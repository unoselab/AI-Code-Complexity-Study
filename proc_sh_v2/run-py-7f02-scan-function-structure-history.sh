...
# =============================================================================
# run-py-7f02-scan-function-structure-history.sh
# -----------------------------------------------------------------------------
# Purpose:
#   Scan the exact Python repository-month Git snapshots for the matched
#   treatment and control repositories before defining function-role groups.
#
# New approach:
...
#     run-py-7f02-decorator-inventory.csv
#     run-py-7f02-decorator-structure-counts.csv
#     run-py-7f02-ast-parent-patterns.csv
#     run-py-7f02-function-structure-counts.csv
#     run-py-7f02-parse-failures.csv
#     run-py-7f02-repository-scan-qc.csv
...
# OUTPUT_DIR="repo_python/tmp/run-py-7f02/smoke/strict" \
# OVERWRITE_OUTPUT=1 \
# bash proc_sh_v2/run-py-7f02-scan-function-structure-history.sh
# =============================================================================
...
if [[ "${PANEL_VARIANT}" != "strict" ]]; then
    echo "ERROR: run-py-7f02 currently supports PANEL_VARIANT=strict only." >&2
    exit 2
fi
...
finish() {
    ...
    echo
    echo "================================================================================"
    echo "run-py-7f02 execution summary"
...
echo "================================================================================"
echo "run-py-7f02: outcome-blind function-structure history scan"
echo "Started:                  ${START_TIME}"
...
expected_outputs=(
    "${OUTPUT_DIR}/run-py-7f02-history-snapshot-manifest.csv"
    "${OUTPUT_DIR}/run-py-7f02-python-file-inventory.csv"
    "${OUTPUT_DIR}/run-py-7f02-python-function-inventory.csv"
    "${OUTPUT_DIR}/run-py-7f02-decorator-inventory.csv"
    "${OUTPUT_DIR}/run-py-7f02-decorator-structure-counts.csv"
    "${OUTPUT_DIR}/run-py-7f02-ast-parent-patterns.csv"
    "${OUTPUT_DIR}/run-py-7f02-function-structure-counts.csv"
    "${OUTPUT_DIR}/run-py-7f02-parse-failures.csv"
    "${OUTPUT_DIR}/run-py-7f02-repository-scan-qc.csv"
    "${OUTPUT_DIR}/run-py-7f02-scan-metadata.json"
)

for expected_file in "${expected_outputs[@]}"; do
    require_file "${expected_file}" "expected run-py-7f02 output"
done

"${PYTHON_BIN}" - "${OUTPUT_DIR}/run-py-7f02-scan-metadata.json" \
    "${OUTPUT_DIR}/run-py-7f02-repository-scan-qc.csv" <<'PY'
import csv
import json
import sys
...
print("================================================================================")
print("run-py-7f02 output verification")
...
echo "run-py-7f02 PASS: raw history inventories are ready for taxonomy review."
