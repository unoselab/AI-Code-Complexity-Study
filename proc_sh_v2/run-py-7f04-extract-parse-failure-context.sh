...
#   - Reproduce each failure with the run-py-7f02 v2 Python 3.13 parser policy.
#   - First parse with type_comments=True; retry with type_comments=False only
#     after a primary failure, and record both attempts and the final mode.
...
EXPECTED_PYTHON_MAJOR_MINOR="${EXPECTED_PYTHON_MAJOR_MINOR:-3.13}"
...
if [[ "${PYTHON_MAJOR_MINOR}" != "${EXPECTED_PYTHON_MAJOR_MINOR}" ]]; then
    echo "ERROR: Python ${EXPECTED_PYTHON_MAJOR_MINOR}.x is required to reproduce run-py-7f02 v2 parse errors; found ${PYTHON_VERSION}." >&2
    exit 2
fi
...
if metadata.get("schema_version") != "run-py-7f04-v2":
    raise SystemExit(
        f"Unexpected schema version: {metadata.get('schema_version')}"
    )
python_version_info = metadata.get("python_version_info", {})
if (
    int(python_version_info.get("major", 0)),
    int(python_version_info.get("minor", 0)),
) != (3, 13):
    raise SystemExit(
        f"Unexpected metadata Python version: {python_version_info}"
    )
...
if counts["dual_parse_failures_reproduced"] != expected_reviews:
    raise SystemExit(
        "Unexpected dual parse failures: "
        f"{counts['dual_parse_failures_reproduced']} != {expected_reviews}"
    )
...
print(f"  dual parse failures reproduced: {counts['dual_parse_failures_reproduced']}")
