...
# =============================================================================
# run-py-7f09-did-reg-fun-binary-groups-borusyak-v1.sh
# -----------------------------------------------------------------------------
...
for index in "${!OUTCOME_GROUPS[@]}"; do
    ...
    dynamic_errors="$(awk -F, '$1 == "dynamic_model_error_count" {gsub(/\r|\"/, "", $2); print $2}' \
        "${out_dir}/${prefix}_final_model_validation.csv")"
    dynamic_complete="$(awk -F, '$1 == "dynamic_effects_complete_13_periods" {gsub(/\r|\"/, "", $2); print $2}' \
        "${out_dir}/${prefix}_final_model_validation.csv")"
...
echo "Dynamic-effect rows:   39"
echo "Uncertainty rows:      42"
