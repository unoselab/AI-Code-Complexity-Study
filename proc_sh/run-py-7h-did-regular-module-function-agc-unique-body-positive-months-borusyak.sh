...
# run-py-7h: Borusyak DiD sensitivity analysis for positive AGC months only.
#
# This wrapper is independent. It reuses the project panel and Borusyak helper
# logic but does not call run-py-7f or run-py-7g.
#
# Inputs:
...
#   Repository-months with a realized outcome of zero are removed. This is a
#   selected-sample sensitivity analysis and must not replace the primary
#   zero-inclusive causal DiD model.
#
# Usage:
#   OVERWRITE_OUTPUT=1 bash proc_sh/run-py-7h-did-regular-module-function-agc-unique-body-positive-months-borusyak.sh
...
R_SCRIPT="${R_SCRIPT:-proc_r/did_regular_module_function_agc_unique_body_positive_months.R}"
...
PANEL_PATH="${PANEL_PATH:-repo_python/run-py-7e/strict/specifications/${SPECIFICATION_NAME}/panel_event_monthly_regular_module_function_agc_unique_body_parse_clean.csv}"
OUTPUT_ROOT="${OUTPUT_ROOT:-repo_python/run-py-7h/strict/specifications/${SPECIFICATION_NAME}}"
...
LOG_DIR="${LOG_DIR:-logs/run-py-7h}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/run-py-7h-positive-agc-months-${SPECIFICATION_NAME}-${RUN_TS}.log}"
...
for required_file in "${R_SCRIPT}" "${HELPER_FILE}" "${PANEL_PATH}"; do
...
{
  echo "================================================================================"
  echo "run-py-7h: positive-outcome-month AGC unique-body DiD sensitivity"
