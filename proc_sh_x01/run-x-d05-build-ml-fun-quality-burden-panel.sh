...
# =============================================================================
# run-x-d05 v1: build ML-selected Python SonarQube quality-burden panel
# =============================================================================
#
# Purpose:
#   1. Reuse frozen run-x-a04 ML file classifications from the ai_detector
...
#   3. Join A04 to D02 exactly by snapshot_id + relative_path + file_sha256.
...
#   8. Stop before causal estimation. D06 will run the Borusyak DiD.
#
# Important semantics:
#   - A04 no_ml_fun and file_not_prepared rows remain unclassified. They are
#     never imputed as HWC.
#   - Primary file selection is frozen upstream:
#       file_ml_agc_share_space_by_token_weighted > 0.50
#   - D05 does not select a new threshold from SonarQube outcomes.
#   - D05 does not compute selected-file issue density because file-level
...
#
# Development files:
#   proc_script_x01/build_ml_fun_quality_burden_panel-v1.py
#   proc_sh_x01/run-x-d05-build-ml-fun-quality-burden-panel-v1.sh
#
# Canonical server deployment names after removing the version suffix:
#   proc_script_x01/build_ml_fun_quality_burden_panel.py
#   proc_sh_x01/run-x-d05-build-ml-fun-quality-burden-panel.sh
#
# This wrapper was created by copying the established run-x-d03 wrapper and
# adapting its standalone execution/provenance/QC pattern for D05. It does not
...
RUN_PREFIX="run-x-d05"
IMPLEMENTATION_VERSION="v1"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"
LOG_DIR="${LOG_DIR:-logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-build-ml-fun-quality-burden-panel-${RUN_TS}.log}"

PYTHON_BIN="${PYTHON_BIN:-python}"
ANALYSIS_SCRIPT="${ANALYSIS_SCRIPT:-proc_script_x01/build_ml_fun_quality_burden_panel.py}"

# Frozen A04 ML file classification from the sibling ai_detector workspace.
A04_ROOT="${A04_ROOT:-${PROJECT_ROOT}/../../ai_detector/src/app/data_did_agc_analysis/run-x-a04}"
A04_FILE="${A04_FILE:-${A04_ROOT}/python_ml_fun_file_scores.csv}"
A04_SUMMARY_FILE="${A04_SUMMARY_FILE:-${A04_ROOT}/summary.json}"
A04_CHECKS_FILE="${A04_CHECKS_FILE:-${A04_ROOT}/checks.csv}"
...
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/run-x-d05}"
PANEL_OUTPUT="${PANEL_OUTPUT:-${OUTPUT_DIR}/quality_ml_fun_repo_month_panel.csv.gz}"
GLOBAL_AUDIT_OUTPUT="${GLOBAL_AUDIT_OUTPUT:-${OUTPUT_DIR}/quality_ml_fun_global_audit.csv}"
TIMING_AUDIT_OUTPUT="${TIMING_AUDIT_OUTPUT:-${OUTPUT_DIR}/quality_ml_fun_by_treatment_timing.csv}"
SAMPLE_SUMMARY_OUTPUT="${SAMPLE_SUMMARY_OUTPUT:-${OUTPUT_DIR}/quality_ml_fun_sample_summary.csv}"
OUTCOME_SPEC_OUTPUT="${OUTCOME_SPEC_OUTPUT:-${OUTPUT_DIR}/quality_ml_fun_outcome_spec.csv}"
CHECKS_OUTPUT="${CHECKS_OUTPUT:-${OUTPUT_DIR}/quality_ml_fun_checks.csv}"
SUMMARY_OUTPUT="${SUMMARY_OUTPUT:-${OUTPUT_DIR}/quality_ml_fun_summary.csv}"
...
require_file "${ANALYSIS_SCRIPT}" "D05 Python program"
mkdir -p "${LOG_DIR}" "${OUTPUT_DIR}"

if [[ "${SELF_TEST_ONLY}" == "0" && "${OVERWRITE}" != "1" && -e "${PANEL_OUTPUT}" ]]; then
  echo "ERROR: output already exists: ${PANEL_OUTPUT}" >&2
  echo "Set OVERWRITE=1 only when intentionally rebuilding D05." >&2
  exit 1
fi
...
echo "============================================================================"
echo "${RUN_LABEL}: build ML-selected Python SonarQube quality-burden panel"
...
echo "A04 file scores:                 ${A04_FILE}"
echo "D02 canonical file burden:       ${D02_FILE}"
echo "B06 quality panel:               ${B06_PANEL_FILE}"
echo "Primary ML metric:               file_ml_agc_share_space_by_token_weighted"
echo "Primary ML rule:                 > 0.50"
echo "No-FUN policy:                   unclassified; never HWC"
...
echo "DiD estimation:                  disabled (D06)"
echo "Output directory:                ${OUTPUT_DIR}"
echo "Log file:                        ${LOG_FILE}"
echo "============================================================================"
echo

echo "** Step 1: Run D05 structural self-test"
echo "----------------------------------------------------------------------------"
"${PYTHON_BIN}" "${ANALYSIS_SCRIPT}" --self-test
echo

echo "** Step 2: Compile D05 Python program"
echo "----------------------------------------------------------------------------"
...
for input_path in \
  ...
  require_file "${input_path}" "required D05 input"
done

echo "** Step 3: Freeze exact input provenance"
echo "----------------------------------------------------------------------------"
echo "A04 file SHA256:                 $(sha256sum "${A04_FILE}" | awk '{print $1}')"
echo "A04 summary SHA256:              $(sha256sum "${A04_SUMMARY_FILE}" | awk '{print $1}')"
echo "A04 checks SHA256:               $(sha256sum "${A04_CHECKS_FILE}" | awk '{print $1}')"
...
echo "** Step 4: Build D05 repo-month panel"
echo "----------------------------------------------------------------------------"
ARGS=(
  --a04-file "${A04_FILE}"
  --a04-summary-file "${A04_SUMMARY_FILE}"
  --a04-checks-file "${A04_CHECKS_FILE}"
  --d02-file "${D02_FILE}"
...
for output_path in \
  ...
  "${SUMMARY_OUTPUT}" \
  "${METADATA_OUTPUT}"; do
  require_file "${output_path}" "D05 output"
done

gzip -t "${PANEL_OUTPUT}"
FAILED_CHECKS="$(awk -F',' 'NR > 1 && $4 != "pass" {n++} END {print n+0}' "${CHECKS_OUTPUT}")"
if [[ "${FAILED_CHECKS}" != "0" ]]; then
  echo "ERROR: D05 checks contain ${FAILED_CHECKS} failures" >&2
  exit 1
fi

SUMMARY_STATUS="$(awk -F',' '$1 == "status" {print $2}' "${SUMMARY_OUTPUT}" | tail -n 1 | tr -d '\r')"
if [[ "${SUMMARY_STATUS}" != "PASS" ]]; then
  echo "ERROR: D05 summary status is not PASS: ${SUMMARY_STATUS}" >&2
  exit 1
fi

echo "D05 output verification: PASS"
