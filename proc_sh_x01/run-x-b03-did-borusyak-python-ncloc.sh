...
#
# Purpose:
#   Estimate the same pooled staggered-DiD velocity specification twice while
#   changing only the Python-NCLOC first-stage covariate:
#     1. SonarQube Python-only NCLOC
#     2. cloc Python-only NCLOC
#
# Design inherited from run-x-a06:
...
#   - The two NCLOC values are never averaged or calibrated to each other.
#   - Both panels must contain the exact same 1,954 repository-month rows.
#   - Legacy cursor/is_treatment/post_event fields are audit-only.
#   - The R program validates that generic ncloc equals the selected backend.
#
# Required inputs:
...
#
# Active analysis program:
#   proc_script_x01/did_borusyak_velocity_python_ncloc.R
#
# Main output directories:
#   repo_x01/run-x-b03/sonarqube
#   repo_x01/run-x-b03/cloc
#   repo_x01/run-x-b03/comparison
#
# Run:
#   bash proc_sh_x01/run-x-b03-did-borusyak-python-ncloc.sh
#
# Optional overrides:
...
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-did-borusyak-python-ncloc-${RUN_TS}.log}"

RSCRIPT_BIN="${RSCRIPT_BIN:-Rscript}"
R_SCRIPT="${R_SCRIPT:-proc_script_x01/did_borusyak_velocity_python_ncloc.R}"
SONARQUBE_INPUT_FILE="${SONARQUBE_INPUT_FILE:-repo_x01/run-x-b02/velocity_did_panel_python_ncloc_sonarqube.csv}"
CLOC_INPUT_FILE="${CLOC_INPUT_FILE:-repo_x01/run-x-b02/velocity_did_panel_python_ncloc_cloc.csv}"
...
SONARQUBE_SUMMARY_OUTPUT="${SONARQUBE_SUMMARY_OUTPUT:-${TMP_OUTPUT_DIR}/velocity_python_ncloc_sonarqube_summary.csv}"
CLOC_SUMMARY_OUTPUT="${CLOC_SUMMARY_OUTPUT:-${TMP_OUTPUT_DIR}/velocity_python_ncloc_cloc_summary.csv}"
COMPARISON_SUMMARY_OUTPUT="${COMPARISON_SUMMARY_OUTPUT:-${TMP_OUTPUT_DIR}/velocity_python_ncloc_backend_comparison_summary.csv}"
SONARQUBE_HTML_OUTPUT="${SONARQUBE_HTML_OUTPUT:-${SONARQUBE_OUTPUT_DIR}/velocity_python_ncloc_sonarqube_report.html}"
CLOC_HTML_OUTPUT="${CLOC_HTML_OUTPUT:-${CLOC_OUTPUT_DIR}/velocity_python_ncloc_cloc_report.html}"
...
{
  echo "============================================================"
  echo "${RUN_LABEL}: Borusyak velocity DiD with Python NCLOC"
  ...
  echo "SonarQube summary:               ${SONARQUBE_SUMMARY_OUTPUT}"
  echo "cloc summary:                    ${CLOC_SUMMARY_OUTPUT}"
...
run_command_logged "Step 1: Run SonarQube Python-NCLOC DiD" "${BACKEND_COMMAND[@]}"
...
build_backend_command \
  ...
  "${CLOC_SUMMARY_OUTPUT}" \
  "${CLOC_HTML_OUTPUT}"
run_command_logged "Step 2: Run cloc Python-NCLOC DiD" "${BACKEND_COMMAND[@]}"
...
run_command_logged "Step 3: Build same-sample backend comparison" "${COMPARISON_COMMAND[@]}"

SONAR_PREFIX="velocity_python_ncloc_sonarqube"
...

{
  ...
  echo "Backend dynamic comparison:      ${COMPARISON_OUTPUT_DIR}/${COMPARISON_PREFIX}_dynamic_differences.csv"
  ...
  echo "Comparison summary:              ${COMPARISON_SUMMARY_OUTPUT}"
  echo "Log file:                        ${LOG_FILE}"
  echo "Next step:                       review SonarQube and cloc DiD estimates together"
  echo "============================================================"
} | tee -a "${LOG_FILE}"
