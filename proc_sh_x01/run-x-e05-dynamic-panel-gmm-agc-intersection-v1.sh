...
# ============================================================
# run-x-e05 v1: NPR-intersection-ML localized quality -> velocity GMM
# ============================================================
#
# Inputs:
...
#   D03: frozen NPR primary reference used only for exact reproduction QC
#   D05: frozen ML primary reference used only for exact reproduction QC
#
# Outputs:
#   1. Exact file-level NPR AND ML intersection aggregated to 1,954 repo-months.
#   2. Detector overlap/support and exact D03/D05 reproduction audits.
#   3. E05 two-step difference-GMM result and diagnostics.
#
# This wrapper is standalone and does not call any previous experiment shell.
# ============================================================

RUN_PREFIX="run-x-e05"
IMPLEMENTATION_VERSION="v1"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"

PYTHON_BIN="${PYTHON_BIN:-python}"
RSCRIPT_BIN="${RSCRIPT_BIN:-Rscript}"
PY_SCRIPT="${PY_SCRIPT:-proc_script_x01/build_agc_detector_intersection_gmm_panel-v1.py}"
R_SCRIPT="${R_SCRIPT:-proc_script_x01/dynamic_panel_gmm_agc_intersection-v1.R}"
...
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/run-x-e05}"
...
EXPECTED_INTERSECTION_EXPANDED_ROWS="${EXPECTED_INTERSECTION_EXPANDED_ROWS:-3619}"
EXPECTED_INTERSECTION_UNIQUE_FILES="${EXPECTED_INTERSECTION_UNIQUE_FILES:-3375}"
EXPECTED_INTERSECTION_ISSUE_STOCK="${EXPECTED_INTERSECTION_ISSUE_STOCK:-3883}"
...
# Frozen upstream hashes. These gates ensure E05 uses the same authoritative
...
LOG_FILE="${LOG_DIR}/${RUN_LABEL}-dynamic-panel-gmm-agc-intersection-${TIMESTAMP}.log"
...
cat <<EOF
============================================================
${RUN_LABEL}: NPR intersection ML localized quality GMM
Started:                         $(date)
...
Intersection rule:                      NPR primary AND ML primary
NPR rule:                        file NPR > ${NPR_THRESHOLD}
ML rule:                         weighted ML AGC share > ${ML_THRESHOLD}
Consensus policy:                selected only when both agree
...
Expected intersection support:   ${EXPECTED_INTERSECTION_EXPANDED_ROWS} expanded / ${EXPECTED_INTERSECTION_UNIQUE_FILES} unique / ${EXPECTED_INTERSECTION_ISSUE_STOCK} issues
Expected active rows/repos:      ${EXPECTED_ACTIVE_ROWS}/${EXPECTED_ACTIVE_REPOSITORIES}
Log file:                        ${LOG_FILE}
============================================================
EOF

printf '\n** Step 1: Build exact NPR AND ML intersection quality panel\n'
printf '%s\n' '------------------------------------------------------------'
...
"${PYTHON_BIN}" "${PY_SCRIPT}" \
  ...
  --expected-intersection-expanded-rows "${EXPECTED_INTERSECTION_EXPANDED_ROWS}" \
  --expected-intersection-unique-files "${EXPECTED_INTERSECTION_UNIQUE_FILES}" \
  --expected-intersection-issue-stock "${EXPECTED_INTERSECTION_ISSUE_STOCK}"

INTERSECTION_PANEL="${OUTPUT_DIR}/agc_detector_intersection_repo_month_panel.csv.gz"

printf '\n** Step 2: Fit E05 intersection-localized dynamic panel GMM\n'
printf '%s\n' '------------------------------------------------------------'
"${RSCRIPT_BIN}" -e "parse(file='${R_SCRIPT}'); cat('R parse: PASS\\n')"
"${RSCRIPT_BIN}" "${R_SCRIPT}" \
  --input-file "${INTERSECTION_PANEL}" \
...
printf '\n** Step 3: Verify E05 artifacts and hard QC\n'
printf '%s\n' '------------------------------------------------------------'
EXPECTED_OUTPUTS=(
  "${OUTPUT_DIR}/agc_detector_intersection_repo_month_panel.csv.gz"
  "${OUTPUT_DIR}/agc_detector_intersection_support.csv"
  "${OUTPUT_DIR}/agc_detector_intersection_reproduction_audit.csv"
  "${OUTPUT_DIR}/agc_detector_intersection_join_audit.csv"
  "${OUTPUT_DIR}/agc_detector_intersection_qc.csv"
  "${OUTPUT_DIR}/agc_detector_intersection_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_coefficients.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_primary_summary.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_sample_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_instrument_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_b06_join_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_numeric_coercion_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_calendar_gap_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_legacy_flag_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_run_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_models.rds"
)
...
"${PYTHON_BIN}" - "${OUTPUT_DIR}/agc_detector_intersection_qc.csv" "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_qc.csv" <<'PY'
import csv
import sys
...
printf '\n** Step 4: E05 intersection support and GMM result\n'
printf '%s\n' '------------------------------------------------------------'
"${PYTHON_BIN}" - "${OUTPUT_DIR}/agc_detector_intersection_support.csv" "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_primary_summary.csv" "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_diagnostics.csv" <<'PY'
import csv
import sys
...
r = primary[0]
print("Primary intersection-quality coefficient:")
print(
...
CAUTION_COUNT="$(${PYTHON_BIN} - "${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_qc.csv" <<'PY'
import csv, sys
...
${RUN_LABEL}: SUCCESS
Finished:                        $(date)
Intersection definition:         NPR > ${NPR_THRESHOLD} AND ML > ${ML_THRESHOLD}
D03 NPR reproduction:            PASS
D05 ML reproduction:             PASS
Consensus selection:             BOTH DETECTORS
QC caution rows:                 ${CAUTION_COUNT}
Primary result:                  ${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_primary_summary.csv
Detector overlap/support:        ${OUTPUT_DIR}/agc_detector_intersection_support.csv
GMM diagnostics:                 ${OUTPUT_DIR}/dynamic_panel_gmm_agc_intersection_diagnostics.csv
Log file:                        ${LOG_FILE}
============================================================
EOF
