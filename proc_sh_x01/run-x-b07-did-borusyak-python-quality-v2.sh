...
#   3. adjusted_density
#      log1p Python issues-per-KLOC outcomes with:
#        log_age + log_contributors + log_stars + log_issues
#        | repo_id + time_index
#      Python NCLOC is excluded because the outcome is already size-normalized.
#   4. fe_only_density
#      The same density outcomes with repository and calendar-month fixed effects.
#
# Treatment timing:
...
R_SCRIPT="${R_SCRIPT:-proc_script_x01/did_borusyak_python_quality-v2.R}"
INPUT_FILE="${INPUT_FILE:-repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv}"
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/${RUN_LABEL}}"
...

{
  ...
  echo "Spec 3:                          adjusted_density"
  echo "Spec 4:                          fe_only_density"
...
EXPECTED_OUTPUTS=(
  ...
  "${OUTPUT_DIR}/python_quality_primary_four_spec_summary.csv"
...
PRIMARY_ROWS="$(( $(wc -l < "${OUTPUT_DIR}/python_quality_primary_four_spec_summary.csv") - 1 ))"

if [[ "${STATIC_ROWS}" -ne 32 || "${DYNAMIC_ROWS}" -ne 384 || "${PRETREND_ROWS}" -ne 160 || "${PRIMARY_ROWS}" -ne 4 ]]; then
...

{
  ...
  echo "Primary four-spec rows:         ${PRIMARY_ROWS}"
  echo "QC status:                       PASS"
  echo "Primary summary:                 ${OUTPUT_DIR}/python_quality_primary_four_spec_summary.csv"
