...
PYTHON_SCRIPT="${PYTHON_SCRIPT:-${PROJECT_ROOT}/proc_script_x01/summarize_python_quality_density_did.py}"
INPUT="${INPUT:-${PROJECT_ROOT}/repo_x01/run-x-b07-v2/python_quality_static_effects.csv}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/repo_x01/run-y-b04/python-quality-density-did-table-v1}"
LOG_DIR="${LOG_DIR:-${PROJECT_ROOT}/logs/run-y-b04}"

CONFIDENCE_LEVEL="${CONFIDENCE_LEVEL:-0.95}"
EXPECTED_FECS_SPEC="${EXPECTED_FECS_SPEC:-adjusted_density}"
EXPECTED_FEOS_SPEC="${EXPECTED_FEOS_SPEC:-fe_only_density}"
...
LOG_FILE="${LOG_DIR}/run-y-b04-v1-${RUN_STAMP}-$$.log"
