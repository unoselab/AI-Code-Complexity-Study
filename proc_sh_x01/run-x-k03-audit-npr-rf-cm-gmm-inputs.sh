#!/usr/bin/env bash
# Audit NPR-RF+CM dynamic-panel GMM inputs before fitting the new RF+CM interaction model.
# v2 fixes the summary/metadata scope label to RF+CM and adds a regression self-test for it.
#
# This wrapper is standalone. Its execution structure was copied from the validated
# run-x-k01 NPR-CM GMM-input audit wrapper and adapted for K03 NPR-RF+CM checks.
# It does not call or depend on any previous shell wrapper.
#
# Versioned delivery files:
#   proc_script_x01/audit_npr_rf_cm_gmm_inputs-v2.py
#   proc_sh_x01/run-x-k03-audit-npr-rf-cm-gmm-inputs-v2.sh
#
# Canonical server filenames after deployment:
#   proc_script_x01/audit_npr_rf_cm_gmm_inputs.py
#   proc_sh_x01/run-x-k03-audit-npr-rf-cm-gmm-inputs.sh
#
# Required inputs:
#   repo_x01/run-x-i04/quality_fun_cfun_npr_threshold_repo_month_panel.csv.gz
#       Existing RF+CM combined-procedure NPR threshold x quality repo-month panel.
...
#
# K03 validates, without fitting a GMM model:
#   1. I04 primary full-sample NPR-RF+CM threshold contract (tau_NPR=1.571637, strict >).
#   2. I04/B06 repo-month identity and treatment-timing agreement.
...
#
# Main outputs under repo_x01/run-x-k03/:
#   k03_npr_rf_cm_gmm_input_checks.csv
#   k03_npr_rf_cm_gmm_input_summary.csv
#   k03_npr_rf_cm_exact_calendar_sample.csv.gz
#   summary.json
#   metadata.json
#
# Runtime:
...
RUN_PREFIX="run-x-k03"
IMPLEMENTATION_VERSION="v2"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"

PYTHON_BIN="${PYTHON_BIN:-python}"
PY_SCRIPT="${PY_SCRIPT:-proc_script_x01/audit_npr_rf_cm_gmm_inputs.py}"
I04_PANEL_FILE="${I04_PANEL_FILE:-repo_x01/run-x-i04/quality_fun_cfun_npr_threshold_repo_month_panel.csv.gz}"
B06_PANEL_FILE="${B06_PANEL_FILE:-repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv}"
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/run-x-k03}"
LOG_DIR="${LOG_DIR:-logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-audit-npr-rf-cm-gmm-inputs-${RUN_TS}.log}"
RUN_SELF_TEST="${RUN_SELF_TEST:-1}"

PRIMARY_THRESHOLD="1.571637"
EXPECTED_I04_LONG_ROWS="85118"
...
for required_file in "${PY_SCRIPT}" "${I04_PANEL_FILE}" "${B06_PANEL_FILE}"; do
...
if [[ "${PYTHON_MAJOR_MINOR}" != "3.11" ]]; then
  echo "ERROR: K03 requires Python 3.11.x; found ${PYTHON_VERSION}" >&2
  exit 2
fi
...
I04_PANEL_SHA256="$(sha256sum "${I04_PANEL_FILE}" | awk '{print $1}')"
...
if [[ "${RUN_SELF_TEST}" == "1" ]]; then
  echo
  echo "** Step 1: Run K03 structural self-test"
  echo "----------------------------------------------------------------------------"
  "${PYTHON_BIN}" "${PY_SCRIPT}" --self-test
fi

echo
echo "** Step 2: Compile K03 Python program"
echo "----------------------------------------------------------------------------"
"${PYTHON_BIN}" -m py_compile "${PY_SCRIPT}"

echo
echo "** Step 3: Audit I04 NPR-RF+CM and B06 GMM inputs"
echo "----------------------------------------------------------------------------"
set +e
"${PYTHON_BIN}" "${PY_SCRIPT}" \
  --i04-panel-file "${I04_PANEL_FILE}" \
  --b06-panel-file "${B06_PANEL_FILE}" \
  --output-dir "${OUTPUT_DIR}" \
  --primary-threshold "${PRIMARY_THRESHOLD}" \
  --expected-i04-long-rows "${EXPECTED_I04_LONG_ROWS}" \
