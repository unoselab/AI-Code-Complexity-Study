#!/usr/bin/env bash
# Audit ML-CM dynamic-panel GMM inputs before fitting the new CM interaction model.
#
# This wrapper is standalone. Its execution structure was copied from the validated
# run-x-k03 NPR-RF+CM GMM-input audit wrapper and adapted for K05 ML-CM checks.
# It does not call or depend on any previous shell wrapper.
#
# Versioned delivery files:
#   proc_script_x01/audit_ml_cm_gmm_inputs-v1.py
#   proc_sh_x01/run-x-k05-audit-ml-cm-gmm-inputs-v1.sh
#
# Canonical server filenames after deployment:
#   proc_script_x01/audit_ml_cm_gmm_inputs.py
#   proc_sh_x01/run-x-k05-audit-ml-cm-gmm-inputs.sh
#
# Required inputs:
#   repo_x01/run-x-h07/quality_ml_cfun_threshold_input_panel.csv.gz
#       Existing CM ML threshold x quality repo-month input panel.
...
#
# K05 validates, without fitting a GMM model:
#   1. H07 primary full-sample ML-CM threshold contract (tau_ML=0.50, strict >).
#   2. H07/B06 repo-month identity and treatment-timing agreement.
...
#
# Main outputs under repo_x01/run-x-k05/:
#   k05_ml_cm_gmm_input_checks.csv
#   k05_ml_cm_gmm_input_summary.csv
#   k05_ml_cm_exact_calendar_sample.csv.gz
#   summary.json
#   metadata.json
#
# Runtime:
...
RUN_PREFIX="run-x-k05"
IMPLEMENTATION_VERSION="v1"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"

PYTHON_BIN="${PYTHON_BIN:-python}"
PY_SCRIPT="${PY_SCRIPT:-proc_script_x01/audit_ml_cm_gmm_inputs.py}"
H07_PANEL_FILE="${H07_PANEL_FILE:-repo_x01/run-x-h07/quality_ml_cfun_threshold_input_panel.csv.gz}"
B06_PANEL_FILE="${B06_PANEL_FILE:-repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv}"
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/run-x-k05}"
LOG_DIR="${LOG_DIR:-logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-audit-ml-cm-gmm-inputs-${RUN_TS}.log}"
RUN_SELF_TEST="${RUN_SELF_TEST:-1}"

PRIMARY_THRESHOLD="0.5"
EXPECTED_H07_LONG_ROWS="81249"
...
EXPECTED_PRIMARY_SELECTED_FILES="37757"
EXPECTED_PRIMARY_ISSUE_STOCK="36432"

for required_file in "${PY_SCRIPT}" "${H07_PANEL_FILE}" "${B06_PANEL_FILE}"; do
...
if [[ "${PYTHON_MAJOR_MINOR}" != "3.11" ]]; then
  echo "ERROR: K05 requires Python 3.11.x; found ${PYTHON_VERSION}" >&2
  exit 2
fi
...
H07_PANEL_SHA256="$(sha256sum "${H07_PANEL_FILE}" | awk '{print $1}')"
...
if [[ "${RUN_SELF_TEST}" == "1" ]]; then
  echo
  echo "** Step 1: Run K05 structural self-test"
  echo "----------------------------------------------------------------------------"
  "${PYTHON_BIN}" "${PY_SCRIPT}" --self-test
fi

echo
echo "** Step 2: Compile K05 Python program"
echo "----------------------------------------------------------------------------"
"${PYTHON_BIN}" -m py_compile "${PY_SCRIPT}"

echo
echo "** Step 3: Audit H07 ML-CM and B06 GMM inputs"
echo "----------------------------------------------------------------------------"
set +e
"${PYTHON_BIN}" "${PY_SCRIPT}" \
  --h07-panel-file "${H07_PANEL_FILE}" \
  --b06-panel-file "${B06_PANEL_FILE}" \
  --output-dir "${OUTPUT_DIR}" \
  --primary-threshold "${PRIMARY_THRESHOLD}" \
  --expected-h07-long-rows "${EXPECTED_H07_LONG_ROWS}" \
