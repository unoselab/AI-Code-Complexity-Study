...
#
# Purpose:
#   Apply the validated run-x-d04/run-x-b07 Borusyak execution and QC pattern
#   to the frozen run-x-d05 ML-selected quality-burden panel.
#
# This wrapper is standalone. It was derived by copying the established D04
# wrapper structure and adapting it for D05 inputs. It never calls an older
# experiment wrapper.
#
# Inputs:
#   repo_x01/run-x-d05/quality_ml_fun_repo_month_panel.csv.gz
#   repo_x01/run-x-d05/quality_ml_fun_summary.csv
#   repo_x01/run-x-d05/quality_ml_fun_sample_summary.csv
#   repo_x01/run-x-d05/quality_ml_fun_global_audit.csv
#   repo_x01/run-x-d05/quality_ml_fun_checks.csv
#   repo_x01/run-x-d05/metadata.json
#
...
RUN_PREFIX="run-x-d06"
IMPLEMENTATION_VERSION="v1"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"
LOG_DIR="${LOG_DIR:-logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-did-borusyak-ml-quality-${RUN_TS}.log}"

RSCRIPT_BIN="${RSCRIPT_BIN:-Rscript}"
R_SCRIPT="${R_SCRIPT:-proc_script_x01/did_borusyak_ml_quality.R}"
INPUT_FILE="${INPUT_FILE:-repo_x01/run-x-d05/quality_ml_fun_repo_month_panel.csv.gz}"
D05_SUMMARY_FILE="${D05_SUMMARY_FILE:-repo_x01/run-x-d05/quality_ml_fun_summary.csv}"
D05_SAMPLE_SUMMARY_FILE="${D05_SAMPLE_SUMMARY_FILE:-repo_x01/run-x-d05/quality_ml_fun_sample_summary.csv}"
D05_GLOBAL_AUDIT_FILE="${D05_GLOBAL_AUDIT_FILE:-repo_x01/run-x-d05/quality_ml_fun_global_audit.csv}"
D05_CHECKS_FILE="${D05_CHECKS_FILE:-repo_x01/run-x-d05/quality_ml_fun_checks.csv}"
D05_METADATA_FILE="${D05_METADATA_FILE:-repo_x01/run-x-d05/metadata.json}"
...
for required_file in \
  ...
  "${INPUT_FILE}" \
  "${D05_SUMMARY_FILE}" \
  "${D05_SAMPLE_SUMMARY_FILE}" \
  "${D05_GLOBAL_AUDIT_FILE}" \
  "${D05_CHECKS_FILE}" \
  "${D05_METADATA_FILE}"; do
...
if [[ -d "${OUTPUT_DIR}" && "${OVERWRITE}" != "1" && "${SELF_TEST_ONLY}" != "1" ]]; then
  ...
  echo "Set OVERWRITE=1 to replace the D06 output directory." >&2
  exit 1
fi
...
D05_SUMMARY_SHA256="$(sha256sum "${D05_SUMMARY_FILE}" | awk '{print $1}')"
D05_SAMPLE_SUMMARY_SHA256="$(sha256sum "${D05_SAMPLE_SUMMARY_FILE}" | awk '{print $1}')"
D05_GLOBAL_AUDIT_SHA256="$(sha256sum "${D05_GLOBAL_AUDIT_FILE}" | awk '{print $1}')"
D05_CHECKS_SHA256="$(sha256sum "${D05_CHECKS_FILE}" | awk '{print $1}')"
D05_METADATA_SHA256="$(sha256sum "${D05_METADATA_FILE}" | awk '{print $1}')"

{
  echo "============================================================================"
  echo "${RUN_LABEL}: Borusyak DiD for frozen ML-selected Python quality burden"
  ...
  echo "D05 panel:                       ${INPUT_FILE}"
  echo "D05 panel SHA256:                ${INPUT_SHA256}"
  echo "D05 summary SHA256:              ${D05_SUMMARY_SHA256}"
  echo "D05 sample summary SHA256:       ${D05_SAMPLE_SUMMARY_SHA256}"
  echo "D05 global audit SHA256:         ${D05_GLOBAL_AUDIT_SHA256}"
  echo "D05 checks SHA256:               ${D05_CHECKS_SHA256}"
  echo "D05 metadata SHA256:             ${D05_METADATA_SHA256}"
  echo "Output directory:                ${OUTPUT_DIR}"
  echo "Primary analysis:                full_sample x all_ml_files"
  echo "Primary ML rule:                 file_ml_agc_share_space_by_token_weighted > ${PRIMARY_THRESHOLD}"
  ...
  echo "Expected D05 panel rows:         ${EXPECTED_LONG_ROWS}"
...
{
  echo
  echo "** Step 1: Run D06 structural self-test"
  echo "----------------------------------------------------------------------------"
} | tee -a "${LOG_FILE}"
...
{
  echo
  echo "** Step 2: Parse D06 R program"
  echo "----------------------------------------------------------------------------"
} | tee -a "${LOG_FILE}"
...
COMMAND=(
  ...
  --d05-summary-file "${D05_SUMMARY_FILE}"
  --d05-sample-summary-file "${D05_SAMPLE_SUMMARY_FILE}"
  --d05-global-audit-file "${D05_GLOBAL_AUDIT_FILE}"
...
{
  echo
  echo "** Step 3: Run D06 Borusyak DiD"
  echo "----------------------------------------------------------------------------"
  printf 'Command:'
  printf ' %q' "${COMMAND[@]}"
  printf '\n\n'
...
EXPECTED_OUTPUTS=(
  "${OUTPUT_DIR}/quality_ml_static_effects.csv"
  "${OUTPUT_DIR}/quality_ml_dynamic_effects.csv"
  "${OUTPUT_DIR}/quality_ml_pretrend_checks.csv"
  "${OUTPUT_DIR}/quality_ml_pretrend_summary.csv"
  "${OUTPUT_DIR}/quality_ml_model_diagnostics.csv"
  "${OUTPUT_DIR}/quality_ml_model_failures.csv"
  "${OUTPUT_DIR}/quality_ml_analysis_support.csv"
  "${OUTPUT_DIR}/quality_ml_event_support.csv"
  "${OUTPUT_DIR}/quality_ml_support_policy.csv"
  "${OUTPUT_DIR}/quality_ml_primary_static.csv"
  "${OUTPUT_DIR}/quality_ml_primary_total_static.csv"
  "${OUTPUT_DIR}/quality_ml_primary_total_dynamic.csv"
  "${OUTPUT_DIR}/quality_ml_total_static_by_analysis.csv"
  "${OUTPUT_DIR}/quality_ml_total_dynamic_by_analysis.csv"
  "${OUTPUT_DIR}/quality_ml_qc.csv"
  "${OUTPUT_DIR}/quality_ml_summary.csv"
  "${OUTPUT_DIR}/quality_ml_run_metadata.csv"
  "${OUTPUT_DIR}/plots/quality_ml_total_static_by_analysis.pdf"
  "${OUTPUT_DIR}/plots/quality_ml_total_static_by_analysis.png"
  "${OUTPUT_DIR}/plots/quality_ml_primary_total_dynamic.pdf"
  "${OUTPUT_DIR}/plots/quality_ml_primary_total_dynamic.png"
)

{
  echo
  echo "** Step 4: Verify D06 output artifacts"
  echo "----------------------------------------------------------------------------"
} | tee -a "${LOG_FILE}"
...
QC_FILE="${OUTPUT_DIR}/quality_ml_qc.csv"
if grep -q ',fail,' "${QC_FILE}" || grep -q ',fail$' "${QC_FILE}"; then
  echo "ERROR: D06 QC file contains one or more hard failed checks." | tee -a "${LOG_FILE}" >&2
  cat "${QC_FILE}" | tee -a "${LOG_FILE}" >&2
  exit 1
fi

STATIC_ROWS="$(( $(wc -l < "${OUTPUT_DIR}/quality_ml_static_effects.csv") - 1 ))"
DYNAMIC_ROWS="$(( $(wc -l < "${OUTPUT_DIR}/quality_ml_dynamic_effects.csv") - 1 ))"
PRETREND_ROWS="$(( $(wc -l < "${OUTPUT_DIR}/quality_ml_pretrend_checks.csv") - 1 ))"
PRIMARY_STATIC_ROWS="$(( $(wc -l < "${OUTPUT_DIR}/quality_ml_primary_static.csv") - 1 ))"
PRIMARY_TOTAL_STATIC_ROWS="$(( $(wc -l < "${OUTPUT_DIR}/quality_ml_primary_total_static.csv") - 1 ))"
PRIMARY_TOTAL_DYNAMIC_ROWS="$(( $(wc -l < "${OUTPUT_DIR}/quality_ml_primary_total_dynamic.csv") - 1 ))"
ANALYSIS_SUPPORT_ROWS="$(( $(wc -l < "${OUTPUT_DIR}/quality_ml_analysis_support.csv") - 1 ))"
EVENT_SUPPORT_ROWS="$(( $(wc -l < "${OUTPUT_DIR}/quality_ml_event_support.csv") - 1 ))"
...
if [[ "${STATIC_ROWS}" -ne 64 || \
  ...
  echo "ERROR: unexpected D06 row counts: static=${STATIC_ROWS}, dynamic=${DYNAMIC_ROWS}, pretrend=${PRETREND_ROWS}, primary_static=${PRIMARY_STATIC_ROWS}, primary_total_static=${PRIMARY_TOTAL_STATIC_ROWS}, primary_total_dynamic=${PRIMARY_TOTAL_DYNAMIC_ROWS}, analysis_support=${ANALYSIS_SUPPORT_ROWS}, event_support=${EVENT_SUPPORT_ROWS}" | tee -a "${LOG_FILE}" >&2
  exit 1
fi

RUN_STATUS="$(awk -F, '$1=="run" && $2=="status" {print $3}' "${OUTPUT_DIR}/quality_ml_summary.csv" | tr -d '\r\"')"
if [[ "${RUN_STATUS}" != "PASS" && "${RUN_STATUS}" != "PASS_WITH_ROBUSTNESS_MODEL_FAILURES" ]]; then
  echo "ERROR: unexpected D06 run status: ${RUN_STATUS}" | tee -a "${LOG_FILE}" >&2
  exit 1
fi

ROBUSTNESS_FAILURES="$(awk -F, '$1=="models" && $2=="robustness_failure_jobs" {print $3}' "${OUTPUT_DIR}/quality_ml_summary.csv" | tr -d '\r\"')"
LOW_SUPPORT_SPECS="$(awk -F, '$1=="support" && $2=="low_support_analysis_specs" {print $3}' "${OUTPUT_DIR}/quality_ml_summary.csv" | tr -d '\r\"')"

{
  ...
  echo "QC status:                       PASS"
  echo "Primary static results:          ${OUTPUT_DIR}/quality_ml_primary_total_static.csv"
  echo "Primary dynamic results:         ${OUTPUT_DIR}/quality_ml_primary_total_dynamic.csv"
  echo "All robustness static results:   ${OUTPUT_DIR}/quality_ml_total_static_by_analysis.csv"
  echo "Pretrend summary:                ${OUTPUT_DIR}/quality_ml_pretrend_summary.csv"
  echo "Model failures:                  ${OUTPUT_DIR}/quality_ml_model_failures.csv"
  echo "Log file:                        ${LOG_FILE}"
  echo "============================================================================"
} | tee -a "${LOG_FILE}"
