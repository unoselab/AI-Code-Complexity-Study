...
# ============================================================
# run-x-e04 v1: NPR-union-ML localized quality -> velocity GMM
# ============================================================
#
# Inputs:
...
#
# Outputs:
#   1. Exact file-level NPR OR ML union aggregated to 1,954 repo-months.
#   2. Detector overlap/support and exact D03/D05 reproduction audits.
#   3. E04 two-step difference-GMM result and diagnostics.
#
# This wrapper is standalone and does not call any previous experiment shell.
# ============================================================

RUN_PREFIX="run-x-e04"
IMPLEMENTATION_VERSION="v1"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"

PYTHON_BIN="${PYTHON_BIN:-python}"
RSCRIPT_BIN="${RSCRIPT_BIN:-Rscript}"
PY_SCRIPT="${PY_SCRIPT:-proc_script_x01/build_agc_detector_union_gmm_panel-v1.py}"
R_SCRIPT="${R_SCRIPT:-proc_script_x01/dynamic_panel_gmm_agc_union-v1.R}"
...
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/run-x-e04}"
...
# Frozen upstream hashes. These gates ensure E04 uses the same authoritative
...
LOG_FILE="${LOG_DIR}/${RUN_LABEL}-dynamic-panel-gmm-agc-union-${TIMESTAMP}.log"
...
cat <<EOF
============================================================
${RUN_LABEL}: NPR union ML localized quality GMM
Started:                         $(date)
...
Union rule:                      NPR primary OR ML primary
NPR rule:                        file NPR > ${NPR_THRESHOLD}
ML rule:                         weighted ML AGC share > ${ML_THRESHOLD}
Overlap policy:                  selected by both counted once
...
printf '\n** Step 1: Build exact NPR OR ML union quality panel\n'
printf '%s\n' '------------------------------------------------------------'
...
UNION_PANEL="${OUTPUT_DIR}/agc_detector_union_repo_month_panel.csv.gz"

printf '\n** Step 2: Fit E04 union-localized dynamic panel GMM\n'
printf '%s\n' '------------------------------------------------------------'
"${RSCRIPT_BIN}" -e "parse(file='${R_SCRIPT}'); cat('R parse: PASS\\n')"
"${RSCRIPT_BIN}" "${R_SCRIPT}" \
  --input-file "${UNION_PANEL}" \
...
printf '\n** Step 3: Verify E04 artifacts and hard QC\n'
printf '%s\n' '------------------------------------------------------------'
EXPECTED_OUTPUTS=(
  "${OUTPUT_DIR}/agc_detector_union_repo_month_panel.csv.gz"
  "${OUTPUT_DIR}/agc_detector_union_support.csv"
  "${OUTPUT_DIR}/agc_detector_union_reproduction_audit.csv"
  "${OUTPUT_DIR}/agc_detector_union_join_audit.csv"
  "${OUTPUT_DIR}/agc_detector_union_qc.csv"
  "${OUTPUT_DIR}/agc_detector_union_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_coefficients.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_primary_summary.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_sample_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_instrument_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_b06_join_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_numeric_coercion_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_calendar_gap_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_legacy_flag_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_run_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_models.rds"
)
...
"${PYTHON_BIN}" - "${OUTPUT_DIR}/agc_detector_union_qc.csv" "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_qc.csv" <<'PY'
import csv
import sys
...
printf '\n** Step 4: E04 union support and GMM result\n'
printf '%s\n' '------------------------------------------------------------'
"${PYTHON_BIN}" - "${OUTPUT_DIR}/agc_detector_union_support.csv" "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_primary_summary.csv" "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_diagnostics.csv" <<'PY'
import csv
import sys
...
r = primary[0]
print("Primary union-quality coefficient:")
print(
...
CAUTION_COUNT="$(${PYTHON_BIN} - "${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_qc.csv" <<'PY'
import csv, sys
...
${RUN_LABEL}: SUCCESS
Finished:                        $(date)
Union definition:                NPR > ${NPR_THRESHOLD} OR ML > ${ML_THRESHOLD}
D03 NPR reproduction:            PASS
D05 ML reproduction:             PASS
Overlap double counting:         NONE
QC caution rows:                 ${CAUTION_COUNT}
Primary result:                  ${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_primary_summary.csv
Detector overlap/support:        ${OUTPUT_DIR}/agc_detector_union_support.csv
GMM diagnostics:                 ${OUTPUT_DIR}/dynamic_panel_gmm_agc_union_diagnostics.csv
Log file:                        ${LOG_FILE}
============================================================
EOF
