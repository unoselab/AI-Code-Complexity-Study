...
# ============================================================
# run-x-k06 v1: ML-CM quality -> future Python velocity GMM
# ============================================================
#
# Purpose:
#   Apply the validated reverse-direction dynamic-panel GMM design to the H07
#   CM-ML primary-threshold quality burden. This run tests whether issue stock
#   localized to CM-localized ML-selected Python files predicts lower subsequent whole-
#   Python development velocity.
#
# Inputs:
#   - H07 long threshold panel:
#       repo_x01/run-x-h07/quality_ml_cfun_threshold_input_panel.csv.gz
#   - H07 summary/sample/global-audit artifacts for provenance and hard QC.
#   - B06 authoritative whole-Python velocity/covariate panel:
#       repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv
#
# Join contract:
#   H07 intentionally omits log_lines_added_py_source. The R analysis joins
#   velocity from B06 on exact repo_id + time_index and verifies overlapping
#   timing/covariate fields before fitting GMM.
#
# Detector contract:
#   - detector: AGCDetector_ML
#   - metric: file_ml_cfun_agc_share_space_by_token_weighted
#   - primary threshold: 0.5
#   - selection: strict ML > threshold
...
# Samples:
#   - full_sample (primary)
#   - exclude_scope_mismatch_repos (pre-specified ML scope robustness)
#
# This wrapper copies the execution structure of the validated K04/E03
# GMM wrappers and adapts it to H07 ML-CM inputs. It is standalone and does
# not call or source any previous experiment wrapper.
#
# Outputs:
#   - Coefficients, diagnostics, sample/instrument QC, provenance metadata,
#     and the fitted pgmm objects under repo_x01/run-x-k06/.
#
#
# Versioned delivery files:
#   proc_script_x01/dynamic_panel_gmm_ml_cm-v1.R
#   proc_sh_x01/run-x-k06-dynamic-panel-gmm-ml-cm-v1.sh
#
# Canonical server filenames after deployment:
#   proc_script_x01/dynamic_panel_gmm_ml_cm.R
#   proc_sh_x01/run-x-k06-dynamic-panel-gmm-ml-cm.sh
#
# Run:
#   bash proc_sh_x01/run-x-k06-dynamic-panel-gmm-ml-cm.sh
# ============================================================
...
RUN_PREFIX="run-x-k06"
IMPLEMENTATION_VERSION="v1"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"
LOG_DIR="${LOG_DIR:-logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-dynamic-panel-gmm-ml-cm-${RUN_TS}.log}"

RSCRIPT_BIN="${RSCRIPT_BIN:-Rscript}"
R_SCRIPT="${R_SCRIPT:-proc_script_x01/dynamic_panel_gmm_ml_cm.R}"
INPUT_FILE="${INPUT_FILE:-repo_x01/run-x-h07/quality_ml_cfun_threshold_input_panel.csv.gz}"
B06_PANEL_FILE="${B06_PANEL_FILE:-repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv}"
H07_SUMMARY_FILE="${H07_SUMMARY_FILE:-repo_x01/run-x-h07/quality_ml_cfun_threshold_input_summary.csv}"
H07_SAMPLE_SUMMARY_FILE="${H07_SAMPLE_SUMMARY_FILE:-repo_x01/run-x-h07/quality_ml_cfun_threshold_input_sample_summary.csv}"
H07_GLOBAL_AUDIT_FILE="${H07_GLOBAL_AUDIT_FILE:-repo_x01/run-x-h07/quality_ml_cfun_threshold_input_global_audit.csv}"
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/${RUN_PREFIX}}"

CONFIDENCE_LEVEL="${CONFIDENCE_LEVEL:-0.95}"
PRIMARY_THRESHOLD="${PRIMARY_THRESHOLD:-0.5}"
...
EXPECTED_PRIMARY_FULL_SELECTED_FILES="${EXPECTED_PRIMARY_FULL_SELECTED_FILES:-37757}"
EXPECTED_PRIMARY_FULL_ISSUE_STOCK="${EXPECTED_PRIMARY_FULL_ISSUE_STOCK:-36432}"
EXPECTED_PRIMARY_SENSITIVITY_SELECTED_FILES="${EXPECTED_PRIMARY_SENSITIVITY_SELECTED_FILES:-37271}"
EXPECTED_PRIMARY_SENSITIVITY_ISSUE_STOCK="${EXPECTED_PRIMARY_SENSITIVITY_ISSUE_STOCK:-36252}"
...
for required_file in "${R_SCRIPT}" "${INPUT_FILE}" "${B06_PANEL_FILE}" "${H07_SUMMARY_FILE}" "${H07_SAMPLE_SUMMARY_FILE}" "${H07_GLOBAL_AUDIT_FILE}"; do
...
H07_SUMMARY_SHA256="$(sha256sum "${H07_SUMMARY_FILE}" | awk '{print $1}')"
H07_SAMPLE_SUMMARY_SHA256="$(sha256sum "${H07_SAMPLE_SUMMARY_FILE}" | awk '{print $1}')"
H07_GLOBAL_AUDIT_SHA256="$(sha256sum "${H07_GLOBAL_AUDIT_FILE}" | awk '{print $1}')"

{
  echo "============================================================"
  echo "${RUN_LABEL}: ML-CM quality -> future velocity GMM"
  ...
  echo "H07 long panel:                  ${INPUT_FILE}"
  echo "H07 long panel SHA256:           ${INPUT_SHA256}"
  echo "B06 velocity panel:              ${B06_PANEL_FILE}"
  echo "B06 velocity panel SHA256:       ${B06_PANEL_SHA256}"
  echo "H07 summary SHA256:              ${H07_SUMMARY_SHA256}"
  echo "H07 sample summary SHA256:       ${H07_SAMPLE_SUMMARY_SHA256}"
  echo "H07 global audit SHA256:         ${H07_GLOBAL_AUDIT_SHA256}"
  echo "Output directory:                ${OUTPUT_DIR}"
  echo "Analysis scope:                  AGCDetector_ML / CM localized quality"
  echo "ML metric:                       file_ml_cfun_agc_share_space_by_token_weighted"
...
COMMAND=(
  ...
  --h07-summary-file "${H07_SUMMARY_FILE}"
  --h07-sample-summary-file "${H07_SAMPLE_SUMMARY_FILE}"
  --h07-global-audit-file "${H07_GLOBAL_AUDIT_FILE}"
...
EXPECTED_OUTPUTS=(
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_coefficients.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_sample_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_instrument_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_model_specifications.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_threshold_input_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_b06_join_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_legacy_flag_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_numeric_coercion_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_calendar_gap_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_run_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_models.rds"
)
...
QC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_qc.csv"
if grep -q ',fail,' "${QC_FILE}" || grep -q ',fail$' "${QC_FILE}"; then
  echo "ERROR: K06 QC file contains one or more failed checks." | tee -a "${LOG_FILE}" >&2
  cat "${QC_FILE}" | tee -a "${LOG_FILE}" >&2
  exit 1
fi

COEFFICIENT_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_coefficients.csv"
DIAGNOSTIC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_diagnostics.csv"
INSTRUMENT_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_instrument_qc.csv"
SAMPLE_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_sample_qc.csv"
METADATA_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_run_metadata.csv"
MODEL_RDS="${OUTPUT_DIR}/dynamic_panel_gmm_ml_cm_models.rds"
...
{
  echo
  echo "** Step 3: ML GMM primary results and diagnostics"
  echo "------------------------------------------------------------"
  echo "Primary lagged ML-quality coefficients:"
  awk 'NR==1 || /,TRUE[[:space:]]*$/' "${COEFFICIENT_FILE}"
  echo
  ...
  echo "Analysis scope:                  AGCDetector_ML / CM localized quality -> velocity"
  echo "Primary threshold:               ${PRIMARY_THRESHOLD}"
