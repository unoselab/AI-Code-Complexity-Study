...
# ============================================================
# run-x-k08 v2: ML-RF+CM quality -> future Python velocity GMM
# ============================================================
#
# Purpose:
#   Apply the validated reverse-direction dynamic-panel GMM design to the I08
#   RF+CM-ML primary-threshold quality burden. This run tests whether issue stock
#   localized to CM-localized ML-selected Python files predicts lower subsequent whole-
#   Python development velocity.
#
# Inputs:
#   - I08 long threshold panel:
#       repo_x01/run-x-i08/quality_ml_fun_cfun_threshold_input_panel.csv.gz
#   - I08 summary/sample/global-audit artifacts for provenance and hard QC.
#   - B06 authoritative whole-Python velocity/covariate panel:
#       repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv
#
# Join contract:
#   I08 intentionally omits log_lines_added_py_source. The R analysis joins
...
# GMM wrappers and adapts it to I08 ML-RF+CM inputs. It is standalone and does
# not call or source any previous experiment wrapper.
#
# Outputs:
#   - Coefficients, diagnostics, sample/instrument QC, provenance metadata,
#     and the fitted pgmm objects under repo_x01/run-x-k08/.
#
#
# Versioned delivery files:
#   proc_script_x01/dynamic_panel_gmm_ml_rf_cm-v2.R
#   proc_sh_x01/run-x-k08-dynamic-panel-gmm-ml-rf-cm-v2.sh
#
# Canonical server filenames after deployment:
#   proc_script_x01/dynamic_panel_gmm_ml_rf_cm.R
#   proc_sh_x01/run-x-k08-dynamic-panel-gmm-ml-rf-cm.sh
#
# Run:
#   bash proc_sh_x01/run-x-k08-dynamic-panel-gmm-ml-rf-cm.sh
# ============================================================
...
RUN_PREFIX="run-x-k08"
IMPLEMENTATION_VERSION="v2"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"
LOG_DIR="${LOG_DIR:-logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-dynamic-panel-gmm-ml-rf-cm-${RUN_TS}.log}"

RSCRIPT_BIN="${RSCRIPT_BIN:-Rscript}"
R_SCRIPT="${R_SCRIPT:-proc_script_x01/dynamic_panel_gmm_ml_rf_cm.R}"
INPUT_FILE="${INPUT_FILE:-repo_x01/run-x-i08/quality_ml_fun_cfun_threshold_input_panel.csv.gz}"
B06_PANEL_FILE="${B06_PANEL_FILE:-repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv}"
I08_SUMMARY_FILE="${I08_SUMMARY_FILE:-repo_x01/run-x-i08/quality_ml_fun_cfun_threshold_input_summary.csv}"
I08_SAMPLE_SUMMARY_FILE="${I08_SAMPLE_SUMMARY_FILE:-repo_x01/run-x-i08/quality_ml_fun_cfun_threshold_input_sample_summary.csv}"
I08_GLOBAL_AUDIT_FILE="${I08_GLOBAL_AUDIT_FILE:-repo_x01/run-x-i08/quality_ml_fun_cfun_threshold_input_global_audit.csv}"
...
EXPECTED_PRIMARY_FULL_SELECTED_FILES="${EXPECTED_PRIMARY_FULL_SELECTED_FILES:-64153}"
EXPECTED_PRIMARY_FULL_ISSUE_STOCK="${EXPECTED_PRIMARY_FULL_ISSUE_STOCK:-35765}"
EXPECTED_PRIMARY_SENSITIVITY_SELECTED_FILES="${EXPECTED_PRIMARY_SENSITIVITY_SELECTED_FILES:-63409}"
EXPECTED_PRIMARY_SENSITIVITY_ISSUE_STOCK="${EXPECTED_PRIMARY_SENSITIVITY_ISSUE_STOCK:-34598}"
...
for required_file in "${R_SCRIPT}" "${INPUT_FILE}" "${B06_PANEL_FILE}" "${I08_SUMMARY_FILE}" "${I08_SAMPLE_SUMMARY_FILE}" "${I08_GLOBAL_AUDIT_FILE}"; do
...
I08_SUMMARY_SHA256="$(sha256sum "${I08_SUMMARY_FILE}" | awk '{print $1}')"
I08_SAMPLE_SUMMARY_SHA256="$(sha256sum "${I08_SAMPLE_SUMMARY_FILE}" | awk '{print $1}')"
I08_GLOBAL_AUDIT_SHA256="$(sha256sum "${I08_GLOBAL_AUDIT_FILE}" | awk '{print $1}')"

{
  echo "============================================================"
  echo "${RUN_LABEL}: ML-RF+CM quality -> future velocity GMM"
  ...
  echo "I08 long panel:                  ${INPUT_FILE}"
  echo "I08 long panel SHA256:           ${INPUT_SHA256}"
  ...
  echo "Analysis scope:                  AGCDetector_ML / RF+CM localized quality"
  echo "ML metric:                       file_ml_fun_cfun_agc_share_space_by_token_weighted"
...
COMMAND=(
  ...
  --i08-summary-file "${I08_SUMMARY_FILE}"
  --i08-sample-summary-file "${I08_SAMPLE_SUMMARY_FILE}"
  --i08-global-audit-file "${I08_GLOBAL_AUDIT_FILE}"
...
EXPECTED_OUTPUTS=(
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_coefficients.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_sample_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_instrument_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_model_specifications.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_threshold_input_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_b06_join_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_legacy_flag_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_numeric_coercion_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_calendar_gap_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_run_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_models.rds"
)
...
QC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_qc.csv"
if grep -q ',fail,' "${QC_FILE}" || grep -q ',fail$' "${QC_FILE}"; then
  echo "ERROR: K08 QC file contains one or more failed checks." | tee -a "${LOG_FILE}" >&2
  cat "${QC_FILE}" | tee -a "${LOG_FILE}" >&2
  exit 1
fi

COEFFICIENT_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_coefficients.csv"
DIAGNOSTIC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_diagnostics.csv"
INSTRUMENT_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_instrument_qc.csv"
SAMPLE_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_sample_qc.csv"
METADATA_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_run_metadata.csv"
MODEL_RDS="${OUTPUT_DIR}/dynamic_panel_gmm_ml_rf_cm_models.rds"
...

{
  ...
  echo "Analysis scope:                  AGCDetector_ML / RF+CM localized quality -> velocity"
  echo "Primary threshold:               ${PRIMARY_THRESHOLD}"
