...
# ============================================================
# run-x-k09 v1: NPR RF+CM threshold-sensitivity dynamic-panel GMM
# ============================================================
#
# Purpose:
#   Fit the same reverse-direction quality -> velocity GMM at every threshold
#   in the prespecified I04 21-point main NPR grid.
#
# Inputs:
#   - I04 long threshold quality panel and provenance/QC summaries
#   - B06 authoritative velocity/covariate panel
#   - K04 coefficient output for exact primary-threshold reproduction
#
# Outputs:
...
#
# Important:
#   This wrapper is derived from the validated K04 NPR GMM and E03 threshold-
#   sensitivity wrappers, but it is self-contained and does not call them.
#   The I04 legacy anchor 1.5183 is not part of the 21-point main sweep.
#
# Versioned delivery files:
#   proc_script_x01/dynamic_panel_gmm_npr_rf_cm_threshold_sensitivity-v1.R
#   proc_sh_x01/run-x-k09-dynamic-panel-gmm-npr-rf-cm-threshold-sensitivity-v1.sh
#
# Canonical server filenames after deployment:
#   proc_script_x01/dynamic_panel_gmm_npr_rf_cm_threshold_sensitivity.R
#   proc_sh_x01/run-x-k09-dynamic-panel-gmm-npr-rf-cm-threshold-sensitivity.sh
#
# Run:
#   bash proc_sh_x01/run-x-k09-dynamic-panel-gmm-npr-rf-cm-threshold-sensitivity.sh
# ============================================================
...
RUN_PREFIX="run-x-k09"
IMPLEMENTATION_VERSION="v1"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"
LOG_DIR="${LOG_DIR:-logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-dynamic-panel-gmm-npr-threshold-sensitivity-${RUN_TS}.log}"

RSCRIPT_BIN="${RSCRIPT_BIN:-Rscript}"
R_SCRIPT="${R_SCRIPT:-proc_script_x01/dynamic_panel_gmm_npr_rf_cm_threshold_sensitivity.R}"
I04_PANEL="${I04_PANEL:-repo_x01/run-x-i04/quality_fun_cfun_npr_threshold_repo_month_panel.csv.gz}"
I04_SUMMARY="${I04_SUMMARY:-repo_x01/run-x-i04/quality_fun_cfun_npr_threshold_summary.csv}"
I04_SAMPLE_SUMMARY="${I04_SAMPLE_SUMMARY:-repo_x01/run-x-i04/quality_fun_cfun_npr_threshold_sample_summary.csv}"
I04_GLOBAL_AUDIT="${I04_GLOBAL_AUDIT:-repo_x01/run-x-i04/quality_fun_cfun_npr_threshold_global_audit.csv}"
B06_PANEL="${B06_PANEL:-repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv}"
K04_REFERENCE_COEFFICIENTS="${K04_REFERENCE_COEFFICIENTS:-repo_x01/run-x-k04/dynamic_panel_gmm_npr_rf_cm_coefficients.csv}"
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/run-x-k09}"
...
EXPECTED_I04_SHA256="${EXPECTED_I04_SHA256:-}"
...
EXPECTED_I04_THRESHOLDS="${EXPECTED_I04_THRESHOLDS:-22}"
...
EXPECTED_PRIMARY_ISSUE_STOCK="${EXPECTED_PRIMARY_ISSUE_STOCK:-14809}"
...
for required_file in \
  "${R_SCRIPT}" "${I04_PANEL}" "${I04_SUMMARY}" "${I04_SAMPLE_SUMMARY}" \
  "${I04_GLOBAL_AUDIT}" "${B06_PANEL}" "${K04_REFERENCE_COEFFICIENTS}"; do
...
I04_SHA256="$(sha256sum "${I04_PANEL}" | awk '{print $1}')"
B06_SHA256="$(sha256sum "${B06_PANEL}" | awk '{print $1}')"
if [[ -n "${EXPECTED_I04_SHA256}" && "${I04_SHA256}" != "${EXPECTED_I04_SHA256}" ]]; then
  echo "ERROR: I04 SHA256 mismatch." >&2
  echo "Expected: ${EXPECTED_I04_SHA256}" >&2
  echo "Observed: ${I04_SHA256}" >&2
  exit 1
fi
...
I04_SUMMARY_SHA256="$(sha256sum "${I04_SUMMARY}" | awk '{print $1}')"
I04_SAMPLE_SUMMARY_SHA256="$(sha256sum "${I04_SAMPLE_SUMMARY}" | awk '{print $1}')"
I04_GLOBAL_AUDIT_SHA256="$(sha256sum "${I04_GLOBAL_AUDIT}" | awk '{print $1}')"
K04_REFERENCE_SHA256="$(sha256sum "${K04_REFERENCE_COEFFICIENTS}" | awk '{print $1}')"

{
  echo "============================================================"
  echo "${RUN_LABEL}: NPR RF+CM threshold-sensitivity quality -> velocity GMM"
  ...
  echo "Experiment role:                 21-threshold NPR RF+CM GMM sweep"
  ...
  echo "Legacy anchor 1.5183:            excluded"
  echo "Comparison operator:             strict >"
  echo "Threshold selection by p-value:  NO"
  echo "I04 threshold panel:             ${I04_PANEL}"
  echo "I04 SHA256:                      ${I04_SHA256}"
  ...
  echo "K04 primary reproduction ref:    ${K04_REFERENCE_COEFFICIENTS}"
  echo "K04 reference SHA256:            ${K04_REFERENCE_SHA256}"
...
COMMAND=(
  ...
  --input-file "${I04_PANEL}"
  --b06-panel-file "${B06_PANEL}"
  --i04-summary-file "${I04_SUMMARY}"
  --i04-sample-summary-file "${I04_SAMPLE_SUMMARY}"
  --i04-global-audit-file "${I04_GLOBAL_AUDIT}"
  --k04-reference-coefficients-file "${K04_REFERENCE_COEFFICIENTS}"
  ...
  --expected-i04-thresholds "${EXPECTED_I04_THRESHOLDS}"
...
{
  echo
  echo "** Step 1: Fit identical reverse GMM across the 21-point NPR grid"
  echo "------------------------------------------------------------"
  printf 'Command:'
  printf ' %q' "${COMMAND[@]}"
  printf '\n\n'
...
EXPECTED_OUTPUTS=(
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_coefficients.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_primary_summary.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_instrument_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_support_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_input_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_b06_join_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_legacy_flag_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_calendar_gap_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_k04_reproduction.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_model_failures.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_run_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_models.rds"
)

{
  echo
  echo "** Step 2: Verify K09 artifacts and hard QC"
  echo "------------------------------------------------------------"
} | tee -a "${LOG_FILE}"
...
QC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_qc.csv"
REPRO_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_k04_reproduction.csv"
SUMMARY_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_primary_summary.csv"
FAILURE_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_model_failures.csv"

if grep -q ',fail,' "${QC_FILE}" || grep -q ',fail$' "${QC_FILE}"; then
  echo "ERROR: K09 QC contains one or more hard failures." | tee -a "${LOG_FILE}" >&2
  exit 1
fi
if grep -q ',fail$' "${REPRO_FILE}" || grep -q ',fail,' "${REPRO_FILE}"; then
  echo "ERROR: K09 primary threshold does not reproduce K04." | tee -a "${LOG_FILE}" >&2
  exit 1
fi
...
if [[ "${SUMMARY_ROWS}" != "${EXPECTED_MAIN_THRESHOLDS}" ]]; then
  echo "ERROR: expected ${EXPECTED_MAIN_THRESHOLDS} K09 threshold-summary rows; observed ${SUMMARY_ROWS}" | tee -a "${LOG_FILE}" >&2
  exit 1
fi
...

{
  ...
  echo "Primary K04 reproduction:        PASS"
  echo "Primary summary:                 ${SUMMARY_FILE}"
  echo "Support diagnostics:             ${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_support_diagnostics.csv"
  echo "GMM diagnostics:                 ${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_diagnostics.csv"
  echo "Log file:                        ${LOG_FILE}"
  echo "============================================================"
} | tee -a "${LOG_FILE}"
