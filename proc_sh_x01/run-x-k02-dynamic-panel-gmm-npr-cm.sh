...
# ============================================================
# run-x-k02 v1: NPR-CM quality -> future Python velocity GMM
# ============================================================
#
# Purpose:
#   Apply the validated run-x-e01 dynamic-panel GMM design to the frozen H03
#   CM-NPR primary-threshold quality burden. This run tests whether issue stock
#   localized to CM-localized NPR-selected Python files predicts lower subsequent whole-
#   Python development velocity.
#
# Inputs:
#   - H03 long threshold panel:
#       repo_x01/run-x-h03/quality_cfun_npr_threshold_repo_month_panel.csv.gz
#   - H03 summary/sample/global-audit artifacts for provenance and hard QC.
#   - B06 authoritative whole-Python velocity/covariate panel:
#       repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv
#
# Join contract:
#   H03 intentionally omits log_lines_added_py_source. The R analysis joins
...
#   proc_script_x01/dynamic_panel_gmm_npr_cm-v1.R
#   proc_sh_x01/run-x-k02-dynamic-panel-gmm-npr-cm-v1.sh
#
# Canonical server filenames after deployment:
#   proc_script_x01/dynamic_panel_gmm_npr_cm.R
#   proc_sh_x01/run-x-k02-dynamic-panel-gmm-npr-cm.sh
#
# Run:
#   bash proc_sh_x01/run-x-k02-dynamic-panel-gmm-npr-cm.sh
# ============================================================
...
RUN_PREFIX="run-x-k02"
IMPLEMENTATION_VERSION="v1"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"
LOG_DIR="${LOG_DIR:-logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-dynamic-panel-gmm-npr-cm-${RUN_TS}.log}"

RSCRIPT_BIN="${RSCRIPT_BIN:-Rscript}"
R_SCRIPT="${R_SCRIPT:-proc_script_x01/dynamic_panel_gmm_npr_cm.R}"
...
EXPECTED_PRIMARY_FULL_SELECTED_FILES="${EXPECTED_PRIMARY_FULL_SELECTED_FILES:-7185}"
EXPECTED_PRIMARY_FULL_ISSUE_STOCK="${EXPECTED_PRIMARY_FULL_ISSUE_STOCK:-9198}"
EXPECTED_PRIMARY_SENSITIVITY_SELECTED_FILES="${EXPECTED_PRIMARY_SENSITIVITY_SELECTED_FILES:-7045}"
EXPECTED_PRIMARY_SENSITIVITY_ISSUE_STOCK="${EXPECTED_PRIMARY_SENSITIVITY_ISSUE_STOCK:-9158}"
...
for required_file in "${R_SCRIPT}" "${INPUT_FILE}" "${B06_PANEL_FILE}" "${H03_SUMMARY_FILE}" "${H03_SAMPLE_SUMMARY_FILE}" "${H03_GLOBAL_AUDIT_FILE}"; do
...
{
  echo "============================================================"
  echo "${RUN_LABEL}: NPR-CM quality -> future velocity GMM"
  ...
  echo "Analysis scope:                  AGCDetector_NPR / CM localized quality"
...
EXPECTED_OUTPUTS=(
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_coefficients.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_sample_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_instrument_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_model_specifications.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_threshold_input_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_b06_join_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_legacy_flag_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_numeric_coercion_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_calendar_gap_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_run_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_models.rds"
)
...
QC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_qc.csv"
...
COEFFICIENT_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_coefficients.csv"
DIAGNOSTIC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_diagnostics.csv"
INSTRUMENT_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_instrument_qc.csv"
SAMPLE_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_sample_qc.csv"
METADATA_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_run_metadata.csv"
MODEL_RDS="${OUTPUT_DIR}/dynamic_panel_gmm_npr_cm_models.rds"
...

{
  ...
  echo "Analysis scope:                  AGCDetector_NPR / CM localized quality -> velocity"
  echo "Primary threshold:               ${PRIMARY_THRESHOLD}"
