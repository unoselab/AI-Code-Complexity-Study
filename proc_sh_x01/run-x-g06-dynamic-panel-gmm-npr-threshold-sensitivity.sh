...
# ============================================================
# run-x-g06 v1: NPR threshold-sensitivity dynamic-panel GMM
# ============================================================
#
# Purpose:
#   Fit the same reverse-direction quality -> velocity GMM at every threshold
#   in the prespecified D03 21-point main NPR grid.
#
# Inputs:
#   - D03 long threshold quality panel and provenance/QC summaries
#   - B06 authoritative velocity/covariate panel
#   - E02 coefficient output for exact primary-threshold reproduction
#
# Outputs:
...
#
# Important:
#   This wrapper is derived from the validated E02 NPR GMM and E03 threshold-
#   sensitivity wrappers, but it is self-contained and does not call them.
#   The D03 legacy anchor 1.5183 is not part of the 21-point main sweep.
#
# Run:
#   bash proc_sh_x01/run-x-g06-dynamic-panel-gmm-npr-threshold-sensitivity.sh
# ============================================================
...
R_SCRIPT="${R_SCRIPT:-proc_script_x01/dynamic_panel_gmm_npr_threshold_sensitivity.R}"
D03_PANEL="${D03_PANEL:-repo_x01/run-x-d03/quality_fun_npr_threshold_repo_month_panel.csv.gz}"
D03_SUMMARY="${D03_SUMMARY:-repo_x01/run-x-d03/quality_fun_npr_threshold_summary.csv}"
D03_SAMPLE_SUMMARY="${D03_SAMPLE_SUMMARY:-repo_x01/run-x-d03/quality_fun_npr_threshold_sample_summary.csv}"
D03_GLOBAL_AUDIT="${D03_GLOBAL_AUDIT:-repo_x01/run-x-d03/quality_fun_npr_threshold_global_audit.csv}"
B06_PANEL="${B06_PANEL:-repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv}"
E02_REFERENCE_COEFFICIENTS="${E02_REFERENCE_COEFFICIENTS:-repo_x01/run-x-e02/dynamic_panel_gmm_npr_coefficients.csv}"
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/run-x-g06}"
...
EXPECTED_D03_THRESHOLDS="${EXPECTED_D03_THRESHOLDS:-22}"
...
EXPECTED_PRIMARY_SELECTED_FILES="${EXPECTED_PRIMARY_SELECTED_FILES:-13739}"
EXPECTED_PRIMARY_ISSUE_STOCK="${EXPECTED_PRIMARY_ISSUE_STOCK:-20306}"
...
for required_file in \
  "${R_SCRIPT}" "${D03_PANEL}" "${D03_SUMMARY}" "${D03_SAMPLE_SUMMARY}" \
  "${D03_GLOBAL_AUDIT}" "${B06_PANEL}" "${E02_REFERENCE_COEFFICIENTS}"; do
...
D03_SHA256="$(sha256sum "${D03_PANEL}" | awk '{print $1}')"
B06_SHA256="$(sha256sum "${B06_PANEL}" | awk '{print $1}')"
if [[ "${D03_SHA256}" != "${EXPECTED_D03_SHA256}" ]]; then
  echo "ERROR: D03 SHA256 mismatch." >&2
  echo "Expected: ${EXPECTED_D03_SHA256}" >&2
  echo "Observed: ${D03_SHA256}" >&2
  exit 1
fi
...
D03_SUMMARY_SHA256="$(sha256sum "${D03_SUMMARY}" | awk '{print $1}')"
D03_SAMPLE_SUMMARY_SHA256="$(sha256sum "${D03_SAMPLE_SUMMARY}" | awk '{print $1}')"
D03_GLOBAL_AUDIT_SHA256="$(sha256sum "${D03_GLOBAL_AUDIT}" | awk '{print $1}')"
E02_REFERENCE_SHA256="$(sha256sum "${E02_REFERENCE_COEFFICIENTS}" | awk '{print $1}')"

{
  echo "============================================================"
  echo "${RUN_LABEL}: NPR threshold-sensitivity quality -> velocity GMM"
  ...
  echo "Experiment role:                 21-threshold NPR GMM sweep"
  ...
  echo "D03 threshold panel:             ${D03_PANEL}"
  echo "D03 SHA256:                      ${D03_SHA256}"
  ...
  echo "E02 primary reproduction ref:    ${E02_REFERENCE_COEFFICIENTS}"
  echo "E02 reference SHA256:            ${E02_REFERENCE_SHA256}"
...
COMMAND=(
  ...
  --input-file "${D03_PANEL}"
  --b06-panel-file "${B06_PANEL}"
  --d03-summary-file "${D03_SUMMARY}"
  --d03-sample-summary-file "${D03_SAMPLE_SUMMARY}"
  --d03-global-audit-file "${D03_GLOBAL_AUDIT}"
  --e02-reference-coefficients-file "${E02_REFERENCE_COEFFICIENTS}"
  ...
  --expected-d03-thresholds "${EXPECTED_D03_THRESHOLDS}"
...
EXPECTED_OUTPUTS=(
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_coefficients.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_primary_summary.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_instrument_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_support_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_input_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_b06_join_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_legacy_flag_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_calendar_gap_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_e02_reproduction.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_model_failures.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_run_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_models.rds"
)

{
  echo
  echo "** Step 2: Verify G06 artifacts and hard QC"
  echo "------------------------------------------------------------"
} | tee -a "${LOG_FILE}"
...
QC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_qc.csv"
REPRO_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_e02_reproduction.csv"
SUMMARY_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_primary_summary.csv"
FAILURE_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_model_failures.csv"

if grep -q ',fail,' "${QC_FILE}" || grep -q ',fail$' "${QC_FILE}"; then
  echo "ERROR: G06 QC contains one or more hard failures." | tee -a "${LOG_FILE}" >&2
  exit 1
fi
if grep -q ',fail$' "${REPRO_FILE}" || grep -q ',fail,' "${REPRO_FILE}"; then
  echo "ERROR: G06 primary threshold does not reproduce E02." | tee -a "${LOG_FILE}" >&2
  exit 1
fi
...
if [[ "${SUMMARY_ROWS}" != "${EXPECTED_MAIN_THRESHOLDS}" ]]; then
  echo "ERROR: expected ${EXPECTED_MAIN_THRESHOLDS} G06 threshold-summary rows; observed ${SUMMARY_ROWS}" | tee -a "${LOG_FILE}" >&2
  exit 1
fi
...

{
  ...
  echo "Primary E02 reproduction:        PASS"
  echo "Primary summary:                 ${SUMMARY_FILE}"
  echo "Support diagnostics:             ${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_support_diagnostics.csv"
  echo "GMM diagnostics:                 ${OUTPUT_DIR}/dynamic_panel_gmm_npr_threshold_diagnostics.csv"
  echo "Log file:                        ${LOG_FILE}"
  echo "============================================================"
} | tee -a "${LOG_FILE}"
