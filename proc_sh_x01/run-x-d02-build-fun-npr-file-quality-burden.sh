...
# ================================================================
# run-x-d02 v2: Build file-level Quality x FUN-NPR burden dataset
# ================================================================
#
# Purpose:
#   1. Reuse the frozen A12 repo-month/file FUN-NPR measurement artifact.
...
#   4. Preserve every A12 Python file row, including zero-issue and no-FUN files.
#   5. Record B05 Python issue-bearing files outside the A12 NPR universe as
...
#
# This wrapper reuses the execution/provenance structure of the earlier B06
# quality-panel wrapper, but it is standalone and never calls an older wrapper.
#
# Exact join:
...
#   - A12 defines the NPR analysis file universe. A file absent from raw B05
#     issues receives zero burden only after B05 completeness checks pass.
#   - B05 issue-bearing Python files outside A12 are exported as explicit scope
#     exclusions because they cannot receive an NPR threshold classification.
#   - No NPR threshold is applied in D02. The frozen D01 threshold grid is
...
#
# Python program:
#   proc_script_x01/build_fun_npr_file_quality_burden.py
#
# Main outputs under repo_x01/run-x-d02/:
#   python_fun_file_quality_burden.csv.gz
#   python_fun_file_quality_snapshot_audit.csv
#   python_sonarqube_issue_files_outside_a12.csv
#   python_fun_file_quality_checks.csv
#   python_fun_file_quality_summary.csv
#   metadata.json
#
# Full run:
#   bash proc_sh_x01/run-x-d02-build-fun-npr-file-quality-burden.sh
#
# Self-test only:
#   SELF_TEST_ONLY=1 bash proc_sh_x01/run-x-d02-build-fun-npr-file-quality-burden.sh
#
# Useful overrides:
...
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-build-fun-npr-file-quality-burden-${RUN_TS}.log}"

PYTHON_BIN="${PYTHON_BIN:-python}"
ANALYSIS_SCRIPT="${ANALYSIS_SCRIPT:-proc_script_x01/build_fun_npr_file_quality_burden.py}"
...
OUTPUT_DIR="${OUTPUT_DIR:-repo_x01/run-x-d02}"
JOINED_OUTPUT="${JOINED_OUTPUT:-${OUTPUT_DIR}/python_fun_file_quality_burden.csv.gz}"
SNAPSHOT_AUDIT_OUTPUT="${SNAPSHOT_AUDIT_OUTPUT:-${OUTPUT_DIR}/python_fun_file_quality_snapshot_audit.csv}"
OUTSIDE_A12_OUTPUT="${OUTSIDE_A12_OUTPUT:-${OUTPUT_DIR}/python_sonarqube_issue_files_outside_a12.csv}"
CHECKS_OUTPUT="${CHECKS_OUTPUT:-${OUTPUT_DIR}/python_fun_file_quality_checks.csv}"
SUMMARY_OUTPUT="${SUMMARY_OUTPUT:-${OUTPUT_DIR}/python_fun_file_quality_summary.csv}"
METADATA_OUTPUT="${METADATA_OUTPUT:-${OUTPUT_DIR}/metadata.json}"

STRICT_EXPECTED_COUNTS="${STRICT_EXPECTED_COUNTS:-1}"
SELF_TEST_ONLY="${SELF_TEST_ONLY:-0}"

EXPECTED_A12_ROWS="${EXPECTED_A12_ROWS:-510297}"
EXPECTED_A12_UNIQUE_SNAPSHOT_FILES="${EXPECTED_A12_UNIQUE_SNAPSHOT_FILES:-494592}"
EXPECTED_A12_FINITE_FUN_ROWS="${EXPECTED_A12_FINITE_FUN_ROWS:-204508}"
...
for numeric_value in \
  "${EXPECTED_A12_ROWS}" \
  "${EXPECTED_A12_UNIQUE_SNAPSHOT_FILES}" \
  "${EXPECTED_A12_FINITE_FUN_ROWS}" \
  "${EXPECTED_SNAPSHOTS}" \
...
if [[ ! -f "${ANALYSIS_SCRIPT}" ]]; then
  echo "ERROR: required D02 Python program not found: ${ANALYSIS_SCRIPT}" >&2
  exit 1
fi
...
{
  echo "============================================================================"
  echo "${RUN_LABEL}: build file-level Quality x FUN-NPR burden dataset"
  ...
  echo "NPR metric preserved:            file_npr_fun_space_by_token_weighted"
  echo "Threshold application:           none (frozen D01 grid remains downstream)"
...
run_command_logged \
  "Step 1: Run D02 structural self-test" \
  "${PYTHON_BIN}" "${ANALYSIS_SCRIPT}" --self-test

run_command_logged \
  "Step 2: Compile D02 Python program" \
...

{
  ...
  echo "----------------------------------------------------------------------------"
  echo "A12 input SHA256:                $(sha256sum "${A12_FILE}" | awk '{print $1}')"
...
D02_COMMAND=(
  ...
  --outside-a12-file "${OUTSIDE_A12_OUTPUT}"
  --checks-file "${CHECKS_OUTPUT}"
  --summary-file "${SUMMARY_OUTPUT}"
  --metadata-file "${METADATA_OUTPUT}"
  --expected-a12-rows "${EXPECTED_A12_ROWS}"
  --expected-a12-unique-snapshot-files "${EXPECTED_A12_UNIQUE_SNAPSHOT_FILES}"
  --expected-a12-finite-fun-rows "${EXPECTED_A12_FINITE_FUN_ROWS}"
...
if [[ "${STRICT_EXPECTED_COUNTS}" == "1" ]]; then
  D02_COMMAND+=(--strict-expected-counts)
fi

run_command_logged "Step 4: Build file-level Quality x FUN-NPR burden dataset" "${D02_COMMAND[@]}"
...

{
  ...
  echo "Outside-A12 exclusion lines:     $(wc -l < "${OUTSIDE_A12_OUTPUT}")"
  ...
  echo "Outside-A12 scope exclusions:     ${OUTSIDE_A12_OUTPUT}"
  echo "QC checks:                        ${CHECKS_OUTPUT}"
  echo "Summary:                          ${SUMMARY_OUTPUT}"
  echo "Metadata:                         ${METADATA_OUTPUT}"
  echo "Log file:                         ${LOG_FILE}"
  echo "Next:                             D03 apply frozen D01 threshold grid and aggregate quality burden"
  echo "============================================================================"
} | tee -a "${LOG_FILE}"
