...
# ============================================================
# run-x-k04 v1: NPR-RF+CM quality -> future Python velocity GMM
# ============================================================
#
# Purpose:
#   Apply the validated run-x-e01 dynamic-panel GMM design to the frozen I04
#   RF+CM-NPR primary-threshold quality burden. This run tests whether issue stock
#   localized to RF+CM-localized NPR-selected Python files predicts lower subsequent whole-
#   Python development velocity.
#
# Inputs:
#   - I04 long threshold panel:
#       repo_x01/run-x-i04/quality_fun_cfun_npr_threshold_repo_month_panel.csv.gz
#   - I04 summary/sample/global-audit artifacts for provenance and hard QC.
#   - B06 authoritative whole-Python velocity/covariate panel:
#       repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv
#
# Join contract:
#   I04 intentionally omits log_lines_added_py_source. The R analysis joins
...
#   proc_script_x01/dynamic_panel_gmm_npr_rf_cm-v1.R
#   proc_sh_x01/run-x-k04-dynamic-panel-gmm-npr-rf-cm-v1.sh
#
# Canonical server filenames after deployment:
#   proc_script_x01/dynamic_panel_gmm_npr_rf_cm.R
#   proc_sh_x01/run-x-k04-dynamic-panel-gmm-npr-rf-cm.sh
#
# Run:
#   bash proc_sh_x01/run-x-k04-dynamic-panel-gmm-npr-rf-cm.sh
# ============================================================
...
RUN_PREFIX="run-x-k04"
IMPLEMENTATION_VERSION="v1"
RUN_LABEL="${RUN_PREFIX}-${IMPLEMENTATION_VERSION}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"
LOG_DIR="${LOG_DIR:-logs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_LABEL}-dynamic-panel-gmm-npr-rf-cm-${RUN_TS}.log}"

RSCRIPT_BIN="${RSCRIPT_BIN:-Rscript}"
R_SCRIPT="${R_SCRIPT:-proc_script_x01/dynamic_panel_gmm_npr_rf_cm.R}"
...
EXPECTED_PRIMARY_FULL_SELECTED_FILES="${EXPECTED_PRIMARY_FULL_SELECTED_FILES:-17071}"
EXPECTED_PRIMARY_FULL_ISSUE_STOCK="${EXPECTED_PRIMARY_FULL_ISSUE_STOCK:-14809}"
EXPECTED_PRIMARY_SENSITIVITY_SELECTED_FILES="${EXPECTED_PRIMARY_SENSITIVITY_SELECTED_FILES:-16885}"
EXPECTED_PRIMARY_SENSITIVITY_ISSUE_STOCK="${EXPECTED_PRIMARY_SENSITIVITY_ISSUE_STOCK:-14748}"
...
for required_file in "${R_SCRIPT}" "${INPUT_FILE}" "${B06_PANEL_FILE}" "${I04_SUMMARY_FILE}" "${I04_SAMPLE_SUMMARY_FILE}" "${I04_GLOBAL_AUDIT_FILE}"; do
...
{
  echo "============================================================"
  echo "${RUN_LABEL}: NPR-RF+CM quality -> future velocity GMM"
  ...
  echo "Analysis scope:                  AGCDetector_NPR / RF+CM localized quality"
...
EXPECTED_OUTPUTS=(
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_coefficients.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_diagnostics.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_sample_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_instrument_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_model_specifications.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_threshold_input_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_b06_join_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_legacy_flag_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_numeric_coercion_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_calendar_gap_audit.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_run_metadata.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_qc.csv"
  "${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_models.rds"
)
...
QC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_qc.csv"
if grep -q ',fail,' "${QC_FILE}" || grep -q ',fail$' "${QC_FILE}"; then
  echo "ERROR: K04 QC file contains one or more failed checks." | tee -a "${LOG_FILE}" >&2
  cat "${QC_FILE}" | tee -a "${LOG_FILE}" >&2
  exit 1
fi

COEFFICIENT_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_coefficients.csv"
DIAGNOSTIC_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_diagnostics.csv"
INSTRUMENT_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_instrument_qc.csv"
SAMPLE_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_sample_qc.csv"
METADATA_FILE="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_run_metadata.csv"
MODEL_RDS="${OUTPUT_DIR}/dynamic_panel_gmm_npr_rf_cm_models.rds"
...

{
  ...
  echo "Analysis scope:                  AGCDetector_NPR / RF+CM localized quality -> velocity"
  echo "Primary threshold:               ${PRIMARY_THRESHOLD}"
