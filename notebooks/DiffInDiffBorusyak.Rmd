...
build_setting_variant <- function(data, setting_label) {
    ...
    if (setting_label == "Orginal") {
        ...
        dt <- filter_with_matched_controls(dt, dt$high_confidence == TRUE)
    } else if (setting_label == "Low Confidence") {
        dt <- filter_with_matched_controls(dt, dt$high_confidence == FALSE)
    } else if (setting_label == "Cursor and Others") {
        dt <- filter_with_matched_controls(dt, dt$other_agents == TRUE)
    } else if (setting_label == "Only Cursor") {
        dt <- filter_with_matched_controls(dt, dt$other_agents == FALSE)
...
# Update setting labels with counts
plot_dyn$setting_label <- settings_with_counts[plot_dyn$setting]
plot_dyn$setting_label <- factor(plot_dyn$setting_label, levels = settings_with_counts)
...
dynamic_plot <- ggplot(plot_dyn, aes(x = time, y = estimate, color = setting_label)) +
    ...
    scale_color_manual(values = setNames(c(
        ...
        "#E69F00",  # Active Months - Orange
        "#56B4E9",  # Very Active - Sky blue
        "#009E73",  # Truncated - Green
        "#F0E442",  # High Adoption - Yellow
        "#0072B2",  # High Confidence - Blue
        "#D55E00",  # Low Confidence - Vermillion
        "#CC79A7",  # Cursor and Others - Pink
        "#999999"   # Only Cursor - Grey
    ), settings_with_counts)) +
    labs(
        x = "Months Relative to Cursor Adoption",
        y = "Treatment Effect",
        color = "Setting",
    ...
    facet_wrap(~ outcome_label, scales = "free_y", nrow = 5, ncol = 1)
...
plot_lang$setting_label <- factor(plot_lang$setting_label, levels = lang_settings_with_counts)
