...
```{r dynamic_effects, fig.width=16, fig.height=2.5}
...
extract_dynamic_effects <- function(results, method_name, variables = NULL) {
    ...
    for (var in vars_to_use) {
        if (method_name == "Callaway and Sant'Anna") {
            ...
            # Drop event-time -1 to avoid anticipation period
            df <- df[df$time != -1, , drop = FALSE]
            effects_list[[var]] <- df
        } else if (method_name == "Borusyak et al.") {
            result <- results[[var]]
...
dynamic_effects <- dynamic_effects %>%
    mutate(
        ...
        # Create linetype variable for scale_linetype_identity
        linetype_sig = ifelse(significant, "solid", "dotted")
    )

# Define dodge position once
pos <- position_dodge(width = 0.5)

# Create comparison plot (top panels)
dynamic_plot <- ggplot(dynamic_effects, aes(x = time, y = estimate, color = method, group = method)) +
    ...
    scale_color_manual(values = c(
        "Two Way Fixed Effects" = "#CC79A7",   # Pink
        "Borusyak et al." = "#000000",         # Black
        "Callaway and Sant'Anna" = "#0072B2"   # Blue
    )) +
...
ggsave("../plots/dynamic_effects.pdf",
  plot = dynamic_plot, width = 16, height = 2.5,
  device = CairoPDF, family = "DejaVu Sans")
```

## Pre-Trend Tests (Heteroskedasticity- and Cluster-Robust Wald)

```{r pretrend_tests}
# Helper: compute Wald test given estimates and a variance-covariance matrix
compute_wald <- function(beta_vec, vcov_mat) {
    # Drop any NA coefficients
    valid <- which(!is.na(beta_vec))
    beta <- as.numeric(beta_vec[valid])
    V <- as.matrix(vcov_mat[valid, valid, drop = FALSE])

    if (length(beta) == 0) {
        return(list(stat = NA_real_, df = 0L, p_value = NA_real_))
    }

    # Try inversion; fall back to generalized inverse if singular
    stat <- tryCatch({
        as.numeric(t(beta) %*% solve(V, beta))
    }, error = function(e) {
        as.numeric(t(beta) %*% MASS::ginv(V) %*% beta)
    })

    df <- length(beta)
    p_value <- 1 - pchisq(stat, df = df)
    list(stat = stat, df = df, p_value = p_value)
}

# TWFE: fit event-study with leads/lags and test all leads == 0
twfe_pretrend_test <- function(outcome_var, data) {
    ...
    V_full <- vcov(model, cluster = ~ repo_name_fac)
    V <- V_full[lead_names, lead_names, drop = FALSE]
...
# Callaway & Sant'Anna: use dynamic AGGTE; prefer full covariance if available
callaway_pretrend_test <- function(attgt_obj) {
    dyn <- aggte(attgt_obj, type = "dynamic", min_e = -6, max_e = 6)
    pre_idx <- which(dyn$egt < 0)
    beta <- dyn$att.egt[pre_idx]

    # Try to use provided covariance across event-times if available
    V_egt <- NULL
    if (!is.null(dyn$V_egt)) V_egt <- dyn$V_egt
    if (is.null(V_egt) && !is.null(dyn$V.egt)) V_egt <- dyn$V.egt

    if (!is.null(V_egt)) {
        V <- as.matrix(V_egt[pre_idx, pre_idx, drop = FALSE])
    } else {
        V <- diag((dyn$se.egt[pre_idx])^2, nrow = length(pre_idx))
    }
...
# Borusyak et al.: use horizon results; build diagonal V from robust SEs
borusyak_pretrend_test <- function(bjs_dyn_df) {
    dyn_df <- bjs_dyn_df[bjs_dyn_df$term != "treat", ]
    dyn_df$time_num <- suppressWarnings(as.numeric(gsub("[\u2212\u2013\u2014]", "-", as.character(dyn_df$term))))
    pre_df <- dyn_df[!is.na(dyn_df$time_num) & dyn_df$time_num < 0, ]
    beta <- pre_df$estimate

    if ("std.error" %in% names(pre_df)) {
        se <- pre_df$std.error
    } else if (all(c("conf.low", "conf.high") %in% names(pre_df))) {
        se <- (pre_df$conf.high - pre_df$conf.low) / (2 * 1.96)
    } else {
        stop("Standard errors not found in did_imputation output.")
    }
...
# Run tests for all outcomes and methods
pretrend_results <- list()

for (var in outcome_vars) {
    # TWFE
    pretrend_results[[paste0(var, "_TWFE")]] <- twfe_pretrend_test(var, panel_data)
    pretrend_results[[paste0(var, "_TWFE")]]$outcome <- var

    # Callaway
    ct <- callaway_pretrend_test(callaway_results[[var]])
    ct$outcome <- var
    pretrend_results[[paste0(var, "_Callaway")]] <- ct

    # Borusyak
    bt <- borusyak_pretrend_test(borusyak_dynamic[[var]])
    bt$outcome <- var
    pretrend_results[[paste0(var, "_BJS")]] <- bt
}

pretrend_table <- bind_rows(pretrend_results) %>%
    mutate(
        outcome = factor(outcome, levels = outcome_vars, labels = outcome_names),
        p_value = as.numeric(p_value),
        stat = as.numeric(stat),
        decision_5pct = ifelse(!is.na(p_value) & p_value < 0.05, "Reject H0", "Fail to Reject")
    ) %>%
    arrange(outcome, method) %>%
    select(outcome, method, num_pre, df, stat, p_value, decision_5pct)

kable(pretrend_table,
      col.names = c("Outcome", "Estimator", "# Pre-Periods", "df",
                    "Chi-square", "p-value", "Decision (5%)"),
      caption = "Pre-Trend Tests (Heteroskedasticity- & Cluster-Robust Wald)",
      digits = c(NA, NA, 0, 0, 3, 3, NA),
      align = c("l", "l", rep("r", 4), "l")) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
