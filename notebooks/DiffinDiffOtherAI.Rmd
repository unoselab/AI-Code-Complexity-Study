---
title: "The Impact of Cursor Adoption on Velocity and Quality - Robustness Check on Other AI Repos using Borusyak et al. (2021) DiD Approach"
...
panel_data <- panel_data %>%
    ...
    ungroup()
panel_data_agent_modified <- panel_data[panel_data$other_agents == TRUE | is.na(panel_data$other_agents), ]
panel_data_all <- panel_data
panel_data_agent_notmodified <- panel_data[panel_data$other_agents == FALSE | is.na(panel_data$other_agents), ]
```
...
borusyak_results_agent_modified <- list()
borusyak_results_all <- list()
borusyak_results_agent_notmodified <- list()

for (var in outcome_vars) {
    borusyak_results_agent_modified[[var]] <- run_borusyak(var, panel_data_agent_modified)
    borusyak_results_all[[var]] <- run_borusyak(var, panel_data_all)
    borusyak_results_agent_notmodified[[var]] <- run_borusyak(var, panel_data_agent_notmodified)
}
```
...
static_effects <- rbind(
    extract_static_effects(borusyak_results_agent_modified, "Borusyak Agent Modified"),
    extract_static_effects(borusyak_results_all, "Borusyak All"),
    extract_static_effects(borusyak_results_agent_notmodified, "Borusyak Agent Not Modified")
)
...
static_table_wide <- static_table %>%
    ...
    select(
        outcome,
        `estimate_sig_Borusyak Agent Modified`, `se_formatted_Borusyak Agent Modified`,
        `estimate_sig_Borusyak All`, `se_formatted_Borusyak All`,
        `estimate_sig_Borusyak Agent Not Modified`, `se_formatted_Borusyak Agent Not Modified`
    )
...
kable(static_table_wide,
    ...
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                  full_width = FALSE,
                  font_size = 12) %>%
    add_header_above(c(" " = 1, "Borusyak Cursor and other Agents" = 2, "Borusyak All" = 2, "Borusyak Only Cursor" = 2)) %>%
...
extract_dynamic_effects <- function(results, method_name, variables = NULL) {
    ...
    for (var in vars_to_use) {
        if (method_name %in% c("Original", "Only Cursor", "Cursor and Others")) {
            result <- results[[var]]
...
borusyak_dynamic_agent_modified <- list()
borusyak_dynamic_all <- list()
borusyak_dynamic_agent_notmodified <- list()

for (var in outcome_vars) {
    borusyak_dynamic_agent_modified[[var]] <- did_imputation(
        data = panel_data_agent_modified,
        yname = var,
        gname = "event",
        tname = "time",
        idname = "repo_name",
    ...
    borusyak_dynamic_agent_notmodified[[var]] <- did_imputation(
        data = panel_data_agent_notmodified,
        yname = var,
        gname = "event",
        tname = "time",
        idname = "repo_name",
...
dynamic_effects <- rbind(
    ...
    extract_dynamic_effects(borusyak_dynamic_agent_notmodified, "Only Cursor"),
    extract_dynamic_effects(borusyak_dynamic_agent_modified, "Cursor and Others")
)
...
dynamic_plot <- ggplot(dynamic_effects, aes(x = time, y = estimate, color = method)) +
    ...
    scale_color_manual(values = c(
        ...
        "Only Cursor" = "#009E73",         # Colorblind-friendly green
        "Cursor and Others" = "#E69F00"    # Colorblind-friendly orange
    )) +
...
ggsave("../plots/dynamic_effects_other_ai_robustness.pdf",
