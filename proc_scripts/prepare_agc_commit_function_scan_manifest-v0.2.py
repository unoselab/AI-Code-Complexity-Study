...
For each repository, the script resolves the most plausible analysis history
reference (normally the current/default branch), enumerates each reachable
commit exactly once, groups commits by committer month, and compares the
reconstructed monthly count and latest commit with the validated panel.
...
def candidate_history_refs(repo_dir: Path) -> list[str]:
    candidates: list[str] = []
    ...
    for candidate in [
        ...
        tip = resolve_ref_commit(repo_dir, text) if text else ""
        if text and tip and tip not in seen_tips:
...
def load_history(repo_dir: Path, ref: str) -> tuple[CommitRecord, ...]:
    output = require_git_ok(
        ...
        f"git log failed for {ref}",
    )

    records: list[CommitRecord] = []
    for history_order, raw_line in enumerate(output.splitlines(), start=1):
        fields = raw_line.split(FIELD_SEPARATOR)
        # require_git_ok() strips trailing control characters from the final
        # output line, so the root commit may omit the empty parent field.
        if len(fields) == 2:
            fields.append("")
        if len(fields) != 3:
            raise RuntimeError(
                f"Unexpected git log record for {ref}: {raw_line!r}"
            )
        ...
        if not valid_commit_text(commit):
            raise RuntimeError(f"Invalid commit in git log for {ref}: {commit!r}")
        records.append(
            CommitRecord(
                commit=commit,
...
def group_history(
    ...
    # The original analyzer retained the first commit encountered when two
    # commits had exactly the same maximum committer timestamp because it used
    # a strict greater-than comparison for latest_commit_time.
    ...
    for month, month_records in grouped.items():
        # Present commit_order from oldest to newest. history_order is used as a
        # deterministic tie breaker while preserving the original traversal.
        ordered = sorted(
            month_records,
            key=lambda record: (
...
def evaluate_history_candidate(
    ...
    records = load_history(repo_dir, ref)
    ...
    return HistoryCandidate(
        ref=ref,
        priority=priority,
        tip_commit=resolve_ref_commit(repo_dir, ref),
        records=records,
        by_commit=by_commit,
        by_month=by_month,
...
def choose_history_candidate(
    ...
    refs = candidate_history_refs(repo_dir)
    if not refs:
        return None, [], "no_history_ref"
    ...
    selection_status = "perfect_panel_match" if perfect else "best_available_ref"
...
def self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="agc-commit-scan-v3-") as temp_dir:
        ...
        records = load_history(repo, "HEAD")
