...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for generated plots. Defaults to OUTPUT_DIR/plots.",
    )
    parser.add_argument(
        "--plot-formats",
        nargs="+",
    ...
    parser.add_argument(
        ...
        type=int,
        default=160,
        help="DPI for raster plot formats. Default: 160.",
    )
    parser.add_argument(
        "--no-plots",
        action="store_true",
        help="Skip plot generation and write only CSV/JSON outputs.",
    )
    parser.add_argument(
        "--thresholds",
        type=int,
...
def save_figure(
    ...
    """Save one figure in all requested formats and return output paths."""
    plot_dir.mkdir(parents=True, exist_ok=True)
    outputs: list[Path] = []
    for plot_format in formats:
        path = plot_dir / f"{stem}.{plot_format}"
...
def plot_descriptive_summary(
    ...
    """Plot median function line span for each source/prediction group."""
    frame = descriptive.copy()
    frame["group_label"] = (
        frame["dataset_source"].astype(str)
        + " / "
        + frame["prediction_group"].astype(str)
    )
    frame = frame.sort_values(["dataset_source", "prediction_group"])

    figure, axis = plt.subplots(figsize=(10, 5.5))
    axis.bar(frame["group_label"], frame["p50"])
    axis.set_title("Median Function Source-Line Span by Group")
    axis.set_xlabel("Dataset source / prediction group")
    axis.set_ylabel("Median inclusive source-line span")
...
def plot_exact_distribution(
    exact_distribution: pd.DataFrame,
    primary_min_lines: int,
    plot_dir: Path,
    formats: list[str],
    dpi: int,
) -> list[Path]:
    """Plot exact line-size frequencies through the weighted 99th percentile."""
    frame = exact_distribution.sort_values("function_line_count").copy()
    if frame.empty:
        raise ValueError("Exact distribution is empty; cannot create plot")

    p99_candidates = frame.loc[frame["cumulative_share_le"].ge(0.99)]
    if p99_candidates.empty:
        max_line = int(frame["function_line_count"].max())
    else:
        max_line = int(p99_candidates.iloc[0]["function_line_count"])
    plot_frame = frame.loc[frame["function_line_count"].le(max_line)]

    figure, axis = plt.subplots(figsize=(10, 5.5))
    axis.plot(
        plot_frame["function_line_count"],
        plot_frame["events"],
        marker="o",
        markersize=2,
    )
    axis.axvline(
        ...
        linestyle="--",
        linewidth=1.2,
        label=f"Primary minimum = {primary_min_lines}",
    )
    axis.set_yscale("log")
    axis.set_title(
        f"Exact Function Line-Span Distribution (through weighted P99 = {max_line})"
    )
    axis.set_xlabel("Inclusive function source-line span")
    axis.set_ylabel("Function events (log scale)")
...
def plot_grouped_distribution(
    ...
    """Plot AGC-like and HWC-like event counts by line-size bin."""
    frame = grouped_distribution.copy()
    labels = frame["line_size_bin"].astype(str)

    figure, axis = plt.subplots(figsize=(10, 5.5))
    axis.bar(labels, frame["hwc_events"], label="HWC-like events")
    axis.bar(
        labels,
        frame["agc_events"],
        bottom=frame["hwc_events"],
        label="AGC-like events",
    )
    axis.set_title("Function Events by Source-Line-Span Bin")
    axis.set_xlabel("Inclusive function source-line span")
    axis.set_ylabel("Function events")
...
def plot_threshold_summary(
    ...
    """Plot retained event share and retained AGC ratio by threshold."""
    frame = threshold_summary.sort_values("min_function_lines")
    figure, axis = plt.subplots(figsize=(9, 5.5))
    axis.plot(
        frame["min_function_lines"],
        frame["retained_share"],
        marker="o",
        label="Retained event share",
    )
    axis.plot(
        frame["min_function_lines"],
        frame["retained_agc_ratio"],
        marker="o",
        label="AGC ratio among retained events",
    )
    axis.axvline(
        primary_min_lines,
        linestyle="--",
    ...
    axis.set_title("Threshold Sensitivity for Function Line Span")
    axis.set_xlabel("Minimum retained function lines")
...
def plot_by_group_retained_share(
    frame: pd.DataFrame,
    group_column: str,
    title: str,
    stem: str,
    ...
    """Plot retained event share by threshold for one grouping column."""
    figure, axis = plt.subplots(figsize=(9, 5.5))
    for group_name, group in frame.groupby(group_column, dropna=False):
        group = group.sort_values("min_function_lines")
        axis.plot(
            group["min_function_lines"],
            group["retained_share"],
            marker="o",
    ...
    axis.set_xlabel("Minimum retained function lines")
    axis.set_ylabel("Retained event share")
    axis.grid(alpha=0.25)
    axis.legend()
    figure.tight_layout()
    return save_figure(figure, plot_dir, stem, formats, dpi)


def plot_repo_month_impact(
    ...
    """Plot retained and newly zero-eligible repository-month counts."""
    frame = preview.sort_values("min_function_lines")
    figure, axis = plt.subplots(figsize=(9, 5.5))
    axis.plot(
        frame["min_function_lines"],
        frame["retained_event_positive_repo_months"],
        marker="o",
        label="Retained event-positive repo-months",
    )
    axis.plot(
        frame["min_function_lines"],
        frame["newly_zero_eligible_repo_months"],
        marker="o",
        label="Newly zero-eligible repo-months",
    )
    axis.axvline(
        primary_min_lines,
        linestyle="--",
    ...
    axis.set_title("Repository-Month Impact of Function Line Thresholds")
    axis.set_xlabel("Minimum retained function lines")
    axis.set_ylabel("Repository-months")
    axis.grid(alpha=0.25)
    axis.legend()
    figure.tight_layout()
    return save_figure(
...
def create_plots(
    ...
    by_period: pd.DataFrame | None,
    repo_month_preview: pd.DataFrame,
    primary_min_lines: int,
    plot_dir: Path,
    formats: list[str],
    dpi: int,
) -> dict[str, list[str]]:
    """Create one simple graph for every main diagnostic table."""
    plot_outputs: dict[str, list[Path]] = {}
    plot_outputs["descriptive_summary"] = plot_descriptive_summary(
        descriptive, plot_dir, formats, dpi
    )
    plot_outputs["exact_distribution"] = plot_exact_distribution(
        exact_distribution, primary_min_lines, plot_dir, formats, dpi
    )
    plot_outputs["grouped_distribution"] = plot_grouped_distribution(
        grouped_distribution, plot_dir, formats, dpi
    )
    plot_outputs["threshold_summary"] = plot_threshold_summary(
        threshold_summary, primary_min_lines, plot_dir, formats, dpi
    )
    plot_outputs["by_treatment"] = plot_by_group_retained_share(
        by_treatment,
        "dataset_source",
        "Retained Function Events by Treatment Group",
    ...
    if by_period is not None:
        plot_outputs["by_period"] = plot_by_group_retained_share(
            by_period,
            "treatment_period",
            "Retained Function Events by Treatment Period",
    ...
    plot_outputs["repo_month_impact_preview"] = plot_repo_month_impact(
        repo_month_preview, primary_min_lines, plot_dir, formats, dpi
    )
    return {
        name: [str(path) for path in paths]
        for name, paths in plot_outputs.items()
    }
...
def analyze(
    ...
    if generate_plots:
        resolved_plot_dir = plot_dir if plot_dir is not None else output_dir / "plots"
        resolved_plot_formats = plot_formats if plot_formats is not None else ["png", "pdf"]
        if plot_dpi < 1:
        ...
        plot_outputs = create_plots(
            ...
            by_period=by_period,
            repo_month_preview=repo_month_preview,
            primary_min_lines=primary_min_lines,
        ...
        expected_plot_groups = {
            ...
            "threshold_summary",
            "by_treatment",
            "repo_month_impact_preview",
        }
        if by_period is not None:
            expected_plot_groups.add("by_period")
        add_check(
            checks,
            "all_expected_plot_groups_created",
            expected_plot_groups.issubset(plot_outputs),
            {
                "expected": sorted(expected_plot_groups),
                "created": sorted(plot_outputs),
            },
        )
        plot_paths = [
            Path(path)
            for paths in plot_outputs.values()
            for path in paths
        ]
        add_check(
            checks,
            "all_plot_files_nonempty",
            all(path.is_file() and path.stat().st_size > 0 for path in plot_paths),
            {
                "plot_file_count": len(plot_paths),
                "missing_or_empty": [
                    str(path)
    ...
    summary = {
        ...
        "plot_generation_enabled": generate_plots,
        "plot_dir": (
            str(plot_dir if plot_dir is not None else output_dir / "plots")
            if generate_plots
            else None
        ),
        "plot_formats": plot_formats if plot_formats is not None else ["png", "pdf"],
...
def self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="agc-line-size-") as temp_dir:
        ...
        if len(plot_files) != 7:
            raise AssertionError(
                f"Self-test expected 7 PNG plots, found {len(plot_files)}"
            )
        if any(path.stat().st_size == 0 for path in plot_files):
            raise AssertionError("Self-test found an empty plot file")
...
def main() -> int:
    ...
    print(f"Plot directory:              {plot_dir if not args.no_plots else '<disabled>'}")
    print(f"Plot formats:                {args.plot_formats if not args.no_plots else '<disabled>'}")
    ...
    if summary["plot_generation_enabled"]:
        plot_file_count = sum(
            len(paths) for paths in summary["plot_outputs"].values()
        )
        print(f"Plot files created:             {plot_file_count}")
        print(f"Plot directory:                 {summary['plot_dir']}")
