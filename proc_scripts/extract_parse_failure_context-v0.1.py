...
writes compact error-line context for manual review. It does not read function
roles, AGC/HWC labels, DiD panels, treatment effects, or confidence intervals.
"""
...
def parse_source(payload: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        result["error_line"] = int(getattr(error, "lineno", 0) or 0)
        result["error_offset"] = int(getattr(error, "offset", 0) or 0)
        result["error_end_line"] = int(getattr(error, "end_lineno", 0) or 0)
        result["error_end_offset"] = int(getattr(error, "end_offset", 0) or 0)
    return result
