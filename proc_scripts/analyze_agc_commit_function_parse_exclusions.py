...
- top-treatment-repository sensitivity by treatment period and event time
...
COUNT_COLUMNS = ADDITIVE_COUNT_COLUMNS + UNIQUE_COUNT_COLUMNS
...
class AnalysisResult:
    ...
    top_treatment_repo_sensitivity_by_period: pd.DataFrame
    top_treatment_repo_sensitivity_by_event_time: pd.DataFrame
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--top-treatment-repo-cutoffs",
        default="1,2,3,5,10",
        help=(
            "Comma-separated treatment-repository cutoffs for period and "
...
def normalize_panel(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    # The source panel uses dataset_source as the time-invariant cohort marker.
    # Its is_treatment field is time varying and becomes 1 only from the event
    # month onward, so it must not be used to define treatment membership.
    panel["treatment_group"] = (
        panel["dataset_source"].eq("treatment").astype(int)
    )
    ...
    panel["active_treatment"] = panel["is_treatment"]
    ...
    panel["treatment_period"] = "control"
    treatment_mask = panel["treatment_group"].eq(1)
    panel.loc[treatment_mask & panel["time_to_event"].lt(0), "treatment_period"] = "pre_event"
    panel.loc[treatment_mask & panel["time_to_event"].eq(0), "treatment_period"] = "event_month"
    panel.loc[treatment_mask & panel["time_to_event"].gt(0), "treatment_period"] = "post_event"
    panel.loc[treatment_mask & panel["time_to_event"].isna(), "treatment_period"] = "unknown_event_time"
...
def enrich_attempts_with_panel_metadata(
        ...
        rows.append(
            {
                ...
                "treatment_group": int(
                    pd.to_numeric(group.get("treatment_group"), errors="coerce")
                    .fillna(0)
                    .max()
                ),
                "is_treatment": int(
...
def aggregate_metrics(
    ...
    enriched_errors: pd.DataFrame | None = None,
) -> pd.DataFrame:
    """Aggregate additive metrics and recompute non-additive unique counts.

    Unique blob counts must never be summed across repository-month rows because
    the same historical blob revision can be observed in multiple months.
    """
    if frame.empty:
    ...
    if attempts is not None and not attempts.empty:
        attempt_unique = (
            attempts.groupby(list(group_columns), dropna=False)
            .agg(unique_blob_revisions_attempted=("blob_revision_key", "nunique"))
            .reset_index()
        )
        result = result.merge(
            attempt_unique,
        ...
        result["unique_blob_revisions_attempted"] = 0

    if enriched_errors is not None and not enriched_errors.empty:
        exclusion_unique = (
            enriched_errors.groupby(list(group_columns), dropna=False)
            .agg(unique_excluded_blob_revisions=("blob_revision_key", "nunique"))
            .reset_index()
        )
        result = result.merge(
            exclusion_unique,
        ...
        result["unique_excluded_blob_revisions"] = 0

    for column in UNIQUE_COUNT_COLUMNS:
        result[column] = (
    ...
    result["current_file_parse_exclusion_rate"] = safe_divide(
        result["current_file_parse_exclusions"],
        result["current_blob_attempts"],
    )
    result["parent_file_parse_exclusion_rate"] = safe_divide(
        result["parent_file_parse_exclusions"],
        result["parent_blob_attempts"],
    )
    result["blob_parse_exclusion_record_rate"] = safe_divide(
        result["parse_exclusion_records"],
        result["blob_parse_attempts"],
    )
...
def build_by_repo(
    ...
    metadata = (
        ...
        .agg(
            treatment_group=("treatment_group", "max"),
            active_treatment=("active_treatment", "max"),
...
def build_by_treatment_period(
    repo_month: pd.DataFrame,
    attempts: pd.DataFrame,
    enriched_errors: pd.DataFrame,
) -> pd.DataFrame:
    def masks(frame: pd.DataFrame) -> list[tuple[str, pd.Series]]:
        treatment = frame["treatment_group"].eq(1)
        event_time = pd.to_numeric(frame["time_to_event"], errors="coerce")
        return [
            ("overall", pd.Series(True, index=frame.index)),
            ("control_all", frame["treatment_group"].eq(0)),
            ("treatment_all", treatment),
            ("treatment_pre_event", treatment & event_time.lt(0)),
            ("treatment_event_month", treatment & event_time.eq(0)),
            ("treatment_post_event", treatment & event_time.gt(0)),
            ("treatment_event_and_post", treatment & event_time.ge(0)),
            ("treatment_unknown_event_time", treatment & event_time.isna()),
        ]

    repo_masks = dict(masks(repo_month))
    attempt_masks = dict(masks(attempts))
    error_masks = dict(masks(enriched_errors)) if not enriched_errors.empty else {}

    rows: list[pd.DataFrame] = []
    for label, repo_mask in repo_masks.items():
        subset = repo_month.loc[repo_mask].copy()
        attempt_subset = attempts.loc[attempt_masks[label]].copy()
        if enriched_errors.empty:
            error_subset = enriched_errors.copy()
        else:
            error_subset = enriched_errors.loc[error_masks[label]].copy()

        if subset.empty:
            row: dict[str, Any] = {"treatment_period": label}
        ...
        subset["analysis_period"] = label
        attempt_subset["analysis_period"] = label
        error_subset["analysis_period"] = label
        aggregate = aggregate_metrics(
            subset,
            ["analysis_period"],
            attempt_subset,
            error_subset,
        ).rename(columns={"analysis_period": "treatment_period"})
    ...
    rate_specs = [
        ...
        (
            "current_file_parse_exclusion_rate",
            "current_file_parse_exclusions",
            "current_blob_attempts",
        ),
        (
            "parent_file_parse_exclusion_rate",
            "parent_file_parse_exclusions",
            "parent_blob_attempts",
        ),
        (
            "blob_parse_exclusion_record_rate",
            "parse_exclusion_records",
            "blob_parse_attempts",
        ),
        (
    ...
    for rate_column, numerator, denominator in rate_specs:
        result[rate_column] = safe_divide(result[numerator], result[denominator])
    return result


def build_by_event_time(
    repo_month: pd.DataFrame,
    attempts: pd.DataFrame,
    enriched_errors: pd.DataFrame,
) -> pd.DataFrame:
    def select(frame: pd.DataFrame) -> pd.DataFrame:
        event_time = pd.to_numeric(frame["time_to_event"], errors="coerce")
        subset = frame.loc[
            frame["treatment_group"].eq(1) & event_time.notna()
        ].copy()
        subset["time_to_event"] = pd.to_numeric(
            subset["time_to_event"], errors="coerce"
        )
        return subset

    repo_subset = select(repo_month)
    if repo_subset.empty:
        return pd.DataFrame(columns=["time_to_event", *COUNT_COLUMNS])
    attempt_subset = select(attempts)
    error_subset = select(enriched_errors) if not enriched_errors.empty else enriched_errors.copy()
...
def build_top_repo_sensitivity(
    ...
    def add_repo_key(frame: pd.DataFrame) -> pd.DataFrame:
        result = frame.copy()
        result["repo_key"] = result["dataset_source"] + "|" + result["repo_name"]
        return result

    repo_month = add_repo_key(repo_month)
    attempts = add_repo_key(attempts)
    enriched_errors = add_repo_key(enriched_errors) if not enriched_errors.empty else enriched_errors.copy()

    rows: list[pd.DataFrame] = []
    for cutoff in cutoffs:
        removed = set(ranked[:cutoff])
        for group_label in ["overall", "control", "treatment"]:
            if group_label == "overall":
                repo_group = pd.Series(True, index=repo_month.index)
                attempt_group = pd.Series(True, index=attempts.index)
                error_group = (
                    pd.Series(True, index=enriched_errors.index)
                    if not enriched_errors.empty
                    else pd.Series(dtype=bool)
                )
            else:
                target = 1 if group_label == "treatment" else 0
                repo_group = repo_month["treatment_group"].eq(target)
                attempt_group = attempts["treatment_group"].eq(target)
                error_group = (
                    enriched_errors["treatment_group"].eq(target)
                    if not enriched_errors.empty
                    else pd.Series(dtype=bool)
                )

            subset = repo_month.loc[
                repo_group & ~repo_month["repo_key"].isin(removed)
            ].copy()
            if subset.empty:
                continue
            attempt_subset = attempts.loc[
                attempt_group & ~attempts["repo_key"].isin(removed)
            ].copy()
            if enriched_errors.empty:
                error_subset = enriched_errors.copy()
            else:
                error_subset = enriched_errors.loc[
                    error_group & ~enriched_errors["repo_key"].isin(removed)
                ].copy()

            subset["analysis_group"] = group_label
            attempt_subset["analysis_group"] = group_label
            error_subset["analysis_group"] = group_label
            aggregate = aggregate_metrics(
                subset,
                ["analysis_group"],
                attempt_subset,
...
def treatment_repository_ranking(by_repo: pd.DataFrame) -> list[str]:
    """Return treatment repositories ranked by parse-exclusion records."""
    ranked = by_repo.loc[
        by_repo["dataset_source"].eq("treatment")
        & by_repo["parse_exclusion_records"].gt(0),
        ["repo_name", "parse_exclusion_records"],
    ].copy()
    ranked = ranked.sort_values(
        ["parse_exclusion_records", "repo_name"],
        ascending=[False, True],
    )
    return ranked["repo_name"].tolist()


def add_repository_key(frame: pd.DataFrame) -> pd.DataFrame:
    result = frame.copy()
    result["repo_key"] = result["dataset_source"] + "|" + result["repo_name"]
    return result


def build_top_treatment_repo_sensitivity_by_period(
    repo_month: pd.DataFrame,
    attempts: pd.DataFrame,
    enriched_errors: pd.DataFrame,
    by_repo: pd.DataFrame,
    ...
    """Recompute treatment-period rates after removing error-heavy treatments."""
    ranked_names = treatment_repository_ranking(by_repo)
    ranked_keys = [f"treatment|{name}" for name in ranked_names]

    repo_with_key = add_repository_key(repo_month)
    attempts_with_key = add_repository_key(attempts)
    errors_with_key = (
        add_repository_key(enriched_errors)
        if not enriched_errors.empty
        else enriched_errors.copy()
    )
    ...
    period_labels = [
        ...
        "treatment_event_and_post",
    ]
    rows: list[pd.DataFrame] = []
    for cutoff in cutoffs:
        removed_keys = set(ranked_keys[:cutoff])
        removed_names = ranked_names[:cutoff]

        repo_subset = repo_with_key.loc[
            repo_with_key["treatment_group"].eq(1)
            & ~repo_with_key["repo_key"].isin(removed_keys)
        ].drop(columns=["repo_key"])
        attempt_subset = attempts_with_key.loc[
            attempts_with_key["treatment_group"].eq(1)
            & ~attempts_with_key["repo_key"].isin(removed_keys)
        ].drop(columns=["repo_key"])
        if enriched_errors.empty:
            error_subset = enriched_errors.copy()
        else:
            error_subset = errors_with_key.loc[
                errors_with_key["treatment_group"].eq(1)
                & ~errors_with_key["repo_key"].isin(removed_keys)
            ].drop(columns=["repo_key"])

        period_table = build_by_treatment_period(
            repo_subset,
            attempt_subset,
            error_subset,
        )
        period_table = period_table.loc[
            period_table["treatment_period"].isin(period_labels)
        ].copy()
        period_table.insert(0, "excluded_top_treatment_repositories", int(cutoff))
        period_table.insert(1, "excluded_treatment_repository_names", ";".join(removed_names))
...
def build_top_treatment_repo_sensitivity_by_event_time(
    repo_month: pd.DataFrame,
    attempts: pd.DataFrame,
    enriched_errors: pd.DataFrame,
    by_repo: pd.DataFrame,
    ...
    """Recompute treatment event-time rates after removing top treatments."""
    ranked_names = treatment_repository_ranking(by_repo)
    ranked_keys = [f"treatment|{name}" for name in ranked_names]
    ...
    for cutoff in cutoffs:
        ...
        event_table = build_by_event_time(
            repo_subset,
            attempt_subset,
            error_subset,
        )
        if event_table.empty:
            continue
        event_table.insert(0, "excluded_top_treatment_repositories", int(cutoff))
        event_table.insert(1, "excluded_treatment_repository_names", ";".join(removed_names))
        rows.append(event_table)
...
def build_sensitivity_checks(
    by_treatment_period: pd.DataFrame,
    by_event_time: pd.DataFrame,
    sensitivity_by_period: pd.DataFrame,
    sensitivity_by_event_time: pd.DataFrame,
) -> pd.DataFrame:
    ...
    expected_period = (
        by_treatment_period.loc[
            by_treatment_period["treatment_period"].isin(baseline_labels),
            ["treatment_period", *count_columns],
        ]
        .sort_values("treatment_period")
        .reset_index(drop=True)
    )
    observed_period = (
        sensitivity_by_period.loc[
            sensitivity_by_period["excluded_top_treatment_repositories"].eq(0),
            ["treatment_period", *count_columns],
        ]
        .sort_values("treatment_period")
        .reset_index(drop=True)
    )
    period_match = expected_period.equals(observed_period)
    add(
        "cutoff_zero_period_matches_baseline",
        period_match,
        f"expected_rows={len(expected_period)};observed_rows={len(observed_period)}",
    )

    expected_event = (
        by_event_time[["time_to_event", *count_columns]]
        .sort_values("time_to_event")
        .reset_index(drop=True)
    )
    observed_event = (
        sensitivity_by_event_time.loc[
            sensitivity_by_event_time["excluded_top_treatment_repositories"].eq(0),
            ["time_to_event", *count_columns],
        ]
        .sort_values("time_to_event")
        .reset_index(drop=True)
    )
    event_match = expected_event.equals(observed_event)
    add(
        "cutoff_zero_event_time_matches_baseline",
        event_match,
        f"expected_rows={len(expected_event)};observed_rows={len(observed_event)}",
    )

    names = pd.concat(
        [
            sensitivity_by_period.get(
                "excluded_treatment_repository_names",
                pd.Series(dtype=str),
            ),
            sensitivity_by_event_time.get(
                "excluded_treatment_repository_names",
                pd.Series(dtype=str),
            ),
        ],
    ...
    treatment_only = all("|" not in name for name in listed_names)
    add(
        "excluded_repository_names_are_treatment_repositories",
        treatment_only,
        len(listed_names),
    )
...
def build_checks(
    ...
    treatment_rows = panel["treatment_group"].eq(1)
    treatment_event_time = pd.to_numeric(
        panel.loc[treatment_rows, "time_to_event"], errors="coerce"
    )
    pre_count = int(treatment_event_time.lt(0).sum())
    event_count = int(treatment_event_time.eq(0).sum())
    post_count = int(treatment_event_time.gt(0).sum())
    unknown_count = int(treatment_event_time.isna().sum())
    treatment_count = int(treatment_rows.sum())
    add(
        "treatment",
        "treatment_group_matches_dataset_source",
        bool(
            panel["treatment_group"].eq(
                panel["dataset_source"].eq("treatment").astype(int)
            ).all()
        ),
        treatment_count,
    ...
    add(
        "treatment",
        "treatment_period_partition_complete",
        pre_count + event_count + post_count + unknown_count == treatment_count,
        (
            f"pre={pre_count};event={event_count};post={post_count};"
            f"unknown={unknown_count};total={treatment_count}"
        ),
    )
    add(
        "treatment",
        "treatment_event_time_missing_zero",
        unknown_count == 0,
        unknown_count,
    )
    ...
    raw_source_counts = errors.groupby("dataset_source").size().to_dict()
    repo_source_counts = (
        repo_month.groupby("dataset_source")["parse_exclusion_records"]
        .sum()
        .astype(int)
        .to_dict()
    )
    source_labels = sorted(set(raw_source_counts) | set(repo_source_counts))
    normalized_raw_source_counts = {
        label: int(raw_source_counts.get(label, 0)) for label in source_labels
    }
    normalized_repo_source_counts = {
        label: int(repo_source_counts.get(label, 0)) for label in source_labels
    }
    add(
        "arithmetic",
        "dataset_source_error_counts_preserved",
        normalized_raw_source_counts == normalized_repo_source_counts,
        (
            f"repo_month={normalized_repo_source_counts};"
            f"raw={normalized_raw_source_counts}"
        ),
    )

    global_unique_excluded = int(enriched_errors["blob_revision_key"].nunique())
    add(
        "uniqueness",
        "global_unique_exclusion_count_matches_unique_table",
        global_unique_excluded == len(unique_exclusions),
        f"{global_unique_excluded}:{len(unique_exclusions)}",
    )
    add(
        "uniqueness",
...
def overall_metrics(
    ...
    totals["unique_blob_revisions_attempted"] = int(
        attempts["blob_revision_key"].nunique()
    )
    totals["unique_excluded_blob_revisions"] = int(
        enriched_errors["blob_revision_key"].nunique()
    )
    ...
    totals["current_file_parse_exclusion_rate"] = scalar_rate(
        int(totals["current_file_parse_exclusions"]),
        int(totals["current_blob_attempts"]),
    )
    totals["parent_file_parse_exclusion_rate"] = scalar_rate(
        int(totals["parent_file_parse_exclusions"]),
        int(totals["parent_blob_attempts"]),
    )
    totals["blob_parse_exclusion_record_rate"] = scalar_rate(
        int(totals["parse_exclusion_records"]),
        int(totals["blob_parse_attempts"]),
    )
...
def build_summary(
    ...
    top_repo_counts = (
        ...
        .head(20)[["dataset_source", "repo_name", "parse_exclusion_records"]]
    ...
    return {
        ...
        "overall": overall_metrics(repo_month, attempts, enriched_errors),
        ...
        "treatment_event_and_post": row_to_metrics(
            by_treatment_period,
            "treatment_event_and_post",
        ),
        "treatment_unknown_event_time": row_to_metrics(
            by_treatment_period,
            "treatment_unknown_event_time",
        ),
        ...
        "outputs": {
            ...
            "top_treatment_repo_sensitivity_by_period": str(
                paths.output_dir
                / "agc_commit_function_parse_exclusions_top_treatment_repo_sensitivity_by_period.csv"
            ),
            "top_treatment_repo_sensitivity_by_event_time": str(
                paths.output_dir
                / "agc_commit_function_parse_exclusions_top_treatment_repo_sensitivity_by_event_time.csv"
            ),
            "checks": str(
                paths.qc_dir
...
def analyze(
    paths: AnalysisPaths,
    cutoffs: Sequence[int],
    treatment_cutoffs: Sequence[int],
) -> AnalysisResult:
    ...
    attempts = enrich_attempts_with_panel_metadata(
        build_blob_attempts(audit),
        panel,
    )
    ...
    by_repo = build_by_repo(repo_month, attempts, enriched_errors)
    by_dataset_source = aggregate_metrics(
        repo_month,
        ["dataset_source"],
        attempts,
        enriched_errors,
    )
    by_treatment_period = build_by_treatment_period(
        repo_month,
        attempts,
        enriched_errors,
    )
    by_event_time = build_by_event_time(
        repo_month,
        attempts,
        enriched_errors,
    )
    ...
    top_treatment_repo_sensitivity_by_period = (
        build_top_treatment_repo_sensitivity_by_period(
            repo_month,
            attempts,
            enriched_errors,
            by_repo,
    ...
    top_treatment_repo_sensitivity_by_event_time = (
        build_top_treatment_repo_sensitivity_by_event_time(
            repo_month,
            attempts,
            enriched_errors,
            by_repo,
    ...
    sensitivity_checks = build_sensitivity_checks(
        by_treatment_period,
        by_event_time,
        top_treatment_repo_sensitivity_by_period,
        top_treatment_repo_sensitivity_by_event_time,
    )
    checks = pd.concat([checks, sensitivity_checks], ignore_index=True)
...
def write_result(result: AnalysisResult, paths: AnalysisPaths) -> None:
    outputs = {
        ...
        paths.output_dir
        / "agc_commit_function_parse_exclusions_top_treatment_repo_sensitivity_by_period.csv": result.top_treatment_repo_sensitivity_by_period,
        paths.output_dir
        / "agc_commit_function_parse_exclusions_top_treatment_repo_sensitivity_by_event_time.csv": result.top_treatment_repo_sensitivity_by_event_time,
        paths.qc_dir
...
def synthetic_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="agc-parse-exclusion-self-test-") as temp:
        ...
        result = analyze(paths, [0, 1, 3], [0, 1, 2, 3])
        ...
        assert result.summary["overall"]["unique_excluded_blob_revisions"] == 1
        assert result.summary["control"]["repository_months"] == 1
        assert result.summary["treatment"]["repository_months"] == 3
        assert result.summary["treatment_pre_event"]["repository_months"] == 1
        assert result.summary["treatment_event_month"]["repository_months"] == 1
        assert result.summary["treatment_post_event"]["repository_months"] == 1
        assert result.summary["treatment_event_and_post"]["repository_months"] == 2
        assert result.summary["treatment_pre_event"]["parse_exclusion_records"] == 0
        assert result.summary["treatment_event_month"]["parse_exclusion_records"] == 1
        assert result.summary["treatment_post_event"]["parse_exclusion_records"] == 1
        assert len(result.by_repo_month) == 4
        assert int(result.checks["passed"].min()) == 1
        assert not result.top_treatment_repo_sensitivity_by_period.empty
        assert not result.top_treatment_repo_sensitivity_by_event_time.empty
        baseline_period = result.top_treatment_repo_sensitivity_by_period.loc[
            result.top_treatment_repo_sensitivity_by_period[
                "excluded_top_treatment_repositories"
            ].eq(0)
        ]
        assert set(baseline_period["treatment_period"]) == {
            "treatment_all",
            "treatment_pre_event",
            "treatment_event_month",
            "treatment_post_event",
...
def main() -> int:
    ...
    treatment_cutoffs = parse_cutoffs(args.top_treatment_repo_cutoffs)
    print("=" * 72)
    ...
    print(
        "Top-treatment cutoffs: "
        f"{','.join(map(str, treatment_cutoffs))}"
    )
    print("=" * 72)

    result = analyze(paths, cutoffs, treatment_cutoffs)
    write_result(result, paths)
    ...
    print(
        "Blob-parse exclusion rate:      "
        f"{overall['blob_parse_exclusion_record_rate']:.6f}"
        if overall["blob_parse_exclusion_record_rate"] is not None
        else "Blob-parse exclusion rate:      NA"
    )
    print(
