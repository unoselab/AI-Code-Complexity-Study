...
def build_monthly_treatment_control_distribution(
    ...
    """Compare treatment and control distributions at aligned event months.

    The output contains one row for each classification, token band, and
    relative event month. For unique-body results, a body is counted once
    within each dataset source and event month. The same body may appear in
    multiple relative months and is intentionally counted once in each month.

    The panel supplies all repository-month exposure counts, including months
    with no selected regular-function event. This prevents event-only rows from
    being used as the comparison denominator.
    """
    ...
    scoped_events["dataset_source"] = (
        scoped_events["dataset_source"].astype("string").str.strip().str.lower()
    )
    scoped_events["event_month"] = pd.to_numeric(
        scoped_events["time_to_event"], errors="coerce"
    )
    scoped_events = scoped_events.loc[
        scoped_events["dataset_source"].isin(sources)
        & scoped_events["event_month"].between(
            minimum_event_time, maximum_event_time, inclusive="both"
        )
    ].copy()
    scoped_events["event_month"] = scoped_events["event_month"].astype("int64")

    scoped_panel = panel.copy()
    scoped_panel["dataset_source"] = (
        scoped_panel["dataset_source"].astype("string").str.strip().str.lower()
    )
    scoped_panel["event_month"] = pd.to_numeric(
        scoped_panel["time_to_event"], errors="coerce"
    )
    scoped_panel = scoped_panel.loc[
        scoped_panel["dataset_source"].isin(sources)
        & scoped_panel["event_month"].between(
            minimum_event_time, maximum_event_time, inclusive="both"
        )
    ].copy()
    scoped_panel["event_month"] = scoped_panel["event_month"].astype("int64")

    exposure = (
        scoped_panel.groupby(["event_month", "dataset_source"], dropna=False)
        .agg(
            panel_repo_months=("repo_name", "size"),
            panel_repositories=("repo_name", "nunique"),
        )
        .reset_index()
    )
    exposure_lookup = {
    ...
    for event_month in event_months:
        for classification in classifications:
            if classification == "ALL":
                class_mask = pd.Series(True, index=scoped_events.index)
            elif classification == "AGC-like":
                class_mask = scoped_events[AGC_COLUMN].eq(1)
            else:
                class_mask = scoped_events[HWC_COLUMN].eq(1)

            for token_range in token_ranges:
                if token_range == "ALL":
                    band_mask = pd.Series(True, index=scoped_events.index)
                else:
                    band_mask = scoped_events["token_range"].astype("string").eq(
                        token_range
                    )
                ...
                for source in sources:
                    source_subset = scoped_events.loc[
                        scoped_events["dataset_source"].eq(source)
                    ...
                    exposure_values = exposure_lookup.get(
                        (event_month, source),
                        {"panel_repo_months": 0, "panel_repositories": 0},
                    )
    ...
    # Add within-month token-band shares. The ALL row is the denominator for
    # each classification, event month, and dataset source.
    for source in sources:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.minimum_body_tokens != 100 or args.maximum_body_tokens != 200:
        raise ValueError(
            "This v3 diagnostic uses the frozen 100-200 pilot bands. "
