...
# ================================================================
# run-x-h07 v1: Borusyak DiD across ML file-composition thresholds
# ================================================================
#
# Purpose:
#   Estimate Cursor-adoption effects on unresolved SonarQube issue burden
#   localized to Python files whose token-weighted C_FUN ML AGC share exceeds each
#   threshold materialized by the H07 C_FUN ML threshold-input stage.
#
# Input contract:
#   - H07 threshold-input has already performed file-level ML thresholding and SonarQube burden
#     aggregation. This script never re-selects files from A07/D02.
...
#
# Provenance gate:
#   The full-sample primary threshold must reproduce the existing run-x-h06
...
outcome_labels <- c(
  ...
  log1p_selected_issue_code_smell = "C_FUN ML-selected code-smell issue stock",
  log1p_selected_issue_bug = "C_FUN ML-selected bug issue stock",
  log1p_selected_issue_vulnerability = "C_FUN ML-selected vulnerability issue stock",
  log1p_selected_issue_high_severity = "C_FUN ML-selected high-severity issue stock",
  log1p_selected_issue_maintainability_impact = "C_FUN ML-selected maintainability-impact issue stock",
  log1p_selected_issue_reliability_impact = "C_FUN ML-selected reliability-impact issue stock",
  log1p_selected_issue_security_impact = "C_FUN ML-selected security-impact issue stock"
)
...
expected_ml_metric <- "file_ml_cfun_agc_share_space_by_token_weighted"
expected_operator <- ">"
...
run_self_test <- function() {
  ...
  stopifnot(identical(
    ...
    "C_FUN ML-selected total unresolved issue stock"
  ))
  stopifnot(identical(
    lookup_named_scalar(outcome_roles, test_outcome, "outcome_roles"),
    "primary_burden"
  ))
  cat("did_borusyak_ml_cfun_threshold_quality self-test: PASS\n")
}
...
h06_static_reference_file <- normalizePath(require_arg(args, "h06_static_reference_file"), mustWork = TRUE)
h06_dynamic_reference_file <- normalizePath(require_arg(args, "h06_dynamic_reference_file"), mustWork = TRUE)
...
h06_reproduction_tolerance <- as_numeric_arg(args, "h06_reproduction_tolerance", 1e-10)

if (plot_min_event != -6L || plot_max_event != 6L) abortf("H07 v1 requires dynamic window -6:6.")
if (pretrend_min != -6L || pretrend_max != -2L) abortf("H07 v1 requires pretrend window -6:-2.")
if (confidence_level <= 0 || confidence_level >= 1) abortf("confidence_level must be between 0 and 1.")
if (expected_threshold_count != 21L || expected_sample_count != 2L) abortf("H07 v1 requires 21 thresholds and 2 sample specifications.")
...
paths <- list(
  static = file.path(output_dir, "quality_ml_cfun_threshold_static_effects.csv"),
  dynamic = file.path(output_dir, "quality_ml_cfun_threshold_dynamic_effects.csv"),
  pretrend_checks = file.path(output_dir, "quality_ml_cfun_threshold_pretrend_checks.csv"),
  pretrend_summary = file.path(output_dir, "quality_ml_cfun_threshold_pretrend_summary.csv"),
  diagnostics = file.path(output_dir, "quality_ml_cfun_threshold_model_diagnostics.csv"),
  failures = file.path(output_dir, "quality_ml_cfun_threshold_model_failures.csv"),
  threshold_support = file.path(output_dir, "quality_ml_cfun_threshold_support.csv"),
  event_support = file.path(output_dir, "quality_ml_cfun_threshold_event_support.csv"),
  support_policy = file.path(output_dir, "quality_ml_cfun_support_policy.csv"),
  primary_threshold_static = file.path(output_dir, "quality_ml_cfun_primary_threshold_static.csv"),
  ...
  total_threshold_static = file.path(output_dir, "quality_ml_cfun_total_threshold_static.csv"),
  total_threshold_dynamic = file.path(output_dir, "quality_ml_cfun_total_threshold_dynamic.csv"),
  h06_reproduction = file.path(output_dir, "quality_ml_cfun_h06_reproduction.csv"),
  qc = file.path(output_dir, "quality_ml_cfun_threshold_qc.csv"),
  summary = file.path(output_dir, "quality_ml_cfun_threshold_summary.csv"),
  metadata = file.path(output_dir, "quality_ml_cfun_threshold_run_metadata.csv")
)

run_started <- Sys.time()
log_message("INFO", "Reading H07 threshold-input long panel: %s", input_file)
...
h06_static_ref <- data.table::fread(h06_static_reference_file, na.strings = c("", "NA", "NaN"))
h06_dynamic_ref <- data.table::fread(h06_dynamic_reference_file, na.strings = c("", "NA", "NaN"))

# Validate H07 threshold-input provenance artifacts before fitting any model.
validate_columns(input_summary, c("metric", "value"), "H07 threshold-input summary")
...
if (!all(required_summary_metrics %in% names(summary_map))) abortf("H07 threshold-input summary is missing required metrics.")
if (summary_map[["script_version"]] != "run-x-h07-v1-prep") abortf("H07 threshold-input summary must report script_version=run-x-h07-v1-prep; observed %s", summary_map[["script_version"]])
if (summary_map[["status"]] != "PASS") abortf("H07 threshold-input summary status is not PASS.")
if (summary_map[["ml_metric"]] != expected_ml_metric) abortf("H07 threshold-input ML metric mismatch.")
if (summary_map[["quality_semantics"]] != "unresolved_sonarqube_issue_stock_at_historical_snapshot") abortf("H07 threshold-input quality semantics mismatch.")
if (summary_map[["mapping_spec"]] != expected_mapping) abortf("H07 threshold-input mapping mismatch.")
expected_summary_numeric <- c(
  ...
  primary_threshold = primary_threshold, primary_full_selected_file_rows = 37757,
  primary_full_selected_issue_total = 36432, density_computed = 0,
...
for (name in names(expected_summary_numeric)) {
  ...
  if (!is.finite(observed) || abs(observed - expected_summary_numeric[[name]]) > 1e-9) {
    abortf("H07 threshold-input summary mismatch for %s: expected %s, observed %s", name, expected_summary_numeric[[name]], summary_map[[name]])
  }
}

validate_columns(input_checks, c("check", "status"), "H07 threshold-input checks")
if (nrow(input_checks) == 0L || any(tolower(trimws(as.character(input_checks$status))) != "pass")) {
  abortf("H07 threshold-input checks contain non-passing rows.")
}
...
), "H07 threshold-input sample summary")
...
for (i in seq_len(nrow(expected_sample_support))) {
  ...
  if (nrow(observed_row) != 1L) abortf("H07 threshold-input sample summary must contain one row for %s.", expected_row$sample_spec)
  for (field in setdiff(names(expected_sample_support), "sample_spec")) {
    if (as.integer(observed_row[[field]]) != as.integer(expected_row[[field]])) {
      abortf("H07 threshold-input sample summary mismatch for %s/%s.", expected_row$sample_spec, field)
    }
  }
}
...
validate_columns(panel_all, required_columns, "H07 threshold-input long panel")

if (nrow(panel_all) != expected_long_rows) abortf("H07 threshold-input long-panel row mismatch: expected %d, observed %d", expected_long_rows, nrow(panel_all))
if (data.table::uniqueN(panel_all$sample_spec) != 2L) abortf("H07 threshold-input long panel must contain two sample specifications.")
if (!setequal(unique(panel_all$sample_spec), expected_samples)) abortf("Unexpected H07 threshold-input sample specifications.")
if (any(panel_all$mapping_spec != expected_mapping)) abortf("H07 threshold-input mapping_spec mismatch.")
if (any(panel_all$ml_metric != expected_ml_metric)) abortf("H07 threshold-input ml_metric mismatch.")
if (any(panel_all$ml_operator != expected_operator)) abortf("H07 threshold-input ml_operator mismatch.")
...
if (!identical(as.character(threshold_map$threshold_spec_id), expected_threshold_ids)) abortf("H07 threshold-input threshold IDs do not match the 21-point ML grid.")
if (any(abs(threshold_map$threshold - expected_thresholds) > 1e-12)) abortf("H07 threshold-input threshold values do not match the expected 0.10:0.90 grid.")
expected_roles <- ifelse(expected_threshold_ids == primary_threshold_id, "primary", "sensitivity_grid")
if (!identical(as.character(threshold_map$threshold_role), expected_roles)) abortf("H07 threshold-input threshold roles are inconsistent with the primary 0.50 threshold.")
...
for (column in c("eligible_ml_file_count", "selected_file_count", raw_issue_columns)) {
  if (anyNA(panel_all[[column]]) || any(!is.finite(panel_all[[column]])) || any(panel_all[[column]] < 0)) {
    abortf("H07 threshold-input column %s contains missing, non-finite, or negative values.", column)
  }
}
for (raw_name in names(raw_log_pairs)) {
  ...
  if (anyNA(observed) || any(!is.finite(observed)) || max(abs(observed - expected)) > 1e-10) {
    abortf("H07 threshold-input log1p mismatch for %s <- %s.", log_name, raw_name)
  }
}

model_fields <- unique(c(all_outcomes, model_specs$adjusted_burden$model_fields))
if (any(!stats::complete.cases(panel_all[, ..model_fields]))) abortf("H07 threshold-input long panel has missing model fields.")
if (any(!vapply(panel_all[, ..model_fields], function(x) all(is.finite(x)), logical(1)))) abortf("H07 threshold-input long panel has non-finite model fields.")

# Check nested strict-threshold construction using totals from the H07 threshold-input long panel.
...
# Reconcile H07 threshold-input global audit to the panel for selected-file and total-issue counts.
...
), "H07 threshold-input global audit")
if (nrow(input_global_audit) != 42L) abortf("H07 threshold-input global audit must contain 42 rows.")
...
if (nrow(audit_check) != 42L || anyNA(audit_check$repo_month_rows)) abortf("H07 threshold-input global audit keys do not reconcile to the long panel.")
if (any(abs(audit_check$audit_repo_month_rows - audit_check$repo_month_rows) > 0) ||
  ...
  abortf("H07 threshold-input global audit values do not reconcile to the long panel.")
}
...
if (nrow(failures_dt) > 0L) abortf("One or more H07 models failed; inspect %s", paths$failures)
...
# Exact numerical reproduction gate against H06 at full_sample x >0.50.
validate_columns(h06_static_ref, c("sample_spec", "model_spec", "outcome", "estimate", "std.error", "conf.low", "conf.high", "p_value"), "H06 static reference")
validate_columns(h06_dynamic_ref, c("sample_spec", "event_time", "model_spec", "outcome", "estimate", "std.error", "conf.low", "conf.high", "p_value"), "H06 dynamic reference")
h07_static_primary <- primary_total_static[sample_spec == "full_sample"]
h07_dynamic_primary <- primary_total_dynamic[sample_spec == "full_sample"]
h06_static_primary <- h06_static_ref[sample_spec == "full_sample" & outcome == "log1p_selected_issue_total"]
h06_dynamic_primary <- h06_dynamic_ref[sample_spec == "full_sample" & outcome == "log1p_selected_issue_total"]
if (nrow(h07_static_primary) != 2L || nrow(h06_static_primary) != 2L || nrow(h07_dynamic_primary) != 24L || nrow(h06_dynamic_primary) != 24L) {
  abortf("H06 reproduction reference row counts are not 2 static and 24 dynamic rows.")
}

reproduction_rows <- list()
metrics <- c("estimate", "std.error", "conf.low", "conf.high", "p_value")
static_join <- merge(
  h07_static_primary[, c("model_spec", metrics), with = FALSE],
  h06_static_primary[, c("model_spec", metrics), with = FALSE],
  by = "model_spec", suffixes = c("_h07", "_h06"), all = TRUE
)
for (row_idx in seq_len(nrow(static_join))) {
  for (metric in metrics) {
    h07_value <- static_join[[paste0(metric, "_h07")]][[row_idx]]
    h06_value <- static_join[[paste0(metric, "_h06")]][[row_idx]]
    difference <- abs(h07_value - h06_value)
    reproduction_rows[[length(reproduction_rows) + 1L]] <- data.table::data.table(
      ...
      metric = metric, h07_value = h07_value, h06_value = h06_value,
      absolute_difference = difference, tolerance = h06_reproduction_tolerance,
      status = ifelse(is.finite(difference) && difference <= h06_reproduction_tolerance, "pass", "fail")
    )
  }
}
dynamic_join <- merge(
  h07_dynamic_primary[, c("model_spec", "event_time", metrics), with = FALSE],
  h06_dynamic_primary[, c("model_spec", "event_time", metrics), with = FALSE],
  by = c("model_spec", "event_time"), suffixes = c("_h07", "_h06"), all = TRUE
)
for (row_idx in seq_len(nrow(dynamic_join))) {
  for (metric in metrics) {
    h07_value <- dynamic_join[[paste0(metric, "_h07")]][[row_idx]]
    h06_value <- dynamic_join[[paste0(metric, "_h06")]][[row_idx]]
    difference <- abs(h07_value - h06_value)
    reproduction_rows[[length(reproduction_rows) + 1L]] <- data.table::data.table(
...
h06_reproduction <- data.table::rbindlist(reproduction_rows)
write_csv(h06_reproduction, paths$h06_reproduction)
if (nrow(h06_reproduction[status != "pass"]) > 0L) abortf("H06 primary 0.50 reproduction failed.")
...
expected_counts <- c(
  ...
  h06_reproduction_rows = 5L * (2L + 24L),
...
observed_counts <- c(
  ...
  h06_reproduction_rows = nrow(h06_reproduction),
...
qc <- data.table::data.table(
  check = c(
    ...
    names(expected_counts), "h06_reproduction_failures", "primary_model_warning_count"
  ),
  observed = c(
    nrow(panel_all), nrow(threshold_map), data.table::uniqueN(panel_all$sample_spec), nrow(key_duplicates),
    sum(tolower(trimws(as.character(input_checks$status))) != "pass"), nrow(input_global_audit), length(monotonic_failures),
    unname(observed_counts), nrow(h06_reproduction[status != "pass"]), primary_warning_count
  ),
  expected = c(
...
qc[, detail := c(
  "H07 threshold-input authoritative long-panel row count.",
  "ML threshold grid count.",
  "Full and scope-sensitivity samples.",
  "sample+threshold+repo+time must be unique.",
  "All H07 threshold-input checks must pass.",
  "21 thresholds x 2 samples.",
  "Strict threshold nesting must be monotone.",
  rep("Strict H07 output row-count contract.", length(expected_counts)),
  "Full-sample >0.50 H07 estimates must numerically reproduce H06.",
  "Primary H07 model path must be warning-free."
)]
write_csv(qc, paths$qc)
failed_qc <- qc[status == "fail"]
if (nrow(failed_qc) > 0L && strict_expected_counts) abortf("H07 hard QC failed: %s", paste(failed_qc$check, collapse = ", "))
...
summary <- data.table::data.table(
  metric = c(
    ...
    "primary_model_warnings", "h06_reproduction_failures", "density_computed", "hard_qc_failures"
  ),
  value = as.character(c(
    paste0("run-x-h07-", implementation_version), ifelse(nrow(failed_qc) == 0L, "PASS", "FAIL"),
    ...
    primary_warning_count, nrow(h06_reproduction[status != "pass"]), 0L, nrow(failed_qc)
  ))
)
...
metadata <- data.table::data.table(
  ...
  metric = c(
    ...
    "input_sample_summary_file", "input_global_audit_file", "h06_static_reference_file", "h06_dynamic_reference_file",
    ...
    "h06_static_reference", "h06_dynamic_reference"
  ),
  value = c(
    ...
    h06_static_reference_file, h06_dynamic_reference_file, script_path,
    ...
    sha256_file(input_file), sha256_file(input_summary_file), sha256_file(input_checks_file), sha256_file(input_sample_summary_file),
    sha256_file(input_global_audit_file), sha256_file(h06_static_reference_file), sha256_file(h06_dynamic_reference_file)
  )
)
...
log_message(
  "INFO",
  "Completed run-x-h07 %s: status=%s; jobs=%d; static=%d; dynamic=%d; sparse threshold-sample rows=%d; warnings=%d; hard QC failures=%d",
