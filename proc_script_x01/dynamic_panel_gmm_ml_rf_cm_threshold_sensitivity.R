...
# ============================================================
# run-x-k10 v1: ML RF+CM threshold sensitivity for dynamic panel GMM
# ============================================================
#
# Purpose:
#   Re-estimate the reverse-direction quality -> velocity dynamic-panel GMM
#   across the prespecified 21-point ML threshold grid already constructed
#   by run-x-d03.
#
# Scientific guardrails:
#   - The prespecified primary threshold remains 0.50.
#   - Main thresholds span primary +/- 0.50 in exact 0.04 increments.
#   - The I08 threshold grid  is intentionally excluded from this sweep.
...
#   file_ml_fun_cfun_agc_share_space_by_token_weighted > tau in the historical snapshot).
#
# Model:
...
#
# Inputs:
#   - I08 ML threshold-long quality panel
#   - I08 summary/sample/global-audit artifacts
#   - B06 authoritative whole-Python velocity/covariate panel
#   - K08 coefficients for exact primary-threshold reproduction
#
# This script is self-contained. It reuses validated analysis logic from the
# existing K08 ML GMM and E03 threshold-sensitivity analyses, but it does not
...
extract_htest <- function(test, threshold_spec_id, threshold, diagnostic_name, primary_analysis) {
  if (is.null(test)) {
    return(data.table::data.table(
      threshold_spec_id = threshold_spec_id,
      threshold = threshold,
...
failed_diagnostics <- function(threshold_spec_id, threshold, primary_analysis, message) {
  data.table::data.table(
    threshold_spec_id = rep(threshold_spec_id, 3L),
...
extract_coefficients <- function(summary_object, threshold_spec_id, threshold, threshold_role, confidence_level, primary_threshold) {
  ...
  data.table::data.table(
    ...
    model = "ml_rf_cm_threshold_quality_to_velocity",
...
i08_summary_file <- normalizePath(require_arg(args, "i08_summary_file"), mustWork = TRUE)
i08_sample_summary_file <- normalizePath(require_arg(args, "i08_sample_summary_file"), mustWork = TRUE)
i08_global_audit_file <- normalizePath(require_arg(args, "i08_global_audit_file"), mustWork = TRUE)
k08_reference_coefficients_file <- normalizePath(require_arg(args, "k08_reference_coefficients_file"), mustWork = TRUE)
...
threshold_min <- as_numeric_arg(args, "threshold_min", 0.10)
threshold_max <- as_numeric_arg(args, "threshold_max", 0.90)
threshold_step <- as_numeric_arg(args, "threshold_step", 0.04)
...
expected_i08_thresholds <- as_integer_arg(args, "expected_i08_thresholds", 21L)
...
expected_primary_selected_files <- as_integer_arg(args, "expected_primary_selected_files", 64153L)
expected_primary_issue_stock <- as_integer_arg(args, "expected_primary_issue_stock", 35765L)
...
if (threshold_step <= 0 || threshold_max <= threshold_min) abortf("Invalid ML threshold-grid bounds or step")
expected_grid <- seq(threshold_min, threshold_max, by = threshold_step)
if (length(expected_grid) != expected_main_thresholds || any(abs(expected_grid - seq(0.10, 0.90, by = 0.04)) > 1e-12)) {
  abortf("Expected the prespecified 21-point ML grid from 0.10 to 0.90 by 0.04")
}
if (!any(abs(expected_grid - primary_threshold) <= 1e-12)) abortf("Primary ML threshold is not present in the sensitivity grid")
...
paths <- list(
  coefficients = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_coefficients.csv"),
  primary_summary = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_primary_summary.csv"),
  diagnostics = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_diagnostics.csv"),
  instrument_qc = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_instrument_qc.csv"),
  support = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_support_diagnostics.csv"),
  threshold_input_audit = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_input_audit.csv"),
  b06_join_audit = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_b06_join_audit.csv"),
  legacy_audit = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_legacy_flag_audit.csv"),
  calendar_gap_audit = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_calendar_gap_audit.csv"),
  reproduction = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_k08_reproduction.csv"),
  model_failures = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_model_failures.csv"),
  qc = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_qc.csv"),
  metadata = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_run_metadata.csv"),
  models = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_models.rds")
)

run_started <- Sys.time()
log_message("INFO", "Reading I08 ML RF+CM threshold panel: %s", input_file)
...
required_i08 <- c(
  ...
  "post_event",
  "log1p_selected_issue_total", "selected_issue_total", "selected_file_count", "eligible_ml_file_count",
  "log_age", "ncloc_py_sonarqube", "log_contributors", "log_stars", "log_issues"
)
validate_columns(long_panel, required_i08, "I08 long panel")
strict_count_check(nrow(long_panel), expected_long_rows, "I08 long-panel rows", strict_expected_counts)
strict_count_check(data.table::uniqueN(long_panel$threshold_spec_id), expected_i08_thresholds, "I08 threshold specifications", strict_expected_counts)
strict_count_check(data.table::uniqueN(long_panel$sample_spec), expected_sample_specs, "I08 sample specifications", strict_expected_counts)

# Validate upstream I08 summary/sample/global-audit artifacts before fitting.
i08_summary <- data.table::fread(i08_summary_file, na.strings = c("", "NA", "NaN"))
validate_columns(i08_summary, c("metric", "value"), "I08 summary")
summary_lookup <- setNames(as.character(i08_summary$value), as.character(i08_summary$metric))
if (!identical(summary_lookup[["status"]], "PASS")) abortf("I08 summary status is not PASS")
if (abs(as.numeric(summary_lookup[["primary_threshold"]]) - primary_threshold) > 1e-12) abortf("I08 primary-threshold summary mismatch")

i08_sample_summary <- data.table::fread(i08_sample_summary_file, na.strings = c("", "NA", "NaN"))
validate_columns(i08_sample_summary, c("sample_spec", "repo_month_rows", "repositories", "control_repositories", "treatment_repositories"), "I08 sample summary")
full_sample_summary <- i08_sample_summary[sample_spec == "full_sample"]
if (nrow(full_sample_summary) != 1L) abortf("I08 sample summary must contain exactly one full_sample row")
strict_count_check(full_sample_summary$repo_month_rows, expected_rows_per_threshold, "I08 full-sample rows", strict_expected_counts)
strict_count_check(full_sample_summary$repositories, expected_repositories, "I08 full-sample repositories", strict_expected_counts)
strict_count_check(full_sample_summary$treatment_repositories, expected_treatment_repositories, "I08 full-sample treatment repositories", strict_expected_counts)
strict_count_check(full_sample_summary$control_repositories, expected_control_repositories, "I08 full-sample control repositories", strict_expected_counts)

global_audit <- data.table::fread(i08_global_audit_file, na.strings = c("", "NA", "NaN"))
...
global_full <- data.table::copy(global_audit[sample_spec == "full_sample" & mapping_spec == "all_ml_files"])
global_full[, threshold_numeric := as.numeric(threshold)]
if (anyNA(global_full$threshold_numeric)) abortf("I08 global audit contains non-numeric thresholds")
...
if (nrow(main_global) != expected_main_thresholds) abortf("Expected %d I08 full-sample main-grid audit rows; observed %d", expected_main_thresholds, nrow(main_global))
if (any(abs(main_global$threshold_numeric - expected_grid) > 1e-10)) abortf("I08 main threshold grid does not match 0.10 to 0.90 in 0.04 increments")
if (any(main_global$repo_month_rows != expected_rows_per_threshold)) abortf("I08 main-grid global audit has unexpected repo-month row counts")
if (any(main_global$repositories != expected_repositories)) abortf("I08 main-grid global audit has unexpected repository counts")
...
if (any(selected_file_diff > 0)) abortf("I08 selected-file support increases as ML threshold increases")
if (any(selected_issue_diff > 0)) abortf("I08 selected-issue support increases as ML threshold increases")

primary_global <- main_global[abs(threshold_numeric - primary_threshold) <= 1e-12]
if (nrow(primary_global) != 1L) abortf("Expected exactly one primary row in I08 full-sample global audit")
...
write_csv(main_global[, .(
  threshold_spec_id, threshold = threshold_numeric, threshold_role, repo_month_rows, repositories,
...
main_panel <- data.table::copy(long_panel[sample_spec == "full_sample" & ml_operator == ">"] )
...
if (nrow(main_panel) != expected_rows_per_threshold * expected_main_thresholds) {
  abortf("Unexpected I08 main-grid row count: expected %d, observed %d", expected_rows_per_threshold * expected_main_thresholds, nrow(main_panel))
}
if (data.table::uniqueN(main_panel$threshold_spec_id) != expected_main_thresholds) abortf("I08 main panel does not contain exactly 21 threshold IDs")
if (any(main_panel$ml_operator != ">")) abortf("K10 requires strict ML > threshold")
...
), by = .(threshold_spec_id, threshold = threshold_numeric, threshold_role)]
data.table::setorder(threshold_key_qc, threshold)
if (any(threshold_key_qc$rows != expected_rows_per_threshold)) abortf("One or more K10 thresholds do not contain 1,954 rows")
if (any(threshold_key_qc$repositories != expected_repositories)) abortf("One or more K10 thresholds do not contain 167 repositories")
if (any(threshold_key_qc$duplicate_repo_time != 0L)) abortf("One or more K10 thresholds contain duplicate repo-month keys")
if (any(abs(threshold_key_qc$threshold - expected_grid) > 1e-10)) abortf("Threshold order/grid mismatch in I08 main panel")
...
if (nrow(support_compare) != expected_main_thresholds || anyNA(support_compare$threshold_spec_id)) abortf("I08 main-panel/global-audit threshold reconciliation failed")
if (any(abs(support_compare$selected_file_rows - support_compare$audit_selected_file_rows) > 1e-12)) abortf("I08 selected-file totals disagree with global audit")
if (any(abs(support_compare$selected_issue_total - support_compare$audit_selected_issue_total) > 1e-12)) abortf("I08 selected-issue totals disagree with global audit")

# Join authoritative B06 velocity once across all threshold rows and audit every
# overlapping field. I08 remains authoritative for detector-localized outcomes.
...
join_compare_fields <- c(
  ...
  "event", "event_index", "time_to_event", "post_event",
  "log_age", "ncloc_py_sonarqube", "log_contributors", "log_stars", "log_issues"
)
b06_lookup_fields <- unique(c("repo_id", "time_index", "log_lines_added_py_source", "is_treatment", "cursor", join_compare_fields))
...
if (nrow(main_panel) != rows_before_join) abortf("B06 join changed K10 main-grid row count")
...
main_panel[, is_treatment := b06__is_treatment]
main_panel[, cursor := b06__cursor]
...
if (nrow(primary_panel) != expected_rows_per_threshold) abortf("Primary K10 panel does not contain 1,954 rows")
primary_panel[, `:=`(
...
model_failures <- data.table::data.table(
  threshold_spec_id = character(), threshold = numeric(), primary_analysis = integer(),
  stage = character(), message = character()
)

threshold_table <- unique(main_panel[, .(threshold_spec_id, threshold = threshold_numeric, threshold_role)])
data.table::setorder(threshold_table, threshold)

for (row_index in seq_len(nrow(threshold_table))) {
  threshold_spec_id_value <- as.character(threshold_table$threshold_spec_id[[row_index]])
  ...
  panel <- data.table::copy(main_panel[threshold_spec_id == threshold_spec_id_value & abs(threshold_numeric - threshold_value) <= 1e-12])
  ...
  support_row <- data.table::data.table(
    threshold_spec_id = threshold_spec_id_value,
  ...
  support_all[[threshold_spec_id_value]] <- support_row
  ...
  log_message("INFO", "Fitting ML threshold %.6f (%s)", threshold_value, threshold_spec_id_value)
  ...
  if (fit_capture$error) {
    ...
    model_failures <- data.table::rbindlist(list(model_failures, data.table::data.table(
      threshold_spec_id = threshold_spec_id_value, threshold = threshold_value, primary_analysis = primary_analysis_value,
      stage = "pgmm", message = message_text
    )), use.names = TRUE)
    if (primary_analysis_value == 1L) abortf("Primary-threshold pgmm failed: %s", message_text)

    diagnostics_all[[threshold_spec_id_value]] <- failed_diagnostics(threshold_spec_id_value, threshold_value, primary_analysis_value, message_text)
    instrument_all[[threshold_spec_id_value]] <- data.table::data.table(
      threshold_spec_id = threshold_spec_id_value, threshold = threshold_value, primary_analysis = primary_analysis_value,
      model = "ml_rf_cm_threshold_quality_to_velocity", collapse = FALSE, instrument_specification = "lag(velocity,2)",
    ...
    summary_all[[threshold_spec_id_value]] <- data.table::data.table(
      threshold_spec_id = threshold_spec_id_value, threshold = threshold_value, threshold_role = threshold_role_value,
    ...
    qc_all[[threshold_spec_id_value]] <- data.table::data.table(
      threshold_spec_id = threshold_spec_id_value, threshold = threshold_value,
      ...
      note = c("Full I08 repo-month sample.", "Exact calendar t-1/t-2 support.", paste0("Non-primary sparse-threshold pgmm failure: ", message_text))
    )
    next
  }
  ...
  if (summary_capture$error) {
    ...
    qc_all[[threshold_spec_id_value]] <- data.table::data.table(
      ...
      expected = c(expected_rows_per_threshold, expected_active_rows, NA_real_),
      status = c("pass", "pass", "caution"),
      note = c("Full I08 repo-month sample.", "Exact calendar t-1/t-2 support.", paste0("Non-primary sparse-threshold summary failure: ", message_text))
    )
    next
  }

  coefficient_capture <- capture_evaluation(
    extract_coefficients(summary_capture$value, threshold_spec_id_value, threshold_value, threshold_role_value, confidence_level, primary_threshold)
  )
  if (coefficient_capture$error) {
    ...
    qc_all[[threshold_spec_id_value]] <- data.table::data.table(
      ...
      expected = c(expected_rows_per_threshold, expected_active_rows, NA_real_),
      status = c("pass", "pass", "caution"),
      note = c("Full I08 repo-month sample.", "Exact calendar t-1/t-2 support.", paste0("Non-primary coefficient extraction failure: ", message_text))
    )
    next
  }
  ...
  if (nrow(primary_term_row) != 1L) {
    message_text <- sprintf("Expected exactly one lagged ML RF+CM quality coefficient; observed %d", nrow(primary_term_row))
    if (primary_analysis_value == 1L) abortf("Primary threshold coefficient extraction failed: %s", message_text)
    model_failures <- data.table::rbindlist(list(model_failures, data.table::data.table(
      threshold_spec_id = threshold_spec_id_value, threshold = threshold_value, primary_analysis = primary_analysis_value,
      stage = "coefficient_extraction", message = message_text
    ...
    qc_all[[threshold_spec_id_value]] <- data.table::data.table(
      ...
      note = c("Full I08 repo-month sample.", "Exact calendar t-1/t-2 support.", paste0("Non-primary primary-term extraction failure: ", message_text))
    )
    next
  }
  ...
  diagnostics <- data.table::rbindlist(list(
    extract_htest(summary_capture$value$sargan, threshold_spec_id_value, threshold_value, "sargan", primary_analysis_value),
    extract_htest(summary_capture$value$m1, threshold_spec_id_value, threshold_value, "ar1", primary_analysis_value),
    extract_htest(summary_capture$value$m2, threshold_spec_id_value, threshold_value, "ar2", primary_analysis_value)
  ...
  threshold_qc <- data.table::data.table(
    ...
    note = c(
      "Full I08 repo-month sample.", "Full-sample repository membership.",
      "Exact calendar t-1/t-2 support.", "Exact-calendar active repositories.",
      "pgmm N should equal exact-calendar support.", "Exactly one lagged ML RF+CM quality coefficient.",
  ...
  coefficients_all[[threshold_spec_id_value]] <- coefficients
  diagnostics_all[[threshold_spec_id_value]] <- diagnostics
  instrument_all[[threshold_spec_id_value]] <- instrument
  summary_all[[threshold_spec_id_value]] <- data.table::data.table(
    threshold_spec_id = threshold_spec_id_value,
    threshold = threshold_value,
    threshold_role = threshold_role_value,
  ...
  qc_all[[threshold_spec_id_value]] <- threshold_qc
  models[[threshold_spec_id_value]] <- fit_capture$value
}
...
if (nrow(summary_output) != expected_main_thresholds) abortf("K10 threshold summary does not contain exactly 21 rows")
if (nrow(support_output) != expected_main_thresholds) abortf("K10 support output does not contain exactly 21 rows")
if (any(abs(summary_output$threshold - expected_grid) > 1e-10)) abortf("K10 result threshold grid is incomplete or out of order")

# Exact K08 full-sample reproduction at the primary ML threshold.
k08_reference <- data.table::fread(k08_reference_coefficients_file, na.strings = c("", "NA", "NaN"))
validate_columns(k08_reference, c("sample_spec", "term", "estimate", "std_error", "p_value", "is_primary_interaction_term"), "K08 coefficients")
k08_primary <- k08_reference[
  sample_spec == "full_sample" &
    bool_to_int(is_primary_interaction_term) == 1L &
    term == "lag(log1p_selected_issue_total, 1)"
]
if (nrow(k08_primary) != 1L) abortf("Expected exactly one K08 full-sample primary ML RF+CM quality coefficient")
k10_primary <- summary_output[abs(threshold - primary_threshold) <= 1e-12 & fit_status == "success"]
if (nrow(k10_primary) != 1L) abortf("Expected exactly one successful K10 primary-threshold result")
reproduction <- data.table::data.table(
  metric = c("estimate", "std_error", "p_value"),
  k08_full_sample = c(k08_primary$estimate[[1L]], k08_primary$std_error[[1L]], k08_primary$p_value[[1L]]),
  k10_primary = c(k10_primary$estimate[[1L]], k10_primary$std_error[[1L]], k10_primary$p_value[[1L]])
)
reproduction[, absolute_difference := abs(as.numeric(k08_full_sample) - as.numeric(k10_primary))]
reproduction[, tolerance := reference_tolerance]
reproduction[, status := ifelse(absolute_difference <= reference_tolerance, "pass", "fail")]
if (any(reproduction$status == "fail")) abortf("K10 primary threshold failed exact K08 full-sample reproduction")
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("K10 threshold GMM QC contains hard failures: %s", paste(paste(failed_qc$threshold_spec_id, failed_qc$check, sep = "/"), collapse = ", "))
}

# Merge support diagnostics into the one-row-per-threshold result file to make
# the downstream RF+CM ML threshold-sensitivity figure and reviewer-facing support checks straightforward.
summary_output <- merge(
  ...
  by = "threshold_spec_id", all.x = TRUE, sort = FALSE
)
data.table::setorder(summary_output, threshold)
...
metadata <- data.table::data.table(
  section = c(
    rep("run", 14), rep("definition", 19), rep("qc", 5), rep("software", 3)
  ),
  metric = c(
    ...
    "i08_summary_file", "i08_summary_sha256", "i08_global_audit_file", "i08_global_audit_sha256",
    "k08_reference_coefficients_file",
    "analysis_scope", "localization_scope", "detector_scope_pair", "detector", "ml_metric", "sample_spec", "primary_threshold",
    "threshold_max", "threshold_step", "main_threshold_count", "threshold_grid_policy",
    "ml_operator", "localized_quality", "velocity", "treatment", "size_control",
    "other_controls", "gmm_specification", "formula",
    "thresholds_successful", "thresholds_failed", "failed_hard_qc", "caution_qc_rows", "k08_reproduction_status",
    "R", "data.table", "plm"
  ),
  value = c(
    "run-x-k10", implementation_version,
    ...
    i08_summary_file, sha256_file(i08_summary_file), i08_global_audit_file, sha256_file(i08_global_audit_file),
    k08_reference_coefficients_file,
    "ml_rf_cm_threshold_sensitivity_quality_to_velocity", "RF+CM", "ML|RF+CM", "AGCDetector_ML", "file_ml_fun_cfun_agc_share_space_by_token_weighted", "full_sample",
    sprintf("%.6f", primary_threshold), sprintf("%.2f", threshold_max), sprintf("%.2f", threshold_step), expected_main_thresholds,
    "fixed prespecified 0.10--0.90 grid by 0.04; no threshold selection", ">", "log1p_selected_issue_total", "log_lines_added_py_source",
...
log_message(
  "INFO",
  "Completed run-x-k10 %s: thresholds=%d; successful=%d; failed=%d; significant=%d; cautions=%d",
