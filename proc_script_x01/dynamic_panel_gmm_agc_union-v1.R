...
# ============================================================
# run-x-e04 v1: Dynamic panel GMM for NPR-union-ML quality burden
# ============================================================
#
# Purpose:
#   Estimate whether unresolved Python static-analysis issue burden localized
#   to the union of the frozen NPR-selected and ML-selected file sets predicts
#   lower subsequent whole-Python development velocity.
#
# Frozen union contract:
...
#
# GMM model:
#   Velocity_t ~ Velocity_{t-1} + UnionQuality_{t-1} + treatment_t + controls_t
#   GMM instrument: Velocity_{t-2}
#   effect=twoways, model=twosteps, transformation=d, collapse=FALSE
#
# Inputs:
#   - E04 union repo-month panel produced by build_agc_detector_union_gmm_panel.py
...
paths <- list(
  coefficients = file.path(output_dir, "dynamic_panel_gmm_agc_union_coefficients.csv"),
  primary_summary = file.path(output_dir, "dynamic_panel_gmm_agc_union_primary_summary.csv"),
  diagnostics = file.path(output_dir, "dynamic_panel_gmm_agc_union_diagnostics.csv"),
  sample_qc = file.path(output_dir, "dynamic_panel_gmm_agc_union_sample_qc.csv"),
  instrument_qc = file.path(output_dir, "dynamic_panel_gmm_agc_union_instrument_qc.csv"),
  b06_join_audit = file.path(output_dir, "dynamic_panel_gmm_agc_union_b06_join_audit.csv"),
  numeric_audit = file.path(output_dir, "dynamic_panel_gmm_agc_union_numeric_coercion_audit.csv"),
  calendar_gap_audit = file.path(output_dir, "dynamic_panel_gmm_agc_union_calendar_gap_audit.csv"),
  legacy_audit = file.path(output_dir, "dynamic_panel_gmm_agc_union_legacy_flag_audit.csv"),
  qc = file.path(output_dir, "dynamic_panel_gmm_agc_union_qc.csv"),
  metadata = file.path(output_dir, "dynamic_panel_gmm_agc_union_run_metadata.csv"),
  models_rds = file.path(output_dir, "dynamic_panel_gmm_agc_union_models.rds")
)

run_started <- Sys.time()
log_message("INFO", "Reading E04 detector-union panel: %s", input_file)
union_panel <- data.table::fread(input_file, na.strings = c("", "NA", "NaN"))
required_union <- c(
  ...
  "union_selected_file_rows", "npr_threshold", "ml_threshold", "detector_union_rule"
)
validate_columns(union_panel, required_union, "E04 union panel")
strict_count_check(nrow(union_panel), expected_rows, "E04 union panel rows", strict_expected_counts)
if (data.table::uniqueN(union_panel[, .(repo_id, time_index)]) != nrow(union_panel)) abortf("E04 union panel has duplicate repo-month keys")
if (any(as.character(union_panel$detector_union_rule) != "NPR_primary_OR_ML_primary")) abortf("Unexpected union rule in E04 input")
if (any(abs(as.numeric(union_panel$npr_threshold) - 1.571637) > 1e-12)) abortf("Unexpected NPR threshold in E04 input")
if (any(abs(as.numeric(union_panel$ml_threshold) - 0.50) > 1e-12)) abortf("Unexpected ML threshold in E04 input")
...
union_panel[, `:=`(repo_id = as.integer(repo_id), time_index = as.integer(time_index))]
b06[, `:=`(repo_id = as.integer(repo_id), time_index = as.integer(time_index))]
if (anyNA(union_panel$repo_id) || anyNA(union_panel$time_index) || anyNA(b06$repo_id) || anyNA(b06$time_index)) abortf("repo_id/time_index must be integer-compatible and complete")
...
rows_before_join <- nrow(union_panel)
panel <- merge(union_panel, b06_lookup, by = c("repo_id", "time_index"), all.x = TRUE, sort = FALSE)
...
if (log_mismatches > 0L) abortf("Union log1p outcome mismatch: %d rows", log_mismatches)
...
model_name <- "agc_union_quality_to_velocity"
primary_term <- "lag(log1p_selected_issue_total, 1)"
direction <- "UnionQuality_{t-1} -> Velocity_t"
...
log_message("INFO", "Fitting NPR-union-ML localized quality -> velocity GMM")
...
if (nrow(primary_summary) != 1L) abortf("Expected exactly one union-quality primary coefficient")
...
if (nrow(failed_qc) > 0L && strict_expected_counts) abortf("run-x-e04 GMM QC failed: %s", paste(failed_qc$check, collapse = ", "))
...
metadata <- data.table::data.table(
  ...
  metric = c(
    ...
    "analysis_scope", "detector_combination", "union_rule", "npr_rule", "ml_rule",
    "localized_quality", "velocity", "treatment", "size_control", "other_controls",
    "gmm_specification", "formula", "failed_checks", "caution_checks"
  ),
  value = c(
    "run-x-e04", implementation_version, format(run_started, "%Y-%m-%d %H:%M:%S %Z"), format(run_finished, "%Y-%m-%d %H:%M:%S %Z"),
    input_file, sha256_file(input_file), b06_panel_file, sha256_file(b06_panel_file), R.version.string, safe_package_version("plm"),
    "agc_detector_union_localized_quality_to_velocity", "AGCDetector_NPR union AGCDetector_ML",
    "NPR selected OR ML selected; overlap counted once",
...
log_message(
  "INFO",
  "Completed run-x-e04 %s: beta=%.9f; se=%.9f; p=%.9f; failed_qc=%d; cautions=%d",
