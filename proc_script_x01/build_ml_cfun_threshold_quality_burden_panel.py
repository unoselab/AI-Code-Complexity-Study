#!/usr/bin/env python3
"""Build the run-x-h07-v1 C_FUN ML-threshold x SonarQube quality burden panel.
...
validated A07 continuous C_FUN file-level ML AGC share with the canonical D02
file-level SonarQube burden and the B06 repo-month panel, then materializes a
zero-inclusive threshold x sample x repo-month panel for downstream H07 Borusyak threshold stage.

Scientific contract
-------------------
- ML file metric: file_ml_cfun_agc_share_space_by_token_weighted
...
- Primary 0.50 burden must exactly reproduce H05 all_ml_files for both samples
- Density is not computed because selected-file SonarQube NCLOC is unavailable

Primary output
--------------
quality_ml_cfun_threshold_input_panel.csv.gz
...
EXPERIMENT_NAME = "run-x-h07-build-ml-cfun-threshold-quality-burden-panel"
ML_METRIC = "file_ml_cfun_agc_share_space_by_token_weighted"
...
BURDEN_SOURCES = {
    ...
    # development fixtures without changing the standardized H07 output names.
...
def audit_missing_sha_identity(a07: pd.DataFrame, unique_d02: pd.DataFrame) -> dict[str, int]:
    a07_missing = a07.loc[a07["file_sha256"].isna(), ["snapshot_id", "relative_path"]].copy()
    d02_missing = unique_d02.loc[
        unique_d02["file_sha256"].isna(), ["snapshot_id", "relative_path"]
    ].copy()
    if a07_missing.duplicated(["snapshot_id", "relative_path"]).any():
        abort("A07 missing-SHA rows are not unique by snapshot_id + relative_path")
    ...
    return {
        "a07_missing_sha_files": int(len(a07_missing)),
        "d02_missing_sha_files": int(len(d02_missing)),
        "d02_only_missing_sha_keys": int((comparison["_merge"] == "left_only").sum()),
        "a07_only_missing_sha_keys": int((comparison["_merge"] == "right_only").sum()),
    }
...
def parse_thresholds(text: str) -> list[float]:
    ...
    if len(values) != len(expected) or any(
        ...
        abort(f"H07 requires exact 21-point grid {expected}; observed {values}")
    return values


def extract_h05_mapping_reference(h05: pd.DataFrame) -> pd.DataFrame:
    require_columns(h05, ["sample_spec", "mapping_spec", "repo_id", "time_index"], "H05")
    ref = h05.loc[h05["mapping_spec"].astype(str).eq(MAPPING_SPEC)].copy()
    if ref.empty:
        abort("H05 all_ml_files mapping reference is empty")
    ref = normalize_repo_time_keys(ref, "H05 all_ml_files reference")
    if ref.duplicated(["sample_spec", "repo_id", "time_index"]).any():
        abort("H05 all_ml_files reference has duplicate sample/repo-month keys")
    return ref


def resolve_h05_file_count_column(h05: pd.DataFrame) -> str:
    for column in ("selected_file_rows", "selected_file_count"):
        if column in h05.columns:
            return column
    abort("H05 reference is missing selected_file_rows/selected_file_count")
    raise AssertionError


def derive_sample_specs(h05_ref: pd.DataFrame, b06: pd.DataFrame) -> tuple[dict[str, set[int]], list[int]]:
    required_samples = {"full_sample", "exclude_scope_mismatch_repos"}
    observed = set(h05_ref["sample_spec"].astype(str).unique())
    if not required_samples.issubset(observed):
        abort(f"H05 reference lacks required sample specs: {sorted(required_samples - observed)}")

    full_ids = set(h05_ref.loc[h05_ref["sample_spec"].eq("full_sample"), "repo_id"].astype(int).unique())
    sensitivity_ids = set(
        h05_ref.loc[h05_ref["sample_spec"].eq("exclude_scope_mismatch_repos"), "repo_id"].astype(int).unique()
    )
    b06_ids = set(b06["repo_id"].astype(int).unique())
    if full_ids != b06_ids:
        abort(f"H05 full-sample repository set differs from B06: H05={len(full_ids)}, B06={len(b06_ids)}")
    if not sensitivity_ids.issubset(full_ids):
        abort("H05 sensitivity repository set is not a subset of the full sample")
...
def compare_primary_to_h05(
    primary_panels: dict[str, pd.DataFrame],
    h05_ref: pd.DataFrame,
    h05_file_count_column: str,
) -> pd.DataFrame:
    audit_rows: list[dict[str, object]] = []
    comparison_columns = ["selected_file_count", *BURDEN_SOURCES.keys()]
    for sample_spec, panel in primary_panels.items():
        reference = h05_ref.loc[h05_ref["sample_spec"].eq(sample_spec)].copy()
        reference = reference.rename(columns={h05_file_count_column: "selected_file_count"})
        required_ref = ["repo_id", "time_index", "selected_file_count", *BURDEN_SOURCES.keys()]
        require_columns(reference, required_ref, f"H05 {sample_spec} reference")
    ...
    if audit["status"].ne("pass").any():
        abort("H05 primary >0.50 reproduction failed; inspect reproduction audit")
    return audit



def run_self_test() -> None:
    """Run lightweight structural checks without production inputs."""
    assert len(DEFAULT_THRESHOLDS) == 21
    assert math.isclose(DEFAULT_THRESHOLDS[10], PRIMARY_THRESHOLD, abs_tol=1e-12)
    assert threshold_spec_id(PRIMARY_THRESHOLD) == "ml_t50"
    assert MAPPING_SPEC == "all_ml_files"
    assert ML_METRIC == "file_ml_cfun_agc_share_space_by_token_weighted"
    assert STRICT_OPERATOR == ">"
    print("build_ml_cfun_threshold_quality_burden_panel self-test: PASS")
...
def main() -> None:
    ...
    parser.add_argument("--h05-reference-file", type=Path)
    parser.add_argument("--output-dir", type=Path)
    parser.add_argument("--thresholds", default=",".join(f"{v:.2f}" for v in DEFAULT_THRESHOLDS))
    parser.add_argument("--expected-a07-rows", type=int, default=494592)
    ...
    parser.add_argument("--expected-eligible-rows", type=int, default=202537)
    parser.add_argument("--expected-eligible-unique-files", type=int, default=196190)
    parser.add_argument("--expected-primary-selected-rows", type=int, default=37757)
    parser.add_argument("--expected-primary-selected-unique-files", type=int, default=36556)
    parser.add_argument("--expected-primary-issue-stock", type=float, default=36432.0)
    ...
    required_args = {"a07_file": args.a07_file, "d02_file": args.d02_file, "b06_file": args.b06_file, "h05_reference_file": args.h05_reference_file, "output_dir": args.output_dir}
    missing_args = [name for name, value in required_args.items() if value is None]
    if missing_args:
        abort(f"Missing required production arguments: {missing_args}")
    for path in (args.a07_file, args.d02_file, args.b06_file, args.h05_reference_file):
        if not path.is_file():
            abort(f"Required input does not exist: {path}")
    args.output_dir.mkdir(parents=True, exist_ok=True)

    print(f"Reading A07 ML file scores: {args.a07_file}")
    a07 = normalize_file_keys(read_table(args.a07_file), "A07")
    ...
    print(f"Reading H05 primary reference: {args.h05_reference_file}")
    h05 = read_table(args.h05_reference_file)
    h05_ref = extract_h05_mapping_reference(h05)

    if len(a07) != args.expected_a07_rows:
        abort(f"A07 row mismatch: expected {args.expected_a07_rows}, observed {len(a07)}")
    ...
    require_columns(a07, [ML_METRIC], "A07")
    a07[ML_METRIC] = pd.to_numeric(a07[ML_METRIC], errors="coerce")
    if a07.duplicated(list(JOIN_FILE_KEYS)).any():
        abort("A07 has duplicate historical file keys")
    ...
    missing_sha = audit_missing_sha_identity(a07, unique_d02)
    if missing_sha["d02_only_missing_sha_keys"] or missing_sha["a07_only_missing_sha_keys"]:
        abort(f"A07/D02 missing-SHA identity mismatch: {missing_sha}")

    universe = unique_d02[list(JOIN_FILE_KEYS)].merge(
        a07[list(JOIN_FILE_KEYS)],
    ...
    a07_only = int((universe["_merge"] == "right_only").sum())
    if d02_only or a07_only:
        abort(f"A07/D02 historical-file universe mismatch: d02_only={d02_only}, a07_only={a07_only}")

    base = d02.merge(
        a07[list(JOIN_FILE_KEYS) + [ML_METRIC]],
        on=list(JOIN_FILE_KEYS),
        how="left",
        validate="many_to_one",
    )
    if len(base) != len(d02):
        abort("A07/D02 expanded join changed D02 row count")
    ...
    sample_repo_ids, excluded_repo_ids = derive_sample_specs(h05_ref, b06)
    ...
    if int(primary_row["selected_file_rows"]) != args.expected_primary_selected_rows:
        abort("Primary selected expanded-file count does not reproduce H05")
    if int(primary_row["selected_unique_historical_files"]) != args.expected_primary_selected_unique_files:
        abort("Primary selected unique-file count does not reproduce A07/H05")
    if not math.isclose(float(primary_row["selected_issue_total"]), args.expected_primary_issue_stock, abs_tol=1e-9):
        abort("Primary selected issue stock does not reproduce H05")

    h05_file_count_column = resolve_h05_file_count_column(h05_ref)
    reproduction = compare_primary_to_h05(primary_panels, h05_ref, h05_file_count_column)
    ...
    add_qc("a07_rows", len(a07), args.expected_a07_rows, "pass")
    ...
    add_qc("a07_missing_sha_files", missing_sha["a07_missing_sha_files"], missing_sha["d02_missing_sha_files"], "pass")
    missing_sha_mismatch = missing_sha["d02_only_missing_sha_keys"] + missing_sha["a07_only_missing_sha_keys"]
    ...
    add_qc("a07_only_file_keys", a07_only, 0, "pass" if a07_only == 0 else "fail")
    ...
    add_qc("primary_h05_reproduction_mismatches", reproduction_failures, 0, "pass" if reproduction_failures == 0 else "fail")
    ...
    if qc["status"].eq("fail").any():
        abort("H07 threshold-input hard QC failed; inspect quality_ml_cfun_threshold_input_checks.csv")

    output_panel = args.output_dir / "quality_ml_cfun_threshold_input_panel.csv.gz"
    output_global = args.output_dir / "quality_ml_cfun_threshold_input_global_audit.csv"
    output_timing = args.output_dir / "quality_ml_cfun_threshold_input_by_treatment_timing.csv"
    output_sample = args.output_dir / "quality_ml_cfun_threshold_input_sample_summary.csv"
    output_outcomes = args.output_dir / "quality_ml_cfun_threshold_input_outcome_spec.csv"
    output_reproduction = args.output_dir / "quality_ml_cfun_threshold_input_h05_reproduction.csv"
    output_checks = args.output_dir / "quality_ml_cfun_threshold_input_checks.csv"
    output_summary = args.output_dir / "quality_ml_cfun_threshold_input_summary.csv"
    output_metadata = args.output_dir / "quality_ml_cfun_threshold_input_metadata.json"
    ...
    summary = pd.DataFrame([
        ("script_version", f"run-x-h07-{IMPLEMENTATION_VERSION}-prep"),
        ("status", "PASS"),
        ("ml_metric", ML_METRIC),
    ...
    metadata = {
        ...
        "inputs": {
            "a07_file": str(args.a07_file), "a07_sha256": sha256_file(args.a07_file),
            ...
            "h05_reference_file": str(args.h05_reference_file), "h05_reference_sha256": sha256_file(args.h05_reference_file),
        },
        "ml_metric": ML_METRIC,
        ...
        "downstream_experiment": "run-x-h07-did-stage",
    ...
    print("=" * 80)
    print("run-x-h07-v1 C_FUN ML threshold x SonarQube burden input")
    print("Status:                                  PASS")
    ...
    print(f"A07/D02 missing file_sha256 files:       {missing_sha['a07_missing_sha_files']}/{missing_sha['d02_missing_sha_files']}")
    print(f"Scope-sensitivity repositories excluded: {len(excluded_repo_ids)}")
    print("H05 primary reproduction:                PASS")
    print("Density computed:                        0")
    print("Hard QC failures:                        0")
    print(f"H07 DiD input panel:                     {output_panel}")
    print("=" * 80)
