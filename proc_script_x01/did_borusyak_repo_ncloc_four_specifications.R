...
# ============================================================
# run-x-c06 v2: Borusyak DiD for four repository-NCLOC specs
# ============================================================
#
...
expected_dynamic_rows <- as_int("expected_dynamic_rows", 96L)
expected_pretrend_rows <- as_int("expected_pretrend_rows", 40L)
...
expected_dynamic_comparison_rows <- as_int("expected_dynamic_comparison_rows", 72L)
...
# didimputation omits event time -1 as the normalized reference period.
# It is requested in horizon but is not returned as an estimated coefficient.
estimated_horizon_values <- setdiff(horizon_values, reference_event_time)
...
expected_horizon_terms <- length(estimated_horizon_values)
...
horizon_set_audit <- dynamic_effects[, .(
  ...
  horizon_matches = identical(sort(unique(event_time)), sort(estimated_horizon_values))
), by = .(specification_id, outcome)]
dynamic_wrong_horizon_models <- horizon_set_audit[!horizon_matches, .N]
add_qc(
  ...
  sprintf(
    ...
    paste(estimated_horizon_values, collapse = ";"),
    reference_event_time
  )
)
...
summary_output <- data.table(
  ...
  estimated_horizon_terms_per_model = length(estimated_horizon_values),
...
log_message(
  paste0(
    "Completed run-x-c06-v2: specifications=%d; outcomes=%d; ",
