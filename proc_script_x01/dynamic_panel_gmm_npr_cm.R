...
# ============================================================
# run-x-k02 v1: Dynamic panel GMM for NPR-localized CM issue burden
# ============================================================
#
# Purpose:
#   Estimate whether unresolved Python static-analysis issue burden localized
#   to files selected by the frozen NPR primary threshold within the CM localization scope predicts lower
...
#   - file NPR metric: file_npr_cfun_space_by_token_weighted
...
#
# Primary GMM model:
#   Velocity_t ~ Velocity_{t-1} + NPRCMQuality_{t-1} + treatment_t + controls_t
...
#   K02 therefore addresses the pre-specified secondary interaction question:
#   NPR-localized CM issue burden_t -> whole-Python velocity_{t+1}.
#
...
#
# Join rule:
#   H03 intentionally contains localized quality outcomes but not the B02/B06
#   whole-Python velocity outcome. K02 therefore joins log_lines_added_py_source
...
h03_summary_file <- normalizePath(require_arg(args, "h03_summary_file"), mustWork = TRUE)
h03_sample_summary_file <- normalizePath(require_arg(args, "h03_sample_summary_file"), mustWork = TRUE)
h03_global_audit_file <- normalizePath(require_arg(args, "h03_global_audit_file"), mustWork = TRUE)
...
expected_primary_full_selected_files <- as_integer_arg(args, "expected_primary_full_selected_files", 7185L)
expected_primary_full_issue_stock <- as_integer_arg(args, "expected_primary_full_issue_stock", 9198L)
expected_primary_sensitivity_selected_files <- as_integer_arg(args, "expected_primary_sensitivity_selected_files", 7045L)
expected_primary_sensitivity_issue_stock <- as_integer_arg(args, "expected_primary_sensitivity_issue_stock", 9158L)
...
paths <- list(
  coefficients = file.path(output_dir, "dynamic_panel_gmm_npr_cm_coefficients.csv"),
  diagnostics = file.path(output_dir, "dynamic_panel_gmm_npr_cm_diagnostics.csv"),
  sample_qc = file.path(output_dir, "dynamic_panel_gmm_npr_cm_sample_qc.csv"),
  instrument_qc = file.path(output_dir, "dynamic_panel_gmm_npr_cm_instrument_qc.csv"),
  model_specs = file.path(output_dir, "dynamic_panel_gmm_npr_cm_model_specifications.csv"),
  threshold_input_audit = file.path(output_dir, "dynamic_panel_gmm_npr_cm_threshold_input_audit.csv"),
  b06_join_audit = file.path(output_dir, "dynamic_panel_gmm_npr_cm_b06_join_audit.csv"),
  legacy_audit = file.path(output_dir, "dynamic_panel_gmm_npr_cm_legacy_flag_audit.csv"),
  coercion_audit = file.path(output_dir, "dynamic_panel_gmm_npr_cm_numeric_coercion_audit.csv"),
  calendar_gap_audit = file.path(output_dir, "dynamic_panel_gmm_npr_cm_calendar_gap_audit.csv"),
  run_metadata = file.path(output_dir, "dynamic_panel_gmm_npr_cm_run_metadata.csv"),
  qc = file.path(output_dir, "dynamic_panel_gmm_npr_cm_qc.csv"),
  models_rds = file.path(output_dir, "dynamic_panel_gmm_npr_cm_models.rds")
)

run_started <- Sys.time()
log_message("INFO", "Reading H03 CM-NPR threshold quality panel: %s", input_file)
...
required_columns <- c(
  ...
  "selected_issue_total", "selected_file_count", "eligible_cfun_file_count",
  "log_age", "ncloc_py_sonarqube", "log_contributors", "log_stars", "log_issues"
)
validate_columns(long_panel, required_columns)

strict_count_check(nrow(long_panel), expected_long_rows, "H03 long-panel rows", strict_expected_counts)
strict_count_check(data.table::uniqueN(long_panel$threshold_id), expected_thresholds, "H03 threshold specifications", strict_expected_counts)
strict_count_check(data.table::uniqueN(long_panel$sample_spec), expected_sample_specs, "H03 sample specifications", strict_expected_counts)

# H03 was built from B06 for quality/timing support, but the H03 long panel does
# not intentionally carry the whole-Python velocity outcome. Read B06 directly
# and use it as the authoritative velocity source for K02.
...
if (nrow(primary_panel) == 0L) abortf("No H03 primary-threshold rows found for strict NPR > %.6f.", primary_threshold)
...
if (!all(sample_specs %in% unique(primary_panel$sample_spec))) {
  abortf("Primary H03 panel is missing required sample specifications: %s", paste(setdiff(sample_specs, unique(primary_panel$sample_spec)), collapse = ", "))
}
primary_panel <- primary_panel[sample_spec %in% sample_specs]

# Join only after freezing the H03 primary threshold and the two pre-specified
...
if (anyNA(primary_panel$repo_id) || anyNA(primary_panel$time_index)) {
  abortf("H03 primary repo_id/time_index must be complete integer-compatible keys before the B06 join.")
}
...
if (nrow(primary_panel) != primary_rows_before_b06_join) {
  abortf("B06 join changed H03 primary row count: before=%d after=%d.", primary_rows_before_b06_join, nrow(primary_panel))
}

# Missing B06 velocity means the H03 repo-month key did not resolve to the
...
if (b06_join_missing_velocity > 0L) {
  abortf("B06 join left %d H03 primary rows without log_lines_added_py_source.", b06_join_missing_velocity)
}
...
join_audit_parts <- list(
  data.table::data.table(
    ...
    note = "H03 primary rows before the B06 velocity join."
  ),
  data.table::data.table(
    ...
    note = "B06 join must not change H03 primary sample membership."
  ),
  data.table::data.table(
    ...
    note = "Every H03 primary repo-month must resolve to authoritative B06 log_lines_added_py_source."
  )
)
for (field in join_compare_fields) {
  ...
  join_audit_parts[[length(join_audit_parts) + 1L]] <- data.table::data.table(
    ...
    note = "H03 field must agree with the authoritative B06 source on matched repo-month keys."
  )
}
...
# Validate H03 summary and sample-support artifacts before estimation.
h03_summary <- data.table::fread(h03_summary_file, na.strings = c("", "NA", "NaN"))
validate_columns(h03_summary, c("metric", "value"))
summary_lookup <- setNames(as.character(h03_summary$value), as.character(h03_summary$metric))
if (!identical(summary_lookup[["status"]], "PASS")) abortf("H03 summary status is not PASS.")
if (abs(as.numeric(summary_lookup[["primary_threshold"]]) - primary_threshold) > 1e-12) abortf("H03 summary primary threshold mismatch.")

sample_summary_input <- data.table::fread(h03_sample_summary_file, na.strings = c("", "NA", "NaN"))
...
if (nrow(sample_summary_input[sample_spec == "full_sample"]) != 1L ||
  ...
  abortf("H03 sample summary must contain exactly one row for each K02 sample specification.")
}
strict_count_check(sample_summary_input[sample_spec == "full_sample", repo_month_rows], expected_full_rows, "H03 sample-summary full rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", repositories], expected_full_repositories, "H03 sample-summary full repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", treatment_repositories], expected_full_treatment_repos, "H03 sample-summary full treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", control_repositories], expected_full_control_repos, "H03 sample-summary full control repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repo_month_rows], expected_sensitivity_rows, "H03 sample-summary sensitivity rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repositories], expected_sensitivity_repositories, "H03 sample-summary sensitivity repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", treatment_repositories], expected_sensitivity_treatment_repos, "H03 sample-summary sensitivity treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", control_repositories], expected_sensitivity_control_repos, "H03 sample-summary sensitivity control repositories", strict_expected_counts)

global_audit <- data.table::fread(h03_global_audit_file, na.strings = c("", "NA", "NaN"))
...
if (nrow(primary_global) != 2L) abortf("Expected two primary rows in H03 global audit; observed %d.", nrow(primary_global))
...
model_specs <- data.table::data.table(
  model = "npr_cm_quality_to_velocity",
  direction = "NPRCMQuality_{t-1} -> Velocity_t",
...
for (sample_spec_value in sample_specs) {
  ...
  model_name <- "npr_cm_quality_to_velocity"
  ...
  qc_parts[[sample_spec]] <- data.table::data.table(
    ...
    note = c(
      "Primary H03 sample invariant.", "Primary H03 sample invariant.", "Primary H03 sample invariant.", "Primary H03 sample invariant.",
      "Must be zero.", "Normalized timing must be exact.", "Numeric parsing must not introduce missingness.",
      "GMM fields must be complete.", "GMM fields must be finite.", "H03 localized quality log outcome must equal log1p(selected_issue_total).",
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("run-x-k02 NPR GMM QC failed: %s", paste(paste(failed_qc$sample_spec, failed_qc$check, sep = "/"), collapse = ", "))
}
...
metadata <- data.table::rbindlist(list(
  make_summary_row("run", "run_prefix", "run-x-k02"),
  ...
  make_summary_row("run", "h03_summary_file", h03_summary_file),
  make_summary_row("run", "h03_summary_sha256", sha256_file(h03_summary_file)),
  make_summary_row("run", "h03_sample_summary_file", h03_sample_summary_file),
  make_summary_row("run", "h03_sample_summary_sha256", sha256_file(h03_sample_summary_file)),
  make_summary_row("run", "h03_global_audit_file", h03_global_audit_file),
  make_summary_row("run", "h03_global_audit_sha256", sha256_file(h03_global_audit_file)),
  ...
  make_summary_row("definition", "analysis_scope", "agc_npr_localized_quality_to_velocity"),
  make_summary_row("definition", "detector", "AGCDetector_NPR"),
  make_summary_row("definition", "npr_metric", "file_npr_cfun_space_by_token_weighted"),
  ...
  make_summary_row("definition", "quality_semantics", "unresolved SonarQube issue stock among Python files with NPR-CM > 1.571637 at the corresponding historical snapshot"),
...
log_message(
  "INFO",
  "Completed run-x-k02 %s: samples=%d; coefficients=%d; diagnostics=%d; failed_qc=%d; cautions=%d",
