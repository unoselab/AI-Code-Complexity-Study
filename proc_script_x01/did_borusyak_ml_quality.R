...
# ============================================================
# run-x-d06 v1: Borusyak DiD for frozen ML-selected quality burden
# ============================================================
#
# Purpose:
#   Estimate Cursor-adoption effects on unresolved SonarQube issue burden
#   concentrated in historical Python files selected by the frozen run-x-a04
#   ML AGC-like file rule and aggregated by run-x-d05.
#
# Primary analysis:
...
#   - Exclude A02/A04 mapping-warning files.
#   - Exclude the two repositories frozen by the prior scope sensitivity.
#   - Apply both exclusions together.
#
# Estimation design copied from the validated run-x-d04 / run-x-b07 logic:
...
#   ML-AGC-like by the frozen detector/file rule. It is not a calibrated defect
...
outcome_labels <- c(
  ...
  log1p_selected_issue_code_smell = "ML-selected code-smell stock",
  log1p_selected_issue_bug = "ML-selected bug stock",
  log1p_selected_issue_vulnerability = "ML-selected vulnerability stock",
  log1p_selected_issue_high_severity = "ML-selected BLOCKER+CRITICAL stock",
  log1p_selected_issue_maintainability_impact = "ML-selected maintainability-impact stock",
  log1p_selected_issue_reliability_impact = "ML-selected reliability-impact stock",
  log1p_selected_issue_security_impact = "ML-selected security-impact stock"
)
...
d05_summary_file <- normalizePath(require_arg(args, "d05_summary_file"), mustWork = TRUE)
d05_sample_summary_file <- normalizePath(require_arg(args, "d05_sample_summary_file"), mustWork = TRUE)
d05_global_audit_file <- normalizePath(require_arg(args, "d05_global_audit_file"), mustWork = TRUE)
...
paths <- list(
  static = file.path(output_dir, "quality_ml_static_effects.csv"),
  dynamic = file.path(output_dir, "quality_ml_dynamic_effects.csv"),
  pretrend_checks = file.path(output_dir, "quality_ml_pretrend_checks.csv"),
  pretrend_summary = file.path(output_dir, "quality_ml_pretrend_summary.csv"),
  diagnostics = file.path(output_dir, "quality_ml_model_diagnostics.csv"),
  failures = file.path(output_dir, "quality_ml_model_failures.csv"),
  analysis_support = file.path(output_dir, "quality_ml_analysis_support.csv"),
  event_support = file.path(output_dir, "quality_ml_event_support.csv"),
  support_policy = file.path(output_dir, "quality_ml_support_policy.csv"),
  primary_static = file.path(output_dir, "quality_ml_primary_static.csv"),
  ...
  total_static_by_analysis = file.path(output_dir, "quality_ml_total_static_by_analysis.csv"),
  total_dynamic_by_analysis = file.path(output_dir, "quality_ml_total_dynamic_by_analysis.csv"),
  qc = file.path(output_dir, "quality_ml_qc.csv"),
  summary = file.path(output_dir, "quality_ml_summary.csv"),
  metadata = file.path(output_dir, "quality_ml_run_metadata.csv"),
  static_plot_pdf = file.path(plot_dir, "quality_ml_total_static_by_analysis.pdf"),
  static_plot_png = file.path(plot_dir, "quality_ml_total_static_by_analysis.png"),
  primary_dynamic_pdf = file.path(plot_dir, "quality_ml_primary_total_dynamic.pdf"),
  primary_dynamic_png = file.path(plot_dir, "quality_ml_primary_total_dynamic.png")
)

run_started <- Sys.time()
log_message("INFO", "Reading D05 panel: %s", input_file)
panel <- data.table::fread(input_file, na.strings = c("", "NA", "NaN"))
d05_summary <- data.table::fread(d05_summary_file, na.strings = c("", "NA", "NaN"))
d05_sample_summary <- data.table::fread(d05_sample_summary_file, na.strings = c("", "NA", "NaN"))
d05_global_audit <- data.table::fread(d05_global_audit_file, na.strings = c("", "NA", "NaN"))

validate_columns(d05_summary, c("metric", "value"))
validate_columns(d05_sample_summary, c(
...
validate_columns(d05_global_audit, c(
...
if (lookup_metric(d05_summary, "status") != "PASS") abortf("D05 summary status is not PASS.")
strict_count_check(as.integer(lookup_metric(d05_summary, "sample_specs")), expected_sample_specs, "D05 sample specs", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(d05_summary, "mapping_specs")), expected_mapping_specs, "D05 mapping specs", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(d05_summary, "b06_repo_month_rows")), 1954L, "D05 B06 repo-month rows", strict_expected_counts)
if (as.integer(lookup_metric(d05_summary, "density_computed")) != 0L) abortf("D05 density_computed must be zero for D06 burden analysis.")
if (abs(as.numeric(lookup_metric(d05_summary, "primary_ml_threshold")) - primary_threshold) > 1e-12) abortf("D05 primary ML threshold does not match D06.")
if (lookup_metric(d05_summary, "primary_ml_operator") != ">") abortf("D05 primary ML operator must be strict >.")
if (lookup_metric(d05_summary, "primary_ml_metric") != "file_ml_agc_share_space_by_token_weighted") abortf("Unexpected D05 primary ML metric.")
...
strict_count_check(input_rows, expected_long_rows, "D06 input rows", strict_expected_counts)

contract_mismatch_rows <- panel[
  ml_primary_operator != ">" |
    ml_primary_metric != "file_ml_agc_share_space_by_token_weighted" |
    abs(ml_primary_threshold - primary_threshold) > 1e-12
]
if (nrow(contract_mismatch_rows) > 0L) abortf("Found %d D05 ML-contract mismatch rows.", nrow(contract_mismatch_rows))
...
if (sum(analysis_catalog$primary_analysis == 1L) != 1L) abortf("D05 primary_analysis must identify exactly one configuration.")
primary_catalog <- analysis_catalog[analysis_role == "primary"]
if (primary_catalog$primary_analysis[[1L]] != 1L) abortf("D05 primary_analysis flag does not match the frozen primary configuration.")
...
if (nrow(timing_mismatch_rows) > 0L) abortf("Found %d D05 timing-reconstruction mismatches.", nrow(timing_mismatch_rows))
...
if (nrow(missing_model_rows) > 0L) abortf("Found %d rows with missing D06 model fields.", nrow(missing_model_rows))
nonfinite_model_rows <- panel[!apply(as.data.frame(panel[, ..all_model_fields]), 1L, function(row) all(is.finite(row)))]
if (nrow(nonfinite_model_rows) > 0L) abortf("Found %d rows with non-finite D06 model fields.", nrow(nonfinite_model_rows))

# Reconcile the D05 panel to its frozen global audit before estimation.
input_global <- panel[, .(
  repo_month_rows = .N,
...
if (any(audit_compare$mismatch)) abortf("D06 input does not reconcile to D05 global audit for %d configurations.", sum(audit_compare$mismatch))

# Reconcile the D05 sample summary independently.
...
if (any(sample_compare$mismatch_count > 0L)) abortf("D05 sample-summary reconciliation failed.")
...
support_policy <- data.table::data.table(
  ...
  prespecified_before_d06_results = 1L,
  ...
  rule = "Flag low support if the minimum positive-issue repository count across event 0:+6 is below the first threshold OR repositories with within-panel variation in log1p selected total issue stock are below the second threshold. Never omit a frozen D05 analysis configuration because of this flag."
)
...
qc <- data.table::data.table(
  check = c(
    ...
    "missing_model_rows", "nonfinite_model_rows", "d05_global_audit_mismatch_rows", "d05_sample_summary_mismatch_rows",
...
qc[, note := c(
  ...
  "Selected file/issue counts must reconcile to D05 global audit.",
  "D05 sample/mapping support must reconcile to D05 sample summary.",
...
summary <- data.table::rbindlist(list(
  make_summary_row("run", "script_version", "run-x-d06-v1"),
...
metadata <- data.table::rbindlist(list(
  ...
  make_summary_row("run", "d05_summary_sha256", sha256_file(d05_summary_file)),
  make_summary_row("run", "d05_sample_summary_sha256", sha256_file(d05_sample_summary_file)),
  make_summary_row("run", "d05_global_audit_sha256", sha256_file(d05_global_audit_file)),
  ...
  make_summary_row("definition", "ml_file_rule", "file_ml_agc_share_space_by_token_weighted > 0.50"),
  make_summary_row("definition", "quality_semantics", "unresolved SonarQube issue burden among historical Python files selected by the frozen ML AGC-like file rule"),
...
if (nrow(plot_static_data) > 0L) {
  ...
  p_static <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "ML-selected quality burden DiD across frozen robustness configurations",
      ...
      caption = "All configurations were specified before D06 causal estimates."
    ) +
...
if (nrow(plot_dynamic_data) > 0L) {
  p_dynamic <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "Primary ML-selected quality event study",
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("D06 hard QC failed: %s", paste(failed_qc$check, collapse = ", "))
}

log_message(
  "INFO",
  "Completed run-x-d06 %s: status=%s; jobs=%d; static=%d; dynamic=%d; robustness failures=%d; hard QC failures=%d",
