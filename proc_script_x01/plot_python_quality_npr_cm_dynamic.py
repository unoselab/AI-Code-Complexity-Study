#!/usr/bin/env python3
"""Create CM NPR threshold-sensitivity event-study figures.
...
DEFAULT_OUTPUT_NAMES = {
    ("adjusted_burden", "below"): "fig_did_quality_npr_cm_dynamic-fecs-below-v1.pdf",
    ("adjusted_burden", "at_or_above"): "fig_did_quality_npr_cm_dynamic-fecs-primary-above-v1.pdf",
    ("fe_only_burden", "below"): "fig_did_quality_npr_cm_dynamic-feos-below-v1.pdf",
    ("fe_only_burden", "at_or_above"): "fig_did_quality_npr_cm_dynamic-feos-primary-above-v1.pdf",
}
REQUIRED_COLUMNS = {
    "sample_spec",
    "threshold_role",
...
# Use the class-method color identity from
# plot_did_quality_cfun_ml_threshold_dynamic-v2.py while retaining the common
# NPR manuscript geometry. Eleven marker shapes are available because the
# at-or-above panel can contain the primary threshold and ten higher thresholds.
MARKERS = ["o", "s", "^", "D", "v", "P", "X", "<", ">", "*", "h"]
CLASS_METHOD_COLORS = ["#c34a36", "#717d11", "#008e79"]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot NPR-localized CM issue-burden event-study estimates."
    )
    parser.add_argument("--input", type=Path)
    parser.add_argument("--figure-dir", type=Path)
    parser.add_argument("--output-dir", type=Path)
    parser.add_argument("--sample-spec", default="exclude_scope_mismatch_repos")
    parser.add_argument("--scope-id", default="cm")
...
def write_outputs(
    ...
    data_path = args.output_dir / "python_quality_npr_cm_dynamic_plot_data.csv"
    checks_path = args.output_dir / "python_quality_npr_cm_dynamic_plot_checks.csv"
    manifest_path = args.output_dir / "python_quality_npr_cm_dynamic_plot_manifest.json"
...
def self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="npr-cm-dynamic-self-test-") as temporary:
        root = Path(temporary)
...
def main() -> int:
    ...
    try:
        if args.self_test:
            ...
            return 0
        paths = run(args)
        print(f"PASS: created {len(paths)} NPR CM dynamic figure panels")
        for path in paths:
            print(f"Figure: {path}")
