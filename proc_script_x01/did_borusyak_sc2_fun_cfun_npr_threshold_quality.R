...
# ============================================================
# run-x-l02 v1: Borusyak DiD across frozen SC2-7B FUN+C_FUN-NPR thresholds
# ============================================================
#
# Purpose:
#   Estimate Cursor-adoption effects on unresolved SonarQube issue burden
#   concentrated in Python files whose contemporaneous SC2-7B FUN+C_FUN-NPR exceeds each
#   threshold frozen by run-x-c04 and aggregated by run-x-l01.
#
# Primary interpretation:
#   The outcome is the log1p unresolved SonarQube issue stock among Python files
#   exceeding a specified FUN+C_FUN-NPR threshold at the corresponding historical
...
#
# Input:
#   repo_x01/run-x-l01/threshold-quality-burden-v2/quality_fun_cfun_npr_threshold_repo_month_panel.csv.gz
#
# Frozen analysis dimensions:
#   - 23 FUN+C_FUN-NPR threshold specifications (21 grid + 1 legacy anchor + 1 prior-primary anchor).
...
#
# Estimation design copied from the validated run-x-d04 FUN-NPR analysis and
# adapted only for the l01 SC2-7B FUN+C_FUN-NPR measurement contract:
...
outcome_labels <- c(
  log1p_selected_issue_total = "Selected high-FUN+C_FUN-NPR total issue stock",
  log1p_selected_issue_code_smell = "Selected high-FUN+C_FUN-NPR code-smell stock",
  log1p_selected_issue_bug = "Selected high-FUN+C_FUN-NPR bug stock",
  log1p_selected_issue_vulnerability = "Selected high-FUN+C_FUN-NPR vulnerability stock",
  log1p_selected_issue_high_severity = "Selected high-FUN+C_FUN-NPR BLOCKER+CRITICAL stock",
  log1p_selected_issue_maintainability_impact = "Selected high-FUN+C_FUN-NPR maintainability-impact stock",
  log1p_selected_issue_reliability_impact = "Selected high-FUN+C_FUN-NPR reliability-impact stock",
  log1p_selected_issue_security_impact = "Selected high-FUN+C_FUN-NPR security-impact stock"
)
...
compute_threshold_support <- function(data, post_values, sparse_min_dynamic_positive_repos, sparse_min_within_variation_repos) {
  ...
  data.table::data.table(
    ...
    eligible_fun_cfun_file_rows = sum(data$eligible_fun_cfun_file_count),
...
run_self_test <- function() {
  ...
  cat("did_borusyak_sc2_fun_cfun_npr_threshold_quality self-test: PASS\n")
}
...
l01_summary_file <- normalizePath(require_arg(args, "l01_summary_file"), mustWork = TRUE)
l01_sample_summary_file <- normalizePath(require_arg(args, "l01_sample_summary_file"), mustWork = TRUE)
l01_global_audit_file <- normalizePath(require_arg(args, "l01_global_audit_file"), mustWork = TRUE)
...
paths <- list(
  static = file.path(output_dir, "quality_fun_cfun_npr_static_effects.csv"),
  dynamic = file.path(output_dir, "quality_fun_cfun_npr_dynamic_effects.csv"),
  pretrend_checks = file.path(output_dir, "quality_fun_cfun_npr_pretrend_checks.csv"),
  pretrend_summary = file.path(output_dir, "quality_fun_cfun_npr_pretrend_summary.csv"),
  diagnostics = file.path(output_dir, "quality_fun_cfun_npr_model_diagnostics.csv"),
  failures = file.path(output_dir, "quality_fun_cfun_npr_model_failures.csv"),
  threshold_support = file.path(output_dir, "quality_fun_cfun_npr_threshold_support.csv"),
  event_support = file.path(output_dir, "quality_fun_cfun_npr_event_support.csv"),
  support_policy = file.path(output_dir, "quality_fun_cfun_npr_support_policy.csv"),
  primary_static = file.path(output_dir, "quality_fun_cfun_npr_primary_threshold_static.csv"),
  primary_total_static = file.path(output_dir, "quality_fun_cfun_npr_primary_total_static.csv"),
  primary_total_dynamic = file.path(output_dir, "quality_fun_cfun_npr_primary_total_dynamic.csv"),
  total_threshold_static = file.path(output_dir, "quality_fun_cfun_npr_total_threshold_static.csv"),
  total_threshold_dynamic = file.path(output_dir, "quality_fun_cfun_npr_total_threshold_dynamic.csv"),
  qc = file.path(output_dir, "quality_fun_cfun_npr_qc.csv"),
  summary = file.path(output_dir, "quality_fun_cfun_npr_summary.csv"),
  metadata = file.path(output_dir, "quality_fun_cfun_npr_run_metadata.csv"),
  static_plot_pdf = file.path(plot_dir, "quality_fun_cfun_npr_total_static_across_thresholds.pdf"),
  static_plot_png = file.path(plot_dir, "quality_fun_cfun_npr_total_static_across_thresholds.png"),
  primary_dynamic_pdf = file.path(plot_dir, "quality_fun_cfun_npr_primary_total_dynamic.pdf"),
  primary_dynamic_png = file.path(plot_dir, "quality_fun_cfun_npr_primary_total_dynamic.png")
)

run_started <- Sys.time()
log_message("INFO", "Reading l01 long panel: %s", input_file)
panel <- data.table::fread(input_file, na.strings = c("", "NA", "NaN"))
l01_summary <- data.table::fread(l01_summary_file, na.strings = c("", "NA", "NaN"))
l01_sample_summary <- data.table::fread(l01_sample_summary_file, na.strings = c("", "NA", "NaN"))
l01_global_audit <- data.table::fread(l01_global_audit_file, na.strings = c("", "NA", "NaN"))

validate_columns(l01_summary, c("metric", "value"))
validate_columns(l01_sample_summary, c(
...
validate_columns(l01_global_audit, c(
  "sample_spec", "threshold_id", "threshold_role", "threshold", "selected_file_rows",
  "eligible_fun_cfun_file_rows", "selected_issue_total"
))

if (lookup_metric(l01_summary, "status") != "PASS") abortf("l01 summary status is not PASS.")
strict_count_check(as.integer(lookup_metric(l01_summary, "thresholds")), expected_thresholds, "l01 thresholds", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(l01_summary, "sample_specs")), expected_sample_specs, "l01 sample specs", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(l01_summary, "long_panel_rows")), expected_long_rows, "l01 long-panel rows", strict_expected_counts)
if (as.integer(lookup_metric(l01_summary, "density_computed")) != 0L) abortf("l01 density_computed must be zero for l02 burden analysis.")
if (abs(as.numeric(lookup_metric(l01_summary, "primary_threshold")) - primary_threshold) > 1e-12) {
  abortf("l01 primary threshold does not match l02 primary threshold.")
}
...
required_columns <- unique(c(
  ...
  "eligible_fun_cfun_file_count", "selected_file_count", "selected_issue_total",
...
numeric_columns <- unique(c(
  ...
  "log_issues", "event_time_normalized", "absorbing_treated", "eligible_fun_cfun_file_count",
...
strict_count_check(input_rows, expected_long_rows, "l02 input rows", strict_expected_counts)

contract_mismatch_rows <- panel[
  comparison_operator != ">" |
    npr_metric != "file_npr_fun_cfun_space_by_token_weighted" |
    quality_scope != "canonical_c04_python_files_with_finite_fun_cfun_npr" |
    quality_count_semantics != "unresolved_sonarqube_issue_stock_at_historical_snapshot" |
    density_computed != 0
]
if (nrow(contract_mismatch_rows) > 0L) abortf("Found %d l01 measurement-contract mismatch rows.", nrow(contract_mismatch_rows))
...
if (nrow(prior_primary_catalog) != expected_sample_specs ||
    any(abs(prior_primary_catalog$threshold - prior_primary_threshold) > 1e-12)) {
  abortf("Prior-primary threshold anchor mismatch.")
}
...
if (nrow(timing_mismatch_rows) > 0L) abortf("Found %d l01 timing-reconstruction mismatches.", nrow(timing_mismatch_rows))
...
if (nrow(missing_model_rows) > 0L) abortf("Found %d rows with missing l02 model fields.", nrow(missing_model_rows))
nonfinite_model_rows <- panel[
  !apply(as.data.frame(panel[, ..all_model_fields]), 1L, function(row) all(is.finite(row)))
]
if (nrow(nonfinite_model_rows) > 0L) abortf("Found %d rows with non-finite l02 model fields.", nrow(nonfinite_model_rows))

# Validate threshold-level aggregate counts against run-x-l01 global audit.
input_global <- panel[, .(
  repo_month_rows = .N,
  eligible_fun_cfun_file_rows = sum(eligible_fun_cfun_file_count),
...
audit_compare <- merge(
  ...
  l01_global_audit[, .(
    ...
    audit_eligible_fun_cfun_file_rows = eligible_fun_cfun_file_rows,
...
audit_compare[, mismatch :=
  is.na(eligible_fun_cfun_file_rows) | is.na(audit_eligible_fun_cfun_file_rows) |
  eligible_fun_cfun_file_rows != audit_eligible_fun_cfun_file_rows |
  selected_file_rows != audit_selected_file_rows |
  selected_issue_total != audit_selected_issue_total
]
if (any(audit_compare$mismatch)) abortf("l02 input does not reconcile to l01 global audit for %d sample-threshold rows.", sum(audit_compare$mismatch))
...
# Validate threshold-invariant sample support against l01 sample summary.
...
for (sample_name in sample_specs) {
  ...
  expected <- l01_sample_summary[sample_spec == sample_name]
  if (nrow(expected) != 1L) abortf("Missing l01 sample-summary row for %s", sample_name)
...
if (any(sample_validation$mismatch_count > 0L)) abortf("l01 sample-support reconciliation failed.")

support_policy <- data.table::data.table(
  policy_id = "prespecified_sparse_support_flag_v1",
  apply_to_model_omission = 0L,
  prespecified_before_l02_results = 1L,
...
qc <- data.table::data.table(
  check = c(
    ...
    "l01_global_audit_mismatch_rows",
    "l01_sample_support_mismatch_samples",
    "model_job_count",
    "static_effect_rows",
    "dynamic_effect_rows",
    "pretrend_check_rows",
...
qc[, note := c(
  ...
  "Must be zero.",
  "l01 measurement contract must remain unchanged.",
  ...
  "Selected file/issue counts must reconcile to l01 global audit.",
  "Threshold-invariant sample support must reconcile to l01 sample summary.",
...
nonprimary_failure_note <- if (nonprimary_failure_job_count > 0L) {
  sprintf("%d non-primary model jobs had a failure or partial dynamic result; inspect quality_fun_cfun_npr_model_failures.csv.", nonprimary_failure_job_count)
} else {
...
summary <- data.table::rbindlist(list(
  make_summary_row("run", "script_version", "run-x-l02-v1"),
  ...
  make_summary_row("design", "prior_primary_threshold", prior_primary_threshold, "Historical I05 primary anchor retained for bridge comparison only."),
...
metadata <- data.table::rbindlist(list(
  ...
  make_summary_row("run", "l01_summary_sha256", sha256_file(l01_summary_file)),
  make_summary_row("run", "l01_sample_summary_sha256", sha256_file(l01_sample_summary_file)),
  make_summary_row("run", "l01_global_audit_sha256", sha256_file(l01_global_audit_file)),
  ...
  make_summary_row("definition", "quality_semantics", "unresolved SonarQube issue burden among Python files exceeding a frozen contemporaneous FUN+C_FUN-NPR threshold"),
...
if (nrow(plot_static_data) > 0L) {
  ...
  p_static <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "Quality burden DiD across frozen FUN+C_FUN-NPR thresholds",
      subtitle = "Total selected-file unresolved SonarQube issue stock",
      x = "FUN+C_FUN-NPR threshold",
...
if (nrow(plot_dynamic_data) > 0L) {
  ...
  p_dynamic <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "Primary-threshold Quality x FUN+C_FUN-NPR event study",
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("l02 hard QC failed: %s", paste(failed_qc$check, collapse = ", "))
}

log_message(
  "INFO",
  "Completed run-x-l02 %s: status=%s; jobs=%d; static=%d; dynamic=%d; nonprimary failures=%d; hard QC failures=%d",
