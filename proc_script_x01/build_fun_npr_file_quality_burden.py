#!/usr/bin/env python3
"""
Build a reusable file-level Quality x FUN-NPR dataset without applying thresholds.

The program joins two frozen production artifacts:

1. A12 repo-month/file FUN-NPR rows.
...
The join is exact at historical snapshot + repository-relative file path:
    ...
    A12.snapshot_id   == B05.snapshot_key
    A12.relative_path == B05.component_path
...
- A12 is the NPR analysis file universe. Therefore a Python file with no B05
  issue row gets an issue burden of zero, but only after B05 snapshot completeness
  is verified.
- B05 may contain SonarQube Python files that are outside the A12 NPR file universe.
  Those files cannot receive an NPR threshold classification, so D02 records them
...
- No NPR threshold is applied here. D01 remains the frozen threshold source;
...
SCRIPT_VERSION = "run-x-d02-v2"
QUALITY_SEMANTICS = "unresolved_python_sonarqube_issue_stock_at_historical_snapshot"
PRIMARY_NPR_METRIC = "file_npr_fun_space_by_token_weighted"
...
FINITE_FUN_STATUSES = {"scored", "scored_with_expected_exclusions"}
...
SNAPSHOT_AUDIT_COLUMNS = [
    ...
    "commit_sha",
    "b05_issue_total",
    "d02_joined_issue_total",
    "outside_a12_issue_total",
    "accounted_issue_total",
    "issue_total_matches",
    "b05_code_smell",
    "d02_joined_code_smell",
    "outside_a12_code_smell",
    ...
    "bug_matches",
    "b05_vulnerability",
    "d02_joined_vulnerability",
    "outside_a12_vulnerability",
    "accounted_vulnerability",
    "vulnerability_matches",
    "b05_high_severity",
    "d02_joined_high_severity",
    "outside_a12_high_severity",
    "accounted_high_severity",
    "high_severity_matches",
    "b05_maintainability_impact",
    "d02_joined_maintainability_impact",
    "outside_a12_maintainability_impact",
    "accounted_maintainability_impact",
    "maintainability_impact_matches",
    "b05_reliability_impact",
    "d02_joined_reliability_impact",
    "outside_a12_reliability_impact",
    "accounted_reliability_impact",
    "reliability_impact_matches",
    "b05_security_impact",
    "d02_joined_security_impact",
    "outside_a12_security_impact",
    "accounted_security_impact",
    "security_impact_matches",
    "outside_a12_issue_bearing_files",
]
...
def open_deterministic_gzip_csv(path: Path, columns: list[str]) -> tuple[Path, TextIO, csv.DictWriter]:
    ...
    setattr(text, "_d02_raw_file", raw)
...
def aggregate_b05_raw_issues(
    ...
    for row in iter_csv(path):
        ...
        if clean(row["component_scope"]) != "python_file":
            raise ValueError(
                f"D02 requires file-attributable Python issues only; observed "
...
def same_file_signature(row: dict[str, str]) -> tuple[str, ...]:
    return (
        ...
        clean(row["file_npr_fun_status"]),
        clean(row[PRIMARY_NPR_METRIC]),
        clean(row["fun_npr_coverage_ratio"]),
    )
...
def build_join(
    *,
    ...
    a12_header = require_columns(a12_path, A12_REQUIRED, "A12 repo-month/file FUN NPR")
    output_columns = a12_header + [column for column in QUALITY_OUTPUT_COLUMNS if column not in a12_header]
    ...
    d02_snapshot_unique_counts: dict[str, IssueCounts] = defaultdict(IssueCounts)
    issue_file_keys_seen: set[tuple[str, str]] = set()
    matched_issue_rows = 0
    zero_issue_rows = 0

    try:
        for row in iter_csv(a12_path):
            rows += 1
            snapshot_id = clean(row["snapshot_id"])
            path_value = clean(row["relative_path"])
            meta = snapshots.get(snapshot_id)
            if meta is None:
                raise ValueError(f"A12 row references snapshot not present in B05: {snapshot_id}")
            source = clean(row["dataset_source"]).casefold()
            repo_name = clean(row["repo_name"])
            ...
            if (source, repo_name.casefold(), commit) != (
                ...
                raise ValueError(f"A12/B05 snapshot identity mismatch for {snapshot_id}")
            if not path_value:
                raise ValueError(f"Missing A12 relative_path at row {rows + 1}")
            ...
            if clean(row["file_npr_fun_status"]) in FINITE_FUN_STATUSES:
                finite_fun_rows += 1
            ...
            if prior is None:
                ...
                d02_snapshot_unique_counts[snapshot_id].add_counts(counts)
            elif prior != signature:
                raise ValueError(f"Repeated A12 snapshot/file has inconsistent fields: {file_key}")
    ...
    for key in snapshots:
        d02_snapshot_unique_counts.setdefault(key, IssueCounts())

    diagnostics = {
        "a12_repo_month_file_rows": rows,
        "a12_finite_fun_rows": finite_fun_rows,
        "a12_unique_snapshot_files": len(unique_file_signatures),
        "a12_unique_snapshots": len(snapshot_ids),
        ...
        "issue_bearing_snapshot_files_seen_in_a12": len(issue_file_keys_seen),
    }
    return diagnostics, dict(d02_snapshot_unique_counts), issue_file_keys_seen


def build_outside_a12_scope_exclusions(
    *,
    snapshots: dict[str, SnapshotMeta],
    file_counts: dict[tuple[str, str], IssueCounts],
    ...
    """Record B05 issue-bearing files that are outside the A12 NPR file universe."""
        ...
        rows.append(
            {
                ...
                "exclusion_class": "outside_a12_npr_file_universe",
                "exclusion_detail": (
                    "B05 contains a Python SonarQube issue-bearing file that is not present "
                    "in the frozen A12 snapshot/file NPR universe; the file cannot be "
    ...
    atomic_csv_rows(rows, output_path, OUTSIDE_A12_COLUMNS)
    diagnostics = {
        "outside_a12_issue_bearing_snapshot_files": len(outside_keys),
        "outside_a12_affected_snapshots": len(snapshot_files),
        "outside_a12_issue_rows": global_counts.total,
        "outside_a12_code_smell": global_counts.code_smell,
        "outside_a12_bug": global_counts.bug,
        "outside_a12_vulnerability": global_counts.vulnerability,
        "outside_a12_high_severity": global_counts.high_severity,
        "outside_a12_maintainability": global_counts.maintainability,
        "outside_a12_reliability": global_counts.reliability,
        "outside_a12_security": global_counts.security,
    }
...
def build_snapshot_audit(
    snapshots: dict[str, SnapshotMeta],
    d02_counts: dict[str, IssueCounts],
    ...
    for key in sorted(snapshots):
        meta = snapshots[key]
        expected = meta.expected
        joined = d02_counts.get(key, IssueCounts())
        ...
        rows.append(
            {
                ...
                "d02_joined_issue_total": joined.total,
                "outside_a12_issue_total": outside.total,
                "accounted_issue_total": accounted.total,
                "b05_code_smell": expected.code_smell,
                "d02_joined_code_smell": joined.code_smell,
                "outside_a12_code_smell": outside.code_smell,
                "accounted_code_smell": accounted.code_smell,
                "b05_bug": expected.bug,
                "d02_joined_bug": joined.bug,
                "outside_a12_bug": outside.bug,
                "accounted_bug": accounted.bug,
                "b05_vulnerability": expected.vulnerability,
                "d02_joined_vulnerability": joined.vulnerability,
                "outside_a12_vulnerability": outside.vulnerability,
                "accounted_vulnerability": accounted.vulnerability,
                "b05_high_severity": expected.high_severity,
                "d02_joined_high_severity": joined.high_severity,
                "outside_a12_high_severity": outside.high_severity,
                "accounted_high_severity": accounted.high_severity,
                "b05_maintainability_impact": expected.maintainability,
                "d02_joined_maintainability_impact": joined.maintainability,
                "outside_a12_maintainability_impact": outside.maintainability,
                "accounted_maintainability_impact": accounted.maintainability,
                "b05_reliability_impact": expected.reliability,
                "d02_joined_reliability_impact": joined.reliability,
                "outside_a12_reliability_impact": outside.reliability,
                "accounted_reliability_impact": accounted.reliability,
                "b05_security_impact": expected.security,
                "d02_joined_security_impact": joined.security,
                "outside_a12_security_impact": outside.security,
                "accounted_security_impact": accounted.security,
                **{name: str(value) for name, value in flags.items()},
                "outside_a12_issue_bearing_files": outside_file_counts.get(key, 0),
            }
        )
...
def production_checks(
    *,
    ...
    expectations = [
        ...
        ("a12_repo_month_file_rows", join_diag["a12_repo_month_file_rows"], args.expected_a12_rows),
        ("a12_unique_snapshot_files", join_diag["a12_unique_snapshot_files"], args.expected_a12_unique_snapshot_files),
        ("a12_finite_fun_rows", join_diag["a12_finite_fun_rows"], args.expected_a12_finite_fun_rows),
        ("a12_unique_snapshots", join_diag["a12_unique_snapshots"], args.expected_snapshots),
    ...
    accounted_file_keys = len(issue_file_keys_seen) + outside_diag["outside_a12_issue_bearing_snapshot_files"]
    add_check(
        rows,
        ...
        "Each B05 issue-bearing Python file must be either joined to A12 or explicitly scope-excluded.",
    )
    add_check(
        rows,
        "outside_a12_issue_bearing_files_recorded",
        outside_diag["outside_a12_issue_bearing_snapshot_files"],
        ">=0 and exported",
        outside_diag["outside_a12_issue_bearing_snapshot_files"] >= 0,
        "These files are outside the frozen A12 NPR file universe and cannot receive an NPR threshold classification.",
    )
    add_check(
        rows,
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    join_diag, d02_snapshot_counts, issue_file_keys_seen = build_join(
        a12_path=args.a12_file,
        snapshots=snapshots,
        file_counts=file_counts,
        output_path=args.output_file,
    )
    outside_snapshot_counts, outside_snapshot_files, outside_diag = build_outside_a12_scope_exclusions(
        snapshots=snapshots,
        file_counts=file_counts,
        issue_file_keys_seen=issue_file_keys_seen,
        output_path=args.outside_a12_file,
    )
    snapshot_audit_rows, snapshot_audit_mismatches = build_snapshot_audit(
        snapshots, d02_snapshot_counts, outside_snapshot_counts, outside_snapshot_files
    )
    ...
    terminal_status = (
        "FAIL"
        if hard_failures
        else ("PASS_WITH_SCOPE_EXCLUSIONS" if outside_diag["outside_a12_issue_bearing_snapshot_files"] else "PASS")
    )
    ...
    summary_metrics: list[tuple[str, Any]] = [
        ...
        ("threshold_applied", 0),
        ("density_computed", 0),
        ("a12_repo_month_file_rows", join_diag["a12_repo_month_file_rows"]),
        ("a12_unique_snapshot_files", join_diag["a12_unique_snapshot_files"]),
        ("a12_finite_fun_rows", join_diag["a12_finite_fun_rows"]),
        ...
        ("b05_issue_bearing_snapshot_files_outside_a12", outside_diag["outside_a12_issue_bearing_snapshot_files"]),
        ("b05_issue_rows_outside_a12", outside_diag["outside_a12_issue_rows"]),
        ("b05_snapshots_with_outside_a12_issue_files", outside_diag["outside_a12_affected_snapshots"]),
    ...
    metadata = {
        ...
        "status": terminal_status,
        "inputs": {
            "a12_file": str(args.a12_file),
            "a12_file_sha256": sha256_file(args.a12_file),
        ...
        "method": {
            "file_universe": "A12 repo-month/file rows",
            "join_keys": ["snapshot_id == snapshot_key", "relative_path == component_path"],
            "zero_issue_policy": "left join from complete A12 file universe; absent B05 issue row => zero",
            "outside_a12_policy": "B05 issue-bearing Python files outside A12 are explicit scope exclusions; they are not threshold-classifiable",
            "quality_semantics": QUALITY_SEMANTICS,
            "npr_threshold": "not applied in D02; use frozen D01 threshold specification downstream",
        ...
        "outputs": {
            ...
            "outside_a12_scope_exclusions": str(args.outside_a12_file),
    ...
    if hard_failures:
        raise RuntimeError(f"D02 hard QC failures: {hard_failures}; see {args.checks_file}")
    return metadata
...
def self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="d02-selftest-") as tmpdir:
        root = Path(tmpdir)
        ...
        a12_columns = sorted(A12_REQUIRED)
        def a12_row(snapshot_id: str, source: str, repo: str, commit: str, repo_id: str, month: str, path_value: str, npr: str, status: str) -> dict[str, str]:
            row = {column: "0" for column in a12_columns}
            row.update(
                {
                    "repo_id": repo_id,
                    ...
                    "fun_occurrences_total": "1",
                    "fun_occurrences_scored": "1" if status == "scored" else "0",
                    "fun_occurrences_excluded": "0",
                    "fun_npr_coverage_ratio": "1.0" if status == "scored" else "",
            ...
            return row
        a12_rows = [
            a12_row(snapshot, "control", "Owner/Repo", "a" * 40, "r1", "2025-01", "src/a.py", "1.7", "scored"),
            a12_row(snapshot, "control", "Owner/Repo", "a" * 40, "r1", "2025-01", "src/b.py", "", "no_fun"),
            # Reuse the same snapshot/file in a later repo-month to test non-duplication of snapshot audit counts.
            a12_row(snapshot, "control", "Owner/Repo", "a" * 40, "r1", "2025-02", "src/a.py", "1.7", "scored"),
            a12_row(snapshot, "control", "Owner/Repo", "a" * 40, "r1", "2025-02", "src/b.py", "", "no_fun"),
            a12_row(snapshot2, "treatment", "Owner2/Repo2", "b" * 40, "r2", "2025-02", "main.py", "1.4", "scored"),
        ]
        a12_path = root / "a12.csv"
        write_fixture_csv(a12_path, a12_columns, a12_rows)
        ...
        args = argparse.Namespace(
            ...
            outside_a12_file=root / "outside_a12.csv",
            checks_file=root / "checks.csv",
            summary_file=root / "d02_summary.csv",
            metadata_file=root / "metadata.json",
            strict_expected_counts=True,
            expected_a12_rows=5,
            expected_a12_unique_snapshot_files=3,
            expected_a12_finite_fun_rows=3,
        ...
        outside = list(iter_csv(args.outside_a12_file))
        assert len(outside) == 1
        assert outside[0]["component_path"] == "generated/outside.py"
        assert outside[0]["sonar_issue_total"] == "1"
    print("build_fun_npr_file_quality_burden self-test: PASS")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expected-a12-rows", type=int, default=510297)
    parser.add_argument("--expected-a12-unique-snapshot-files", type=int, default=494592)
    parser.add_argument("--expected-a12-finite-fun-rows", type=int, default=204508)
...
def main() -> int:
    ...
    print("=" * 80)
    print("run-x-d02 FUN-NPR x file-level SonarQube burden join")
    print(f"Status:                              {metadata['status']}")
    print(f"A12 repo-month/file rows:            {diagnostics['a12_repo_month_file_rows']}")
    print(f"A12 unique snapshot/files:           {diagnostics['a12_unique_snapshot_files']}")
    print(f"A12 finite FUN NPR rows:             {diagnostics['a12_finite_fun_rows']}")
    print(f"B05 snapshots:                       {diagnostics['a12_unique_snapshots']}")
    ...
    print(f"Issue-bearing files joined to A12:   {diagnostics['issue_bearing_snapshot_files_seen_in_a12']}")
    print(f"Issue-bearing files outside A12:     {diagnostics['outside_a12_issue_bearing_snapshot_files']}")
    print(f"Issue rows outside A12:              {diagnostics['outside_a12_issue_rows']}")
    ...
    print(f"Outside-A12 scope exclusions:        {args.outside_a12_file.resolve()}")
    ...
    print("Thresholds:                          not applied; frozen D01 grid will be used downstream")
    print("=" * 80)
    return 0
