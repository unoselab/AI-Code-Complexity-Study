...
# ============================================================
# run-x-k09 v1: NPR RF+CM threshold sensitivity for dynamic panel GMM
# ============================================================
#
# Purpose:
...
#   - The I04 legacy anchor 1.5183 is intentionally excluded from this sweep.
...
#   file_npr_fun_cfun_space_by_token_weighted > tau in the historical snapshot).
#
# Model:
...
#
# Inputs:
#   - I04 NPR threshold-long quality panel
#   - I04 summary/sample/global-audit artifacts
#   - B06 authoritative whole-Python velocity/covariate panel
#   - K04 coefficients for exact primary-threshold reproduction
#
# This script is self-contained. It reuses validated analysis logic from the
# existing K04 NPR GMM and E03 threshold-sensitivity analyses, but it does not
...
extract_coefficients <- function(summary_object, threshold_id, threshold, threshold_role, confidence_level, primary_threshold) {
  ...
  data.table::data.table(
    ...
    model = "npr_rf_cm_threshold_quality_to_velocity",
...
i04_summary_file <- normalizePath(require_arg(args, "i04_summary_file"), mustWork = TRUE)
i04_sample_summary_file <- normalizePath(require_arg(args, "i04_sample_summary_file"), mustWork = TRUE)
i04_global_audit_file <- normalizePath(require_arg(args, "i04_global_audit_file"), mustWork = TRUE)
k04_reference_coefficients_file <- normalizePath(require_arg(args, "k04_reference_coefficients_file"), mustWork = TRUE)
...
expected_i04_thresholds <- as_integer_arg(args, "expected_i04_thresholds", 22L)
...
paths <- list(
  coefficients = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_coefficients.csv"),
  primary_summary = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_primary_summary.csv"),
  diagnostics = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_diagnostics.csv"),
  instrument_qc = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_instrument_qc.csv"),
  support = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_support_diagnostics.csv"),
  threshold_input_audit = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_input_audit.csv"),
  b06_join_audit = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_b06_join_audit.csv"),
  legacy_audit = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_legacy_flag_audit.csv"),
  calendar_gap_audit = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_calendar_gap_audit.csv"),
  reproduction = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_k04_reproduction.csv"),
  model_failures = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_model_failures.csv"),
  qc = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_qc.csv"),
  metadata = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_run_metadata.csv"),
  models = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_models.rds")
)

run_started <- Sys.time()
log_message("INFO", "Reading I04 NPR RF+CM threshold panel: %s", input_file)
...
required_d03 <- c(
  ...
  "log1p_selected_issue_total", "selected_issue_total", "selected_file_count", "eligible_fun_cfun_file_count",
  "log_age", "ncloc_py_sonarqube", "log_contributors", "log_stars", "log_issues"
)
validate_columns(long_panel, required_d03, "I04 long panel")
strict_count_check(nrow(long_panel), expected_long_rows, "I04 long-panel rows", strict_expected_counts)
strict_count_check(data.table::uniqueN(long_panel$threshold_id), expected_i04_thresholds, "I04 threshold specifications", strict_expected_counts)
strict_count_check(data.table::uniqueN(long_panel$sample_spec), expected_sample_specs, "I04 sample specifications", strict_expected_counts)

# Validate upstream I04 summary/sample/global-audit artifacts before fitting.
i04_summary <- data.table::fread(i04_summary_file, na.strings = c("", "NA", "NaN"))
validate_columns(i04_summary, c("metric", "value"), "I04 summary")
summary_lookup <- setNames(as.character(i04_summary$value), as.character(i04_summary$metric))
if (!identical(summary_lookup[["status"]], "PASS")) abortf("I04 summary status is not PASS")
if (abs(as.numeric(summary_lookup[["primary_threshold"]]) - primary_threshold) > 1e-12) abortf("I04 primary-threshold summary mismatch")

i04_sample_summary <- data.table::fread(i04_sample_summary_file, na.strings = c("", "NA", "NaN"))
validate_columns(i04_sample_summary, c("sample_spec", "repo_month_rows", "repositories", "control_repositories", "treatment_repositories"), "I04 sample summary")
full_sample_summary <- i04_sample_summary[sample_spec == "full_sample"]
if (nrow(full_sample_summary) != 1L) abortf("I04 sample summary must contain exactly one full_sample row")
strict_count_check(full_sample_summary$repo_month_rows, expected_rows_per_threshold, "I04 full-sample rows", strict_expected_counts)
strict_count_check(full_sample_summary$repositories, expected_repositories, "I04 full-sample repositories", strict_expected_counts)
strict_count_check(full_sample_summary$treatment_repositories, expected_treatment_repositories, "I04 full-sample treatment repositories", strict_expected_counts)
strict_count_check(full_sample_summary$control_repositories, expected_control_repositories, "I04 full-sample control repositories", strict_expected_counts)

global_audit <- data.table::fread(i04_global_audit_file, na.strings = c("", "NA", "NaN"))
...
if (anyNA(global_full$threshold_numeric)) abortf("I04 global audit contains non-numeric thresholds")
...
if (nrow(main_global) != expected_main_thresholds) abortf("Expected %d I04 full-sample main-grid audit rows; observed %d", expected_main_thresholds, nrow(main_global))
if (any(abs(main_global$threshold_numeric - expected_grid) > 1e-10)) abortf("I04 main threshold grid does not match primary +/- radius in 0.05 increments")
if (any(main_global$repo_month_rows != expected_rows_per_threshold)) abortf("I04 main-grid global audit has unexpected repo-month row counts")
if (any(main_global$repositories != expected_repositories)) abortf("I04 main-grid global audit has unexpected repository counts")
...
if (any(selected_file_diff > 0)) abortf("I04 selected-file support increases as NPR threshold increases")
if (any(selected_issue_diff > 0)) abortf("I04 selected-issue support increases as NPR threshold increases")

primary_global <- main_global[abs(threshold_numeric - primary_threshold) <= 1e-12]
if (nrow(primary_global) != 1L) abortf("Expected exactly one primary row in I04 full-sample global audit")
...
if (nrow(main_panel) != expected_rows_per_threshold * expected_main_thresholds) {
  abortf("Unexpected I04 main-grid row count: expected %d, observed %d", expected_rows_per_threshold * expected_main_thresholds, nrow(main_panel))
}
if (data.table::uniqueN(main_panel$threshold_id) != expected_main_thresholds) abortf("I04 main panel does not contain exactly 21 threshold IDs")
if (any(main_panel$comparison_operator != ">")) abortf("K09 requires strict NPR > threshold")
...
if (any(threshold_key_qc$rows != expected_rows_per_threshold)) abortf("One or more K09 thresholds do not contain 1,954 rows")
if (any(threshold_key_qc$repositories != expected_repositories)) abortf("One or more K09 thresholds do not contain 167 repositories")
if (any(threshold_key_qc$duplicate_repo_time != 0L)) abortf("One or more K09 thresholds contain duplicate repo-month keys")
if (any(abs(threshold_key_qc$threshold - expected_grid) > 1e-10)) abortf("Threshold order/grid mismatch in I04 main panel")
...
if (nrow(support_compare) != expected_main_thresholds || anyNA(support_compare$threshold_id)) abortf("I04 main-panel/global-audit threshold reconciliation failed")
if (any(abs(support_compare$selected_file_rows - support_compare$audit_selected_file_rows) > 1e-12)) abortf("I04 selected-file totals disagree with global audit")
if (any(abs(support_compare$selected_issue_total - support_compare$audit_selected_issue_total) > 1e-12)) abortf("I04 selected-issue totals disagree with global audit")

# Join authoritative B06 velocity once across all threshold rows and audit every
# overlapping field. I04 remains authoritative for detector-localized outcomes.
...
if (nrow(main_panel) != rows_before_join) abortf("B06 join changed K09 main-grid row count")
...
if (nrow(primary_panel) != expected_rows_per_threshold) abortf("Primary K09 panel does not contain 1,954 rows")
primary_panel[, `:=`(
...
for (row_index in seq_len(nrow(threshold_table))) {
  ...
  if (fit_capture$error) {
    ...
    instrument_all[[threshold_id_value]] <- data.table::data.table(
      threshold_id = threshold_id_value, threshold = threshold_value, primary_analysis = primary_analysis_value,
      model = "npr_rf_cm_threshold_quality_to_velocity", collapse = FALSE, instrument_specification = "lag(velocity,2)",
    ...
    qc_all[[threshold_id_value]] <- data.table::data.table(
      ...
      note = c("Full I04 repo-month sample.", "Exact calendar t-1/t-2 support.", paste0("Non-primary sparse-threshold pgmm failure: ", message_text))
    )
    next
  }
  ...
  if (summary_capture$error) {
    ...
    qc_all[[threshold_id_value]] <- data.table::data.table(
      ...
      expected = c(expected_rows_per_threshold, expected_active_rows, NA_real_),
      status = c("pass", "pass", "caution"),
      note = c("Full I04 repo-month sample.", "Exact calendar t-1/t-2 support.", paste0("Non-primary sparse-threshold summary failure: ", message_text))
    )
    next
  }
  ...
  if (coefficient_capture$error) {
    ...
    qc_all[[threshold_id_value]] <- data.table::data.table(
      ...
      expected = c(expected_rows_per_threshold, expected_active_rows, NA_real_),
      status = c("pass", "pass", "caution"),
      note = c("Full I04 repo-month sample.", "Exact calendar t-1/t-2 support.", paste0("Non-primary coefficient extraction failure: ", message_text))
    )
    next
  }
  ...
  if (nrow(primary_term_row) != 1L) {
    message_text <- sprintf("Expected exactly one lagged NPR RF+CM quality coefficient; observed %d", nrow(primary_term_row))
    if (primary_analysis_value == 1L) abortf("Primary threshold coefficient extraction failed: %s", message_text)
    model_failures <- data.table::rbindlist(list(model_failures, data.table::data.table(
      threshold_id = threshold_id_value, threshold = threshold_value, primary_analysis = primary_analysis_value,
      stage = "coefficient_extraction", message = message_text
    ...
    qc_all[[threshold_id_value]] <- data.table::data.table(
      ...
      note = c("Full I04 repo-month sample.", "Exact calendar t-1/t-2 support.", paste0("Non-primary primary-term extraction failure: ", message_text))
    )
    next
  }
  ...
  threshold_qc <- data.table::data.table(
    ...
    note = c(
      "Full I04 repo-month sample.", "Full-sample repository membership.",
      "Exact calendar t-1/t-2 support.", "Exact-calendar active repositories.",
      "pgmm N should equal exact-calendar support.", "Exactly one lagged NPR RF+CM quality coefficient.",
...
if (nrow(summary_output) != expected_main_thresholds) abortf("K09 threshold summary does not contain exactly 21 rows")
if (nrow(support_output) != expected_main_thresholds) abortf("K09 support output does not contain exactly 21 rows")
if (any(abs(summary_output$threshold - expected_grid) > 1e-10)) abortf("K09 result threshold grid is incomplete or out of order")

# Exact K04 full-sample reproduction at the primary NPR threshold.
k04_reference <- data.table::fread(k04_reference_coefficients_file, na.strings = c("", "NA", "NaN"))
validate_columns(k04_reference, c("sample_spec", "term", "estimate", "std_error", "p_value", "is_primary_interaction_term"), "K04 coefficients")
k04_primary <- k04_reference[
  sample_spec == "full_sample" &
    bool_to_int(is_primary_interaction_term) == 1L &
    term == "lag(log1p_selected_issue_total, 1)"
]
if (nrow(k04_primary) != 1L) abortf("Expected exactly one K04 full-sample primary NPR RF+CM quality coefficient")
g06_primary <- summary_output[abs(threshold - primary_threshold) <= 1e-12 & fit_status == "success"]
if (nrow(g06_primary) != 1L) abortf("Expected exactly one successful K09 primary-threshold result")
reproduction <- data.table::data.table(
  metric = c("estimate", "std_error", "p_value"),
  k04_full_sample = c(k04_primary$estimate[[1L]], k04_primary$std_error[[1L]], k04_primary$p_value[[1L]]),
  g06_primary = c(g06_primary$estimate[[1L]], g06_primary$std_error[[1L]], g06_primary$p_value[[1L]])
)
reproduction[, absolute_difference := abs(as.numeric(k04_full_sample) - as.numeric(g06_primary))]
reproduction[, tolerance := reference_tolerance]
reproduction[, status := ifelse(absolute_difference <= reference_tolerance, "pass", "fail")]
if (any(reproduction$status == "fail")) abortf("K09 primary threshold failed exact K04 full-sample reproduction")
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("K09 threshold GMM QC contains hard failures: %s", paste(paste(failed_qc$threshold_id, failed_qc$check, sep = "/"), collapse = ", "))
}
...
metadata <- data.table::data.table(
  ...
  metric = c(
    ...
    "i04_summary_file", "i04_summary_sha256", "i04_global_audit_file", "i04_global_audit_sha256",
    "k04_reference_coefficients_file",
    "analysis_scope", "localization_scope", "detector_scope_pair", "detector", "npr_metric", "sample_spec", "primary_threshold",
    ...
    "thresholds_successful", "thresholds_failed", "failed_hard_qc", "caution_qc_rows", "k04_reproduction_status",
    "R", "data.table", "plm"
  ),
  value = c(
    "run-x-k09", implementation_version,
    ...
    i04_summary_file, sha256_file(i04_summary_file), i04_global_audit_file, sha256_file(i04_global_audit_file),
    k04_reference_coefficients_file,
    "npr_rf_cm_threshold_sensitivity_quality_to_velocity", "RF+CM", "NPR|RF+CM", "AGCDetector_NPR", "file_npr_fun_cfun_space_by_token_weighted", "full_sample",
    sprintf("%.6f", primary_threshold), sprintf("%.2f", threshold_radius), sprintf("%.2f", threshold_step), expected_main_thresholds,
    "exclude I04 legacy anchor 1.5183 from the main sweep", ">", "log1p_selected_issue_total", "log_lines_added_py_source",
...
log_message(
  "INFO",
  "Completed run-x-k09 %s: thresholds=%d; successful=%d; failed=%d; significant=%d; cautions=%d",
