#!/usr/bin/env python3
"""
Audit readiness of the NPR-RF+CM quality-to-velocity dynamic-panel GMM inputs.

This K03 audit is intentionally model-free. It validates that the existing I04
RF+CM-localized NPR quality panel can be joined to the authoritative B06 Python
...
- Detector: NPR
- Localization scope: RF+CM (regular functions and class methods)
- Primary detector cutoff: tau_NPR = 1.571637
- Selection operator inherited from I04: strict >
...
def run_audit(args: argparse.Namespace) -> int:
    i04_path = Path(args.i04_panel_file)
    b06_path = Path(args.b06_panel_file)
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    i04 = pd.read_csv(i04_path, compression="infer", low_memory=False)
    ...
    require_columns(i04, i04_required, "I04 panel")
    require_columns(b06, b06_required, "B06 panel")

    numeric(i04, ["threshold", "repo_id", "treatment_group", "time_index", "event_index",
                  "selected_issue_total", "log1p_selected_issue_total"], "I04 panel")
    ...
    primary = i04[
        i04["sample_spec"].map(clean_text).eq("full_sample")
        & i04["threshold_role"].map(clean_text).eq("primary")
        & np.isclose(i04["threshold"].astype(float), args.primary_threshold, atol=1e-12, rtol=0.0)
    ].copy()
    ...
    add_check(checks, "i04_long_rows", len(i04), args.expected_i04_long_rows,
              len(i04) == args.expected_i04_long_rows, "Expected I04 threshold x sample long-panel rows.")
    ...
    add_check(checks, "primary_repositories", int(primary["repo_id"].nunique()), args.expected_repositories,
              int(primary["repo_id"].nunique()) == args.expected_repositories, "Primary I04 repository count.")
    add_check(checks, "primary_treatment_repositories",
              ...
              "Primary I04 treated repositories.")
    add_check(checks, "primary_control_repositories",
              ...
              "Primary I04 control repositories.")
    ...
    add_check(checks, "primary_npr_metric", "|".join(metrics), EXPECTED_NPR_METRIC,
              metrics == [EXPECTED_NPR_METRIC], "K03 must audit RF+CM combined-procedure NPR, not RF or CM.")
    ...
    merged = primary[identity_cols].merge(
        ...
        suffixes=("_i04", "_b06"), indicator=True, validate="one_to_one"
    )
    unmatched = int(merged["_merge"].ne("both").sum())
    add_check(checks, "i04_b06_unmatched_repo_months", unmatched, 0, unmatched == 0,
              "Primary I04 and B06 must have the same full-sample repo-month universe.")

    mismatch_count = 0
    if unmatched == 0:
        for col in ["repo_name", "dataset_source", "treatment_group", "event_index"]:
            left = merged[f"{col}_i04"].map(clean_text)
            right = merged[f"{col}_b06"].map(clean_text)
            mismatch_count += int(left.ne(right).sum())
    add_check(checks, "i04_b06_identity_field_mismatches", mismatch_count, 0, mismatch_count == 0,
    ...
    audit_summary = pd.DataFrame([
        {
            "analysis": "NPR-RF+CM quality_{t-1} -> velocity_t",
            "detector": "NPR",
            ...
            "i04_primary_rows": len(primary),
            "i04_primary_repositories": int(primary["repo_id"].nunique()),
    ...
    checks_path = output_dir / "k03_npr_rf_cm_gmm_input_checks.csv"
    summary_path = output_dir / "k03_npr_rf_cm_gmm_input_summary.csv"
    active_path = output_dir / "k03_npr_rf_cm_exact_calendar_sample.csv.gz"
    ...
    metadata = {
        ...
        "i04_panel_file": str(i04_path),
        "i04_panel_sha256": sha256_file(i04_path),
    ...
    print(f"K03 audit status:                         {status}")
    print(f"Hard QC failures:                        {hard_failures}")
    print(f"Primary I04 rows/repositories:           {len(primary)}/{int(primary['repo_id'].nunique())}")
...
def self_test() -> int:
    ...
    print("audit_npr_rf_cm_gmm_inputs self-test: PASS")
    return 0
...
def main() -> int:
    ...
    if args.self_test:
        return self_test()
    missing = [name for name in ["i04_panel_file", "b06_panel_file", "output_dir"] if not getattr(args, name)]
    if missing:
