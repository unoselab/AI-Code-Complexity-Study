...
# ============================================================
# run-x-b03 v1: Borusyak velocity DiD with Python NCLOC
# ============================================================
#
# Purpose:
#   Estimate static and dynamic treatment effects of Cursor adoption using each
#   Python-only NCLOC backend prepared by run-x-b02-v1, and build a same-sample
#   descriptive comparison of the SonarQube and cloc estimates.
#
# Outcomes:
#   - log_commits
#   - log_lines_added
...
#
# Inputs and outputs are supplied by the shell wrapper. Backend mode fits one
# specification; comparison mode verifies panel alignment and stacks the two
# result sets. This script does not call any earlier experiment script.
# ============================================================
...
render_html_report <- function(
  ...
  report_lines <- c(
    ...
    "subtitle: \"Borusyak et al. (2021) Difference-in-Differences Imputation — Python NCLOC\"",
    "output:",
    "  html_document:",
    "    toc: true",
    "    toc_depth: 3",
    ...
    "# Overview",
    "",
    "This report was generated automatically by **run-x-b03** for the **`r backend_label_report`** Python-NCLOC backend. It follows the presentation logic of the original `DiffInDiffBorusyak.Rmd` and uses the run-x-b02 panel without changing outcomes, timing, fixed effects, clustering, or the matched sample.",
    "",
    ...
    "```",
    "",
    "<div class=\"note\">The 11 legacy-flag mismatches are expected audit findings. The models use normalized absorbing treatment timing. The generic `ncloc` model column is verified against the selected backend metric: `r backend_metric_report`.</div>",
    "",
...
run_backend_comparison <- function(args) {
  ...
  sq_prefix <- "velocity_python_ncloc_sonarqube"
  cloc_prefix <- "velocity_python_ncloc_cloc"
  comparison_prefix <- "velocity_python_ncloc_backend"
  ...
  panel_qc <- data.table::data.table(
    check_name = c(
      ...
      "cloc_ncloc_alias_mismatches", "static_effect_rows", "dynamic_effect_rows",
      "pretrend_check_rows"
    ),
    status = c(
      ...
      ifelse(nrow(static_all) == 4L, "pass", "fail"),
      ifelse(nrow(dynamic_all) == 48L, "pass", "fail"),
      ifelse(nrow(pretrend_checks_all) == 20L, "pass", "fail")
    ),
    observed = c(
      nrow(sq_panel), nrow(cloc_panel), key_mismatches, non_ncloc_mismatches,
      sq_alias_mismatches, cloc_alias_mismatches, nrow(static_all),
      nrow(dynamic_all), nrow(pretrend_checks_all)
    ),
    expected = c(
      expected_rows, expected_rows, 0, 0, 0, 0, 4, 48, 20
    ),
    note = c(
      "", "", "", "Only ncloc and backend-specific provenance may differ.",
      "", "", "Two outcomes times two backends.",
      "Twenty-four package-native terms per backend; forty-eight stacked rows.",
      "Five placebo terms times two outcomes times two backends."
    )
  )
  ...
  plot_object <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      ...
      caption = "Both specifications use the same 1,954 repository-month sample; only the Python-NCLOC covariate changes."
    ) +
  ...
  ggplot2::ggsave(output_paths$plot_pdf, plot_object, width = 11, height = 2.8, units = "in", bg = "white")
  ggplot2::ggsave(output_paths$plot_png, plot_object, width = 11, height = 2.8, units = "in", dpi = 180, bg = "white")

  summary_rows <- list(
    make_summary_row("implementation", "version", "v1"),
    make_summary_row("definition", "comparison", "sonarqube_vs_cloc_same_sample"),
  ...
  log_message(
    "INFO",
    "Completed run-x-b03-v1 backend comparison: static=%d; dynamic=%d; pretrend=%d; panel_rows=%d",
...
artifact_prefix <- paste0("velocity_python_ncloc_", backend)
...
if (is.null(html_output) || !nzchar(as.character(html_output))) {
  html_output <- file.path(output_dir, paste0("velocity_python_ncloc_", backend, "_report.html"))
}
...
log_message("INFO", "Reading %s Python-NCLOC panel: %s", backend_label, input_file)
...
metadata <- data.table::data.table(
  ...
  value = c(
    "v1", backend, backend_label, backend_metric,
...
log_message("INFO", "Completed run-x-b03-v1 backend=%s: %d static effects, %d package-native dynamic/pretrend terms, %d pretrend checks, and 1 HTML report",
            backend,
