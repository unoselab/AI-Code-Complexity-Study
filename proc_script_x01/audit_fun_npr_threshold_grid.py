#!/usr/bin/env python3
"""Freeze and audit the FUN file-NPR threshold grid before quality outcomes are used.

A13 consumes the A12 repo-month/Python-file FUN NPR artifact and applies a
pre-specified threshold grid to the continuous file-level FUN NPR signal.  It
intentionally does not read SonarQube or any other quality outcome.  The goal is
therefore threshold pre-specification and coverage/sample-composition auditing,
not treatment-effect estimation.
...
- Decision rule: AGC-like/high-FUN-NPR file iff NPR > threshold

Treatment timing
----------------
A13 follows the normalized Model A timing used by the quality DiD design:
...
Outputs are aggregate audits only.  A13 does not generate 22 duplicated
file-level classification datasets; A14 can reapply the frozen threshold spec
when joining the A12 file rows to SonarQube file-level outcomes.
"""
...
SCRIPT_VERSION = "run-x-d01-v1"
METRIC_COLUMN = "file_npr_fun_space_by_token_weighted"
COMPARISON_OPERATOR = ">"
...
EXPECTED_A12_SCRIPT_VERSION = "run-x-a12-v2"
...
def validate_a12_summary(path: Path) -> dict[str, Any]:
    with path.open("r", encoding="utf-8") as stream:
        summary = json.load(stream)
    if clean(summary.get("script_version")) != EXPECTED_A12_SCRIPT_VERSION:
        raise ValueError(
            f"Unexpected A12 script version: {summary.get('script_version')!r}; expected {EXPECTED_A12_SCRIPT_VERSION!r}"
        )
    if int(summary.get("hard_check_failures", -1)) != 0:
        raise ValueError(f"A12 summary reports hard_check_failures={summary.get('hard_check_failures')}")
    status = clean(summary.get("status"))
    if status not in {"PASS", "PASS_WITH_EXPECTED_EXCLUSIONS"}:
        raise ValueError(f"A12 summary status is not a successful terminal state: {status!r}")
    return summary
...
def audit_input(
    ...
    require_columns(input_path, INPUT_REQUIRED_COLUMNS, "A12 repo-month/file FUN NPR input")
    ...
    for row in iter_csv(input_path):
        ...
        status = clean(row["file_npr_fun_status"])
        ...
        coverage = parse_float(row["fun_npr_coverage_ratio"], f"row {total_rows} fun_npr_coverage_ratio", allow_blank=True)
        ...
        if prior_unique is None:
            ...
            raise ValueError(f"Repeated snapshot/file has inconsistent FUN NPR fields: {snapshot_key}")
    ...
    for index, spec in enumerate(specs):
        ...
        global_rows.append(
            ...
            | {
                ...
                "eligible_finite_fun_rows": eligible_rows,
        ...
        for name in group_names:
            ...
            group_rows.append(
                ...
                | {
                    ...
                    "eligible_finite_fun_rows": group.eligible,
        ...
        for rm in sorted_repo_months:
            ...
            repo_month_rows.append(
                ...
                | {
                    ...
                    "eligible_finite_fun_rows": rm.eligible,
...
def make_checks(
    ...
    add_check(checks, "threshold_total_count_with_legacy", len(specs) == 22, len(specs), 22, "A13 must have the 21-point grid plus one named legacy anchor.")
    ...
    if args.strict_expected_counts:
        expected_pairs = [
            ...
            ("eligible_repo_month_file_rows", diagnostics["eligible_finite_fun_rows"], args.expected_eligible_rows),
        ...
        for name, observed, expected in expected_pairs:
            add_check(checks, name, observed == expected, observed, expected, "Strict production-count reconciliation against the completed A12/Model A sample.")

    add_check(checks, "unexpected_file_status_rows", diagnostics["unexpected_status_rows"] == 0, diagnostics["unexpected_status_rows"], 0, "A13 accepts only A12 v2 file-status values.")
    add_check(checks, "unexpected_missing_score_rows", diagnostics["unexpected_missing_score_rows"] == 0, diagnostics["unexpected_missing_score_rows"], 0, "A12 unexpected_missing_score must remain zero before thresholding.")
...
def self_test() -> None:
    ...
    print("audit_fun_npr_threshold_grid self-test: PASS")


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input-file", type=Path, help="A12 python_fun_repo_month_file_npr_scores.csv")
    parser.add_argument("--a12-summary-file", type=Path, help="A12 summary.json adjacent to the input artifact")
    parser.add_argument("--output-dir", type=Path, help="A13 output directory")
    ...
    parser.add_argument("--expected-eligible-unique-snapshot-files", type=int, default=196643)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.self_test:
        self_test()
        return 0
    if args.input_file is None or args.a12_summary_file is None or args.output_dir is None:
        raise SystemExit("--input-file, --a12-summary-file, and --output-dir are required unless --self-test is used")

    args.input_file = args.input_file.expanduser().resolve()
    args.a12_summary_file = args.a12_summary_file.expanduser().resolve()
    args.output_dir = args.output_dir.expanduser().resolve()
    for path in (args.input_file, args.a12_summary_file):
        if not path.is_file():
            raise FileNotFoundError(path)

    if args.primary_threshold != DEFAULT_PRIMARY_THRESHOLD:
        raise ValueError(f"A13 v1 freezes primary threshold at {DEFAULT_PRIMARY_THRESHOLD}; observed override={args.primary_threshold}")
    if args.grid_step != DEFAULT_GRID_STEP:
        raise ValueError(f"A13 v1 freezes grid step at {DEFAULT_GRID_STEP}; observed override={args.grid_step}")
    if args.grid_radius != DEFAULT_GRID_RADIUS:
        raise ValueError(f"A13 v1 freezes grid radius at {DEFAULT_GRID_RADIUS}; observed override={args.grid_radius}")
    if args.legacy_threshold != DEFAULT_LEGACY_THRESHOLD:
        raise ValueError(f"A13 v1 freezes legacy threshold at {DEFAULT_LEGACY_THRESHOLD}; observed override={args.legacy_threshold}")

    started = utc_now()
    a12_summary = validate_a12_summary(args.a12_summary_file)
    ...
    spec_path = args.output_dir / "fun_npr_threshold_spec.csv"
    global_path = args.output_dir / "fun_npr_threshold_audit.csv"
    group_path = args.output_dir / "fun_npr_threshold_by_treatment_timing.csv"
    repo_month_path = args.output_dir / "fun_npr_threshold_repo_month_audit.csv"
    distribution_path = args.output_dir / "fun_npr_distribution_summary.csv"
    checks_path = args.output_dir / "fun_npr_threshold_checks.csv"
    ...
    summary = {
        ...
        "methodology": {
            ...
            "threshold_selection_note": "Threshold grid was frozen and audited without reading SonarQube or any quality outcome.",
        },
        "input": diagnostics,
        "a12": {
            "status": a12_summary.get("status"),
            "script_version": a12_summary.get("script_version"),
            "hard_check_failures": a12_summary.get("hard_check_failures"),
        },
    ...
    atomic_json(
        {
            ...
            "created_utc": utc_now(),
            "inputs": {
                "a12_repo_month_file_npr": str(args.input_file),
                "a12_repo_month_file_npr_sha256": sha256_file(args.input_file),
                "a12_summary": str(args.a12_summary_file),
                "a12_summary_sha256": sha256_file(args.a12_summary_file),
            },
    ...
    print("=" * 80)
    print("run-x-d01 FUN file-NPR threshold-grid audit")
    print(f"Status:                              {summary['status']}")
    print(f"Input repo-month/file rows:          {diagnostics['input_rows']}")
    print(f"Eligible finite FUN NPR rows:        {diagnostics['eligible_finite_fun_rows']}")
