...
# ============================================================
# run-x-e02 v3: Dynamic panel GMM for NPR-localized quality burden
# ============================================================
#
# Purpose:
#   Estimate whether unresolved Python static-analysis issue burden localized
#   to files selected by the frozen FUN-NPR primary threshold predicts lower
#   subsequent whole-Python development velocity.
#
# Primary detector/localization contract:
#   - detector: AGCDetector_NPR / FUN-NPR
#   - file NPR metric: file_npr_fun_space_by_token_weighted
...
#   E02 therefore addresses the pre-specified secondary interaction question:
#   NPR-localized quality burden_t -> whole-Python velocity_{t+1}.
#
...
#
# Join rule:
#   D03 intentionally contains localized quality outcomes but not the B02/B06
#   whole-Python velocity outcome. E02 therefore joins log_lines_added_py_source
...
expected_primary_full_selected_files <- as_integer_arg(args, "expected_primary_full_selected_files", 13739L)
expected_primary_full_issue_stock <- as_integer_arg(args, "expected_primary_full_issue_stock", 20306L)
expected_primary_sensitivity_selected_files <- as_integer_arg(args, "expected_primary_sensitivity_selected_files", 13659L)
expected_primary_sensitivity_issue_stock <- as_integer_arg(args, "expected_primary_sensitivity_issue_stock", 20251L)
...
paths <- list(
  coefficients = file.path(output_dir, "dynamic_panel_gmm_npr_coefficients.csv"),
  diagnostics = file.path(output_dir, "dynamic_panel_gmm_npr_diagnostics.csv"),
  sample_qc = file.path(output_dir, "dynamic_panel_gmm_npr_sample_qc.csv"),
  instrument_qc = file.path(output_dir, "dynamic_panel_gmm_npr_instrument_qc.csv"),
  model_specs = file.path(output_dir, "dynamic_panel_gmm_npr_model_specifications.csv"),
  threshold_input_audit = file.path(output_dir, "dynamic_panel_gmm_npr_threshold_input_audit.csv"),
  b06_join_audit = file.path(output_dir, "dynamic_panel_gmm_npr_b06_join_audit.csv"),
  legacy_audit = file.path(output_dir, "dynamic_panel_gmm_npr_legacy_flag_audit.csv"),
  coercion_audit = file.path(output_dir, "dynamic_panel_gmm_npr_numeric_coercion_audit.csv"),
  calendar_gap_audit = file.path(output_dir, "dynamic_panel_gmm_npr_calendar_gap_audit.csv"),
  run_metadata = file.path(output_dir, "dynamic_panel_gmm_npr_run_metadata.csv"),
  qc = file.path(output_dir, "dynamic_panel_gmm_npr_qc.csv"),
  models_rds = file.path(output_dir, "dynamic_panel_gmm_npr_models.rds")
)

run_started <- Sys.time()
log_message("INFO", "Reading D03 FUN-NPR threshold quality panel: %s", input_file)
...
required_columns <- c(
  ...
  "selected_issue_total", "selected_file_count", "eligible_fun_file_count",
...
strict_count_check(data.table::uniqueN(long_panel$threshold_id), expected_thresholds, "D03 threshold specifications", strict_expected_counts)
...
# D03 was built from B06 for quality/timing support, but the D03 long panel does
# not intentionally carry the whole-Python velocity outcome. Read B06 directly
# and use it as the authoritative velocity source for E02.
...
if (nrow(primary_panel) == 0L) abortf("No D03 primary-threshold rows found for strict NPR > %.6f.", primary_threshold)
...
if (!all(sample_specs %in% unique(primary_panel$sample_spec))) {
  abortf("Primary D03 panel is missing required sample specifications: %s", paste(setdiff(sample_specs, unique(primary_panel$sample_spec)), collapse = ", "))
}
primary_panel <- primary_panel[sample_spec %in% sample_specs]

# Join only after freezing the D03 primary threshold and the two pre-specified
...
if (anyNA(primary_panel$repo_id) || anyNA(primary_panel$time_index)) {
  abortf("D03 primary repo_id/time_index must be complete integer-compatible keys before the B06 join.")
}
...
if (nrow(primary_panel) != primary_rows_before_b06_join) {
  abortf("B06 join changed D03 primary row count: before=%d after=%d.", primary_rows_before_b06_join, nrow(primary_panel))
}

# Missing B06 velocity means the D03 repo-month key did not resolve to the
...
if (b06_join_missing_velocity > 0L) {
  abortf("B06 join left %d D03 primary rows without log_lines_added_py_source.", b06_join_missing_velocity)
}
...
join_audit_parts <- list(
  data.table::data.table(
    ...
    note = "D03 primary rows before the B06 velocity join."
  ),
  data.table::data.table(
    ...
    note = "B06 join must not change D03 primary sample membership."
  ),
  data.table::data.table(
    ...
    note = "Every D03 primary repo-month must resolve to authoritative B06 log_lines_added_py_source."
  )
)
for (field in join_compare_fields) {
  ...
  join_audit_parts[[length(join_audit_parts) + 1L]] <- data.table::data.table(
    ...
    note = "D03 field must agree with the authoritative B06 source on matched repo-month keys."
  )
}
...
# Validate D03 summary and sample-support artifacts before estimation.
...
if (abs(as.numeric(summary_lookup[["primary_threshold"]]) - primary_threshold) > 1e-12) abortf("D03 summary primary threshold mismatch.")

sample_summary_input <- data.table::fread(d03_sample_summary_file, na.strings = c("", "NA", "NaN"))
...
if (nrow(sample_summary_input[sample_spec == "full_sample"]) != 1L ||
  ...
  abortf("D03 sample summary must contain exactly one row for each E02 sample specification.")
}
strict_count_check(sample_summary_input[sample_spec == "full_sample", repo_month_rows], expected_full_rows, "D03 sample-summary full rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", repositories], expected_full_repositories, "D03 sample-summary full repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", treatment_repositories], expected_full_treatment_repos, "D03 sample-summary full treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", control_repositories], expected_full_control_repos, "D03 sample-summary full control repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repo_month_rows], expected_sensitivity_rows, "D03 sample-summary sensitivity rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repositories], expected_sensitivity_repositories, "D03 sample-summary sensitivity repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", treatment_repositories], expected_sensitivity_treatment_repos, "D03 sample-summary sensitivity treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", control_repositories], expected_sensitivity_control_repos, "D03 sample-summary sensitivity control repositories", strict_expected_counts)
...
if (nrow(primary_global) != 2L) abortf("Expected two primary rows in D03 global audit; observed %d.", nrow(primary_global))
...
for (sample_spec_value in sample_specs) {
  ...
  model_name <- "npr_quality_to_velocity"
  ...
  qc_parts[[sample_spec]] <- data.table::data.table(
    ...
    note = c(
      "Primary D03 sample invariant.", "Primary D03 sample invariant.", "Primary D03 sample invariant.", "Primary D03 sample invariant.",
      "Must be zero.", "Normalized timing must be exact.", "Numeric parsing must not introduce missingness.",
      "GMM fields must be complete.", "GMM fields must be finite.", "D03 localized quality log outcome must equal log1p(selected_issue_total).",
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("run-x-e02 NPR GMM QC failed: %s", paste(paste(failed_qc$sample_spec, failed_qc$check, sep = "/"), collapse = ", "))
}
...
metadata <- data.table::rbindlist(list(
  make_summary_row("run", "run_prefix", "run-x-e02"),
  ...
  make_summary_row("run", "d03_summary_file", d03_summary_file),
  make_summary_row("run", "d03_summary_sha256", sha256_file(d03_summary_file)),
  make_summary_row("run", "d03_sample_summary_file", d03_sample_summary_file),
  make_summary_row("run", "d03_sample_summary_sha256", sha256_file(d03_sample_summary_file)),
  make_summary_row("run", "d03_global_audit_file", d03_global_audit_file),
  ...
  make_summary_row("definition", "npr_metric", "file_npr_fun_space_by_token_weighted"),
  ...
  make_summary_row("definition", "quality_semantics", "unresolved SonarQube issue stock among Python files with FUN-NPR > 1.571637 at the corresponding historical snapshot"),
...
log_message(
  "INFO",
  "Completed run-x-e02 %s: samples=%d; coefficients=%d; diagnostics=%d; failed_qc=%d; cautions=%d",
