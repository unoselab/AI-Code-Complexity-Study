...
# ============================================================
# run-x-k08 v2: Dynamic panel GMM for ML-localized RF+CM issue burden
# ============================================================
#
# Purpose:
#   Estimate whether unresolved Python static-analysis issue burden localized
#   to files selected by the primary ML threshold within the jointly recomputed RF+CM localization scope predicts lower
#   subsequent whole-Python development velocity.
#
# Primary detector/localization contract:
#   - detector: AGCDetector_ML; localization scope: RF+CM
#   - file ML metric: file_ml_fun_cfun_agc_share_space_by_token_weighted
...
#
# Primary GMM model:
#   Velocity_t ~ Velocity_{t-1} + MLRFCMQuality_{t-1} + treatment_t + controls_t
...
#   K08 therefore addresses the pre-specified secondary interaction question:
#   ML-localized RF+CM issue burden_t -> whole-Python velocity_{t+1}.
#
...
#
# Inputs:
#   - ML-localized RF+CM quality panel:
...
#
# Join rule:
#   I08 intentionally contains localized quality outcomes but not the B02/B06
#   whole-Python velocity outcome. K08 therefore joins log_lines_added_py_source
...
# the validated K04/E03 reverse-direction GMM logic and adapted to I08 ML-RF+CM
...
expected_primary_full_selected_files <- as_integer_arg(args, "expected_primary_full_selected_files", 64153L)
expected_primary_full_issue_stock <- as_integer_arg(args, "expected_primary_full_issue_stock", 35765L)
expected_primary_sensitivity_selected_files <- as_integer_arg(args, "expected_primary_sensitivity_selected_files", 63409L)
expected_primary_sensitivity_issue_stock <- as_integer_arg(args, "expected_primary_sensitivity_issue_stock", 34598L)
...
paths <- list(
  coefficients = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_coefficients.csv"),
  diagnostics = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_diagnostics.csv"),
  sample_qc = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_sample_qc.csv"),
  instrument_qc = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_instrument_qc.csv"),
  model_specs = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_model_specifications.csv"),
  threshold_input_audit = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_threshold_input_audit.csv"),
  b06_join_audit = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_b06_join_audit.csv"),
  legacy_audit = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_legacy_flag_audit.csv"),
  coercion_audit = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_numeric_coercion_audit.csv"),
  calendar_gap_audit = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_calendar_gap_audit.csv"),
  run_metadata = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_run_metadata.csv"),
  qc = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_qc.csv"),
  models_rds = file.path(output_dir, "dynamic_panel_gmm_ml_rf_cm_models.rds")
)

run_started <- Sys.time()
log_message("INFO", "Reading I08 RF+CM-ML threshold quality panel: %s", input_file)
...
strict_count_check(data.table::uniqueN(long_panel$threshold_spec_id), expected_thresholds, "I08 threshold specifications", strict_expected_counts)
...
# I08 was built from B06 for quality/timing support, but the I08 long panel does
# not intentionally carry the whole-Python velocity outcome. Read B06 directly
# and use it as the authoritative velocity source for K08.
...
if (nrow(primary_panel) == 0L) abortf("No I08 primary-threshold rows found for strict ML > %.6f.", primary_threshold)
...
if (!all(sample_specs %in% unique(primary_panel$sample_spec))) {
  abortf("Primary I08 panel is missing required sample specifications: %s", paste(setdiff(sample_specs, unique(primary_panel$sample_spec)), collapse = ", "))
}
primary_panel <- primary_panel[sample_spec %in% sample_specs]

# Join only after freezing the I08 primary threshold and the two pre-specified
...
if (anyNA(primary_panel$repo_id) || anyNA(primary_panel$time_index)) {
  abortf("I08 primary repo_id/time_index must be complete integer-compatible keys before the B06 join.")
}

# I08 intentionally omits the legacy is_treatment/cursor fields. Compare only
# fields present in both I08 and B06, while sourcing those audit-only flags
# from authoritative B06 after the exact repo_id + time_index join.
join_compare_fields <- c(
  "repo_name", "dataset_source", "scope_role", "treatment_group", "time",
  "event", "event_index", "time_to_event", "post_event",
  "log_age", "ncloc_py_sonarqube", "log_contributors", "log_stars", "log_issues"
)
b06_legacy_fields <- c("is_treatment", "cursor")
b06_lookup_fields <- unique(c(
  "repo_id", "time_index", "log_lines_added_py_source",
  join_compare_fields, b06_legacy_fields
))
...
if (nrow(primary_panel) != primary_rows_before_b06_join) {
  abortf("B06 join changed I08 primary row count: before=%d after=%d.", primary_rows_before_b06_join, nrow(primary_panel))
}

# Missing B06 velocity means the I08 repo-month key did not resolve to the
...
if (b06_join_missing_velocity > 0L) {
  abortf("B06 join left %d I08 primary rows without log_lines_added_py_source.", b06_join_missing_velocity)
}
...
join_audit_parts <- list(
  data.table::data.table(
    ...
    note = "I08 primary rows before the B06 velocity join."
  ),
  data.table::data.table(
    ...
    note = "B06 join must not change I08 primary sample membership."
  ),
  data.table::data.table(
    ...
    note = "Every I08 primary repo-month must resolve to authoritative B06 log_lines_added_py_source."
  )
)
for (field in join_compare_fields) {
  ...
  join_audit_parts[[length(join_audit_parts) + 1L]] <- data.table::data.table(
    ...
    note = "I08 field must agree with the authoritative B06 source on matched repo-month keys."
  )
}
...
# Preserve legacy treatment flags only for the existing audit. The estimator
# continues to use normalized absorbing_treated reconstructed from
# treatment_group and event_index.
primary_panel[, is_treatment := b06__is_treatment]
primary_panel[, cursor := b06__cursor]
drop_b06_columns <- grep("^b06__", names(primary_panel), value = TRUE)
primary_panel[, (drop_b06_columns) := NULL]

# Validate I08 summary and sample-support artifacts before estimation.
i08_summary <- data.table::fread(i08_summary_file, na.strings = c("", "NA", "NaN"))
validate_columns(i08_summary, c("metric", "value"))
...
if (abs(as.numeric(summary_lookup[["primary_threshold"]]) - primary_threshold) > 1e-12) abortf("I08 summary primary threshold mismatch.")

sample_summary_input <- data.table::fread(i08_sample_summary_file, na.strings = c("", "NA", "NaN"))
...
if (nrow(sample_summary_input[sample_spec == "full_sample"]) != 1L ||
  ...
  abortf("I08 sample summary must contain exactly one row for each K08 sample specification.")
}
strict_count_check(sample_summary_input[sample_spec == "full_sample", repo_month_rows], expected_full_rows, "I08 sample-summary full rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", repositories], expected_full_repositories, "I08 sample-summary full repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", treatment_repositories], expected_full_treatment_repos, "I08 sample-summary full treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", control_repositories], expected_full_control_repos, "I08 sample-summary full control repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repo_month_rows], expected_sensitivity_rows, "I08 sample-summary sensitivity rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repositories], expected_sensitivity_repositories, "I08 sample-summary sensitivity repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", treatment_repositories], expected_sensitivity_treatment_repos, "I08 sample-summary sensitivity treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", control_repositories], expected_sensitivity_control_repos, "I08 sample-summary sensitivity control repositories", strict_expected_counts)
...
if (nrow(primary_global) != 2L) abortf("Expected two primary rows in I08 global audit; observed %d.", nrow(primary_global))
...
model_specs <- data.table::data.table(
  model = "ml_rf_cm_quality_to_velocity",
  direction = "MLRFCMQuality_{t-1} -> Velocity_t",
...
for (sample_spec_value in sample_specs) {
  ...
  model_name <- "ml_rf_cm_quality_to_velocity"
  ...
  qc_parts[[sample_spec]] <- data.table::data.table(
    ...
    note = c(
      "Primary I08 sample invariant.", "Primary I08 sample invariant.", "Primary I08 sample invariant.", "Primary I08 sample invariant.",
      "Must be zero.", "Normalized timing must be exact.", "Numeric parsing must not introduce missingness.",
      "GMM fields must be complete.", "GMM fields must be finite.", "I08 localized quality log outcome must equal log1p(selected_issue_total).",
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("run-x-k08 ML GMM QC failed: %s", paste(paste(failed_qc$sample_spec, failed_qc$check, sep = "/"), collapse = ", "))
}
...
metadata <- data.table::rbindlist(list(
  make_summary_row("run", "run_prefix", "run-x-k08"),
  ...
  make_summary_row("run", "i08_summary_file", i08_summary_file),
  make_summary_row("run", "i08_summary_sha256", sha256_file(i08_summary_file)),
  make_summary_row("run", "i08_sample_summary_file", i08_sample_summary_file),
  make_summary_row("run", "i08_sample_summary_sha256", sha256_file(i08_sample_summary_file)),
  make_summary_row("run", "i08_global_audit_file", i08_global_audit_file),
  make_summary_row("run", "i08_global_audit_sha256", sha256_file(i08_global_audit_file)),
  ...
  make_summary_row("definition", "analysis_scope", "agc_ml_rf_cm_localized_quality_to_velocity"),
  make_summary_row("definition", "localization_scope", "RF+CM"),
  make_summary_row("definition", "detector_scope_pair", "ML|RF+CM"),
  make_summary_row("definition", "detector", "AGCDetector_ML"),
  make_summary_row("definition", "ml_metric", "file_ml_fun_cfun_agc_share_space_by_token_weighted"),
  ...
  make_summary_row("definition", "quality_semantics", "unresolved SonarQube issue stock among Python files with ML-RF+CM share > 0.50 at the corresponding historical snapshot"),
...
log_message(
  "INFO",
  "Completed run-x-k08 %s: samples=%d; coefficients=%d; diagnostics=%d; failed_qc=%d; cautions=%d",
