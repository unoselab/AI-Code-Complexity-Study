#!/usr/bin/env python3
"""Freeze and audit the C_FUN file-NPR threshold grid before C_FUN quality outcomes are joined.

H01 consumes the A15 repo-month/Python-file C_FUN NPR artifact and applies a
pre-specified threshold grid to the continuous file-level C_FUN NPR signal.  It
...
auditing for the C_FUN extension, not treatment-effect estimation.
...
- H01 does not recalibrate thresholds from the C_FUN distribution or quality outcomes

Treatment timing
----------------
H01 follows the normalized Model A timing used by the quality DiD design:
...
Outputs are aggregate audits only. H01 does not generate 22 duplicated
file-level classification datasets; a later H-series quality-panel stage can
reapply the frozen threshold specification when joining A15 file rows to
...
def make_checks(
    ...
    add_check(checks, "threshold_total_count_with_legacy", len(specs) == 22, len(specs), 22, "H01 must have the 21-point grid plus one named legacy anchor.")
    ...
    if args.strict_expected_counts:
        ...
        for name, observed, expected in expected_pairs:
            add_check(checks, name, observed == expected, observed, expected, "Strict production-count reconciliation against the completed A15 C_FUN/Model A sample.")

    add_check(checks, "unexpected_file_status_rows", diagnostics["unexpected_status_rows"] == 0, diagnostics["unexpected_status_rows"], 0, "H01 accepts only A15 v1 C_FUN file-status values.")
...
def self_test() -> None:
    ...
    print("audit_cfun_npr_threshold_grid self-test: PASS")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, help="H01 output directory")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.self_test:
        self_test()
        return 0
    if args.input_file is None or args.a15_summary_file is None or args.output_dir is None:
        raise SystemExit("--input-file, --a15-summary-file, and --output-dir are required unless --self-test is used")
    ...
    for path in (args.input_file, args.a15_summary_file):
        if not path.is_file():
            raise FileNotFoundError(path)

    if args.primary_threshold != DEFAULT_PRIMARY_THRESHOLD:
        raise ValueError(f"H01 v1 freezes primary threshold at {DEFAULT_PRIMARY_THRESHOLD}; observed override={args.primary_threshold}")
    if args.grid_step != DEFAULT_GRID_STEP:
        raise ValueError(f"H01 v1 freezes grid step at {DEFAULT_GRID_STEP}; observed override={args.grid_step}")
    if args.grid_radius != DEFAULT_GRID_RADIUS:
        raise ValueError(f"H01 v1 freezes grid radius at {DEFAULT_GRID_RADIUS}; observed override={args.grid_radius}")
    if args.legacy_threshold != DEFAULT_LEGACY_THRESHOLD:
        raise ValueError(f"H01 v1 freezes legacy threshold at {DEFAULT_LEGACY_THRESHOLD}; observed override={args.legacy_threshold}")
    ...
    summary = {
        ...
        "methodology": {
            ...
            "threshold_selection_note": "The previously frozen SC2-7B FUN threshold/grid is transferred unchanged to C_FUN and audited without reading SonarQube or any quality outcome; H01 performs no C_FUN recalibration.",
        },
        "input": diagnostics,
        "a15": {
    ...
    print("=" * 80)
    print("run-x-h01 C_FUN file-NPR threshold-grid audit")
