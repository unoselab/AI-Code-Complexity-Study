...
# ============================================================
# run-x-b07 v2: Borusyak DiD for Python-only SonarQube quality
# ============================================================
#
# Purpose:
...
#      effects only.
#   3. adjusted_density
#      Python issues per KLOC with contemporaneous project covariates except
#      Python NCLOC, because the outcome is already normalized by code size.
#   4. fe_only_density
#      The same density outcomes with repository and calendar-month fixed
#      effects only.
#
...
make_model_specs <- function() {
  ...
  list(
    ...
    adjusted_density = list(
      label = "Adjusted density",
      family = "density",
      primary = "log_issues_per_kloc_py_sonarqube",
      outcomes = density_outcomes,
      first_stage = ~ log_age + log_contributors + log_stars + log_issues | repo_id + time_index,
      first_stage_text = "~ log_age + log_contributors + log_stars + log_issues | repo_id + time_index",
      model_fields = c("log_age", "log_contributors", "log_stars", "log_issues")
    ),
...
plot_primary_density <- function(dynamic_all, output_pdf, output_png) {
  data <- dynamic_all[
    ...
    model_spec %in% c("adjusted_density", "fe_only_density")
  ]
  data[, model_display := factor(model_spec_label, levels = c("Adjusted density", "FE-only density"))]
  p <- ggplot2::ggplot(
    data,
    ggplot2::aes(x = event_time, y = estimate, shape = model_display, linetype = model_display)
  ) +
    ...
    ggplot2::labs(
      ...
      subtitle = "Adjusted density versus repository/month FE-only density",
      x = "Months relative to first observed Cursor adoption",
...
paths <- list(
  ...
  primary_four_spec = file.path(output_dir, "python_quality_primary_four_spec_summary.csv"),
...
primary_four_spec <- static_all[
  (model_spec %in% c("adjusted_burden", "fe_only_burden") & outcome == "log_issue_total_py_sonarqube") |
  (model_spec %in% c("adjusted_density", "fe_only_density") & outcome == "log_issues_per_kloc_py_sonarqube")
]
primary_four_spec[, spec_order := match(
  model_spec,
  c("adjusted_burden", "fe_only_burden", "adjusted_density", "fe_only_density")
)]
data.table::setorder(primary_four_spec, spec_order)
primary_four_spec[, spec_order := NULL]
write_csv(primary_four_spec, paths$primary_four_spec)
...
adjusted_static <- static_all[model_spec %in% c("adjusted_burden", "adjusted_density"), .(
  outcome_family, outcome, outcome_label,
...
fe_static <- static_all[model_spec %in% c("fe_only_burden", "fe_only_density"), .(
...
covariate_static <- merge(
  adjusted_static, fe_static,
  by = c("outcome_family", "outcome"), all = TRUE, sort = FALSE
)
covariate_static[, estimate_difference_fe_minus_adjusted := fe_only_estimate - adjusted_estimate]
data.table::setorder(covariate_static, outcome_family, outcome)
write_csv(covariate_static, paths$covariate_static)

adjusted_dynamic <- dynamic_all[model_spec %in% c("adjusted_burden", "adjusted_density"), .(
  outcome_family, outcome, outcome_label, event_time, term_type,
  adjusted_estimate = estimate, adjusted_std_error = std.error,
  adjusted_conf_low = conf.low, adjusted_conf_high = conf.high, adjusted_p_value = p_value
)]
fe_dynamic <- dynamic_all[model_spec %in% c("fe_only_burden", "fe_only_density"), .(
  outcome_family, outcome, event_time, term_type,
  fe_only_estimate = estimate, fe_only_std_error = std.error,
  fe_only_conf_low = conf.low, fe_only_conf_high = conf.high, fe_only_p_value = p_value
)]
covariate_dynamic <- merge(
  adjusted_dynamic, fe_dynamic,
  by = c("outcome_family", "outcome", "event_time", "term_type"), all = TRUE, sort = FALSE
)
covariate_dynamic[, estimate_difference_fe_minus_adjusted := fe_only_estimate - adjusted_estimate]
data.table::setorder(covariate_dynamic, outcome_family, outcome, event_time)
...
expected_primary_rows <- 4L
expected_key_rows <- 4L * length(key_events)
expected_covariate_static_rows <- 16L
expected_covariate_dynamic_rows <- 16L * expected_dynamic_terms_per_model
...
qc <- data.table::data.table(
  check = c(
    ...
    "pretrend_summary_rows", "primary_four_spec_rows", "primary_key_term_rows",
  ...
  observed = c(
    ...
    nrow(pretrend_summary), nrow(primary_four_spec), nrow(primary_key_terms),
...
qc[, note := c(
  ...
  "4 specs x 8 outcomes.", "32 models x 12 reported dynamic/placebo terms.",
  "32 models x 5 placebo terms.", "One summary per model.",
  "Adjusted/FE-only burden and adjusted/FE-only density.",
  "Four primary specifications x eight key event times.",
  "Sixteen burden/density outcomes: FE-only versus adjusted.",
  "Sixteen burden/density outcomes x twelve event terms."
)]
...
metadata <- data.table::rbindlist(list(
  ...
  make_summary_row("definition", "adjusted_burden_first_stage", model_specs$adjusted_burden$first_stage_text),
  make_summary_row("definition", "adjusted_density_first_stage", model_specs$adjusted_density$first_stage_text),
...
log_message(
  ...
  implementation_version, nrow(static_all), nrow(dynamic_all), nrow(pretrend_all), nrow(primary_four_spec), nrow(failed_qc)
)
