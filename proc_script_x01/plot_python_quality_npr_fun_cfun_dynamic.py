#!/usr/bin/env python3
"""Create RF+CM NPR threshold-sensitivity event-study figures.
...
DEFAULT_OUTPUT_NAMES = {
    ("adjusted_burden", "below"): "fig_did_quality_npr_fun_cfun_dynamic-fecs-below-v1.pdf",
    ("adjusted_burden", "at_or_above"): "fig_did_quality_npr_fun_cfun_dynamic-fecs-primary-above-v1.pdf",
    ("fe_only_burden", "below"): "fig_did_quality_npr_fun_cfun_dynamic-feos-below-v1.pdf",
    ("fe_only_burden", "at_or_above"): "fig_did_quality_npr_fun_cfun_dynamic-feos-primary-above-v1.pdf",
}
REQUIRED_COLUMNS = {
    "sample_spec",
    "threshold_role",
...
# Match the color and marker styling of the supplied RF+CM ML reference.
# Eleven marker shapes are available because the at-or-above panel can contain
# the primary threshold and ten higher thresholds.
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot NPR-localized RF+CM issue-burden event-study estimates."
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "threshold estimates within each event month. Default: 0.36."
        ),
    )
...
def load_and_validate(args: argparse.Namespace) -> tuple[pd.DataFrame, list[dict[str, object]]]:
    ...
    duplicate_columns = ["sample_spec", "model_spec", "outcome", "threshold", "event_time"]
...
def plot_panel(
    ...
    cmap = LinearSegmentedColormap.from_list(
        "rf_cm_npr_thresholds",
        ["#845ec2", "#bb3bb0", "#d13a28"],
    )
...
def write_outputs(
    ...
    data_path = args.output_dir / "python_quality_npr_fun_cfun_dynamic_plot_data.csv"
    checks_path = args.output_dir / "python_quality_npr_fun_cfun_dynamic_plot_checks.csv"
    manifest_path = args.output_dir / "python_quality_npr_fun_cfun_dynamic_plot_manifest.json"
...
def self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="npr-fun-cfun-dynamic-self-test-") as temporary:
        root = Path(temporary)
...
def main() -> int:
    ...
    try:
        if args.self_test:
            ...
            return 0
        paths = run(args)
        print(f"PASS: created {len(paths)} NPR RF+CM dynamic figure panels")
        for path in paths:
            print(f"Figure: {path}")
