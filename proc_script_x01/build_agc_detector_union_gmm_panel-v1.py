#!/usr/bin/env python3
"""Build the NPR-union-ML AGC-like file quality panel for run-x-e04.
...
primary definitions from the existing NPR and ML analyses:
    ...
    Union selected: NPR selected OR ML selected

The union is formed at exact historical file identity before SonarQube issue
burden is aggregated. A file selected by both detectors is therefore counted
exactly once in the union burden.
...
Outputs
-------
agc_detector_union_repo_month_panel.csv.gz
    One row per B06 repo-month with detector-specific and union support, union
    unresolved issue burden, and log1p union burden used by E04 GMM.
agc_detector_union_support.csv
    Global support and overlap statistics for NPR-only, ML-only, intersection,
    and union selections.
agc_detector_union_reproduction_audit.csv
    Exact D03 NPR-primary and D05 ML-primary reproduction checks.
agc_detector_union_join_audit.csv
    Exact file-identity join diagnostics.
agc_detector_union_qc.csv
    Hard implementation checks.
agc_detector_union_metadata.csv
...
EXPERIMENT_NAME = "run-x-e04-dynamic-panel-gmm-agc-union"
...
def main() -> None:
    ...
    panel["selected_file_rows"] = panel["union_selected_file_rows"]
    panel["selected_issue_total"] = panel["union_selected_issue_total"]
    ...
    if not bool(count_identity.all() and disjoint_count_identity.all() and issue_identity.all() and disjoint_issue_identity.all()):
        abort("Union algebra failed; possible duplicate counting in repo-month aggregation")

    # Reconstruct existing frozen detector-only panels before trusting the union.
    ...
    if not (reproduction_audit["status"] == "pass").all():
        abort("Detector-only reproduction failed; union panel is not trusted")
    ...
    union_zero_share = float((panel["selected_issue_total"] == 0).mean())
    union_within_variation_repos = int(
    ...
    qc_rows = [
        ...
        ("union_smaller_than_npr_rows", int((panel["union_selected_file_rows"] < panel["npr_selected_file_rows"]).sum()), 0),
        ("union_smaller_than_ml_rows", int((panel["union_selected_file_rows"] < panel["ml_selected_file_rows"]).sum()), 0),
    ...
    if (qc["status"] != "pass").any() or (join_audit["status"] != "pass").any():
        abort("E04 union builder hard QC failed")
    ...
    metadata = pd.DataFrame(
        [
            ...
            ("definition", "double_count_policy", "files selected by both detectors are counted exactly once"),
            ("definition", "quality_semantics", "unresolved SonarQube issue stock among union-selected historical Python files"),
            ("support", "union_zero_issue_repo_month_share", union_zero_share),
            ("support", "union_repositories_with_within_quality_variation", union_within_variation_repos),
            ("support", "jaccard_npr_ml", jaccard),
            ("support", "overlap_coefficient_npr_ml", overlap_coefficient),
        ],
    ...
    panel_path = args.output_dir / "agc_detector_union_repo_month_panel.csv.gz"
    support_path = args.output_dir / "agc_detector_union_support.csv"
    reproduction_path = args.output_dir / "agc_detector_union_reproduction_audit.csv"
    join_path = args.output_dir / "agc_detector_union_join_audit.csv"
    qc_path = args.output_dir / "agc_detector_union_qc.csv"
    metadata_path = args.output_dir / "agc_detector_union_metadata.csv"
    ...
    print("E04 detector-union panel: PASS")
    ...
    print(f"Union issue stock: {panel['selected_issue_total'].sum():.0f}")
    print(f"Union zero-burden repo-month share: {union_zero_share:.6f}")
    print(f"Union within-quality-variation repositories: {union_within_variation_repos}")
