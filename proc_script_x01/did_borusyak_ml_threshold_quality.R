...
#   localized to Python files whose token-weighted ML AGC share exceeds each
#   threshold prepared by run-x-d07.
#
# Input contract:
#   - D07 has already performed file-level ML thresholding and SonarQube burden
#     aggregation. This script never re-selects files from A04/D02.
#   - 21 strict thresholds: 0.10, 0.14, ..., 0.50, ..., 0.86, 0.90.
#   - Two samples: full_sample and exclude_scope_mismatch_repos.
#   - Mapping: all_ml_files.
#   - Primary threshold: weighted ML AGC share > 0.50.
#
# Estimation:
...
#   - event 0:+6 dynamic effects and package-native placebo terms -6:-2.
#   - event -1 is the omitted reference period.
#
# Provenance gate:
#   The full-sample primary threshold must reproduce the existing run-x-d06
#   primary total-issue static and dynamic estimates within a strict tolerance.
#
# This program is standalone. It reuses the validated statistical structure of
# the prior Borusyak analyses but does not call any prior experiment script.
# ================================================================
...
outcome_labels <- c(
  log1p_selected_issue_total = "ML-selected total unresolved issue stock",
  log1p_selected_issue_code_smell = "ML-selected code-smell issue stock",
  log1p_selected_issue_bug = "ML-selected bug issue stock",
  log1p_selected_issue_vulnerability = "ML-selected vulnerability issue stock",
  log1p_selected_issue_high_severity = "ML-selected high-severity issue stock",
  log1p_selected_issue_maintainability_impact = "ML-selected maintainability-impact issue stock",
  log1p_selected_issue_reliability_impact = "ML-selected reliability-impact issue stock",
  log1p_selected_issue_security_impact = "ML-selected security-impact issue stock"
)
...
expected_ml_metric <- "file_ml_agc_share_space_by_token_weighted"
expected_operator <- ">"
...
run_self_test <- function() {
  ...
  stopifnot(identical(
    ...
    "ML-selected total unresolved issue stock"
  ))
  stopifnot(identical(
    lookup_named_scalar(outcome_roles, test_outcome, "outcome_roles"),
    "primary_burden"
  ))
  cat("did_borusyak_ml_threshold_quality self-test: PASS\n")
}
...
d07_summary_file <- normalizePath(require_arg(args, "d07_summary_file"), mustWork = TRUE)
d07_checks_file <- normalizePath(require_arg(args, "d07_checks_file"), mustWork = TRUE)
d07_sample_summary_file <- normalizePath(require_arg(args, "d07_sample_summary_file"), mustWork = TRUE)
d07_global_audit_file <- normalizePath(require_arg(args, "d07_global_audit_file"), mustWork = TRUE)
d06_static_reference_file <- normalizePath(require_arg(args, "d06_static_reference_file"), mustWork = TRUE)
d06_dynamic_reference_file <- normalizePath(require_arg(args, "d06_dynamic_reference_file"), mustWork = TRUE)
...
d06_reproduction_tolerance <- as_numeric_arg(args, "d06_reproduction_tolerance", 1e-10)

if (plot_min_event != -6L || plot_max_event != 6L) abortf("D08 v2 requires dynamic window -6:6.")
if (pretrend_min != -6L || pretrend_max != -2L) abortf("D08 v2 requires pretrend window -6:-2.")
if (confidence_level <= 0 || confidence_level >= 1) abortf("confidence_level must be between 0 and 1.")
if (expected_threshold_count != 21L || expected_sample_count != 2L) abortf("D08 v2 requires 21 thresholds and 2 sample specifications.")
...
paths <- list(
  static = file.path(output_dir, "quality_ml_threshold_static_effects.csv"),
  dynamic = file.path(output_dir, "quality_ml_threshold_dynamic_effects.csv"),
  pretrend_checks = file.path(output_dir, "quality_ml_threshold_pretrend_checks.csv"),
  pretrend_summary = file.path(output_dir, "quality_ml_threshold_pretrend_summary.csv"),
  diagnostics = file.path(output_dir, "quality_ml_threshold_model_diagnostics.csv"),
  failures = file.path(output_dir, "quality_ml_threshold_model_failures.csv"),
  threshold_support = file.path(output_dir, "quality_ml_threshold_support.csv"),
  event_support = file.path(output_dir, "quality_ml_threshold_event_support.csv"),
  support_policy = file.path(output_dir, "quality_ml_support_policy.csv"),
  primary_threshold_static = file.path(output_dir, "quality_ml_primary_threshold_static.csv"),
  primary_total_static = file.path(output_dir, "quality_ml_primary_total_static.csv"),
  primary_total_dynamic = file.path(output_dir, "quality_ml_primary_total_dynamic.csv"),
  total_threshold_static = file.path(output_dir, "quality_ml_total_threshold_static.csv"),
  total_threshold_dynamic = file.path(output_dir, "quality_ml_total_threshold_dynamic.csv"),
  d06_reproduction = file.path(output_dir, "quality_ml_d06_reproduction.csv"),
  qc = file.path(output_dir, "quality_ml_threshold_qc.csv"),
  summary = file.path(output_dir, "quality_ml_threshold_summary.csv"),
  metadata = file.path(output_dir, "quality_ml_threshold_run_metadata.csv")
)

run_started <- Sys.time()
log_message("INFO", "Reading D07 long panel: %s", input_file)
panel_all <- data.table::fread(input_file, na.strings = c("", "NA", "NaN"))
d07_summary <- data.table::fread(d07_summary_file, na.strings = c("", "NA", "NaN"))
d07_checks <- data.table::fread(d07_checks_file, na.strings = c("", "NA", "NaN"))
d07_sample_summary <- data.table::fread(d07_sample_summary_file, na.strings = c("", "NA", "NaN"))
d07_global_audit <- data.table::fread(d07_global_audit_file, na.strings = c("", "NA", "NaN"))
d06_static_ref <- data.table::fread(d06_static_reference_file, na.strings = c("", "NA", "NaN"))
d06_dynamic_ref <- data.table::fread(d06_dynamic_reference_file, na.strings = c("", "NA", "NaN"))

# Validate D07 provenance artifacts before fitting any model.
validate_columns(d07_summary, c("metric", "value"), "D07 summary")
summary_map <- setNames(as.character(d07_summary$value), as.character(d07_summary$metric))
required_summary_metrics <- c(
  ...
  "primary_full_selected_file_rows", "primary_full_selected_issue_total",
  "density_computed", "scope_sensitivity_repositories", "hard_qc_failures"
)
if (!all(required_summary_metrics %in% names(summary_map))) abortf("D07 summary is missing required metrics.")
if (summary_map[["script_version"]] != "run-x-d07-v3") abortf("D07 summary must report script_version=run-x-d07-v3; observed %s", summary_map[["script_version"]])
if (summary_map[["status"]] != "PASS") abortf("D07 summary status is not PASS.")
if (summary_map[["ml_metric"]] != expected_ml_metric) abortf("D07 ML metric mismatch.")
if (summary_map[["quality_semantics"]] != "unresolved_sonarqube_issue_stock_at_historical_snapshot") abortf("D07 quality semantics mismatch.")
if (summary_map[["mapping_spec"]] != expected_mapping) abortf("D07 mapping mismatch.")
expected_summary_numeric <- c(
  ...
  primary_threshold = primary_threshold, primary_full_selected_file_rows = 43325,
  primary_full_selected_issue_total = 48478, density_computed = 0,
  scope_sensitivity_repositories = 2, hard_qc_failures = 0
)
for (name in names(expected_summary_numeric)) {
  ...
  if (!is.finite(observed) || abs(observed - expected_summary_numeric[[name]]) > 1e-9) {
    abortf("D07 summary mismatch for %s: expected %s, observed %s", name, expected_summary_numeric[[name]], summary_map[[name]])
  }
}

validate_columns(d07_checks, c("check", "status"), "D07 checks")
if (nrow(d07_checks) == 0L || any(tolower(trimws(as.character(d07_checks$status))) != "pass")) {
  abortf("D07 checks contain non-passing rows.")
}

validate_columns(d07_sample_summary, c(
  "repo_month_rows", "repositories", "control_repositories", "treatment_repositories",
  "control_rows", "treatment_pre_rows", "treatment_post_rows", "untreated_first_stage_rows",
...
for (i in seq_len(nrow(expected_sample_support))) {
  ...
  observed_row <- d07_sample_summary[sample_spec == expected_row$sample_spec]
  if (nrow(observed_row) != 1L) abortf("D07 sample summary must contain one row for %s.", expected_row$sample_spec)
  for (field in setdiff(names(expected_sample_support), "sample_spec")) {
    if (as.integer(observed_row[[field]]) != as.integer(expected_row[[field]])) {
      abortf("D07 sample summary mismatch for %s/%s.", expected_row$sample_spec, field)
    }
  }
}
...
validate_columns(panel_all, required_columns, "D07 long panel")

if (nrow(panel_all) != expected_long_rows) abortf("D07 long-panel row mismatch: expected %d, observed %d", expected_long_rows, nrow(panel_all))
if (data.table::uniqueN(panel_all$sample_spec) != 2L) abortf("D07 long panel must contain two sample specifications.")
if (!setequal(unique(panel_all$sample_spec), expected_samples)) abortf("Unexpected D07 sample specifications.")
if (any(panel_all$mapping_spec != expected_mapping)) abortf("D07 mapping_spec mismatch.")
if (any(panel_all$ml_metric != expected_ml_metric)) abortf("D07 ml_metric mismatch.")
if (any(panel_all$ml_operator != expected_operator)) abortf("D07 ml_operator mismatch.")

numeric_columns <- unique(c(
  "repo_id", "treatment_group", "time_index", "event_index", "time_to_event", "threshold",
  "primary_threshold", "primary_analysis", "eligible_ml_file_count", "selected_file_count",
  "log_age", "ncloc_py_sonarqube", "log_contributors", "log_stars", "log_issues",
...
if (!identical(as.character(threshold_map$threshold_spec_id), expected_threshold_ids)) abortf("D07 threshold IDs do not match the 21-point ML grid.")
if (any(abs(threshold_map$threshold - expected_thresholds) > 1e-12)) abortf("D07 threshold values do not match the expected 0.10:0.90 grid.")
expected_roles <- ifelse(expected_threshold_ids == primary_threshold_id, "primary", "sensitivity_grid")
if (!identical(as.character(threshold_map$threshold_role), expected_roles)) abortf("D07 threshold roles are inconsistent with the primary 0.50 threshold.")
...
for (column in c("eligible_ml_file_count", "selected_file_count", raw_issue_columns)) {
  if (anyNA(panel_all[[column]]) || any(!is.finite(panel_all[[column]])) || any(panel_all[[column]] < 0)) {
    abortf("D07 column %s contains missing, non-finite, or negative values.", column)
  }
}
for (raw_name in names(raw_log_pairs)) {
  ...
  if (anyNA(observed) || any(!is.finite(observed)) || max(abs(observed - expected)) > 1e-10) {
    abortf("D07 log1p mismatch for %s <- %s.", log_name, raw_name)
  }
}

model_fields <- unique(c(all_outcomes, model_specs$adjusted_burden$model_fields))
if (any(!stats::complete.cases(panel_all[, ..model_fields]))) abortf("D07 long panel has missing model fields.")
if (any(!vapply(panel_all[, ..model_fields], function(x) all(is.finite(x)), logical(1)))) abortf("D07 long panel has non-finite model fields.")

# Check nested strict-threshold construction using totals from the D07 long panel.
...
# Reconcile D07 global audit to the panel for selected-file and total-issue counts.
validate_columns(d07_global_audit, c(
  "sample_spec", "mapping_spec", "threshold_spec_id", "threshold", "threshold_role",
  "repo_month_rows", "repositories", "selected_file_rows", "selected_issue_total",
  "repositories_with_within_quality_variation"
), "D07 global audit")
if (nrow(d07_global_audit) != 42L) abortf("D07 global audit must contain 42 rows.")
...
audit_check <- merge(
  d07_global_audit[, .(sample_spec, mapping_spec, threshold_spec_id, threshold,
...
if (nrow(audit_check) != 42L || anyNA(audit_check$repo_month_rows)) abortf("D07 global audit keys do not reconcile to the long panel.")
if (any(abs(audit_check$audit_repo_month_rows - audit_check$repo_month_rows) > 0) ||
  ...
  abortf("D07 global audit values do not reconcile to the long panel.")
}
...
for (sample_name in expected_samples) {
  for (i in seq_along(expected_threshold_ids)) {
    ...
    min_dynamic_positive <- min(support_grid$positive_issue_repositories)
...
if (nrow(failures_dt) > 0L) abortf("One or more D08 models failed; inspect %s", paths$failures)
...
# Exact numerical reproduction gate against D06 at full_sample x >0.50.
validate_columns(d06_static_ref, c("sample_spec", "model_spec", "outcome", "estimate", "std.error", "conf.low", "conf.high", "p_value"), "D06 static reference")
validate_columns(d06_dynamic_ref, c("sample_spec", "event_time", "model_spec", "outcome", "estimate", "std.error", "conf.low", "conf.high", "p_value"), "D06 dynamic reference")
d08_static_primary <- primary_total_static[sample_spec == "full_sample"]
d08_dynamic_primary <- primary_total_dynamic[sample_spec == "full_sample"]
d06_static_primary <- d06_static_ref[sample_spec == "full_sample" & outcome == "log1p_selected_issue_total"]
d06_dynamic_primary <- d06_dynamic_ref[sample_spec == "full_sample" & outcome == "log1p_selected_issue_total"]
if (nrow(d08_static_primary) != 2L || nrow(d06_static_primary) != 2L || nrow(d08_dynamic_primary) != 24L || nrow(d06_dynamic_primary) != 24L) {
  abortf("D06 reproduction reference row counts are not 2 static and 24 dynamic rows.")
}

reproduction_rows <- list()
metrics <- c("estimate", "std.error", "conf.low", "conf.high", "p_value")
static_join <- merge(
  d08_static_primary[, c("model_spec", metrics), with = FALSE],
  d06_static_primary[, c("model_spec", metrics), with = FALSE],
  by = "model_spec", suffixes = c("_d08", "_d06"), all = TRUE
)
for (row_idx in seq_len(nrow(static_join))) {
  for (metric in metrics) {
    d08_value <- static_join[[paste0(metric, "_d08")]][[row_idx]]
    d06_value <- static_join[[paste0(metric, "_d06")]][[row_idx]]
    difference <- abs(d08_value - d06_value)
    reproduction_rows[[length(reproduction_rows) + 1L]] <- data.table::data.table(
      result_type = "static", model_spec = static_join$model_spec[[row_idx]], event_time = NA_integer_,
      metric = metric, d08_value = d08_value, d06_value = d06_value,
      absolute_difference = difference, tolerance = d06_reproduction_tolerance,
      status = ifelse(is.finite(difference) && difference <= d06_reproduction_tolerance, "pass", "fail")
    )
  }
}
dynamic_join <- merge(
  d08_dynamic_primary[, c("model_spec", "event_time", metrics), with = FALSE],
  d06_dynamic_primary[, c("model_spec", "event_time", metrics), with = FALSE],
  by = c("model_spec", "event_time"), suffixes = c("_d08", "_d06"), all = TRUE
)
for (row_idx in seq_len(nrow(dynamic_join))) {
  for (metric in metrics) {
    d08_value <- dynamic_join[[paste0(metric, "_d08")]][[row_idx]]
    d06_value <- dynamic_join[[paste0(metric, "_d06")]][[row_idx]]
    difference <- abs(d08_value - d06_value)
    reproduction_rows[[length(reproduction_rows) + 1L]] <- data.table::data.table(
      result_type = "dynamic", model_spec = dynamic_join$model_spec[[row_idx]], event_time = dynamic_join$event_time[[row_idx]],
      metric = metric, d08_value = d08_value, d06_value = d06_value,
      absolute_difference = difference, tolerance = d06_reproduction_tolerance,
      status = ifelse(is.finite(difference) && difference <= d06_reproduction_tolerance, "pass", "fail")
    )
...
d06_reproduction <- data.table::rbindlist(reproduction_rows)
write_csv(d06_reproduction, paths$d06_reproduction)
if (nrow(d06_reproduction[status != "pass"]) > 0L) abortf("D06 primary 0.50 reproduction failed.")
...
expected_counts <- c(
  ...
  d06_reproduction_rows = 5L * (2L + 24L),
...
observed_counts <- c(
  ...
  d06_reproduction_rows = nrow(d06_reproduction),
...
qc <- data.table::data.table(
  check = c(
    "d07_long_rows", "threshold_count", "sample_spec_count", "duplicate_long_panel_keys",
    "d07_checks_nonpass", "d07_global_audit_rows", "threshold_nesting_failures",
    names(expected_counts), "d06_reproduction_failures", "primary_model_warning_count"
  ),
  observed = c(
    nrow(panel_all), nrow(threshold_map), data.table::uniqueN(panel_all$sample_spec), nrow(key_duplicates),
    sum(tolower(trimws(as.character(d07_checks$status))) != "pass"), nrow(d07_global_audit), length(monotonic_failures),
    unname(observed_counts), nrow(d06_reproduction[status != "pass"]), primary_warning_count
  ),
  expected = c(
    expected_long_rows, 21L, 2L, 0L,
    0L, 42L, 0L,
    unname(expected_counts), 0L, 0L
  )
)
qc[, status := ifelse(as.numeric(observed) == as.numeric(expected), "pass", "fail")]
qc[, detail := c(
  "D07 authoritative long-panel row count.",
  "ML threshold grid count.",
  "Full and scope-sensitivity samples.",
  "sample+threshold+repo+time must be unique.",
  "All D07 checks must pass.",
  "21 thresholds x 2 samples.",
  "Strict threshold nesting must be monotone.",
  rep("Strict D08 output row-count contract.", length(expected_counts)),
  "Full-sample >0.50 D08 estimates must numerically reproduce D06.",
  "Primary D08 model path must be warning-free."
)]
write_csv(qc, paths$qc)
failed_qc <- qc[status == "fail"]
if (nrow(failed_qc) > 0L && strict_expected_counts) abortf("D08 hard QC failed: %s", paste(failed_qc$check, collapse = ", "))
...
summary <- data.table::data.table(
  metric = c(
    ...
    "primary_model_warnings", "d06_reproduction_failures", "density_computed", "hard_qc_failures"
  ),
  value = as.character(c(
    paste0("run-x-d08-", implementation_version), ifelse(nrow(failed_qc) == 0L, "PASS", "FAIL"),
    ...
    primary_warning_count, nrow(d06_reproduction[status != "pass"]), 0L, nrow(failed_qc)
  ))
)
...
metadata <- data.table::data.table(
  section = c(
    rep("run", 12), rep("software", 4), rep("definition", 12), rep("input_sha256", 7)
  ),
  metric = c(
    "implementation_version", "started", "finished", "input_file", "d07_summary_file", "d07_checks_file",
    "d07_sample_summary_file", "d07_global_audit_file", "d06_static_reference_file", "d06_dynamic_reference_file",
    ...
    "cluster_variable", "dynamic_window", "pretrend_window", "reference_event", "confidence_level", "density_computed",
    "d07_long_panel", "d07_summary", "d07_checks", "d07_sample_summary", "d07_global_audit",
    "d06_static_reference", "d06_dynamic_reference"
  ),
  value = c(
    implementation_version, format(run_started, "%Y-%m-%d %H:%M:%S %Z"), format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z"),
    input_file, d07_summary_file, d07_checks_file, d07_sample_summary_file, d07_global_audit_file,
    d06_static_reference_file, d06_dynamic_reference_file, script_path,
    ifelse(is.na(script_path), NA_character_, sha256_file(script_path)),
    ...
    sha256_file(input_file), sha256_file(d07_summary_file), sha256_file(d07_checks_file), sha256_file(d07_sample_summary_file),
    sha256_file(d07_global_audit_file), sha256_file(d06_static_reference_file), sha256_file(d06_dynamic_reference_file)
  )
)
...
log_message(
  "INFO",
  "Completed run-x-d08 %s: status=%s; jobs=%d; static=%d; dynamic=%d; sparse threshold-sample rows=%d; warnings=%d; hard QC failures=%d",
