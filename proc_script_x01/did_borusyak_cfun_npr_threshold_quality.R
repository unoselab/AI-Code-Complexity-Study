...
# ============================================================
# run-x-h04 v1: Borusyak DiD across frozen C_FUN-NPR thresholds
# ============================================================
#
# Purpose:
#   Estimate Cursor-adoption effects on unresolved SonarQube issue burden
#   concentrated in Python files whose contemporaneous C_FUN-NPR exceeds each
#   threshold frozen by run-x-h01 and aggregated by run-x-h03.
#
# Primary interpretation:
#   The outcome is the log1p unresolved SonarQube issue stock among Python files
#   exceeding a specified C_FUN-NPR threshold at the corresponding historical
...
#   - 22 C_FUN-NPR threshold specifications (21 grid + 1 legacy anchor).
...
# adapted only for the H03 C_FUN-NPR measurement contract:
...
outcome_labels <- c(
  log1p_selected_issue_total = "Selected high-C_FUN-NPR total issue stock",
  log1p_selected_issue_code_smell = "Selected high-C_FUN-NPR code-smell stock",
  log1p_selected_issue_bug = "Selected high-C_FUN-NPR bug stock",
  log1p_selected_issue_vulnerability = "Selected high-C_FUN-NPR vulnerability stock",
  log1p_selected_issue_high_severity = "Selected high-C_FUN-NPR BLOCKER+CRITICAL stock",
  log1p_selected_issue_maintainability_impact = "Selected high-C_FUN-NPR maintainability-impact stock",
  log1p_selected_issue_reliability_impact = "Selected high-C_FUN-NPR reliability-impact stock",
  log1p_selected_issue_security_impact = "Selected high-C_FUN-NPR security-impact stock"
)
...
compute_threshold_support <- function(data, post_values, sparse_min_dynamic_positive_repos, sparse_min_within_variation_repos) {
  ...
  data.table::data.table(
    ...
    eligible_cfun_file_rows = sum(data$eligible_cfun_file_count),
...
run_self_test <- function() {
  ...
  cat("did_borusyak_cfun_npr_threshold_quality self-test: PASS\n")
}
...
paths <- list(
  static = file.path(output_dir, "quality_cfun_npr_static_effects.csv"),
  dynamic = file.path(output_dir, "quality_cfun_npr_dynamic_effects.csv"),
  pretrend_checks = file.path(output_dir, "quality_cfun_npr_pretrend_checks.csv"),
  pretrend_summary = file.path(output_dir, "quality_cfun_npr_pretrend_summary.csv"),
  diagnostics = file.path(output_dir, "quality_cfun_npr_model_diagnostics.csv"),
  failures = file.path(output_dir, "quality_cfun_npr_model_failures.csv"),
  threshold_support = file.path(output_dir, "quality_cfun_npr_threshold_support.csv"),
  event_support = file.path(output_dir, "quality_cfun_npr_event_support.csv"),
  support_policy = file.path(output_dir, "quality_cfun_npr_support_policy.csv"),
  primary_static = file.path(output_dir, "quality_cfun_npr_primary_threshold_static.csv"),
  primary_total_static = file.path(output_dir, "quality_cfun_npr_primary_total_static.csv"),
  primary_total_dynamic = file.path(output_dir, "quality_cfun_npr_primary_total_dynamic.csv"),
  total_threshold_static = file.path(output_dir, "quality_cfun_npr_total_threshold_static.csv"),
  total_threshold_dynamic = file.path(output_dir, "quality_cfun_npr_total_threshold_dynamic.csv"),
  qc = file.path(output_dir, "quality_cfun_npr_qc.csv"),
  summary = file.path(output_dir, "quality_cfun_npr_summary.csv"),
  metadata = file.path(output_dir, "quality_cfun_npr_run_metadata.csv"),
  static_plot_pdf = file.path(plot_dir, "quality_cfun_npr_total_static_across_thresholds.pdf"),
  static_plot_png = file.path(plot_dir, "quality_cfun_npr_total_static_across_thresholds.png"),
  primary_dynamic_pdf = file.path(plot_dir, "quality_cfun_npr_primary_total_dynamic.pdf"),
  primary_dynamic_png = file.path(plot_dir, "quality_cfun_npr_primary_total_dynamic.png")
)

run_started <- Sys.time()
log_message("INFO", "Reading H03 long panel: %s", input_file)
panel <- data.table::fread(input_file, na.strings = c("", "NA", "NaN"))
h03_summary <- data.table::fread(h03_summary_file, na.strings = c("", "NA", "NaN"))
h03_sample_summary <- data.table::fread(h03_sample_summary_file, na.strings = c("", "NA", "NaN"))
h03_global_audit <- data.table::fread(h03_global_audit_file, na.strings = c("", "NA", "NaN"))

validate_columns(h03_summary, c("metric", "value"))
validate_columns(h03_sample_summary, c(
...
validate_columns(h03_global_audit, c(
  "sample_spec", "threshold_id", "threshold_role", "threshold", "selected_file_rows",
  "eligible_cfun_file_rows", "selected_issue_total"
))

if (lookup_metric(h03_summary, "status") != "PASS") abortf("H03 summary status is not PASS.")
strict_count_check(as.integer(lookup_metric(h03_summary, "thresholds")), expected_thresholds, "H03 thresholds", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(h03_summary, "sample_specs")), expected_sample_specs, "H03 sample specs", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(h03_summary, "long_panel_rows")), expected_long_rows, "H03 long-panel rows", strict_expected_counts)
if (as.integer(lookup_metric(h03_summary, "density_computed")) != 0L) abortf("H03 density_computed must be zero for H04 burden analysis.")
if (abs(as.numeric(lookup_metric(h03_summary, "primary_threshold")) - primary_threshold) > 1e-12) {
  abortf("H03 primary threshold does not match H04 primary threshold.")
}
...
required_columns <- unique(c(
  ...
  "eligible_cfun_file_count", "selected_file_count", "selected_issue_total",
...
numeric_columns <- unique(c(
  ...
  "log_issues", "event_time_normalized", "absorbing_treated", "eligible_cfun_file_count",
...
strict_count_check(input_rows, expected_long_rows, "H04 input rows", strict_expected_counts)

contract_mismatch_rows <- panel[
  comparison_operator != ">" |
    npr_metric != "file_npr_cfun_space_by_token_weighted" |
    quality_scope != "canonical_a15_python_files_with_finite_cfun_npr" |
    quality_count_semantics != "unresolved_sonarqube_issue_stock_at_historical_snapshot" |
    density_computed != 0
]
if (nrow(contract_mismatch_rows) > 0L) abortf("Found %d H03 measurement-contract mismatch rows.", nrow(contract_mismatch_rows))
...
if (nrow(timing_mismatch_rows) > 0L) abortf("Found %d H03 timing-reconstruction mismatches.", nrow(timing_mismatch_rows))
...
if (nrow(missing_model_rows) > 0L) abortf("Found %d rows with missing H04 model fields.", nrow(missing_model_rows))
nonfinite_model_rows <- panel[
  !apply(as.data.frame(panel[, ..all_model_fields]), 1L, function(row) all(is.finite(row)))
]
if (nrow(nonfinite_model_rows) > 0L) abortf("Found %d rows with non-finite H04 model fields.", nrow(nonfinite_model_rows))

# Validate threshold-level aggregate counts against run-x-h03 global audit.
input_global <- panel[, .(
  repo_month_rows = .N,
  eligible_cfun_file_rows = sum(eligible_cfun_file_count),
...
audit_compare <- merge(
  ...
  h03_global_audit[, .(
    ...
    audit_eligible_cfun_file_rows = eligible_cfun_file_rows,
...
audit_compare[, mismatch :=
  is.na(eligible_cfun_file_rows) | is.na(audit_eligible_cfun_file_rows) |
  eligible_cfun_file_rows != audit_eligible_cfun_file_rows |
  selected_file_rows != audit_selected_file_rows |
  selected_issue_total != audit_selected_issue_total
]
if (any(audit_compare$mismatch)) abortf("H04 input does not reconcile to H03 global audit for %d sample-threshold rows.", sum(audit_compare$mismatch))
...
# Validate threshold-invariant sample support against H03 sample summary.
...
for (sample_name in sample_specs) {
  ...
  expected <- h03_sample_summary[sample_spec == sample_name]
  if (nrow(expected) != 1L) abortf("Missing H03 sample-summary row for %s", sample_name)
...
if (any(sample_validation$mismatch_count > 0L)) abortf("H03 sample-support reconciliation failed.")

support_policy <- data.table::data.table(
  policy_id = "prespecified_sparse_support_flag_v1",
  apply_to_model_omission = 0L,
  prespecified_before_h04_results = 1L,
...
qc <- data.table::data.table(
  check = c(
    ...
    "h03_global_audit_mismatch_rows",
    "h03_sample_support_mismatch_samples",
    "model_job_count",
    "static_effect_rows",
    "dynamic_effect_rows",
    "pretrend_check_rows",
...
qc[, note := c(
  ...
  "Selected file/issue counts must reconcile to H03 global audit.",
  "Threshold-invariant sample support must reconcile to H03 sample summary.",
...
nonprimary_failure_note <- if (nonprimary_failure_job_count > 0L) {
  sprintf("%d non-primary model jobs had a failure or partial dynamic result; inspect quality_cfun_npr_model_failures.csv.", nonprimary_failure_job_count)
} else {
...
summary <- data.table::rbindlist(list(
  make_summary_row("run", "script_version", "run-x-h04-v1"),
...
metadata <- data.table::rbindlist(list(
  ...
  make_summary_row("definition", "quality_semantics", "unresolved SonarQube issue burden among Python files exceeding a frozen contemporaneous C_FUN-NPR threshold"),
...
if (nrow(plot_static_data) > 0L) {
  ...
  p_static <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "Quality burden DiD across frozen C_FUN-NPR thresholds",
      subtitle = "Total selected-file unresolved SonarQube issue stock",
      x = "C_FUN-NPR threshold",
...
if (nrow(plot_dynamic_data) > 0L) {
  ...
  p_dynamic <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "Primary-threshold Quality x C_FUN-NPR event study",
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("H04 hard QC failed: %s", paste(failed_qc$check, collapse = ", "))
}

log_message(
  "INFO",
  "Completed run-x-h04 %s: status=%s; jobs=%d; static=%d; dynamic=%d; nonprimary failures=%d; hard QC failures=%d",
