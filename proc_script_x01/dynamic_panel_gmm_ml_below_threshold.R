...
# ============================================================
# run-x-g05 v1: Dynamic panel GMM for ML-below-threshold quality burden
# ============================================================
#
# Purpose:
#   Estimate whether static-analysis issue burden in finite-ML-share Python files
#   at or below the primary ML threshold predicts lower subsequent
#   whole-Python development velocity.
#
# Input contract:
#   G04 quality subset: finite file_ml_agc_share_space_by_token_weighted <= 0.50
#   ML-share-missing/non-finite files are excluded from this subset by G04.
#   G05 does not recompute file selection; it validates and consumes G04 output.
#
# GMM model:
#   Velocity_t ~ Velocity_{t-1} + BelowMLQuality_{t-1} + treatment_t + controls_t
#   GMM instrument: Velocity_{t-2}
#   effect=twoways, model=twosteps, transformation=d, collapse=FALSE
#
# Inputs:
#   - G04 zero-inclusive ML-below-threshold repo-month panel
#   - G04 hard-QC and metadata files
...
expected_selected_file_rows <- as_integer_arg(args, "expected_selected_file_rows", 161184L)
expected_selected_issue_stock <- as_integer_arg(args, "expected_selected_issue_stock", 295977L)
...
paths <- list(
  coefficients = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_coefficients.csv"),
  primary_summary = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_primary_summary.csv"),
  diagnostics = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_diagnostics.csv"),
  sample_qc = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_sample_qc.csv"),
  instrument_qc = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_instrument_qc.csv"),
  b06_join_audit = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_b06_join_audit.csv"),
  numeric_audit = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_numeric_coercion_audit.csv"),
  calendar_gap_audit = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_calendar_gap_audit.csv"),
  legacy_audit = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_legacy_flag_audit.csv"),
  qc = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_qc.csv"),
  metadata = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_run_metadata.csv"),
  models_rds = file.path(output_dir, "dynamic_panel_gmm_ml_below_threshold_models.rds")
)

run_started <- Sys.time()
log_message("INFO", "Reading G04 ML-below-threshold panel: %s", input_file)
g04_panel <- data.table::fread(input_file, na.strings = c("", "NA", "NaN"))
required_g04 <- c(
  "repo_id", "time_index", "selected_file_rows", "selected_issue_total", "log1p_selected_issue_total",
  "ml_eligible_file_rows", "ml_eligible_issue_total",
  "ml_above_threshold_file_rows", "ml_above_threshold_issue_total",
  "ml_below_threshold_file_rows", "ml_below_threshold_issue_total",
  "ml_unclassified_file_rows", "ml_unclassified_issue_total",
  "ml_threshold", "ml_eligibility_rule", "ml_above_rule", "ml_below_rule", "ml_unclassified_rule"
)
validate_columns(g04_panel, required_g04, "G04 ML-below-threshold panel")
strict_count_check(nrow(g04_panel), expected_rows, "G04 panel rows", strict_expected_counts)
if (data.table::uniqueN(g04_panel[, .(repo_id, time_index)]) != nrow(g04_panel)) abortf("G04 panel has duplicate repo-month keys")
if (any(abs(as.numeric(g04_panel$ml_threshold) - 0.5) > 1e-12)) abortf("Unexpected ML threshold in G04 input")
if (any(as.character(g04_panel$ml_eligibility_rule) != "finite_weighted_ML_AGC_share")) abortf("Unexpected ML eligibility rule in G04 input")
if (any(as.character(g04_panel$ml_above_rule) != "finite_ML_share_GT_primary_threshold")) abortf("Unexpected ML above-threshold rule in G04 input")
if (any(as.character(g04_panel$ml_below_rule) != "finite_ML_share_LE_primary_threshold")) abortf("Unexpected ML below-threshold rule in G04 input")
if (any(as.character(g04_panel$ml_unclassified_rule) != "ML_share_missing_or_nonfinite")) abortf("Unexpected ML unclassified rule in G04 input")
if (any(as.numeric(g04_panel$selected_file_rows) != as.numeric(g04_panel$ml_below_threshold_file_rows))) abortf("G04 selected_file_rows is not the below-threshold file count")
if (any(abs(as.numeric(g04_panel$selected_issue_total) - as.numeric(g04_panel$ml_below_threshold_issue_total)) > 1e-12)) abortf("G04 selected_issue_total is not the below-threshold issue burden")
strict_count_check(sum(as.numeric(g04_panel$selected_file_rows)), expected_selected_file_rows, "G04 selected file rows", strict_expected_counts)
strict_count_check(round(sum(as.numeric(g04_panel$selected_issue_total))), expected_selected_issue_stock, "G04 selected issue stock", strict_expected_counts)

log_message("INFO", "Reading G04 hard QC: %s", input_qc_file)
g04_qc <- data.table::fread(input_qc_file, na.strings = c("", "NA", "NaN"))
validate_columns(g04_qc, c("check", "status"), "G04 QC")
if (nrow(g04_qc[tolower(as.character(status)) == "fail"]) > 0L) abortf("G04 input QC contains failed checks")
required_g04_checks <- c(
  "repo_month_rows", "duplicate_repo_month_keys",
  "a04_d02_d02_only_keys", "a04_d02_a04_only_keys",
  "d02_only_missing_sha_keys", "a04_only_missing_sha_keys",
  "ml_eligible_rows", "ml_above_threshold_rows", "ml_below_threshold_rows",
  "d05_primary_reproduction_failures",
...
missing_g04_checks <- setdiff(required_g04_checks, as.character(g04_qc$check))
if (length(missing_g04_checks) > 0L) abortf("G04 QC is missing required checks: %s", paste(missing_g04_checks, collapse = ", "))
if (nrow(g04_qc[check %in% required_g04_checks & tolower(as.character(status)) != "pass"]) > 0L) abortf("Required G04 QC checks are not all PASS")

log_message("INFO", "Reading G04 metadata: %s", input_metadata_file)
g04_metadata <- data.table::fread(input_metadata_file, na.strings = c("", "NA", "NaN"))
validate_columns(g04_metadata, c("section", "metric", "value"), "G04 metadata")
metadata_value <- function(metric_name) {
  values <- as.character(g04_metadata[metric == metric_name, value])
  if (length(values) != 1L) abortf("Expected exactly one G04 metadata value for %s", metric_name)
  values[[1L]]
}
if (metadata_value("experiment") != "run-x-g04-prepare-ml-below-threshold-gmm-input") abortf("Unexpected G04 experiment provenance")
if (metadata_value("implementation_version") != "v2") abortf("Unexpected G04 implementation version")
if (metadata_value("ml_metric") != "file_ml_agc_share_space_by_token_weighted") abortf("Unexpected G04 ML metric")
if (abs(as.numeric(metadata_value("ml_threshold")) - 0.5) > 1e-12) abortf("Unexpected G04 ML threshold metadata")
if (metadata_value("downstream_experiment") != "run-x-g05") abortf("G04 metadata does not designate run-x-g05 as downstream experiment")
...
g04_panel[, `:=`(repo_id = as.integer(repo_id), time_index = as.integer(time_index))]
b06[, `:=`(repo_id = as.integer(repo_id), time_index = as.integer(time_index))]
if (anyNA(g04_panel$repo_id) || anyNA(g04_panel$time_index) || anyNA(b06$repo_id) || anyNA(b06$time_index)) abortf("repo_id/time_index must be integer-compatible and complete")
...
rows_before_join <- nrow(g04_panel)
panel <- merge(g04_panel, b06_lookup, by = c("repo_id", "time_index"), all.x = TRUE, sort = FALSE)
...
if (log_mismatches > 0L) abortf("ML-below-threshold log1p outcome mismatch: %d rows", log_mismatches)
...
model_name <- "ml_below_threshold_quality_to_velocity"
primary_term <- "lag(log1p_selected_issue_total, 1)"
direction <- "MLBelowThresholdQuality_{t-1} -> Velocity_t"
...
log_message("INFO", "Fitting ML-below-threshold quality -> velocity GMM")
...
if (nrow(primary_summary) != 1L) abortf("Expected exactly one ML-below-threshold primary coefficient")
...
qc <- data.table::data.table(
  check = c(
    "source_rows", "source_repositories", "treatment_repositories", "control_repositories",
    "g04_selected_file_rows", "g04_selected_issue_stock",
...
if (nrow(failed_qc) > 0L && strict_expected_counts) abortf("run-x-g05 GMM QC failed: %s", paste(failed_qc$check, collapse = ", "))
...
metadata <- data.table::data.table(
  ...
  metric = c(
    ...
    "g04_panel_file", "g04_panel_sha256", "g04_qc_file", "g04_qc_sha256",
    "g04_metadata_file", "g04_metadata_sha256", "b06_panel_file", "b06_panel_sha256",
    "R", "plm",
    "analysis_scope", "ml_metric", "ml_threshold", "ml_selection_rule",
    "localized_quality", "velocity", "treatment", "size_control", "other_controls",
    "gmm_specification", "failed_checks", "caution_checks"
  ),
  value = c(
    "run-x-g05", implementation_version,
    ...
    "ml_below_threshold_quality_to_velocity", "file_ml_agc_share_space_by_token_weighted",
    "0.5", "finite weighted ML AGC share <= 0.50; ML share missing/non-finite excluded by G04",
...
log_message(
  "INFO",
  "Completed run-x-g05 %s: beta=%.9f; se=%.9f; p=%.9f; failed_qc=%d; cautions=%d",
