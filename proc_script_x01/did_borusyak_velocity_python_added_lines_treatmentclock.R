...
# ============================================================
# run-x-b03 v4: Borusyak DiD timing sensitivity for Python added-lines velocity
# ============================================================
#
# Purpose:
...
#   The recorded adoption cohort is preserved as event_index_recorded. Backend
#   mode constructs event_index_model using one of three effective clocks:
#   recorded T, T-1, or T-2. Legacy month-specific columns such as cursor,
#   is_treatment, post_event, lead_*, and lag_* are audit-only and never define
#   model treatment.
#
...
# holding backend, outcome, and sample fixed. Timing-comparison mode stacks the
# three effective adoption clocks and reports their static, dynamic, and
# pretrend sensitivity.
...
run_did_model <- function(data, outcome, first_stage_formula, horizon = NULL, pretrends = NULL, gname = "event_index_model") {
  capture_evaluation(
...
render_html_report <- function(
  ...
  timing_spec, timing_spec_label, adoption_shift_months
) {
  ...
  report_lines <- c(
    ...
    "- Calendar time uses `time_index`; the recorded adoption cohort is preserved as `event_index_recorded`.",
    "- This specification uses **`r timing_spec_label_report`** with an adoption shift of **`r adoption_shift_months_report` month(s)** when constructing the effective treatment cohort.",
  ...
  report_env$timing_spec_report <- timing_spec
  report_env$timing_spec_label_report <- timing_spec_label
  report_env$adoption_shift_months_report <- adoption_shift_months
...
run_backend_comparison <- function(args) {
  ...
  summary_rows <- list(
    make_summary_row("implementation", "version", "v4"),
  ...
  log_message(
    "INFO",
    "Completed run-x-b03-v6 backend comparison covariate_spec=%s: static=%d; dynamic=%d; pretrend=%d; panel_rows=%d",
...
run_covariate_comparison <- function(args) {
  ...
  log_message(
    "INFO",
    "Completed run-x-b03-v6 covariate comparison: static=%d; dynamic=%d; pretrend_checks=%d; paired_static=%d; paired_dynamic=%d",
    nrow(static_all), nrow(dynamic_all), nrow(pretrend_checks_all), nrow(static_differences), nrow(dynamic_differences)
  )
}


run_timing_comparison <- function(args) {
  check_packages(c("data.table", "ggplot2"))
  spec_root <- normalizePath(require_arg(args, "spec_root"), mustWork = TRUE)
  comparison_output_dir <- require_arg(args, "comparison_output_dir")
  comparison_summary_output <- require_arg(args, "comparison_summary_output")
  strict_expected_counts <- as_logical_arg(args, "strict_expected_counts", TRUE)
  tolerance <- as_numeric_arg(args, "backend_equivalence_tolerance", 1e-8)
  ...
  timings <- data.table::data.table(
    ...
    timing_spec_label = c("Recorded adoption T", "Effective adoption T-1", "Effective adoption T-2"),
    adoption_shift_months = c(0L, -1L, -2L)
  )
  covariates <- data.table::data.table(
    covariate_spec = c("python_ncloc_adjusted", "no_covariates"),
    covariate_spec_label = c("Python NCLOC adjusted", "No contemporaneous covariates")
  )
  backends <- data.table::data.table(
    ncloc_backend = c("sonarqube", "cloc"),
    ncloc_backend_label = c("SonarQube", "cloc")
  )
  outcomes <- c(
    "log_lines_added_py_source",
    "log_lines_added_py_no_merge",
  ...
  pretrend_check_parts <- list()
  pretrend_summary_parts <- list()
  event_support_parts <- list()
  sample_summary_parts <- list()

  part_index <- 0L
  for (ti in seq_len(nrow(timings))) {
    trow <- timings[ti]
    for (ci in seq_len(nrow(covariates))) {
      crow <- covariates[ci]
      for (bi in seq_len(nrow(backends))) {
        brow <- backends[bi]
        part_index <- part_index + 1L
        output_dir <- file.path(spec_root, trow$timing_spec, crow$covariate_spec, brow$ncloc_backend)
        prefix <- paste0("velocity_python_added_lines_", brow$ncloc_backend, "_", crow$covariate_spec)
        files <- list(
          static = file.path(output_dir, paste0(prefix, "_static_effects.csv")),
          dynamic = file.path(output_dir, paste0(prefix, "_dynamic_effects.csv")),
          pretrend_checks = file.path(output_dir, paste0(prefix, "_pretrend_checks.csv")),
          pretrend_summary = file.path(output_dir, paste0(prefix, "_pretrend_summary.csv")),
          event_support = file.path(output_dir, paste0(prefix, "_event_support.csv")),
          sample_summary = file.path(output_dir, paste0(prefix, "_sample_summary.csv"))
        )
        missing <- names(files)[!vapply(files, file.exists, logical(1))]
        if (length(missing) > 0L) {
          abortf("Timing comparison is missing %s outputs for %s/%s/%s", paste(missing, collapse = ","), trow$timing_spec, crow$covariate_spec, brow$ncloc_backend)
        }
        tables <- lapply(files, data.table::fread)
        for (name in c("static", "dynamic", "pretrend_checks", "pretrend_summary")) {
          tab <- tables[[name]]
          validate_columns(tab, c("timing_spec", "adoption_shift_months", "covariate_spec", "ncloc_backend"))
          if (any(tab$timing_spec != trow$timing_spec) || any(tab$adoption_shift_months != trow$adoption_shift_months) ||
              any(tab$covariate_spec != crow$covariate_spec) || any(tab$ncloc_backend != brow$ncloc_backend)) {
            abortf("Timing metadata mismatch in %s/%s/%s %s output.", trow$timing_spec, crow$covariate_spec, brow$ncloc_backend, name)
          }
        }
        static_parts[[part_index]] <- tables$static
        dynamic_parts[[part_index]] <- tables$dynamic
        pretrend_check_parts[[part_index]] <- tables$pretrend_checks
        pretrend_summary_parts[[part_index]] <- tables$pretrend_summary
        es <- tables$event_support
        es[, `:=`(timing_spec = trow$timing_spec, timing_spec_label = trow$timing_spec_label, adoption_shift_months = trow$adoption_shift_months, covariate_spec = crow$covariate_spec, ncloc_backend = brow$ncloc_backend)]
        event_support_parts[[part_index]] <- es
        ss <- tables$sample_summary
        ss[, `:=`(timing_spec = trow$timing_spec, covariate_spec = crow$covariate_spec, ncloc_backend = brow$ncloc_backend)]
        sample_summary_parts[[part_index]] <- ss
      }
    }
  }
  ...
  pretrend_checks_all <- data.table::rbindlist(pretrend_check_parts, fill = TRUE, use.names = TRUE)
  pretrend_summary_all <- data.table::rbindlist(pretrend_summary_parts, fill = TRUE, use.names = TRUE)
  event_support_all <- data.table::rbindlist(event_support_parts, fill = TRUE, use.names = TRUE)
  sample_summary_all <- data.table::rbindlist(sample_summary_parts, fill = TRUE, use.names = TRUE)

  data.table::setorder(static_all, timing_spec, covariate_spec, ncloc_backend, outcome)
  data.table::setorder(dynamic_all, timing_spec, covariate_spec, ncloc_backend, outcome, event_time)
  data.table::setorder(pretrend_checks_all, timing_spec, covariate_spec, ncloc_backend, outcome, event_time)
  data.table::setorder(pretrend_summary_all, timing_spec, covariate_spec, ncloc_backend, outcome)

  primary_static <- static_all[outcome == primary_outcome, .(
    timing_spec, timing_spec_label, adoption_shift_months, ncloc_backend, ncloc_backend_label,
    covariate_spec, covariate_spec_label, outcome, outcome_label,
    estimate, std.error, conf.low, conf.high, p_value, significant
  )]
  primary_pretrend <- pretrend_summary_all[outcome == primary_outcome, .(
    timing_spec, ncloc_backend, covariate_spec,
    all_ci_include_zero, periods_excluding_zero, minimum_p_value
  )]
  primary_summary <- merge(primary_static, primary_pretrend, by = c("timing_spec", "ncloc_backend", "covariate_spec"), all.x = TRUE, sort = FALSE)
  data.table::setorder(primary_summary, adoption_shift_months, covariate_spec, ncloc_backend)

  primary_key_terms <- dynamic_all[outcome == primary_outcome & event_time %in% c(-2L, 0L), .(
    timing_spec, timing_spec_label, adoption_shift_months, ncloc_backend, ncloc_backend_label,
    covariate_spec, covariate_spec_label, event_time, term_type, estimate, std.error,
    conf.low, conf.high, p_value, significant, support_rows, support_repositories
  )]
  data.table::setorder(primary_key_terms, adoption_shift_months, covariate_spec, ncloc_backend, event_time)

  # Re-express estimated event terms on the original recorded-adoption clock.
  # original_event_time = effective_event_time + adoption_shift_months.
  recorded_clock_dynamic <- data.table::copy(dynamic_all)
  recorded_clock_dynamic[, recorded_event_time := event_time + adoption_shift_months]
  data.table::setorder(recorded_clock_dynamic, outcome, covariate_spec, ncloc_backend, recorded_event_time, adoption_shift_months)

  anchor_grid <- data.table::CJ(
    timing_spec = timings$timing_spec,
    covariate_spec = covariates$covariate_spec,
    ncloc_backend = backends$ncloc_backend,
    unique = TRUE
  )
  anchor_grid <- merge(anchor_grid, timings, by = "timing_spec", all.x = TRUE, sort = FALSE)
  anchor_grid[, analysis_event_time := -2L - adoption_shift_months]
  anchor_grid[, recorded_event_time := -2L]
  anchor_values <- recorded_clock_dynamic[
    outcome == primary_outcome & recorded_event_time == -2L,
    .(timing_spec, covariate_spec, ncloc_backend, analysis_event_time = event_time,
      estimate, std.error, conf.low, conf.high, p_value, significant, support_rows, support_repositories)
  ]
  primary_recorded_minus2_anchor <- merge(
    anchor_grid, anchor_values,
    by = c("timing_spec", "covariate_spec", "ncloc_backend", "analysis_event_time"),
    all.x = TRUE, sort = FALSE
  )
  primary_recorded_minus2_anchor[, coefficient_status := data.table::fifelse(
    analysis_event_time == -1L, "omitted_reference",
    data.table::fifelse(is.na(estimate), "not_estimated", "estimated")
  )]
  data.table::setorder(primary_recorded_minus2_anchor, adoption_shift_months, covariate_spec, ncloc_backend)

  recorded_static <- static_all[timing_spec == "recorded_t", .(
    ncloc_backend, covariate_spec, outcome,
    recorded_estimate = estimate, recorded_std_error = std.error, recorded_p_value = p_value
  )]
  shifted_static <- static_all[timing_spec != "recorded_t", .(
    timing_spec, timing_spec_label, adoption_shift_months, ncloc_backend, covariate_spec, outcome, outcome_label,
    shifted_estimate = estimate, shifted_std_error = std.error, shifted_p_value = p_value
  )]
  timing_static_differences <- merge(shifted_static, recorded_static, by = c("ncloc_backend", "covariate_spec", "outcome"), all.x = TRUE, sort = FALSE)
  timing_static_differences[, `:=`(
    estimate_difference_shifted_minus_recorded = shifted_estimate - recorded_estimate,
    absolute_estimate_difference = abs(shifted_estimate - recorded_estimate)
  )]
  data.table::setorder(timing_static_differences, adoption_shift_months, covariate_spec, ncloc_backend, outcome)

  # QC: no-covariate results must be backend-independent for each timing.
  no_cov_static_sq <- static_all[covariate_spec == "no_covariates" & ncloc_backend == "sonarqube", .(timing_spec, outcome, sq = estimate)]
  no_cov_static_cl <- static_all[covariate_spec == "no_covariates" & ncloc_backend == "cloc", .(timing_spec, outcome, cloc = estimate)]
  no_cov_static <- merge(no_cov_static_sq, no_cov_static_cl, by = c("timing_spec", "outcome"), all = TRUE)
  no_cov_static_max_diff <- max(abs(no_cov_static$sq - no_cov_static$cloc), na.rm = TRUE)
  no_cov_dynamic_sq <- dynamic_all[covariate_spec == "no_covariates" & ncloc_backend == "sonarqube", .(timing_spec, outcome, event_time, sq = estimate)]
  no_cov_dynamic_cl <- dynamic_all[covariate_spec == "no_covariates" & ncloc_backend == "cloc", .(timing_spec, outcome, event_time, cloc = estimate)]
  no_cov_dynamic <- merge(no_cov_dynamic_sq, no_cov_dynamic_cl, by = c("timing_spec", "outcome", "event_time"), all = TRUE)
  no_cov_dynamic_max_diff <- max(abs(no_cov_dynamic$sq - no_cov_dynamic$cloc), na.rm = TRUE)

  expected_static_rows <- 3L * 2L * 2L * length(outcomes)
  expected_dynamic_rows <- expected_static_rows * 12L
  expected_pretrend_rows <- expected_static_rows * 5L
  expected_pretrend_summary_rows <- expected_static_rows
  expected_primary_summary_rows <- 3L * 2L * 2L
  expected_key_term_rows <- expected_primary_summary_rows * 2L
  expected_static_difference_rows <- 2L * 2L * 2L * length(outcomes)

  qc <- data.table::data.table(
    check_name = c(
      "static_rows", "dynamic_rows", "pretrend_check_rows", "pretrend_summary_rows",
      "primary_summary_rows", "primary_key_term_rows", "timing_static_difference_rows",
      "no_covariates_static_backend_equivalence", "no_covariates_dynamic_backend_equivalence"
    ),
    status = c(
      ifelse(nrow(static_all) == expected_static_rows, "pass", "fail"),
      ...
      ifelse(nrow(pretrend_checks_all) == expected_pretrend_rows, "pass", "fail"),
      ifelse(nrow(pretrend_summary_all) == expected_pretrend_summary_rows, "pass", "fail"),
      ifelse(nrow(primary_summary) == expected_primary_summary_rows, "pass", "fail"),
      ifelse(nrow(primary_key_terms) == expected_key_term_rows, "pass", "fail"),
      ifelse(nrow(timing_static_differences) == expected_static_difference_rows, "pass", "fail"),
      ifelse(is.finite(no_cov_static_max_diff) && no_cov_static_max_diff <= tolerance, "pass", "fail"),
      ifelse(is.finite(no_cov_dynamic_max_diff) && no_cov_dynamic_max_diff <= tolerance, "pass", "fail")
    ),
    observed = c(
      nrow(static_all), nrow(dynamic_all), nrow(pretrend_checks_all), nrow(pretrend_summary_all),
      nrow(primary_summary), nrow(primary_key_terms), nrow(timing_static_differences),
      no_cov_static_max_diff, no_cov_dynamic_max_diff
    ),
    expected = c(
      expected_static_rows, expected_dynamic_rows, expected_pretrend_rows, expected_pretrend_summary_rows,
      expected_primary_summary_rows, expected_key_term_rows, expected_static_difference_rows,
      tolerance, tolerance
    ),
    note = c(
      "3 timings x 2 covariate specs x 2 backends x 4 outcomes.",
      "Twelve estimated event terms (-6:-2 and 0:+6) per static specification.",
      "Five package-native placebo terms per static specification.",
      "One pretrend summary per static specification.",
      "Primary outcome across every timing/covariate/backend combination.",
      "Primary outcome event -2 and event 0 across every combination.",
      "T-1 and T-2 static estimates minus recorded-T within backend/covariate/outcome.",
      "No-covariate model does not use NCLOC backend.",
      "No-covariate dynamic model does not use NCLOC backend."
    )
  )
  if (strict_expected_counts && any(qc$status == "fail")) {
    abortf("Timing sensitivity QC failed: %s", paste(qc[status == "fail", check_name], collapse = ", "))
  }
  ...
  paths <- list(
    static_all = file.path(comparison_output_dir, paste0(prefix, "_static_effects.csv")),
    dynamic_all = file.path(comparison_output_dir, paste0(prefix, "_dynamic_effects.csv")),
    recorded_clock_dynamic = file.path(comparison_output_dir, paste0(prefix, "_recorded_clock_dynamic_effects.csv")),
    pretrend_checks_all = file.path(comparison_output_dir, paste0(prefix, "_pretrend_checks.csv")),
    pretrend_summary_all = file.path(comparison_output_dir, paste0(prefix, "_pretrend_summary.csv")),
    primary_summary = file.path(comparison_output_dir, paste0(prefix, "_primary_summary.csv")),
    primary_key_terms = file.path(comparison_output_dir, paste0(prefix, "_primary_key_terms.csv")),
    primary_recorded_minus2_anchor = file.path(comparison_output_dir, paste0(prefix, "_primary_recorded_minus2_anchor.csv")),
    timing_static_differences = file.path(comparison_output_dir, paste0(prefix, "_static_differences_vs_recorded.csv")),
    event_support_all = file.path(comparison_output_dir, paste0(prefix, "_event_support.csv")),
    sample_summary_all = file.path(comparison_output_dir, paste0(prefix, "_sample_summaries.csv")),
    qc = file.path(comparison_output_dir, paste0(prefix, "_qc.csv")),
    plot_pdf = file.path(plot_dir, paste0(prefix, "_primary_dynamic_effects.pdf")),
    plot_png = file.path(plot_dir, paste0(prefix, "_primary_dynamic_effects.png")),
    recorded_plot_pdf = file.path(plot_dir, paste0(prefix, "_primary_recorded_clock_effects.pdf")),
    recorded_plot_png = file.path(plot_dir, paste0(prefix, "_primary_recorded_clock_effects.png"))
  )

  write_csv(static_all, paths$static_all)
  write_csv(dynamic_all, paths$dynamic_all)
  write_csv(recorded_clock_dynamic, paths$recorded_clock_dynamic)
  write_csv(pretrend_checks_all, paths$pretrend_checks_all)
  write_csv(pretrend_summary_all, paths$pretrend_summary_all)
  write_csv(primary_summary, paths$primary_summary)
  write_csv(primary_key_terms, paths$primary_key_terms)
  write_csv(primary_recorded_minus2_anchor, paths$primary_recorded_minus2_anchor)
  write_csv(timing_static_differences, paths$timing_static_differences)
  write_csv(event_support_all, paths$event_support_all)
  write_csv(sample_summary_all, paths$sample_summary_all)
  write_csv(qc, paths$qc)

  # Use SonarQube as the representative backend for plotting timing sensitivity;
  # the no-covariate backend-equivalence QC verifies that backend choice is irrelevant there.
  plot_data <- dynamic_all[outcome == primary_outcome & ncloc_backend == "sonarqube" & event_time >= -6L & event_time <= 6L]
  plot_data[, timing_display := factor(timing_spec_label, levels = timings$timing_spec_label)]
  plot_data[, covariate_display := factor(covariate_spec_label, levels = covariates$covariate_spec_label)]
  p_effective <- ggplot2::ggplot(plot_data, ggplot2::aes(x = event_time, y = estimate, shape = timing_display, linetype = timing_display)) +
    ggplot2::geom_errorbar(ggplot2::aes(ymin = conf.low, ymax = conf.high), width = 0.20, position = ggplot2::position_dodge(width = 0.35)) +
    ggplot2::geom_point(size = 1.6, position = ggplot2::position_dodge(width = 0.35)) +
    ggplot2::geom_line(position = ggplot2::position_dodge(width = 0.35)) +
    ggplot2::geom_hline(yintercept = 0, linetype = "dashed") +
    ggplot2::geom_vline(xintercept = -0.5, linetype = "dashed") +
    ggplot2::scale_x_continuous(breaks = seq.int(-6L, 6L)) +
    ggplot2::coord_cartesian(xlim = c(-6.5, 6.5)) +
    ggplot2::facet_wrap(~covariate_display, nrow = 1) +
    ggplot2::labs(x = "Months Relative to Effective Adoption Timing", y = "Treatment Effect", shape = "Timing", linetype = "Timing", caption = "Primary Python source-added-lines outcome; SonarQube panel; T, T-1, and T-2 timing definitions.") +
    ggplot2::theme_minimal() + ggplot2::theme(legend.position = "bottom", panel.grid.major.x = ggplot2::element_blank(), panel.grid.minor.x = ggplot2::element_blank())
  ggplot2::ggsave(paths$plot_pdf, p_effective, width = 11, height = 5.5, units = "in", bg = "white")
  ggplot2::ggsave(paths$plot_png, p_effective, width = 11, height = 5.5, units = "in", dpi = 180, bg = "white")

  recorded_plot_data <- recorded_clock_dynamic[outcome == primary_outcome & ncloc_backend == "sonarqube" & recorded_event_time >= -6L & recorded_event_time <= 6L]
  recorded_plot_data[, timing_display := factor(timing_spec_label, levels = timings$timing_spec_label)]
  recorded_plot_data[, covariate_display := factor(covariate_spec_label, levels = covariates$covariate_spec_label)]
  p_recorded <- ggplot2::ggplot(recorded_plot_data, ggplot2::aes(x = recorded_event_time, y = estimate, shape = timing_display, linetype = timing_display)) +
    ggplot2::geom_point(size = 1.6, position = ggplot2::position_dodge(width = 0.35)) +
    ggplot2::geom_line(position = ggplot2::position_dodge(width = 0.35)) +
    ggplot2::geom_hline(yintercept = 0, linetype = "dashed") +
    ggplot2::geom_vline(xintercept = -0.5, linetype = "dashed") +
    ...
    ggplot2::coord_cartesian(xlim = c(-6.5, 6.5)) +
    ggplot2::facet_wrap(~covariate_display, nrow = 1) +
    ggplot2::labs(x = "Months Relative to Recorded Adoption T", y = "Treatment Effect", shape = "Timing", linetype = "Timing", caption = "The same estimated terms re-indexed to the recorded-adoption clock; omitted reference periods differ across timing specifications.") +
    ggplot2::theme_minimal() + ggplot2::theme(legend.position = "bottom", panel.grid.major.x = ggplot2::element_blank(), panel.grid.minor.x = ggplot2::element_blank())
  ggplot2::ggsave(paths$recorded_plot_pdf, p_recorded, width = 11, height = 5.5, units = "in", bg = "white")
  ggplot2::ggsave(paths$recorded_plot_png, p_recorded, width = 11, height = 5.5, units = "in", dpi = 180, bg = "white")

  summary_rows <- list(
    make_summary_row("implementation", "version", "v4"),
    make_summary_row("definition", "comparison", "recorded_t_vs_t_minus_1_vs_t_minus_2"),
    make_summary_row("definition", "primary_outcome", primary_outcome),
    make_summary_row("definition", "timing_specs", paste(timings$timing_spec, collapse = "|")),
    make_summary_row("definition", "covariate_specs", paste(covariates$covariate_spec, collapse = "|")),
    make_summary_row("definition", "backend_equivalence_tolerance", tolerance),
    make_summary_row("output", "static_rows", nrow(static_all)),
    make_summary_row("output", "dynamic_rows", nrow(dynamic_all)),
    make_summary_row("output", "pretrend_check_rows", nrow(pretrend_checks_all)),
    make_summary_row("output", "primary_summary_rows", nrow(primary_summary)),
    make_summary_row("qc", "no_covariates_static_backend_max_abs_difference", no_cov_static_max_diff),
    make_summary_row("qc", "no_covariates_dynamic_backend_max_abs_difference", no_cov_dynamic_max_diff),
    make_summary_row("artifact", "primary_dynamic_plot_pdf", paths$plot_pdf),
    make_summary_row("artifact", "primary_recorded_clock_plot_pdf", paths$recorded_plot_pdf)
  )
  for (idx in seq_len(nrow(primary_summary))) {
    row <- primary_summary[idx]
    key <- paste(row$timing_spec, row$covariate_spec, row$ncloc_backend, sep = "__")
    summary_rows[[length(summary_rows) + 1L]] <- make_summary_row("primary_static", paste0(key, "__estimate"), row$estimate)
    summary_rows[[length(summary_rows) + 1L]] <- make_summary_row("primary_static", paste0(key, "__p_value"), row$p_value)
    summary_rows[[length(summary_rows) + 1L]] <- make_summary_row("primary_pretrend", paste0(key, "__all_ci_include_zero"), row$all_ci_include_zero)
    summary_rows[[length(summary_rows) + 1L]] <- make_summary_row("primary_pretrend", paste0(key, "__periods_excluding_zero"), row$periods_excluding_zero)
    summary_rows[[length(summary_rows) + 1L]] <- make_summary_row("primary_pretrend", paste0(key, "__minimum_p_value"), row$minimum_p_value)
  }
  write_csv(data.table::rbindlist(summary_rows, fill = TRUE, use.names = TRUE), comparison_summary_output)

  log_message("INFO", "Completed run-x-b03-v6 timing comparison: static=%d; dynamic=%d; pretrend_checks=%d; primary=%d", nrow(static_all), nrow(dynamic_all), nrow(pretrend_checks_all), nrow(primary_summary))
}


args <- parse_cli_args(commandArgs(trailingOnly = TRUE))
mode <- tolower(if (is.null(args$mode)) "backend" else as.character(args$mode))
if (mode == "timing_compare") {
...
if (mode != "backend") abortf("--mode must be backend, compare, covariate_compare, or timing_compare: %s", mode)
...
timing_spec <- tolower(require_arg(args, "timing_spec"))
timing_specs <- list(
  recorded_t = list(label = "Recorded adoption T", adoption_shift_months = 0L),
  t_minus_1 = list(label = "Effective adoption T-1", adoption_shift_months = -1L),
  t_minus_2 = list(label = "Effective adoption T-2", adoption_shift_months = -2L)
)
if (!timing_spec %in% names(timing_specs)) {
  abortf("--timing-spec must be recorded_t, t_minus_1, or t_minus_2: %s", timing_spec)
}
timing_spec_label <- timing_specs[[timing_spec]]$label
adoption_shift_months <- timing_specs[[timing_spec]]$adoption_shift_months
...
expected_timing_shifted_rows <- as_integer_arg(args, "expected_timing_shifted_rows", -1L)
...
paths <- list(
  ...
  timing_shift_audit = file.path(output_dir, paste0(artifact_prefix, "_timing_shift_audit.csv")),
  common_support_audit = file.path(output_dir, paste0(artifact_prefix, "_common_support_audit.csv")),
...
# Keep the recorded adoption timing for provenance and legacy-flag audit, then
# construct an alternative effective cohort for the timing-sensitivity model.
# A negative adoption_shift_months moves adoption earlier: T-1 or T-2.
panel[, event_index_recorded := event_index]

# Use one common repository sample for all timing-sensitivity specifications.
# A repository fixed effect estimated from untreated observations cannot be
# predicted for a treated repository that has no untreated observation under
# an earlier treatment clock. Therefore, define support using the earliest
# sensitivity clock (T-2), drop unsupported treated repositories once, and
# reuse that same sample for recorded T, T-1, and T-2. Controls are retained.
common_support_shift_months <- -2L
panel[, common_support_event_index := data.table::fifelse(
  treatment_group == 1L,
  event_index_recorded + common_support_shift_months,
  0L
)]
common_support_by_repo <- panel[treatment_group == 1L, .(
  common_support_pre_rows = sum(time_index < common_support_event_index),
  common_support_total_rows = .N,
  common_support_event_index = unique(common_support_event_index)[1L]
), by = .(repo_id, repo_name)]
common_support_by_repo[, common_support_keep := common_support_pre_rows > 0L]
common_support_audit <- common_support_by_repo[common_support_keep == FALSE]
write_csv(common_support_audit, paths$common_support_audit)
unsupported_treatment_repos <- common_support_audit$repo_id
if (length(unsupported_treatment_repos) > 0L) {
  log_message(
    "INFO",
    "Common T-2 support filter removes %d treated repositories (%d repository-month rows) before all timing specifications.",
    length(unsupported_treatment_repos),
    nrow(panel[repo_id %in% unsupported_treatment_repos])
  )
  panel <- panel[!(treatment_group == 1L & repo_id %in% unsupported_treatment_repos)]
}
retained_support_check <- panel[treatment_group == 1L, .(
  has_common_support_pre = any(time_index < event_index_recorded + common_support_shift_months)
), by = repo_id]
if (any(!retained_support_check$has_common_support_pre)) {
  abortf("Common-support filtering failed: at least one retained treated repository lacks an untreated T-2 observation.")
}

panel[, event_time_recorded := data.table::fifelse(
  treatment_group == 1L,
  time_index - event_index_recorded,
  NA_integer_
)]
panel[, absorbing_treated_recorded := as.integer(
  treatment_group == 1L & event_index_recorded > 0L & time_index >= event_index_recorded
)]
panel[, event_index_model := data.table::fifelse(
  treatment_group == 1L,
  event_index_recorded + adoption_shift_months,
  0L
)]
if (any(panel[treatment_group == 1L, event_index_model <= 0L])) {
  abortf("Timing shift %s produces a nonpositive effective adoption index.", timing_spec)
}
panel[, event_time_normalized := data.table::fifelse(
  treatment_group == 1L,
  time_index - event_index_model,
  NA_integer_
)]
panel[, absorbing_treated := as.integer(
  treatment_group == 1L & event_index_model > 0L & time_index >= event_index_model
)]
panel[, timing_shift_changed_treatment := absorbing_treated != absorbing_treated_recorded]
...
# Legacy flags are defined against the recorded adoption timing, not the
# intentionally shifted sensitivity timing.
panel[, mismatch_cursor := legacy_cursor_flag != absorbing_treated_recorded]
panel[, mismatch_is_treatment := legacy_is_treatment != absorbing_treated_recorded]
panel[, mismatch_post_event := legacy_post_event != absorbing_treated_recorded]
...
timing_shifted_rows <- nrow(panel[timing_shift_changed_treatment == TRUE])
...
strict_count_check(timing_shifted_rows, expected_timing_shifted_rows, "timing-shifted treatment rows", strict_expected_counts)
...
timing_errors <- panel[
  treatment_group == 1L &
  (is.na(time_to_event) | as.integer(time_to_event) != event_time_recorded)
]
if (nrow(timing_errors) > 0L) abortf("Found %d recorded event-time normalization errors.", nrow(timing_errors))
...
legacy_audit <- panel[legacy_mismatch_any == TRUE, .(
  repo_id, repo_name, time, time_index, event, event_index = event_index_recorded,
  timing_spec = timing_spec, timing_spec_label = timing_spec_label, adoption_shift_months = adoption_shift_months,
  time_to_event, event_time_recorded, absorbing_treated_recorded,
...
timing_shift_audit <- panel[treatment_group == 1L & timing_shift_changed_treatment == TRUE, .(
  repo_id, repo_name, time, time_index,
  timing_spec = timing_spec, timing_spec_label = timing_spec_label,
  adoption_shift_months = adoption_shift_months,
  event_index_recorded, event_index_model,
  event_time_recorded, event_time_model = event_time_normalized,
  absorbing_treated_recorded, absorbing_treated_model = absorbing_treated
)]
data.table::setorder(timing_shift_audit, repo_name, time_index)
write_csv(timing_shift_audit, paths$timing_shift_audit)
...
all_event_support[, `:=`(
  ...
  timing_spec_label = timing_spec_label,
  adoption_shift_months = adoption_shift_months
)]
...
), by = .(event, event_index_recorded, event_index_model)]
cohort_support[, `:=`(
  timing_spec = timing_spec,
  timing_spec_label = timing_spec_label,
  adoption_shift_months = adoption_shift_months
)]
data.table::setorder(cohort_support, event_index_model)
...
sample_summary <- data.table::rbindlist(list(
  ...
  make_summary_row("definition", "timing_spec", timing_spec),
  make_summary_row("definition", "timing_spec_label", timing_spec_label),
  make_summary_row("definition", "adoption_shift_months", adoption_shift_months),
  ...
  make_summary_row("qc", "recorded_timing_errors", nrow(timing_errors)),
  ...
  make_summary_row("qc", "timing_shift_changed_treatment_rows", timing_shifted_rows, "Rows intentionally reclassified by the alternative effective adoption timing.")
), use.names = TRUE)
...
for (outcome in outcomes) {
  ...
  pretrend_check_tables[[outcome]] <- pretrend_check[, .(
    ...
    covariate_spec, covariate_spec_label, timing_spec, timing_spec_label, adoption_shift_months, first_stage_formula,
...
metadata <- data.table::data.table(
  metadata_key = c(
    ...
    "covariate_spec", "covariate_spec_label", "timing_spec", "timing_spec_label", "adoption_shift_months",
  ...
  value = c(
    "v4", backend, backend_label, backend_metric, covariate_spec, covariate_spec_label,
    timing_spec, timing_spec_label, as.character(adoption_shift_months),
...
summary_rows <- list(
  ...
  make_summary_row("definition", "timing_spec_label", timing_spec_label),
  make_summary_row("definition", "adoption_shift_months", adoption_shift_months),
  make_summary_row("definition", "treatment_timing", "absorbing_effective_adoption", "Recorded event_index is preserved; the effective cohort applies the timing sensitivity shift. Legacy monthly usage flags are audit-only."),
...
log_message("INFO", "Completed run-x-b03-v6 backend=%s covariate_spec=%s timing_spec=%s: %d static effects, %d package-native dynamic/pretrend terms, %d pretrend checks, and 1 HTML report",
            backend, covariate_spec, timing_spec,
