...
# ============================================================
# run-x-l04 v1: Borusyak DiD across frozen SC2-7B NPR RF/CM thresholds
# ============================================================
#
# Purpose:
#   Estimate Cursor-adoption effects on unresolved SonarQube issue burden
#   concentrated in Python files whose contemporaneous scope-specific SC2-7B NPR
#   exceeds each threshold frozen by run-x-c02/c03 and aggregated by run-x-l03.
#
# Primary interpretation:
#   The outcome is the log1p unresolved SonarQube issue stock among Python files
#   exceeding a specified scope-specific NPR threshold at the corresponding historical
...
#
# Input:
#   repo_x01/run-x-l03/npr-rf-cm-threshold-quality-burden-v1/quality_npr_rf_cm_threshold_repo_month_panel.csv.gz
#
# Frozen analysis dimensions:
#   - 23 NPR threshold specifications per localization scope (21 grid + 1 legacy anchor + 1 prior-primary anchor).
...
#
# Estimation design copied from the validated run-x-l02 Borusyak analysis and
# adapted only for the l03 scope-specific RF/CM measurement contract:
...
outcome_labels <- c(
  log1p_selected_issue_total = "Selected high-NPR total issue stock",
  log1p_selected_issue_code_smell = "Selected high-NPR code-smell stock",
  log1p_selected_issue_bug = "Selected high-NPR bug stock",
  log1p_selected_issue_vulnerability = "Selected high-NPR vulnerability stock",
  log1p_selected_issue_high_severity = "Selected high-NPR BLOCKER+CRITICAL stock",
  log1p_selected_issue_maintainability_impact = "Selected high-NPR maintainability-impact stock",
  log1p_selected_issue_reliability_impact = "Selected high-NPR reliability-impact stock",
  log1p_selected_issue_security_impact = "Selected high-NPR security-impact stock"
)
...
make_static_placeholder <- function(job, model_status, error_message = "", warning_count = 0L, warning_messages = "") {
  data.table::data.table(
    localization_scope = job$localization_scope,
    scope_id = job$scope_id,
    npr_metric = job$npr_metric,
...
compute_threshold_support <- function(data, post_values, sparse_min_dynamic_positive_repos, sparse_min_within_variation_repos) {
  ...
  data.table::data.table(
    ...
    eligible_file_rows = sum(data$eligible_file_count),
...
run_self_test <- function() {
  ...
  cat("did_borusyak_sc2_npr_rf_cm_threshold_quality self-test: PASS\n")
}
...
l03_summary_file <- normalizePath(require_arg(args, "l03_summary_file"), mustWork = TRUE)
l03_sample_summary_file <- normalizePath(require_arg(args, "l03_sample_summary_file"), mustWork = TRUE)
l03_global_audit_file <- normalizePath(require_arg(args, "l03_global_audit_file"), mustWork = TRUE)
scope_name <- toupper(as.character(require_arg(args, "localization_scope")))
if (!scope_name %in% c("RF", "CM")) abortf("localization_scope must be RF or CM; observed %s", scope_name)
scope_id_value <- tolower(scope_name)
expected_npr_metric <- if (scope_name == "RF") {
  "file_npr_fun_space_by_token_weighted"
} else {
  "file_npr_cfun_space_by_token_weighted"
}
expected_quality_scope <- if (scope_name == "RF") {
  "canonical_rf_python_files_with_finite_scope_npr"
} else {
  "canonical_cm_python_files_with_finite_scope_npr"
}
...
primary_threshold <- as_numeric_arg(args, "primary_threshold", 1.515059)
prior_primary_threshold <- as_numeric_arg(args, "prior_primary_threshold", 1.571637)
expected_scopes <- as_integer_arg(args, "expected_scopes", 2L)
expected_thresholds <- as_integer_arg(args, "expected_thresholds", 23L)
expected_sample_specs <- as_integer_arg(args, "expected_sample_specs", 2L)
expected_total_long_rows <- as_integer_arg(args, "expected_total_long_rows", 177974L)
expected_scope_long_rows <- as_integer_arg(args, "expected_scope_long_rows", 88987L)
...
file_prefix <- sprintf("quality_npr_%s", scope_id_value)
paths <- list(
  static = file.path(output_dir, sprintf("%s_static_effects.csv", file_prefix)),
  dynamic = file.path(output_dir, sprintf("%s_dynamic_effects.csv", file_prefix)),
  pretrend_checks = file.path(output_dir, sprintf("%s_pretrend_checks.csv", file_prefix)),
  pretrend_summary = file.path(output_dir, sprintf("%s_pretrend_summary.csv", file_prefix)),
  diagnostics = file.path(output_dir, sprintf("%s_model_diagnostics.csv", file_prefix)),
  failures = file.path(output_dir, sprintf("%s_model_failures.csv", file_prefix)),
  threshold_support = file.path(output_dir, sprintf("%s_threshold_support.csv", file_prefix)),
  event_support = file.path(output_dir, sprintf("%s_event_support.csv", file_prefix)),
  support_policy = file.path(output_dir, sprintf("%s_support_policy.csv", file_prefix)),
  primary_static = file.path(output_dir, sprintf("%s_primary_threshold_static.csv", file_prefix)),
  primary_total_static = file.path(output_dir, sprintf("%s_primary_total_static.csv", file_prefix)),
  primary_total_dynamic = file.path(output_dir, sprintf("%s_primary_total_dynamic.csv", file_prefix)),
  total_threshold_static = file.path(output_dir, sprintf("%s_total_threshold_static.csv", file_prefix)),
  total_threshold_dynamic = file.path(output_dir, sprintf("%s_total_threshold_dynamic.csv", file_prefix)),
  qc = file.path(output_dir, sprintf("%s_qc.csv", file_prefix)),
  summary = file.path(output_dir, sprintf("%s_summary.csv", file_prefix)),
  metadata = file.path(output_dir, sprintf("%s_run_metadata.csv", file_prefix)),
  static_plot_pdf = file.path(plot_dir, sprintf("%s_total_static_across_thresholds.pdf", file_prefix)),
  static_plot_png = file.path(plot_dir, sprintf("%s_total_static_across_thresholds.png", file_prefix)),
  primary_dynamic_pdf = file.path(plot_dir, sprintf("%s_primary_total_dynamic.pdf", file_prefix)),
  primary_dynamic_png = file.path(plot_dir, sprintf("%s_primary_total_dynamic.png", file_prefix))
)

run_started <- Sys.time()
log_message("INFO", "Reading l03 RF/CM long panel for scope=%s: %s", scope_name, input_file)
panel_all <- data.table::fread(input_file, na.strings = c("", "NA", "NaN"))
l03_summary <- data.table::fread(l03_summary_file, na.strings = c("", "NA", "NaN"))
l03_sample_summary <- data.table::fread(l03_sample_summary_file, na.strings = c("", "NA", "NaN"))
l03_global_audit <- data.table::fread(l03_global_audit_file, na.strings = c("", "NA", "NaN"))

validate_columns(l03_summary, c("metric", "value"))
validate_columns(l03_sample_summary, c(
  "localization_scope", "scope_id", "sample_spec", "repo_month_rows", "repositories",
  "control_repositories", "treatment_repositories", "untreated_first_stage_rows",
  "treatment_post_rows", "dynamic_event_0_to_6_rows"
))
validate_columns(l03_global_audit, c(
  "localization_scope", "scope_id", "npr_metric", "sample_spec", "threshold_id",
  "threshold_role", "threshold", "selected_file_rows", "eligible_file_rows",
  "selected_issue_total"
))
validate_columns(panel_all, c("localization_scope", "scope_id"))

if (lookup_metric(l03_summary, "status") != "PASS") abortf("l03 summary status is not PASS.")
strict_count_check(as.integer(lookup_metric(l03_summary, "localization_scopes")), expected_scopes, "l03 localization scopes", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(l03_summary, "thresholds_per_scope")), expected_thresholds, "l03 thresholds per scope", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(l03_summary, "sample_specs_per_scope")), expected_sample_specs, "l03 sample specs per scope", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(l03_summary, "long_panel_rows")), expected_total_long_rows, "l03 total long-panel rows", strict_expected_counts)
if (as.integer(lookup_metric(l03_summary, "density_computed")) != 0L) abortf("l03 density_computed must be zero for l04 burden analysis.")
if (abs(as.numeric(lookup_metric(l03_summary, "primary_threshold")) - primary_threshold) > 1e-12) {
  abortf("l03 primary threshold does not match l04 primary threshold.")
}
if (abs(as.numeric(lookup_metric(l03_summary, "prior_primary_threshold")) - prior_primary_threshold) > 1e-12) {
  abortf("l03 prior-primary threshold does not match l04 prior-primary threshold.")
}
strict_count_check(nrow(panel_all), expected_total_long_rows, "l03 input total rows", strict_expected_counts)

observed_scopes <- sort(unique(panel_all$localization_scope))
if (!identical(observed_scopes, sort(c("CM", "RF")))) {
  abortf("Unexpected l03 localization scopes: %s", paste(observed_scopes, collapse = ", "))
}
panel <- panel_all[localization_scope == scope_name]
rm(panel_all)
strict_count_check(nrow(panel), expected_scope_long_rows, sprintf("l04 %s input rows", scope_name), strict_expected_counts)

l03_sample_summary <- l03_sample_summary[localization_scope == scope_name]
l03_global_audit <- l03_global_audit[localization_scope == scope_name]
if (nrow(l03_sample_summary) != expected_sample_specs) abortf("Unexpected l03 sample-summary rows for %s.", scope_name)
if (nrow(l03_global_audit) != expected_thresholds * expected_sample_specs) abortf("Unexpected l03 global-audit rows for %s.", scope_name)

model_specs <- make_model_specs()
outcomes <- names(outcome_labels)
required_columns <- unique(c(
  "localization_scope", "scope_id", "npr_metric", "sample_spec", "threshold_id",
  "threshold_role", "grid_order", "delta_from_primary", "threshold", "comparison_operator",
  "quality_scope", "repo_id", "repo_name", "dataset_source", "scope_role",
  ...
  "eligible_file_count", "selected_file_count", "selected_issue_total",
...
numeric_columns <- unique(c(
  "grid_order", "delta_from_primary", "threshold", "repo_id", "treatment_group",
  "time_index", "event_index", "time_to_event", "is_treatment", "post_event",
  "log_age", "ncloc_py_sonarqube", "log_contributors", "log_stars", "log_issues",
  "event_time_normalized", "absorbing_treated", "eligible_file_count", "selected_file_count",
  unname(raw_outcome_map), outcomes
))
...
# Build the contract mask with explicit column access. This avoids data.table's
# `..name` parent-scope pronoun inside `i`, which is not supported consistently
# across data.table versions and caused v1 to look for a literal column named
# `..expected_npr_metric` on the production server.
contract_mismatch_mask <-
  is.na(panel[["localization_scope"]]) |
  panel[["localization_scope"]] != scope_name |
  is.na(panel[["scope_id"]]) |
  panel[["scope_id"]] != scope_id_value |
  is.na(panel[["comparison_operator"]]) |
  panel[["comparison_operator"]] != ">" |
  is.na(panel[["npr_metric"]]) |
  panel[["npr_metric"]] != expected_npr_metric |
  is.na(panel[["quality_scope"]]) |
  panel[["quality_scope"]] != expected_quality_scope
contract_mismatch_rows <- panel[contract_mismatch_mask]
if (nrow(contract_mismatch_rows) > 0L) abortf("Found %d l03 measurement-contract mismatch rows for %s.", nrow(contract_mismatch_rows), scope_name)
...
if (any(threshold_identity$sample_count != expected_sample_specs) ||
    ...
    any(threshold_identity$operator_count != 1L)) abortf("Threshold catalog is not invariant across samples.")

primary_catalog <- threshold_catalog[threshold_role == "primary"]
if (nrow(primary_catalog) != expected_sample_specs || any(abs(primary_catalog$threshold - primary_threshold) > 1e-12)) abortf("Primary-threshold catalog mismatch.")
legacy_catalog <- threshold_catalog[threshold_role == "legacy_anchor"]
if (nrow(legacy_catalog) != expected_sample_specs || any(abs(legacy_catalog$threshold - 1.5183) > 1e-12)) abortf("Legacy threshold anchor mismatch.")
prior_primary_catalog <- threshold_catalog[threshold_role == "prior_primary_anchor"]
if (nrow(prior_primary_catalog) != expected_sample_specs || any(abs(prior_primary_catalog$threshold - prior_primary_threshold) > 1e-12)) abortf("Prior-primary threshold anchor mismatch.")

duplicate_keys <- panel[, .N, by = .(sample_spec, threshold_id, repo_id, time_index)][N > 1L]
if (nrow(duplicate_keys) > 0L) abortf("Found %d duplicate sample-threshold-repo-time keys for %s.", nrow(duplicate_keys), scope_name)
...
if (nrow(timing_mismatch_rows) > 0L) abortf("Found %d l03 timing-reconstruction mismatches for %s.", nrow(timing_mismatch_rows), scope_name)
...
if (nrow(missing_model_rows) > 0L) abortf("Found %d rows with missing l04 model fields.", nrow(missing_model_rows))
nonfinite_model_rows <- panel[!apply(as.data.frame(panel[, ..all_model_fields]), 1L, function(row) all(is.finite(row)))]
if (nrow(nonfinite_model_rows) > 0L) abortf("Found %d rows with non-finite l04 model fields.", nrow(nonfinite_model_rows))

# Validate threshold-level aggregate counts against the frozen run-x-l03 global audit.
input_global <- panel[, .(
  repo_month_rows = .N,
  eligible_file_rows = sum(eligible_file_count),
...
audit_compare[, mismatch :=
  is.na(eligible_file_rows) | is.na(audit_eligible_file_rows) |
...
if (any(audit_compare$mismatch)) abortf("l04 %s input does not reconcile to l03 global audit for %d sample-threshold rows.", scope_name, sum(audit_compare$mismatch))
...
# Validate threshold-invariant causal-panel support against the frozen l03 sample summary.
...
for (sample_name in sample_specs) {
  ...
  actual <- data.table::data.table(
    ...
    dynamic_event_0_to_6_rows = nrow(part[
      absorbing_treated_recomputed == 1L & event_time_recomputed >= 0L & event_time_recomputed <= plot_max_event
    ])
  )
  expected <- l03_sample_summary[sample_spec == sample_name]
  if (nrow(expected) != 1L) abortf("Missing l03 sample-summary row for %s/%s", scope_name, sample_name)
...
if (any(sample_validation$mismatch_count > 0L)) abortf("l03 sample-support reconciliation failed for %s.", scope_name)

support_policy <- data.table::data.table(
  localization_scope = scope_name,
  scope_id = scope_id_value,
  policy_id = "prespecified_sparse_support_flag_v1",
  apply_to_model_omission = 0L,
  prespecified_before_l04_results = 1L,
...
for (sample_name in sample_specs) {
  ...
  for (i in seq_len(nrow(sample_thresholds))) {
    ...
    support <- compute_threshold_support(part, post_values, sparse_min_dynamic_positive_repos, sparse_min_within_variation_repos)
    support[, `:=`(
      localization_scope = scope_name,
      scope_id = scope_id_value,
      npr_metric = expected_npr_metric,
      sample_spec = sample_name,
    ...
    data.table::setcolorder(support, c(
      "localization_scope", "scope_id", "npr_metric", "sample_spec", "threshold_id",
      "threshold_role", "grid_order", "delta_from_primary", "threshold", "comparison_operator",
      setdiff(names(support), c("localization_scope", "scope_id", "npr_metric", "sample_spec", "threshold_id", "threshold_role", "grid_order", "delta_from_primary", "threshold", "comparison_operator"))
    ))
...
if (length(job_failure_rows) > 0L) {
  ...
  failures_all <- data.table::data.table(
    localization_scope = character(), scope_id = character(), npr_metric = character(),
...
data.table::setorder(static_all, localization_scope, sample_spec, threshold, model_spec, outcome)
data.table::setorder(dynamic_all, localization_scope, sample_spec, threshold, model_spec, outcome, event_time)
data.table::setorder(diagnostics_all, localization_scope, sample_spec, threshold, model_spec, outcome, stage)
...
pretrend_summary <- pretrend_all[, .(
  ...
  localization_scope, scope_id, npr_metric, sample_spec, threshold_id, threshold_role, grid_order, delta_from_primary, threshold,
  model_spec, model_spec_label, outcome, outcome_label, outcome_role
)]
data.table::setorder(pretrend_all, localization_scope, sample_spec, threshold, model_spec, outcome, event_time)
data.table::setorder(pretrend_summary, localization_scope, sample_spec, threshold, model_spec, outcome)
...
qc <- data.table::data.table(
  check = c(
    ...
    "l03_global_audit_mismatch_rows",
    "l03_sample_support_mismatch_samples",
    "model_job_count",
    "static_effect_rows",
    "dynamic_effect_rows",
    "pretrend_check_rows",
  ...
  observed = c(
    ...
    max(abs(prior_primary_catalog$threshold - prior_primary_threshold))
  ),
  expected = c(
    expected_scope_long_rows,
    expected_thresholds,
...
qc[, note := c(
  sprintf("Frozen run-x-l03 %s scope panel.", scope_name),
  "21 main-grid thresholds plus one legacy anchor and one prior-primary anchor.",
  "Full sample plus pre-specified two-repository exclusion sensitivity.",
  "Must be zero.",
  "l03 scope-specific measurement contract must remain unchanged.",
  ...
  "Selected file/issue counts must reconcile to l03 global audit for the requested scope.",
  "Threshold-invariant sample support must reconcile to l03 sample summary for the requested scope.",
  "23 thresholds x 2 samples x 2 model specs x 8 outcomes.",
  ...
  "Prior-primary anchor must equal 1.571637 exactly within numerical tolerance for historical bridge comparison."
)]
write_csv(qc, paths$qc)

failed_qc <- qc[status == "fail"]
nonprimary_failure_note <- if (nonprimary_failure_job_count > 0L) {
  sprintf("%d non-primary model jobs had a failure or partial dynamic result; inspect scope-specific model-failures CSV.", nonprimary_failure_job_count)
} else {
...
summary <- data.table::rbindlist(list(
  make_summary_row("run", "script_version", "run-x-l04-v2"),
  make_summary_row("run", "status", run_status),
  make_summary_row("design", "localization_scope", scope_name),
  make_summary_row("design", "scope_id", scope_id_value),
  make_summary_row("design", "npr_metric", expected_npr_metric),
  ...
  make_summary_row("design", "prior_primary_threshold", prior_primary_threshold, "Prior-paper primary anchor retained for RF/CM historical bridge comparison only."),
...
metadata <- data.table::rbindlist(list(
  ...
  make_summary_row("run", "l03_summary_sha256", sha256_file(l03_summary_file)),
  make_summary_row("run", "l03_sample_summary_sha256", sha256_file(l03_sample_summary_file)),
  make_summary_row("run", "l03_global_audit_sha256", sha256_file(l03_global_audit_file)),
  ...
  make_summary_row("definition", "localization_scope", scope_name),
  make_summary_row("definition", "scope_id", scope_id_value),
  make_summary_row("definition", "npr_metric", expected_npr_metric),
  make_summary_row("definition", "quality_scope", expected_quality_scope),
  ...
  make_summary_row("definition", "prior_primary_threshold", prior_primary_threshold),
  make_summary_row("definition", "quality_semantics", sprintf("unresolved SonarQube issue burden among Python files exceeding the frozen contemporaneous %s NPR threshold", scope_name)),
...
if (nrow(plot_static_data) > 0L) {
  ...
  p_static <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = sprintf("Quality burden DiD across frozen %s NPR thresholds", scope_name),
      subtitle = "Total selected-file unresolved SonarQube issue stock",
      x = sprintf("%s NPR threshold", scope_name),
      y = "Static ATT on log1p selected issue burden",
      linetype = "Sample / specification",
      shape = "Sample / specification",
      caption = sprintf("Dotted line: frozen primary threshold %.6f. Sparse thresholds are retained.", primary_threshold)
    ) +
...
if (nrow(plot_dynamic_data) > 0L) {
  ...
  p_dynamic <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = sprintf("Primary-threshold Quality x %s NPR event study", scope_name),
      subtitle = sprintf("Frozen threshold T = %.6f; full and pre-specified scope-sensitivity samples", primary_threshold),
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("l04 %s hard QC failed: %s", scope_name, paste(failed_qc$check, collapse = ", "))
}

log_message(
  "INFO",
  "Completed run-x-l04 %s scope=%s: status=%s; jobs=%d; static=%d; dynamic=%d; nonprimary failures=%d; hard QC failures=%d",
  implementation_version, scope_name, run_status, model_job_count, nrow(static_all), nrow(dynamic_all),
