#!/usr/bin/env python3
"""
Audit readiness of the ML-CM quality-to-velocity dynamic-panel GMM inputs.

This K05 audit is intentionally model-free. It validates that the existing H07
CM-localized ML quality panel can be joined to the authoritative B06 Python
...
- Selection operator inherited from H07: strict >
...
def run_audit(args: argparse.Namespace) -> int:
    h07_path = Path(args.h07_panel_file)
    b06_path = Path(args.b06_panel_file)
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    h07 = pd.read_csv(h07_path, compression="infer", low_memory=False)
    ...
    require_columns(h07, h07_required, "H07 panel")
    require_columns(b06, b06_required, "B06 panel")

    numeric(h07, ["threshold", "repo_id", "treatment_group", "time_index", "event_index",
                  "selected_issue_total", "log1p_selected_issue_total"], "H07 panel")
    ...
    primary = h07[
        h07["sample_spec"].map(clean_text).eq("full_sample")
        & h07["threshold_role"].map(clean_text).eq("primary")
        & np.isclose(h07["threshold"].astype(float), args.primary_threshold, atol=1e-12, rtol=0.0)
    ].copy()
    ...
    add_check(checks, "h07_long_rows", len(h07), args.expected_h07_long_rows,
              len(h07) == args.expected_h07_long_rows, "Expected H07 threshold x sample long-panel rows.")
    ...
    add_check(checks, "primary_repositories", int(primary["repo_id"].nunique()), args.expected_repositories,
              int(primary["repo_id"].nunique()) == args.expected_repositories, "Primary H07 repository count.")
    add_check(checks, "primary_treatment_repositories",
              ...
              "Primary H07 treated repositories.")
    add_check(checks, "primary_control_repositories",
              ...
              "Primary H07 control repositories.")
    ...
    add_check(checks, "primary_ml_metric", "|".join(metrics), EXPECTED_ML_METRIC,
              metrics == [EXPECTED_ML_METRIC], "K05 must audit CM/class-method ML, not RF or RF+CM.")
    ...
    add_check(checks, "primary_selected_file_rows", int(primary["selected_file_count"].sum()),
              ...
              "Expected primary ML-CM selected file-occurrence count.")
    add_check(checks, "primary_selected_issue_stock", int(round(primary["selected_issue_total"].sum())),
              ...
              "Expected primary ML-CM unresolved issue stock.")
    ...
    merged = primary[identity_cols].merge(
        ...
        suffixes=("_h07", "_b06"), indicator=True, validate="one_to_one"
    )
    unmatched = int(merged["_merge"].ne("both").sum())
    add_check(checks, "h07_b06_unmatched_repo_months", unmatched, 0, unmatched == 0,
              "Primary H07 and B06 must have the same full-sample repo-month universe.")

    mismatch_count = 0
    if unmatched == 0:
        for col in ["repo_name", "dataset_source", "treatment_group", "event_index"]:
            left = merged[f"{col}_h07"].map(clean_text)
            right = merged[f"{col}_b06"].map(clean_text)
            mismatch_count += int(left.ne(right).sum())
    add_check(checks, "h07_b06_identity_field_mismatches", mismatch_count, 0, mismatch_count == 0,
    ...
    audit_summary = pd.DataFrame([
        {
            "analysis": "ML-CM quality_{t-1} -> velocity_t",
            "detector": "ML",
            ...
            "h07_primary_rows": len(primary),
            "h07_primary_repositories": int(primary["repo_id"].nunique()),
    ...
    checks_path = output_dir / "k05_ml_cm_gmm_input_checks.csv"
    summary_path = output_dir / "k05_ml_cm_gmm_input_summary.csv"
    active_path = output_dir / "k05_ml_cm_exact_calendar_sample.csv.gz"
    ...
    metadata = {
        ...
        "h07_panel_file": str(h07_path),
        "h07_panel_sha256": sha256_file(h07_path),
    ...
    print(f"K05 audit status:                         {status}")
    print(f"Hard QC failures:                        {hard_failures}")
    print(f"Primary H07 rows/repositories:           {len(primary)}/{int(primary['repo_id'].nunique())}")
...
def self_test() -> int:
    ...
    assert EXPECTED_SCOPE == "CM", EXPECTED_SCOPE
    print("audit_ml_cm_gmm_inputs self-test: PASS")
    return 0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expected-primary-selected-files", type=int, default=37757)
    parser.add_argument("--expected-primary-issue-stock", type=int, default=36432)
...
def main() -> int:
    ...
    if args.self_test:
        return self_test()
    missing = [name for name in ["h07_panel_file", "b06_panel_file", "output_dir"] if not getattr(args, name)]
    if missing:
