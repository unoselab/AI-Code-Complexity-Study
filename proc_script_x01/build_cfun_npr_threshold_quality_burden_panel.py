#!/usr/bin/env python3
"""Build threshold-specific Quality x C_FUN-NPR repository-month burden panels.

This H03 experiment is a standalone adaptation of the validated D03 FUN-NPR
threshold-quality implementation. It does not call H03, rerun SonarQube, rescore
NPR, or choose thresholds from quality outcomes.

Frozen upstream contracts:
1. H01: outcome-blind C_FUN-NPR threshold specification and audit.
2. H02: A15-backed file-level C_FUN-NPR joined to unresolved SonarQube issue stock.
3. B06: authoritative 1,954-row Python SonarQube quality DiD base panel.

The H02 scope-exclusion artifact is also consumed deterministically. Repositories
with H02 issue-bearing Python files outside the frozen A15 NPR file universe are
...
Only files with finite ``file_npr_cfun_space_by_token_weighted`` are threshold
eligible. Files without finite C_FUN NPR are unclassified; they are never treated
...
NCLOC is not part of the frozen H02 input contract.
"""
...
def load_thresholds(spec_path: Path, audit_path: Path) -> tuple[list[ThresholdSpec], pd.DataFrame]:
    """Load and validate the frozen H01 threshold contract."""
    ...
    require_columns(spec, THRESHOLD_SPEC_REQUIRED, "H01 threshold specification")
    require_columns(audit, THRESHOLD_AUDIT_REQUIRED, "H01 threshold audit")

    if spec["threshold_id"].duplicated().any():
        raise ValueError("H01 threshold specification contains duplicate threshold_id values")
    if audit["threshold_id"].duplicated().any():
        raise ValueError("H01 threshold audit contains duplicate threshold_id values")
    ...
    if any(item.comparison_operator != ">" for item in rows):
        raise ValueError("H03 requires the frozen strict '>' comparison operator")
    if any(item.metric != NPR_METRIC for item in rows):
        raise ValueError(f"H03 requires the frozen NPR metric {NPR_METRIC}")
    ...
    if audit_ids != spec_ids:
        raise ValueError("H01 threshold specification and threshold audit IDs do not match")
    return rows, audit


def validate_upstream_contracts(
    h01_summary_path: Path,
    h02_summary_path: Path,
    h02_outside_scope_path: Path,
) -> tuple[pd.DataFrame, dict[str, Any], dict[str, str]]:
    """Validate the frozen H01/H02 gates and derive the scope-sensitivity repos."""
    h01 = read_json(h01_summary_path)
    h02 = read_metric_csv(h02_summary_path)

    if clean(h01.get("script_version")) != "run-x-h01-v1":
        raise ValueError(f"Unexpected H01 script version: {h01.get('script_version')}")
    if clean(h01.get("status")) != "PASS" or int(h01.get("hard_check_failures", -1)) != 0:
        raise ValueError("H01 threshold audit is not a clean PASS")
    if bool(h01.get("quality_outcome_inputs_consumed", True)):
        raise ValueError("H01 reports quality-outcome inputs were consumed")
    methodology = h01.get("methodology", {})
    if clean(methodology.get("metric")) != NPR_METRIC:
        raise ValueError("H01 metric does not match the H03 C_FUN NPR metric")
    if not math.isclose(float(methodology.get("primary_threshold")), PRIMARY_THRESHOLD, abs_tol=1e-12):
        raise ValueError("H01 primary threshold does not match the frozen value")

    if h02.get("script_version") != "run-x-h02-v1":
        raise ValueError(f"Unexpected H02 script version: {h02.get('script_version')}")
    if h02.get("status") != "PASS_WITH_SCOPE_EXCLUSIONS":
        raise ValueError(f"H02 status is not PASS_WITH_SCOPE_EXCLUSIONS: {h02.get('status')}")
    if h02.get("hard_qc_failures") != "0" or h02.get("threshold_applied") != "0":
        raise ValueError("H02 summary does not satisfy the no-failure/no-threshold contract")
    if h02.get("npr_metric_preserved") != NPR_METRIC:
        raise ValueError("H02 did not preserve the expected C_FUN NPR metric")

    outside = read_csv(
        h02_outside_scope_path,
        ["snapshot_key", "dataset_source", "repo_name", "commit_sha", "component_path", "exclusion_class"],
    )
    require_columns(outside, H02_OUTSIDE_REQUIRED, "H02 outside-A15 scope exclusions")
    if outside.empty:
        raise ValueError("H02 outside-A15 scope exclusion file is unexpectedly empty")
    if not outside["exclusion_class"].map(clean).eq("outside_a15_npr_file_universe").all():
        raise ValueError("Unexpected H02 scope exclusion class")
    ...
    if outside["sonar_issue_total"].isna().any() or (outside["sonar_issue_total"] < 0).any():
        raise ValueError("H02 outside-scope issue totals must be complete and non-negative")
    ...
    return scope_repos, h01, h02

def load_h02_file_table(path: Path) -> pd.DataFrame:
    """Load only the H02 columns needed for H03."""
    header = pd.read_csv(path, nrows=0)
    require_columns(header, H02_REQUIRED_COLUMNS, "H02 file-quality table")
    usecols = sorted(H02_REQUIRED_COLUMNS)
    string_columns = {
        "dataset_source",
        "repo_name",
        "repo_month",
    ...
    for column in [
        ...
        "cfun_space_by_tokens_scored", "cfun_npr_coverage_ratio", NPR_METRIC,
        *ISSUE_COLUMNS,
    ]:
        data[column] = pd.to_numeric(data[column], errors="coerce")
    if data[["repo_id", "time_index"]].isna().any().any():
        raise ValueError("H02 repo_id/time_index contains missing numeric values")
    for column in ISSUE_COLUMNS:
        if data[column].isna().any() or (data[column] < 0).any():
            raise ValueError(f"H02 issue column must be complete non-negative: {column}")
...
def validate_repo_month_identity(h02: pd.DataFrame, b06: pd.DataFrame) -> pd.DataFrame:
    """Validate that H02 and B06 describe the same 1,954 repo-month observations."""
    identity_cols = [
        "repo_id",
        "time_index",
        "dataset_source",
    ...
    grouped = h02[identity_cols].drop_duplicates()
    dup_key = grouped.duplicated(["repo_id", "time_index"], keep=False)
    if dup_key.any():
        raise ValueError("H02 has inconsistent repo-month identity within repo_id/time_index")

    right = b06[["repo_id", "time_index", "dataset_source", "repo_name", "time", "event", "event_index", "snapshot_key", "latest_commit_effective"]].copy()
    merged = right.merge(grouped, on=["repo_id", "time_index"], how="outer", suffixes=("_b06", "_h02"), indicator=True, validate="one_to_one")
    if not merged["_merge"].eq("both").all():
        raise ValueError("H02/B06 repo-month key universes do not match")
    comparisons = {
        "dataset_source": merged["dataset_source_b06"].map(clean).str.casefold().eq(merged["dataset_source_h02"].map(clean).str.casefold()),
        "repo_name": merged["repo_name_b06"].map(clean).str.casefold().eq(merged["repo_name_h02"].map(clean).str.casefold()),
        "time": merged["time"].map(clean).eq(merged["repo_month"].map(clean)),
        "event": merged["event_b06"].map(clean).eq(merged["event_h02"].map(clean)),
        "event_index": pd.to_numeric(merged["event_index_b06"], errors="coerce").eq(pd.to_numeric(merged["event_index_h02"], errors="coerce")),
    ...
    if any(failures.values()):
        raise ValueError(f"H02/B06 identity mismatches: {failures}")
    return grouped
...
def aggregate_one_threshold(files: pd.DataFrame, threshold: float, base_keys: pd.DataFrame) -> pd.DataFrame:
    ...
    work = files[["repo_id", "time_index", "python_lines", "cfun_space_by_tokens_scored", "sonar_issue_total", *[c for c in ISSUE_COLUMNS if c != "sonar_issue_total"]]].copy()
    work["eligible_cfun_file_count"] = eligible.astype("int64")
    ...
    work["selected_cfun_space_by_tokens"] = pd.to_numeric(work["cfun_space_by_tokens_scored"], errors="coerce").fillna(0).where(selected, 0)
    for column in ISSUE_COLUMNS:
        work[column] = work[column].where(selected, 0)

    agg_map: dict[str, str] = {
        "eligible_cfun_file_count": "sum",
        "selected_file_count": "sum",
        "selected_file_with_any_issue_count": "sum",
        "selected_python_lines": "sum",
        "selected_cfun_space_by_tokens": "sum",
    }
    ...
    grouped["selected_file_share_of_eligible"] = np.where(
        grouped["eligible_cfun_file_count"].gt(0),
        grouped["selected_file_count"] / grouped["eligible_cfun_file_count"],
        np.nan,
    )
    grouped["has_eligible_cfun_files"] = grouped["eligible_cfun_file_count"].gt(0).astype("int64")
    ...
    output["selected_file_share_of_eligible"] = np.where(
        output["eligible_cfun_file_count"].gt(0),
        output["selected_file_count"] / output["eligible_cfun_file_count"],
        np.nan,
    )
    return output
...
def build_outputs(
    ...
    h01_audit: pd.DataFrame,
    scope_repos: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Build the long H03 panel and audit outputs."""
    validate_repo_month_identity(h02, b06)
    ...
    scope_spec["reason"] = (
        "H02 contains issue-bearing Python file paths outside the frozen A15 C_FUN NPR file universe; "
        "exclude the affected repository as a scope-sensitivity analysis."
    )

    audit_by_id = h01_audit.set_index(h01_audit["threshold_id"].map(clean), drop=False)
    panel_parts: list[pd.DataFrame] = []
    global_rows: list[dict[str, Any]] = []
    timing_rows: list[dict[str, Any]] = []

    for spec in specs:
        aggregated = aggregate_one_threshold(h02, spec.threshold, base_keys)
        ...
        if not full[["repo_id", "time_index"]].equals(base[["repo_id", "time_index"]]):
            raise ValueError("Internal H03 aggregation order mismatch")
        ...
        for sample_name, sample_base in sample_specs.items():
            ...
            sample.insert(8, "quality_scope", "canonical_a15_python_files_with_finite_cfun_npr")
            ...
            eligible_files = int(sample["eligible_cfun_file_count"].sum())
            global_rows.append(
                {
                    ...
                    "eligible_cfun_file_rows": eligible_files,
            ...
            for stratum, group in timing_groups:
                eligible = int(group["eligible_cfun_file_count"].sum())
                ...
                timing_rows.append(
                    {
                        ...
                        "eligible_cfun_file_rows": eligible,
    ...
    outcome_spec = pd.DataFrame(
        [
            {"outcome": "log1p_selected_issue_total", "role": "primary_burden", "count_column": "selected_issue_total", "description": "log1p unresolved SonarQube issue stock in finite-C_FUN-NPR files exceeding the frozen threshold"},
...
def make_checks(
    ...
    expected_h02_rows: int,
    expected_finite_rows: int,
    expected_scope_repos: int,
) -> pd.DataFrame:
    """Construct strong structural and accounting checks for H03."""
    rows: list[dict[str, Any]] = []
    ...
    add_check(rows, "h02_file_rows", len(h02), expected_h02_rows, (len(h02) == expected_h02_rows) or not strict)
    finite = int(np.isfinite(pd.to_numeric(h02[NPR_METRIC], errors="coerce").to_numpy(dtype=float)).sum())
    add_check(rows, "h02_finite_cfun_rows", finite, expected_finite_rows, (finite == expected_finite_rows) or not strict)
    ...
    h01 = h01_audit.set_index(h01_audit["threshold_id"].map(clean), drop=False)
    ...
    for spec in specs:
        ...
        expected_selected = int(pd.to_numeric(h01.loc[spec.threshold_id, "selected_file_rows"]))
        expected_eligible = int(pd.to_numeric(h01.loc[spec.threshold_id, "eligible_finite_cfun_rows"]))
        observed_eligible = int(full_global.loc[spec.threshold_id, "eligible_cfun_file_rows"])
        mismatch_count += int(observed_selected != expected_selected or observed_eligible != expected_eligible)
    add_check(rows, "h01_selected_and_eligible_counts_reconcile_all_thresholds", mismatch_count, 0, mismatch_count == 0)
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    """Execute the H03 build."""
    started = utc_now()
    specs, h01_audit = load_thresholds(args.threshold_spec_file, args.threshold_audit_file)
    scope_repos, h01_summary, h02_summary = validate_upstream_contracts(
        args.h01_summary_file,
        args.h02_summary_file,
        args.h02_outside_scope_file,
    )
    h02 = load_h02_file_table(args.h02_file)
    ...
    panel, global_audit, timing_audit, sample_summary, outcome_spec, scope_spec = build_outputs(
        h02, b06, specs, h01_audit, scope_repos
    )
    checks = make_checks(
        panel,
        global_audit,
        ...
        args.expected_h02_rows,
        args.expected_finite_cfun_rows,
    ...
    summary_rows = [
        ...
        ("h02_outside_scope_file_rows", int(pd.to_numeric(read_metric_csv(args.h02_summary_file).get("b05_issue_bearing_snapshot_files_outside_a15", "0")))),
    ...
    metadata = {
        ...
        "status": "PASS" if hard_failures == 0 else "FAIL",
        "inputs": {
            "h02_file": str(args.h02_file.resolve()),
            "h02_summary_file": str(args.h02_summary_file.resolve()),
            "h02_outside_scope_file": str(args.h02_outside_scope_file.resolve()),
            "b06_panel_file": str(args.b06_panel_file.resolve()),
            "h01_summary_file": str(args.h01_summary_file.resolve()),
            "threshold_spec_file": str(args.threshold_spec_file.resolve()),
            "threshold_audit_file": str(args.threshold_audit_file.resolve()),
        },
        "input_sha256": {
            "h02_file": sha256_file(args.h02_file),
            "h02_summary_file": sha256_file(args.h02_summary_file),
            "h02_outside_scope_file": sha256_file(args.h02_outside_scope_file),
            "b06_panel_file": sha256_file(args.b06_panel_file),
            "h01_summary_file": sha256_file(args.h01_summary_file),
            "threshold_spec_file": sha256_file(args.threshold_spec_file),
            "threshold_audit_file": sha256_file(args.threshold_audit_file),
        },
        "frozen_contracts": {
            "h01_status": clean(h01_summary.get("status")),
            "h02_status": h02_summary.get("status"),
            "comparison_operator": ">",
            "primary_threshold": PRIMARY_THRESHOLD,
        ...
        "semantics": {
            "eligible_file": "finite file_npr_cfun_space_by_token_weighted",
            "selected_file": "eligible file with NPR strictly greater than the frozen H01 threshold",
            "nonfinite_file": "unclassified; never interpreted as below-threshold/human",
            "quality_burden": "unresolved SonarQube issue stock on canonical A15-backed Python file paths",
    ...
    print("=" * 80)
    print("run-x-h03 frozen C_FUN-NPR threshold x SonarQube burden panel")
    ...
    if hard_failures:
        raise RuntimeError(f"H03 hard QC failures: {hard_failures}; see {args.checks_output}")
    return metadata

def run_self_test() -> None:
    """Exercise strict-threshold, zero-selection, and C_FUN aggregation logic."""
    base = pd.DataFrame(
        {
            "repo_id": [1, 1, 2, 2],
            ...
            file_rows.append(
                {
                    ...
                    "python_lines": 10,
                    "cfun_space_by_tokens_scored": 10,
                    "cfun_npr_coverage_ratio": 1.0 if np.isfinite(npr) else np.nan,
                    NPR_METRIC: npr,
                    "file_npr_cfun_status": "scored" if np.isfinite(npr) else "no_cfun",
    ...
    assert int(selected.loc[(1, 1), "eligible_cfun_file_count"]) == 2
    ...
    print("build_cfun_npr_threshold_quality_burden_panel self-test: PASS")

def parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Build H03 frozen-threshold Quality x C_FUN-NPR burden panel.")
    parser.add_argument("--self-test", action="store_true")
    parser.add_argument("--h02-file", type=Path)
    parser.add_argument("--h02-summary-file", type=Path)
    parser.add_argument("--h02-outside-scope-file", type=Path)
    parser.add_argument("--b06-panel-file", type=Path)
    parser.add_argument("--h01-summary-file", type=Path)
    ...
    parser.add_argument("--expected-h02-rows", type=int, default=510297)
    parser.add_argument("--expected-finite-cfun-rows", type=int, default=202027)
    parser.add_argument("--expected-scope-repositories", type=int, default=2)
    args = parser.parse_args()
    if args.self_test:
        return args
    required = [
        "h02_file", "h02_summary_file", "h02_outside_scope_file", "b06_panel_file",
        "h01_summary_file", "threshold_spec_file", "threshold_audit_file",
