...
# ============================================================
# run-x-k04 v1: Dynamic panel GMM for NPR-localized RF+CM issue burden
# ============================================================
#
# Purpose:
#   Estimate whether unresolved Python static-analysis issue burden localized
#   to files selected by the frozen NPR primary threshold within the RF+CM localization scope predicts lower
#   subsequent whole-Python development velocity.
#
# Primary detector/localization contract:
#   - detector: AGCDetector_NPR; localization scope: CM
#   - file NPR metric: file_npr_fun_cfun_space_by_token_weighted
#   - frozen primary threshold: 1.571637
#   - comparison operator: strict >
#   - localized quality outcome: log1p_selected_issue_total
#   - velocity outcome: log_lines_added_py_source
#
# Primary GMM model:
#   Velocity_t ~ Velocity_{t-1} + NPRRFCMQuality_{t-1} + treatment_t + controls_t
...
#   2. exclude_scope_mismatch_repos (pre-specified NPR scope robustness)
#
# Important scope decision:
#   This run does not estimate Velocity_t -> NPR-localized Quality_t because a
#   detector-localized velocity-flow outcome has not yet been constructed.
#   K04 therefore addresses the pre-specified secondary interaction question:
#   NPR-localized RF+CM issue burden_t -> whole-Python velocity_{t+1}.
#
...
#
# Inputs:
#   - NPR-localized CM quality panel:
#       repo_x01/run-x-i04/quality_fun_cfun_npr_threshold_repo_month_panel.csv.gz
#   - Authoritative whole-Python velocity/covariate panel:
#       repo_x01/run-x-b06/panels/quality_did_panel_python_sonarqube.csv
#
# Join rule:
#   I04 intentionally contains localized quality outcomes but not the B02/B06
#   whole-Python velocity outcome. K04 therefore joins log_lines_added_py_source
#   from the frozen B06 panel by exact (repo_id, time_index), while auditing
#   all overlapping timing/covariate fields before estimation.
#
# This script is self-contained. It reuses validated GMM/QC/provenance logic
# from run-x-e01 but does not call any prior experiment script.
# ============================================================
...
expected_primary_full_selected_files <- as_integer_arg(args, "expected_primary_full_selected_files", 17071L)
expected_primary_full_issue_stock <- as_integer_arg(args, "expected_primary_full_issue_stock", 14809L)
expected_primary_sensitivity_selected_files <- as_integer_arg(args, "expected_primary_sensitivity_selected_files", 16885L)
expected_primary_sensitivity_issue_stock <- as_integer_arg(args, "expected_primary_sensitivity_issue_stock", 14748L)
...
paths <- list(
  coefficients = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_coefficients.csv"),
  diagnostics = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_diagnostics.csv"),
  sample_qc = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_sample_qc.csv"),
  instrument_qc = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_instrument_qc.csv"),
  model_specs = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_model_specifications.csv"),
  threshold_input_audit = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_threshold_input_audit.csv"),
  b06_join_audit = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_b06_join_audit.csv"),
  legacy_audit = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_legacy_flag_audit.csv"),
  coercion_audit = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_numeric_coercion_audit.csv"),
  calendar_gap_audit = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_calendar_gap_audit.csv"),
  run_metadata = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_run_metadata.csv"),
  qc = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_qc.csv"),
  models_rds = file.path(output_dir, "dynamic_panel_gmm_npr_rf_cm_models.rds")
)

run_started <- Sys.time()
log_message("INFO", "Reading I04 RF+CM-NPR threshold quality panel: %s", input_file)
...
required_columns <- c(
  ...
  "selected_issue_total", "selected_file_count", "eligible_fun_cfun_file_count",
...
strict_count_check(data.table::uniqueN(long_panel$threshold_id), expected_thresholds, "I04 threshold specifications", strict_expected_counts)
...
# I04 was built from B06 for quality/timing support, but the I04 long panel does
# not intentionally carry the whole-Python velocity outcome. Read B06 directly
# and use it as the authoritative velocity source for K04.
...
if (nrow(primary_panel) == 0L) abortf("No I04 primary-threshold rows found for strict NPR > %.6f.", primary_threshold)
...
if (!all(sample_specs %in% unique(primary_panel$sample_spec))) {
  abortf("Primary I04 panel is missing required sample specifications: %s", paste(setdiff(sample_specs, unique(primary_panel$sample_spec)), collapse = ", "))
}
primary_panel <- primary_panel[sample_spec %in% sample_specs]

# Join only after freezing the I04 primary threshold and the two pre-specified
...
if (anyNA(primary_panel$repo_id) || anyNA(primary_panel$time_index)) {
  abortf("I04 primary repo_id/time_index must be complete integer-compatible keys before the B06 join.")
}
...
if (nrow(primary_panel) != primary_rows_before_b06_join) {
  abortf("B06 join changed I04 primary row count: before=%d after=%d.", primary_rows_before_b06_join, nrow(primary_panel))
}

# Missing B06 velocity means the I04 repo-month key did not resolve to the
...
if (b06_join_missing_velocity > 0L) {
  abortf("B06 join left %d I04 primary rows without log_lines_added_py_source.", b06_join_missing_velocity)
}
...
join_audit_parts <- list(
  data.table::data.table(
    ...
    note = "I04 primary rows before the B06 velocity join."
  ),
  data.table::data.table(
    ...
    note = "B06 join must not change I04 primary sample membership."
  ),
  data.table::data.table(
    ...
    note = "Every I04 primary repo-month must resolve to authoritative B06 log_lines_added_py_source."
  )
)
for (field in join_compare_fields) {
  ...
  join_audit_parts[[length(join_audit_parts) + 1L]] <- data.table::data.table(
    ...
    note = "I04 field must agree with the authoritative B06 source on matched repo-month keys."
  )
}
...
# Validate I04 summary and sample-support artifacts before estimation.
i04_summary <- data.table::fread(i04_summary_file, na.strings = c("", "NA", "NaN"))
validate_columns(i04_summary, c("metric", "value"))
...
if (abs(as.numeric(summary_lookup[["primary_threshold"]]) - primary_threshold) > 1e-12) abortf("I04 summary primary threshold mismatch.")

sample_summary_input <- data.table::fread(i04_sample_summary_file, na.strings = c("", "NA", "NaN"))
...
if (nrow(sample_summary_input[sample_spec == "full_sample"]) != 1L ||
  ...
  abortf("I04 sample summary must contain exactly one row for each K04 sample specification.")
}
strict_count_check(sample_summary_input[sample_spec == "full_sample", repo_month_rows], expected_full_rows, "I04 sample-summary full rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", repositories], expected_full_repositories, "I04 sample-summary full repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", treatment_repositories], expected_full_treatment_repos, "I04 sample-summary full treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", control_repositories], expected_full_control_repos, "I04 sample-summary full control repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repo_month_rows], expected_sensitivity_rows, "I04 sample-summary sensitivity rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repositories], expected_sensitivity_repositories, "I04 sample-summary sensitivity repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", treatment_repositories], expected_sensitivity_treatment_repos, "I04 sample-summary sensitivity treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", control_repositories], expected_sensitivity_control_repos, "I04 sample-summary sensitivity control repositories", strict_expected_counts)
...
if (nrow(primary_global) != 2L) abortf("Expected two primary rows in I04 global audit; observed %d.", nrow(primary_global))
...
model_specs <- data.table::data.table(
  model = "npr_rf_cm_quality_to_velocity",
  direction = "NPRRFCMQuality_{t-1} -> Velocity_t",
  conceptual_direction = "NPR-localized Quality_t -> Velocity_{t+1}",
...
for (sample_spec_value in sample_specs) {
  ...
  model_name <- "npr_rf_cm_quality_to_velocity"
  ...
  qc_parts[[sample_spec]] <- data.table::data.table(
    ...
    note = c(
      "Primary I04 sample invariant.", "Primary I04 sample invariant.", "Primary I04 sample invariant.", "Primary I04 sample invariant.",
      "Must be zero.", "Normalized timing must be exact.", "Numeric parsing must not introduce missingness.",
      "GMM fields must be complete.", "GMM fields must be finite.", "I04 localized quality log outcome must equal log1p(selected_issue_total).",
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("run-x-k04 NPR GMM QC failed: %s", paste(paste(failed_qc$sample_spec, failed_qc$check, sep = "/"), collapse = ", "))
}
...
metadata <- data.table::rbindlist(list(
  make_summary_row("run", "run_prefix", "run-x-k04"),
  ...
  make_summary_row("run", "i04_summary_file", i04_summary_file),
  make_summary_row("run", "i04_summary_sha256", sha256_file(i04_summary_file)),
  make_summary_row("run", "i04_sample_summary_file", i04_sample_summary_file),
  make_summary_row("run", "i04_sample_summary_sha256", sha256_file(i04_sample_summary_file)),
  make_summary_row("run", "i04_global_audit_file", i04_global_audit_file),
  make_summary_row("run", "i04_global_audit_sha256", sha256_file(i04_global_audit_file)),
  ...
  make_summary_row("definition", "analysis_scope", "agc_npr_rf_cm_localized_quality_to_velocity"),
  make_summary_row("definition", "localization_scope", "RF+CM"),
  make_summary_row("definition", "detector_scope_pair", "NPR|RF+CM"),
  make_summary_row("definition", "detector", "AGCDetector_NPR"),
  make_summary_row("definition", "npr_metric", "file_npr_fun_cfun_space_by_token_weighted"),
  ...
  make_summary_row("definition", "scope_note", "Secondary detector-localized GMM: lagged NPR-selected issue burden predicts subsequent whole-Python velocity; detector-localized velocity flow is not yet constructed."),
  make_summary_row("definition", "quality_semantics", "unresolved SonarQube issue stock among Python files with NPR-RF+CM > 1.571637 at the corresponding historical snapshot"),
...
log_message(
  "INFO",
  "Completed run-x-k04 %s: samples=%d; coefficients=%d; diagnostics=%d; failed_qc=%d; cautions=%d",
