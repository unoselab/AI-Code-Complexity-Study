...
# ============================================================
# run-x-g01 v3: Dynamic panel GMM for neither-detector quality burden
# ============================================================
#
# Purpose:
#   Estimate whether unresolved Python static-analysis issue burden localized
#   to files selected by neither the NPR detector nor the ML detector predicts
#   lower subsequent whole-Python development velocity.
#
# Detector-complement contract:
#   NPR file rule: file_npr_fun_space_by_token_weighted > 1.571637
#   ML file rule:  file_ml_agc_share_space_by_token_weighted > 0.50
#   Union rule:    NPR selected OR ML selected
#   Neither rule:  NOT(NPR selected OR ML selected)
#
# GMM model:
#   Velocity_t ~ Velocity_{t-1} + NeitherQuality_{t-1} + treatment_t + controls_t
#   GMM instrument: Velocity_{t-2}
#   effect=twoways, model=twosteps, transformation=d, collapse=FALSE
#
# Inputs:
#   - G01 neither-selected repo-month panel produced by build_agc_detector_neither_gmm_panel.py
...
paths <- list(
  coefficients = file.path(output_dir, "dynamic_panel_gmm_agc_neither_coefficients.csv"),
  primary_summary = file.path(output_dir, "dynamic_panel_gmm_agc_neither_primary_summary.csv"),
  diagnostics = file.path(output_dir, "dynamic_panel_gmm_agc_neither_diagnostics.csv"),
  sample_qc = file.path(output_dir, "dynamic_panel_gmm_agc_neither_sample_qc.csv"),
  instrument_qc = file.path(output_dir, "dynamic_panel_gmm_agc_neither_instrument_qc.csv"),
  b06_join_audit = file.path(output_dir, "dynamic_panel_gmm_agc_neither_b06_join_audit.csv"),
  numeric_audit = file.path(output_dir, "dynamic_panel_gmm_agc_neither_numeric_coercion_audit.csv"),
  calendar_gap_audit = file.path(output_dir, "dynamic_panel_gmm_agc_neither_calendar_gap_audit.csv"),
  legacy_audit = file.path(output_dir, "dynamic_panel_gmm_agc_neither_legacy_flag_audit.csv"),
  qc = file.path(output_dir, "dynamic_panel_gmm_agc_neither_qc.csv"),
  metadata = file.path(output_dir, "dynamic_panel_gmm_agc_neither_run_metadata.csv"),
  models_rds = file.path(output_dir, "dynamic_panel_gmm_agc_neither_models.rds")
)

run_started <- Sys.time()
log_message("INFO", "Reading G01 detector-neither panel: %s", input_file)
neither_panel <- data.table::fread(input_file, na.strings = c("", "NA", "NaN"))
required_neither <- c(
  "repo_id", "time_index", "selected_file_rows", "selected_issue_total", "log1p_selected_issue_total",
  "npr_selected_file_rows", "ml_selected_file_rows", "intersection_selected_file_rows",
  "neither_selected_file_rows", "union_selected_file_rows", "npr_threshold", "ml_threshold", "detector_union_rule", "detector_neither_rule"
)
validate_columns(neither_panel, required_neither, "G01 neither panel")
strict_count_check(nrow(neither_panel), expected_rows, "G01 neither panel rows", strict_expected_counts)
if (data.table::uniqueN(neither_panel[, .(repo_id, time_index)]) != nrow(neither_panel)) abortf("G01 neither panel has duplicate repo-month keys")
if (any(as.character(neither_panel$detector_union_rule) != "NPR_primary_OR_ML_primary")) abortf("Unexpected union audit rule in G01 input")
if (any(as.character(neither_panel$detector_neither_rule) != "NOT(NPR_primary_OR_ML_primary)")) abortf("Unexpected neither rule in G01 input")
if (any(abs(as.numeric(neither_panel$npr_threshold) - 1.571637) > 1e-12)) abortf("Unexpected NPR threshold in G01 input")
if (any(abs(as.numeric(neither_panel$ml_threshold) - 0.50) > 1e-12)) abortf("Unexpected ML threshold in G01 input")
...
neither_panel[, `:=`(repo_id = as.integer(repo_id), time_index = as.integer(time_index))]
b06[, `:=`(repo_id = as.integer(repo_id), time_index = as.integer(time_index))]
if (anyNA(neither_panel$repo_id) || anyNA(neither_panel$time_index) || anyNA(b06$repo_id) || anyNA(b06$time_index)) abortf("repo_id/time_index must be integer-compatible and complete")
...
rows_before_join <- nrow(neither_panel)
panel <- merge(neither_panel, b06_lookup, by = c("repo_id", "time_index"), all.x = TRUE, sort = FALSE)
...
if (log_mismatches > 0L) abortf("Neither log1p outcome mismatch: %d rows", log_mismatches)
...
model_name <- "agc_neither_quality_to_velocity"
primary_term <- "lag(log1p_selected_issue_total, 1)"
direction <- "NeitherQuality_{t-1} -> Velocity_t"
...
log_message("INFO", "Fitting neither-detector localized quality -> velocity GMM")
...
if (nrow(primary_summary) != 1L) abortf("Expected exactly one neither-quality primary coefficient")
...
if (nrow(failed_qc) > 0L && strict_expected_counts) abortf("run-x-g01 GMM QC failed: %s", paste(failed_qc$check, collapse = ", "))
...
metadata <- data.table::data.table(
  ...
  metric = c(
    ...
    "analysis_scope", "detector_combination", "neither_rule", "npr_rule", "ml_rule",
    "localized_quality", "velocity", "treatment", "size_control", "other_controls",
    "gmm_specification", "formula", "failed_checks", "caution_checks"
  ),
  value = c(
    "run-x-g01", implementation_version, format(run_started, "%Y-%m-%d %H:%M:%S %Z"), format(run_finished, "%Y-%m-%d %H:%M:%S %Z"),
    input_file, sha256_file(input_file), b06_panel_file, sha256_file(b06_panel_file), R.version.string, safe_package_version("plm"),
    "agc_detector_neither_localized_quality_to_velocity", "neither AGCDetector_NPR nor AGCDetector_ML",
    "NOT(NPR selected OR ML selected)",
...
log_message(
  "INFO",
  "Completed run-x-g01 %s: beta=%.9f; se=%.9f; p=%.9f; failed_qc=%d; cautions=%d",
