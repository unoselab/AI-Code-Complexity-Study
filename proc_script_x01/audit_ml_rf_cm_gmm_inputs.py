#!/usr/bin/env python3
"""
Audit readiness of the ML-RF+CM quality-to-velocity dynamic-panel GMM inputs.

This K07 audit is intentionally model-free. It validates that the existing I08
RF+CM-localized ML quality panel can be joined to the authoritative B06 Python
...
- Detector: ML
- Localization scope: RF+CM (jointly recomputed regular-function + class-method scope)
- Primary detector cutoff: tau_ML = 0.50
- Selection operator inherited from I08: strict >
...
SCRIPT_VERSION = "run-x-k07-v1"
EXPECTED_SCOPE = "RF+CM"
...
def build_exact_calendar_sample(primary: pd.DataFrame, b06: pd.DataFrame) -> pd.DataFrame:
    ...
    base = primary.drop(columns=["log_lines_added_py_source"], errors="ignore").merge(
...
def run_audit(args: argparse.Namespace) -> int:
    i08_path = Path(args.i08_panel_file)
    b06_path = Path(args.b06_panel_file)
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    i08 = pd.read_csv(i08_path, compression="infer", low_memory=False)
    b06 = pd.read_csv(b06_path, low_memory=False)

    i08_required = [
        "sample_spec", "threshold_spec_id", "threshold_role", "threshold",
        "ml_operator", "ml_metric", "repo_id", "repo_name",
        "dataset_source", "treatment_group", "time_index", "event_index",
        "selected_file_count", "selected_issue_total", "log1p_selected_issue_total",
    ]
    b06_required = [
    ...
    require_columns(i08, i08_required, "I08 panel")
    require_columns(b06, b06_required, "B06 panel")

    numeric(i08, ["threshold", "repo_id", "treatment_group", "time_index", "event_index",
                  "selected_issue_total", "log1p_selected_issue_total"], "I08 panel")
    ...
    primary = i08[
        i08["sample_spec"].map(clean_text).eq("full_sample")
        & i08["threshold_role"].map(clean_text).eq("primary")
        & np.isclose(i08["threshold"].astype(float), args.primary_threshold, atol=1e-12, rtol=0.0)
    ].copy()
    ...
    add_check(checks, "i08_long_rows", len(i08), args.expected_i08_long_rows,
              len(i08) == args.expected_i08_long_rows, "Expected I08 threshold x sample long-panel rows.")
    ...
    add_check(checks, "primary_repositories", int(primary["repo_id"].nunique()), args.expected_repositories,
              int(primary["repo_id"].nunique()) == args.expected_repositories, "Primary I08 repository count.")
    add_check(checks, "primary_treatment_repositories",
              ...
              "Primary I08 treated repositories.")
    add_check(checks, "primary_control_repositories",
              ...
              "Primary I08 control repositories.")
    ...
    operators = sorted({clean_text(x) for x in primary["ml_operator"].dropna().unique()})
    metrics = sorted({clean_text(x) for x in primary["ml_metric"].dropna().unique()})
    add_check(checks, "primary_comparison_operator", "|".join(operators), EXPECTED_COMPARISON,
              operators == [EXPECTED_COMPARISON], "Primary ML selection must remain strict >.")
    add_check(checks, "primary_ml_metric", "|".join(metrics), EXPECTED_ML_METRIC,
              metrics == [EXPECTED_ML_METRIC], "K07 must audit jointly recomputed RF+CM ML, not RF or CM.")

    add_check(checks, "primary_selected_file_rows", int(primary["selected_file_count"].sum()),
              args.expected_primary_selected_files,
              int(primary["selected_file_count"].sum()) == args.expected_primary_selected_files,
              "Expected primary ML-RF+CM selected file-occurrence count.")
    add_check(checks, "primary_selected_issue_stock", int(round(primary["selected_issue_total"].sum())),
              args.expected_primary_issue_stock,
              int(round(primary["selected_issue_total"].sum())) == args.expected_primary_issue_stock,
              "Expected primary ML-RF+CM unresolved issue stock.")
    ...
    merged = primary[identity_cols].merge(
        ...
        suffixes=("_i08", "_b06"), indicator=True, validate="one_to_one"
    )
    unmatched = int(merged["_merge"].ne("both").sum())
    add_check(checks, "i08_b06_unmatched_repo_months", unmatched, 0, unmatched == 0,
              "Primary I08 and B06 must have the same full-sample repo-month universe.")

    mismatch_count = 0
    if unmatched == 0:
        for col in ["repo_name", "dataset_source", "treatment_group", "event_index"]:
            left = merged[f"{col}_i08"].map(clean_text)
            right = merged[f"{col}_b06"].map(clean_text)
            mismatch_count += int(left.ne(right).sum())
    add_check(checks, "i08_b06_identity_field_mismatches", mismatch_count, 0, mismatch_count == 0,
    ...
    audit_summary = pd.DataFrame([
        {
            "analysis": "ML-RF+CM quality_{t-1} -> velocity_t",
            "detector": "ML",
            ...
            "i08_primary_rows": len(primary),
            "i08_primary_repositories": int(primary["repo_id"].nunique()),
    ...
    checks_path = output_dir / "k07_ml_rf_cm_gmm_input_checks.csv"
    summary_path = output_dir / "k07_ml_rf_cm_gmm_input_summary.csv"
    active_path = output_dir / "k07_ml_rf_cm_exact_calendar_sample.csv.gz"
    ...
    metadata = {
        ...
        "i08_panel_file": str(i08_path),
        "i08_panel_sha256": sha256_file(i08_path),
    ...
    print(f"K07 audit status:                         {status}")
    print(f"Hard QC failures:                        {hard_failures}")
    print(f"Primary I08 rows/repositories:           {len(primary)}/{int(primary['repo_id'].nunique())}")
...
def self_test() -> int:
    ...
    assert EXPECTED_SCOPE == "RF+CM", EXPECTED_SCOPE
    print("audit_ml_rf_cm_gmm_inputs self-test: PASS")
    return 0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expected-primary-selected-files", type=int, default=64153)
    parser.add_argument("--expected-primary-issue-stock", type=int, default=35765)
...
def main() -> int:
    ...
    if args.self_test:
        return self_test()
    missing = [name for name in ["i08_panel_file", "b06_panel_file", "output_dir"] if not getattr(args, name)]
    if missing:
