...
# ============================================================
# run-x-k11 v2: Dynamic-panel GMM for NPR-localized quality burden
# ============================================================
#
# Purpose:
#   Estimate the association between lagged NPR-localized issue burden and
#   subsequent whole-Python development velocity for one localization scope.
#   The shell wrapper invokes this self-contained analysis for RF, CM, and
#   RF+CM using the prespecified NPR threshold of 1.515059.
#
# Model:
...
#
# Inputs:
#   - Scope-specific threshold-long quality panel from run-x-l03 (RF and CM)
#     or run-x-l01-v2 (RF+CM)
#   - Matching source summary, sample-summary, and global-audit artifacts
#   - Authoritative whole-Python velocity and covariate panel from run-x-b06
#
# The implementation preserves the validated K02/K04 estimator, diagnostics,
# provenance checks, and sample checks without invoking earlier experiment
# scripts.
# ============================================================
...
analysis_scope_id <- tolower(require_arg(args, "scope_id"))
scope_label <- require_arg(args, "scope_label")
npr_metric <- require_arg(args, "npr_metric")
...
expected_long_rows <- as_integer_arg(args, "expected_long_rows", 88987L)
...
expected_primary_full_selected_files <- as_integer_arg(args, "expected_primary_full_selected_files")
expected_primary_full_issue_stock <- as_integer_arg(args, "expected_primary_full_issue_stock")
expected_primary_sensitivity_selected_files <- as_integer_arg(args, "expected_primary_sensitivity_selected_files")
expected_primary_sensitivity_issue_stock <- as_integer_arg(args, "expected_primary_sensitivity_issue_stock")
...
if (!analysis_scope_id %in% c("rf", "cm", "rf_cm")) {
  abortf("--scope-id must be one of rf, cm, or rf_cm: %s", analysis_scope_id)
}
...
file_stem <- sprintf("dynamic_panel_gmm_npr_%s", analysis_scope_id)
paths <- list(
  coefficients = file.path(output_dir, paste0(file_stem, "_coefficients.csv")),
  diagnostics = file.path(output_dir, paste0(file_stem, "_diagnostics.csv")),
  sample_qc = file.path(output_dir, paste0(file_stem, "_sample_qc.csv")),
  instrument_qc = file.path(output_dir, paste0(file_stem, "_instrument_qc.csv")),
  model_specs = file.path(output_dir, paste0(file_stem, "_model_specifications.csv")),
  threshold_input_audit = file.path(output_dir, paste0(file_stem, "_threshold_input_audit.csv")),
  b06_join_audit = file.path(output_dir, paste0(file_stem, "_b06_join_audit.csv")),
  legacy_audit = file.path(output_dir, paste0(file_stem, "_legacy_flag_audit.csv")),
  coercion_audit = file.path(output_dir, paste0(file_stem, "_numeric_coercion_audit.csv")),
  calendar_gap_audit = file.path(output_dir, paste0(file_stem, "_calendar_gap_audit.csv")),
  run_metadata = file.path(output_dir, paste0(file_stem, "_run_metadata.csv")),
  qc = file.path(output_dir, paste0(file_stem, "_qc.csv")),
  models_rds = file.path(output_dir, paste0(file_stem, "_models.rds"))
)

run_started <- Sys.time()
log_message("INFO", "Reading %s NPR threshold quality panel for scope=%s: %s", source_label, scope_label, input_file)
long_panel <- data.table::fread(input_file, na.strings = c("", "NA", "NaN"))
if ("scope_id" %in% names(long_panel)) {
  long_panel <- long_panel[tolower(as.character(long_panel$scope_id)) == analysis_scope_id]
}
if ("npr_metric" %in% names(long_panel) &&
    any(as.character(long_panel$npr_metric) != npr_metric, na.rm = TRUE)) {
  abortf("NPR metric mismatch for localization scope %s.", scope_label)
}
if (!"eligible_file_count" %in% names(long_panel) &&
    "eligible_fun_cfun_file_count" %in% names(long_panel)) {
  data.table::setnames(long_panel, "eligible_fun_cfun_file_count", "eligible_file_count")
}
if (nrow(long_panel) == 0L) abortf("No rows remain for localization scope %s.", scope_label)
...
required_columns <- c(
  ...
  "selected_issue_total", "selected_file_count", "eligible_file_count",
  "log_age", "ncloc_py_sonarqube", "log_contributors", "log_stars", "log_issues"
)
validate_columns(long_panel, required_columns)

strict_count_check(nrow(long_panel), expected_long_rows, "source input long-panel rows", strict_expected_counts)
strict_count_check(data.table::uniqueN(long_panel$threshold_id), expected_thresholds, "source input threshold specifications", strict_expected_counts)
strict_count_check(data.table::uniqueN(long_panel$sample_spec), expected_sample_specs, "source input sample specifications", strict_expected_counts)

# The quality panels were constructed from B06 timing support but do not
# contain the whole-Python velocity outcome. We therefore use B06 as the
# authoritative velocity source for K11.
...
if (nrow(primary_panel) == 0L) abortf("No source input primary-threshold rows found for strict NPR > %.6f.", primary_threshold)
...
if (!all(sample_specs %in% unique(primary_panel$sample_spec))) {
  abortf("Primary source input panel is missing required sample specifications: %s", paste(setdiff(sample_specs, unique(primary_panel$sample_spec)), collapse = ", "))
}
primary_panel <- primary_panel[sample_spec %in% sample_specs]

# Join only after selecting the primary threshold and the two prespecified
...
if (anyNA(primary_panel$repo_id) || anyNA(primary_panel$time_index)) {
  abortf("source input primary repo_id/time_index must be complete integer-compatible keys before the B06 join.")
}
...
if (nrow(primary_panel) != primary_rows_before_b06_join) {
  abortf("B06 join changed source input primary row count: before=%d after=%d.", primary_rows_before_b06_join, nrow(primary_panel))
}

# Missing B06 velocity means the source input repo-month key did not resolve to the
...
if (b06_join_missing_velocity > 0L) {
  abortf("B06 join left %d source input primary rows without log_lines_added_py_source.", b06_join_missing_velocity)
}
...
join_audit_parts <- list(
  data.table::data.table(
    ...
    note = "source input primary rows before the B06 velocity join."
  ),
  data.table::data.table(
    ...
    note = "B06 join must not change source input primary sample membership."
  ),
  data.table::data.table(
    ...
    note = "Every source input primary repo-month must resolve to authoritative B06 log_lines_added_py_source."
  )
)
for (field in join_compare_fields) {
  ...
  join_audit_parts[[length(join_audit_parts) + 1L]] <- data.table::data.table(
    ...
    note = "source input field must agree with the authoritative B06 source on matched repo-month keys."
  )
}
...
# Validate source input summary and sample-support artifacts before estimation.
source_summary <- data.table::fread(source_summary_file, na.strings = c("", "NA", "NaN"))
validate_columns(source_summary, c("metric", "value"))
summary_lookup <- setNames(as.character(source_summary$value), as.character(source_summary$metric))
if (!identical(summary_lookup[["status"]], "PASS")) abortf("source input summary status is not PASS.")
if (abs(as.numeric(summary_lookup[["primary_threshold"]]) - primary_threshold) > 1e-12) abortf("source input summary primary threshold mismatch.")

sample_summary_input <- data.table::fread(source_sample_summary_file, na.strings = c("", "NA", "NaN"))
if ("scope_id" %in% names(sample_summary_input)) {
  sample_summary_input <- sample_summary_input[
    tolower(as.character(sample_summary_input$scope_id)) == analysis_scope_id
  ]
}
...
if (nrow(sample_summary_input[sample_spec == "full_sample"]) != 1L ||
  ...
  abortf("source input sample summary must contain exactly one row for each K11 sample specification.")
}
strict_count_check(sample_summary_input[sample_spec == "full_sample", repo_month_rows], expected_full_rows, "source input sample-summary full rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", repositories], expected_full_repositories, "source input sample-summary full repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", treatment_repositories], expected_full_treatment_repos, "source input sample-summary full treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "full_sample", control_repositories], expected_full_control_repos, "source input sample-summary full control repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repo_month_rows], expected_sensitivity_rows, "source input sample-summary sensitivity rows", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", repositories], expected_sensitivity_repositories, "source input sample-summary sensitivity repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", treatment_repositories], expected_sensitivity_treatment_repos, "source input sample-summary sensitivity treatment repositories", strict_expected_counts)
strict_count_check(sample_summary_input[sample_spec == "exclude_scope_mismatch_repos", control_repositories], expected_sensitivity_control_repos, "source input sample-summary sensitivity control repositories", strict_expected_counts)

global_audit <- data.table::fread(source_global_audit_file, na.strings = c("", "NA", "NaN"))
if ("scope_id" %in% names(global_audit)) {
  global_audit <- global_audit[
    tolower(as.character(global_audit$scope_id)) == analysis_scope_id
  ]
}
# Repository-month support is validated from the source sample summary.
# The global audit is used for threshold identity and selected-burden totals.
validate_columns(global_audit, c(
  "sample_spec", "threshold_id", "threshold", "comparison_operator",
  "selected_file_rows", "selected_issue_total"
))
primary_global <- global_audit[
  sample_spec %in% sample_specs & threshold_id == "primary" &
  abs(as.numeric(threshold) - primary_threshold) <= 1e-12 & comparison_operator == ">"
]
if (nrow(primary_global) != 2L) abortf("Expected two primary rows in source input global audit; observed %d.", nrow(primary_global))
...
model_name <- sprintf("npr_%s_quality_to_velocity", analysis_scope_id)
direction_label <- sprintf("NPR-%s Quality_{t-1} -> Velocity_t", scope_label)
model_specs <- data.table::data.table(
  scope_id = analysis_scope_id,
  localization_scope = scope_label,
  model = model_name,
  direction = direction_label,
  conceptual_direction = sprintf("NPR-%s Quality_t -> Velocity_{t+1}", scope_label),
...
for (sample_spec_value in sample_specs) {
  ...
  log_message("INFO", "Fitting NPR-localized quality -> velocity GMM for sample=%s", sample_spec)
  ...
  coefficients_parts[[sample_spec]][, `:=`(
    scope_id = analysis_scope_id,
    localization_scope = scope_label
  )]
  ...
  qc_parts[[sample_spec]] <- data.table::data.table(
    ...
    note = c(
      "Primary source input sample invariant.", "Primary source input sample invariant.", "Primary source input sample invariant.", "Primary source input sample invariant.",
      "Must be zero.", "Normalized timing must be exact.", "Numeric parsing must not introduce missingness.",
      "GMM fields must be complete.", "GMM fields must be finite.", "source input localized quality log outcome must equal log1p(selected_issue_total).",
      "Legacy mismatch is audit-only.", "Legacy mismatch is audit-only.", "Exactly one lagged NPR-quality coefficient is required.",
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("run-x-k11 NPR GMM QC failed: %s", paste(paste(failed_qc$sample_spec, failed_qc$check, sep = "/"), collapse = ", "))
}
...
metadata <- data.table::rbindlist(list(
  make_summary_row("run", "run_prefix", "run-x-k11"),
  make_summary_row("run", "implementation_version", implementation_version),
  make_summary_row("run", "source_label", source_label),
  make_summary_row("run", "scope_id", analysis_scope_id),
  make_summary_row("run", "scope_label", scope_label),
  ...
  make_summary_row("run", "source_summary_file", source_summary_file),
  make_summary_row("run", "source_summary_sha256", sha256_file(source_summary_file)),
  make_summary_row("run", "source_sample_summary_file", source_sample_summary_file),
  make_summary_row("run", "source_sample_summary_sha256", sha256_file(source_sample_summary_file)),
  make_summary_row("run", "source_global_audit_file", source_global_audit_file),
  make_summary_row("run", "source_global_audit_sha256", sha256_file(source_global_audit_file)),
  ...
  make_summary_row("definition", "analysis_scope", sprintf("agc_npr_%s_localized_quality_to_velocity", analysis_scope_id)),
  make_summary_row("definition", "localization_scope", scope_label),
  make_summary_row("definition", "detector_scope_pair", paste("NPR", scope_label, sep = "|")),
  make_summary_row("definition", "detector", "AGCDetector_NPR"),
  make_summary_row("definition", "npr_metric", npr_metric),
  make_summary_row("definition", "threshold_id", "primary"),
  make_summary_row("definition", "primary_threshold", primary_threshold),
  make_summary_row("definition", "comparison_operator", ">"),
  ...
  make_summary_row("definition", "scope_note", sprintf("Lagged NPR-%s issue burden is associated with subsequent whole-Python velocity; this model does not estimate localized velocity.", scope_label)),
  make_summary_row("definition", "quality_semantics", sprintf("unresolved SonarQube issue stock among Python files with NPR-%s > %.6f at the corresponding historical snapshot", scope_label, primary_threshold)),
...
log_message(
  "INFO",
  "Completed run-x-k11 %s for scope=%s: samples=%d; coefficients=%d; diagnostics=%d; failed_qc=%d; cautions=%d",
  implementation_version, scope_label, length(sample_specs), nrow(coefficients), nrow(diagnostics),
