...
# ============================================================
# run-x-i05 v1: Borusyak DiD across frozen FUN+C_FUN-NPR thresholds
# ============================================================
#
# Purpose:
#   Estimate Cursor-adoption effects on unresolved SonarQube issue burden
#   concentrated in Python files whose contemporaneous FUN+C_FUN-NPR exceeds each
#   threshold frozen by run-x-i02 and aggregated by run-x-i04.
#
...
#   - 22 FUN+C_FUN-NPR threshold specifications (21 grid + 1 legacy anchor).
...
# adapted only for the I04 FUN+C_FUN-NPR measurement contract:
...
run_self_test <- function() {
  ...
  cat("did_borusyak_fun_cfun_npr_threshold_quality self-test: PASS\n")
}
...
log_message("INFO", "Reading I04 long panel: %s", input_file)
...
i04_global_audit <- data.table::fread(i04_global_audit_file, na.strings = c("", "NA", "NaN"))

validate_columns(i04_summary, c("metric", "value"))
validate_columns(i04_sample_summary, c(
...
validate_columns(i04_global_audit, c(
  "sample_spec", "threshold_id", "threshold_role", "threshold", "selected_file_rows",
  "eligible_fun_cfun_file_rows", "selected_issue_total"
))

if (lookup_metric(i04_summary, "status") != "PASS") abortf("I04 summary status is not PASS.")
strict_count_check(as.integer(lookup_metric(i04_summary, "thresholds")), expected_thresholds, "I04 thresholds", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(i04_summary, "sample_specs")), expected_sample_specs, "I04 sample specs", strict_expected_counts)
strict_count_check(as.integer(lookup_metric(i04_summary, "long_panel_rows")), expected_long_rows, "I04 long-panel rows", strict_expected_counts)
if (as.integer(lookup_metric(i04_summary, "density_computed")) != 0L) abortf("I04 density_computed must be zero for I05 burden analysis.")
if (abs(as.numeric(lookup_metric(i04_summary, "primary_threshold")) - primary_threshold) > 1e-12) {
  abortf("I04 primary threshold does not match I05 primary threshold.")
}
...
strict_count_check(input_rows, expected_long_rows, "I05 input rows", strict_expected_counts)

contract_mismatch_rows <- panel[
  comparison_operator != ">" |
    npr_metric != "file_npr_fun_cfun_space_by_token_weighted" |
    quality_scope != "canonical_i01_python_files_with_finite_fun_cfun_npr" |
    quality_count_semantics != "unresolved_sonarqube_issue_stock_at_historical_snapshot" |
    density_computed != 0
]
if (nrow(contract_mismatch_rows) > 0L) abortf("Found %d I04 measurement-contract mismatch rows.", nrow(contract_mismatch_rows))
...
if (nrow(timing_mismatch_rows) > 0L) abortf("Found %d I04 timing-reconstruction mismatches.", nrow(timing_mismatch_rows))
...
if (nrow(missing_model_rows) > 0L) abortf("Found %d rows with missing I05 model fields.", nrow(missing_model_rows))
nonfinite_model_rows <- panel[
  !apply(as.data.frame(panel[, ..all_model_fields]), 1L, function(row) all(is.finite(row)))
]
if (nrow(nonfinite_model_rows) > 0L) abortf("Found %d rows with non-finite I05 model fields.", nrow(nonfinite_model_rows))

# Validate threshold-level aggregate counts against run-x-i04 global audit.
input_global <- panel[, .(
  repo_month_rows = .N,
...
if (any(audit_compare$mismatch)) abortf("I05 input does not reconcile to I04 global audit for %d sample-threshold rows.", sum(audit_compare$mismatch))
...
# Validate threshold-invariant sample support against I04 sample summary.
...
for (sample_name in sample_specs) {
  ...
  expected <- i04_sample_summary[sample_spec == sample_name]
  if (nrow(expected) != 1L) abortf("Missing I04 sample-summary row for %s", sample_name)
...
if (any(sample_validation$mismatch_count > 0L)) abortf("I04 sample-support reconciliation failed.")

support_policy <- data.table::data.table(
  policy_id = "prespecified_sparse_support_flag_v1",
  apply_to_model_omission = 0L,
  prespecified_before_i05_results = 1L,
...
qc <- data.table::data.table(
  check = c(
    ...
    "i04_global_audit_mismatch_rows",
    "i04_sample_support_mismatch_samples",
    "model_job_count",
    "static_effect_rows",
    "dynamic_effect_rows",
    "pretrend_check_rows",
...
qc[, note := c(
  ...
  "Selected file/issue counts must reconcile to I04 global audit.",
  "Threshold-invariant sample support must reconcile to I04 sample summary.",
...
summary <- data.table::rbindlist(list(
  make_summary_row("run", "script_version", "run-x-i05-v1"),
...
if (nrow(failed_qc) > 0L && strict_expected_counts) {
  abortf("I05 hard QC failed: %s", paste(failed_qc$check, collapse = ", "))
}

log_message(
  "INFO",
  "Completed run-x-i05 %s: status=%s; jobs=%d; static=%d; dynamic=%d; nonprimary failures=%d; hard QC failures=%d",
