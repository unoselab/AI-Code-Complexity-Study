#!/usr/bin/env python3
"""
run-x-h05-v1: build the ML C_FUN-selected Python quality-burden repo-month panel.

Scientific purpose
------------------
H05 connects the frozen run-x-a07 ML C_FUN file classification to the already-canonical
D02 Python SonarQube file-burden table, then aggregates C_FUN-selected-file issue stock
to the B06 1,954-row repository-month quality panel.

The program deliberately does NOT:
- rerun the ML detector,
- change the frozen A07 C_FUN primary file threshold,
...
Only A07 rows with file_ml_cfun_agc_status == "scored" are ML-classifiable.
Rows with no_ml_cfun or file_not_prepared remain unclassified and are never
...
the canonical historical repo-month/file universe. Its legacy filename contains FUN,
but the table itself is the detector-neutral historical Python file-quality source. H05 reuses that canonical join instead of touching raw
SonarQube data again.

Exact A07 x D02 join
--------------------
...
unique A07 snapshot/file row. H05 requires the UNIQUE D02 snapshot/file universe
to match A07 exactly.
...
ML C_FUN mapping specifications:
1. all_ml_files                 (primary)
2. exclude_mapping_warning_files (robustness)

The mapping-warning robustness excludes warning-bearing A07 C_FUN file rows from both
the ML-eligible denominator and the selected-file numerator.

Primary downstream outcome for H06
----------------------------------
    log1p_selected_issue_total

This is the log1p unresolved SonarQube issue stock in contemporaneously selected
ML C_FUN-AGC-like Python files at the historical repository-month snapshot. It is a
...
SCRIPT_VERSION = "run-x-h05-v1"
A07_EXPECTED_RUN = "run-x-a07-v1"
A07_EXPECTED_STATUS = "PASS_WITH_WARNINGS"
D02_EXPECTED_RUN = "run-x-d02-v2"
D02_EXPECTED_STATUS = "PASS_WITH_SCOPE_EXCLUSIONS"
PRIMARY_ML_METRIC = "file_ml_cfun_agc_share_space_by_token_weighted"
PRIMARY_ML_THRESHOLD = 0.50
PRIMARY_ML_OPERATOR = ">"
QUALITY_SEMANTICS = "unresolved_sonarqube_issue_stock_at_historical_snapshot"

EXPECTED_A07_FILE_ROWS = 494_592
EXPECTED_A07_FILES_WITH_CFUN = 196_190
EXPECTED_A07_NO_CFUN = 298_142
EXPECTED_A07_NOT_PREPARED = 260
EXPECTED_A07_PRIMARY_SELECTED = 36_556
EXPECTED_A07_MAPPING_WARNING_FILES = 3_152
...
def validate_a07_contract(summary_path: Path, checks_path: Path) -> dict[str, Any]:
    """Validate the frozen A07 primary file-classification contract."""
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    if clean(summary.get("run")) != A07_EXPECTED_RUN:
        raise ValueError(f"Unexpected A07 run: {summary.get('run')}")
    if clean(summary.get("mode")) != "full":
        raise ValueError(f"A07 mode must be full: {summary.get('mode')}")
    if clean(summary.get("status")) != A07_EXPECTED_STATUS:
        raise ValueError(f"Unexpected A07 status: {summary.get('status')}")
    if int(summary.get("failed_hard_checks", -1)) != 0:
        raise ValueError("A07 has failed hard checks")

    rule = summary.get("primary_file_rule", {})
    if clean(rule.get("metric")) != PRIMARY_ML_METRIC:
        raise ValueError("A07 primary metric mismatch")
    if clean(rule.get("operator")) != PRIMARY_ML_OPERATOR:
        raise ValueError("A07 primary operator mismatch")
    if not math.isclose(float(rule.get("threshold")), PRIMARY_ML_THRESHOLD, abs_tol=1e-12):
        raise ValueError("A07 primary threshold mismatch")

    checks = read_csv(checks_path, ["check", "severity", "passed"])
    require_columns(checks, {"check", "severity", "passed"}, "A07 checks")
    ...
    if not passed.eq(1).all():
        ...
        raise ValueError(f"A07 hard checks are not clean: {failed}")
    return summary
...
def load_a07(path: Path) -> pd.DataFrame:
    """Load and validate the unique A07 historical snapshot/file table."""
    header = pd.read_csv(path, nrows=0)
    require_columns(header, A07_REQUIRED, "A07 file scores")
    usecols = sorted(A07_REQUIRED)
    string_cols = {
        "snapshot_id",
        "dataset_source",
        "repo_name",
    ...
    # authoritative, and A07 intentionally keeps file_sha256 blank for them.
    if data[["snapshot_id", "relative_path"]].astype(str).apply(lambda col: col.str.len().eq(0)).any().any():
        raise ValueError("A07 contains missing snapshot_id or relative_path")
    if data.duplicated(key).any():
        raise ValueError("A07 contains duplicate snapshot/path/file_sha256 keys")

    allowed_status = {"scored", "no_ml_cfun", "file_not_prepared"}
    if not set(data["file_ml_cfun_agc_status"]) <= allowed_status:
        raise ValueError(f"Unexpected A07 status values: {sorted(set(data['file_ml_cfun_agc_status']) - allowed_status)}")

    scored = data["file_ml_cfun_agc_status"].eq("scored")
    unclassified = ~scored
    if data.loc[scored, PRIMARY_ML_METRIC].isna().any():
        raise ValueError("A07 scored rows contain missing weighted AGC share")
    if data.loc[unclassified, PRIMARY_ML_METRIC].notna().any():
        raise ValueError("A07 unclassified rows unexpectedly contain weighted AGC share")
    if data.loc[unclassified, "file_ml_cfun_agc_like_primary"].notna().any():
        raise ValueError("A07 unclassified rows unexpectedly contain primary classification")

    expected_flags = data.loc[scored, PRIMARY_ML_METRIC].gt(PRIMARY_ML_THRESHOLD).astype("int64")
    observed_flags = data.loc[scored, "file_ml_cfun_agc_like_primary"].astype("int64")
    if not expected_flags.equals(observed_flags):
        raise ValueError("A07 primary classification does not reproduce weighted-share > 0.50")
    if not data.loc[scored, "file_ml_cfun_agc_primary_operator"].eq(PRIMARY_ML_OPERATOR).all():
        raise ValueError("A07 primary operator mismatch in scored rows")
    if not np.allclose(
        data.loc[scored, "file_ml_cfun_agc_primary_threshold"].astype(float),
        PRIMARY_ML_THRESHOLD,
        atol=1e-12,
        rtol=0,
    ):
        raise ValueError("A07 primary threshold mismatch in scored rows")
    return data
...
def attach_a07_to_d02(d02: pd.DataFrame, a07: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    """Attach one unique A07 file classification to every D02 repo-month/file row."""
    key = ["snapshot_id", "relative_path", "file_sha256"]
    d02_unique = d02[key].drop_duplicates()
    a07_unique = a07[key].drop_duplicates()

    universe = d02_unique.merge(a07_unique, on=key, how="outer", indicator=True, validate="one_to_one")
    d02_only = int(universe["_merge"].eq("left_only").sum())
    a07_only = int(universe["_merge"].eq("right_only").sum())
    if d02_only or a07_only:
        raise ValueError(f"A07/D02 historical file universes differ: d02_only={d02_only}, a07_only={a07_only}")
    ...
    if len(merged) != len(d02):
        raise ValueError("A07 attachment changed D02 row count")

    identity_failures = {
        "dataset_source": int((~merged["dataset_source_d02"].str.casefold().eq(merged["dataset_source_a07"].str.casefold())).sum()),
        "repo_name": int((~merged["repo_name_d02"].str.casefold().eq(merged["repo_name_a07"].str.casefold())).sum()),
        "snapshot_commit": int((~merged["snapshot_commit_d02"].str.casefold().eq(merged["snapshot_commit_a07"].str.casefold())).sum()),
    }
    if any(identity_failures.values()):
        raise ValueError(f"A07/D02 identity mismatches: {identity_failures}")

    d02_lines = pd.to_numeric(merged["python_lines_d02"], errors="coerce")
    a07_lines = pd.to_numeric(merged["python_lines_a07"], errors="coerce")
    line_mismatch = int((d02_lines.fillna(-1) != a07_lines.fillna(-1)).sum())
    if line_mismatch:
        raise ValueError(f"A07/D02 python_lines mismatch rows: {line_mismatch}")
    ...
    merged = merged.drop(
        columns=["dataset_source_a07", "repo_name_a07", "snapshot_commit_a07", "python_lines_a07"]
    )
    diagnostics = {
        "d02_rows": len(d02),
        "d02_unique_snapshot_files": len(d02_unique),
        "a07_rows": len(a07),
        "a07_unique_snapshot_files": len(a07_unique),
        "d02_only_file_keys": d02_only,
        "a07_only_file_keys": a07_only,
...
def aggregate_mapping_spec(files: pd.DataFrame, base_keys: pd.DataFrame, exclude_mapping_warnings: bool) -> pd.DataFrame:
    ...
    scored = files["file_ml_cfun_agc_status"].eq("scored").to_numpy(dtype=bool)
    warning = files["ml_cfun_mapping_warning_present"].fillna(0).astype("int64").eq(1).to_numpy(dtype=bool)
    eligible = scored & (~warning if exclude_mapping_warnings else True)
    primary = files["file_ml_cfun_agc_like_primary"].fillna(0).astype("int64").eq(1).to_numpy(dtype=bool)
    ...
    work["eligible_ml_cfun_space_by_tokens"] = pd.to_numeric(
        files["ml_cfun_space_by_tokens_total"], errors="coerce"
    ).fillna(0).where(eligible, 0)
    work["eligible_ml_agc_space_by_tokens"] = pd.to_numeric(
        files["ml_cfun_agc_space_by_tokens"], errors="coerce"
    ).fillna(0).where(eligible, 0)
    work["selected_ml_cfun_space_by_tokens"] = pd.to_numeric(
        files["ml_cfun_space_by_tokens_total"], errors="coerce"
    ...
    agg_map: dict[str, str] = {
        ...
        "eligible_ml_cfun_space_by_tokens": "sum",
        "eligible_ml_agc_space_by_tokens": "sum",
        "selected_ml_cfun_space_by_tokens": "sum",
    ...
    zero_fill = [
        ...
        "eligible_ml_cfun_space_by_tokens",
        "eligible_ml_agc_space_by_tokens",
        "selected_ml_cfun_space_by_tokens",
...
def build_outputs(
    ...
    """Build long-form H05 panel and descriptive audits."""
    ...
    for mapping_name, exclude_warnings in mapping_specs.items():
        ...
        for sample_name, sample_base in sample_specs.items():
            ...
            scored = file_sample["file_ml_cfun_agc_status"].eq("scored")
            warning = file_sample["ml_cfun_mapping_warning_present"].fillna(0).astype("int64").eq(1)
            eligible = scored & (~warning if exclude_warnings else True)
            selected = eligible & file_sample["file_ml_cfun_agc_like_primary"].fillna(0).astype("int64").eq(1)
...
def production_checks(
    ...
    """Build strict H05 QC checks."""
    rows: list[dict[str, Any]] = []
    strict = args.strict_expected_counts

    status_counts = a07["file_ml_cfun_agc_status"].value_counts().to_dict()
    selected_unique = int(a07["file_ml_cfun_agc_like_primary"].fillna(0).eq(1).sum())
    warning_unique = int(a07["ml_cfun_mapping_warning_present"].fillna(0).eq(1).sum())
    d02_unique = len(d02[["snapshot_id", "relative_path", "file_sha256"]].drop_duplicates())

    expected_pairs = [
        ("a07_file_rows", len(a07), EXPECTED_A07_FILE_ROWS),
        ("a07_files_with_cfun", int(status_counts.get("scored", 0)), EXPECTED_A07_FILES_WITH_CFUN),
        ("a07_no_ml_cfun", int(status_counts.get("no_ml_cfun", 0)), EXPECTED_A07_NO_CFUN),
        ("a07_file_not_prepared", int(status_counts.get("file_not_prepared", 0)), EXPECTED_A07_NOT_PREPARED),
        ("a07_primary_selected_unique_files", selected_unique, EXPECTED_A07_PRIMARY_SELECTED),
        ("a07_mapping_warning_unique_files", warning_unique, EXPECTED_A07_MAPPING_WARNING_FILES),
    ...
    add_check(rows, "a07_failed_hard_checks", a07_summary.get("failed_hard_checks"), 0, int(a07_summary.get("failed_hard_checks", -1)) == 0)
    add_check(rows, "a07_d02_d02_only_file_keys", join_diag["d02_only_file_keys"], 0, join_diag["d02_only_file_keys"] == 0)
    add_check(rows, "a07_d02_a07_only_file_keys", join_diag["a07_only_file_keys"], 0, join_diag["a07_only_file_keys"] == 0)
    add_check(rows, "a07_d02_identity_mismatch_rows", join_diag["identity_mismatch_rows"], 0, join_diag["identity_mismatch_rows"] == 0)
    add_check(rows, "a07_d02_python_lines_mismatch_rows", join_diag["python_lines_mismatch_rows"], 0, join_diag["python_lines_mismatch_rows"] == 0)
    add_check(rows, "joined_d02_row_conservation", len(joined), len(d02), len(joined) == len(d02))

    invalid_selected = int((joined["file_ml_cfun_agc_like_primary"].fillna(0).eq(1) & ~joined["file_ml_cfun_agc_status"].eq("scored")).sum())
    ...
    add_check(
        ...
        EXPECTED_A07_PRIMARY_SELECTED,
        int(primary_global["selected_unique_snapshot_files"]) == EXPECTED_A07_PRIMARY_SELECTED,
    )
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    """Run the full H05 build."""
    for path in [
        args.a07_file,
        args.a07_summary_file,
        args.a07_checks_file,
        args.d02_file,
    ...
    a07_summary = validate_a07_contract(args.a07_summary_file, args.a07_checks_file)
    ...
    summary_rows = [
        ...
        ("density_computed", 0),
        ("a07_file_rows", len(a07)),
        ("a07_files_with_cfun", int(a07["file_ml_cfun_agc_status"].eq("scored").sum())),
        ("a07_primary_selected_unique_files", int(a07["file_ml_cfun_agc_like_primary"].fillna(0).eq(1).sum())),
        ("a07_mapping_warning_unique_files", int(a07["ml_cfun_mapping_warning_present"].fillna(0).eq(1).sum())),
    ...
    metadata = {
        ...
        "inputs": {
            "a07_file": str(args.a07_file.resolve()),
            "a07_file_sha256": sha256_file(args.a07_file),
            "a07_summary_file": str(args.a07_summary_file.resolve()),
            "a07_summary_file_sha256": sha256_file(args.a07_summary_file),
            "a07_checks_file": str(args.a07_checks_file.resolve()),
            "a07_checks_file_sha256": sha256_file(args.a07_checks_file),
        ...
        "upstream_contracts": {
            "a07_run": a07_summary.get("run"),
            "a07_status": a07_summary.get("status"),
        ...
        "method": {
            ...
            "join_cardinality": "D02 repo-month/file rows many-to-one A07 unique snapshot/files",
            "file_universe_equality": "unique D02 snapshot/files must equal A07 exactly",
            "primary_selection": f"{PRIMARY_ML_METRIC} {PRIMARY_ML_OPERATOR} {PRIMARY_ML_THRESHOLD}",
            "no_cfun_policy": "A07 no_ml_cfun remains unclassified and is never treated as HWC",
            ...
            "mapping_sensitivity": "exclude A07 ml_cfun_mapping_warning_present == 1 from both eligible and selected sets",
            ...
            "did": "not estimated in H05; downstream H06 reuses the validated Borusyak design",
        },
    ...
    if hard_failures:
        raise RuntimeError(f"H05 hard QC failures: {hard_failures}; see {args.checks_output}")

    print("=" * 80)
    print("run-x-h05 ML C_FUN-selected Python quality-burden panel")
    print(f"Status:                              {status}")
    print(f"A07 unique historical files:        {len(a07)}")
...
def run_self_test() -> None:
    ...
    files = pd.DataFrame(
        {
            ...
            "file_ml_cfun_agc_status": ["scored", "scored", "no_ml_cfun", "scored"],
            "file_ml_cfun_agc_like_primary": [1, 1, np.nan, 0],
            "ml_cfun_mapping_warning_present": [0, 1, 0, 0],
            "ml_cfun_space_by_tokens_total": [100, 50, 0, 80],
            "ml_cfun_agc_space_by_tokens": [80, 40, 0, 20],
        }
    )
    ...
    print("build_ml_cfun_quality_burden_panel self-test: PASS")


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description="Build run-x-h05 ML C_FUN-selected Python quality burden panel.")
    parser.add_argument("--self-test", action="store_true")
    parser.add_argument("--a07-file", type=Path)
    parser.add_argument("--a07-summary-file", type=Path)
    parser.add_argument("--a07-checks-file", type=Path)
